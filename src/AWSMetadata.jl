...
const services_path = joinpath(@__DIR__, "AWSServices.gen.jl")
...
Low level wrappers are written into `src/AWSServices.gen.jl`, while high level wrappers API
wrappers are written into their respective files in `src/services/{service}.gen.jl`.
"""
