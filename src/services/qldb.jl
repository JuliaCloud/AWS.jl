...
"""
    cancel_journal_kinesis_stream(ledger_name, stream_id)
    cancel_journal_kinesis_stream(ledger_name, stream_id, params::Dict{String,<:Any})

Ends a given Amazon QLDB journal stream. Before a stream can be canceled, its current status
must be `ACTIVE`.

You can't restart a stream after you cancel it. Canceled QLDB stream resources are subject
to a 7-day retention period, so they are automatically deleted after this limit expires.
...
function cancel_journal_kinesis_stream(
    LedgerName, StreamId; aws_config::AbstractAWSConfig=current_aws_config()
)
    return qldb(
        "DELETE",
...
- `permissions_mode`: The permissions mode to assign to the ledger that you want to create.
  ...
  all PartiQL commands (hence, `ALLOW_ALL`) on any tables in the specified ledger. This mode
  disregards any table-level or command-level IAM permissions policies that you create for
  the ledger.
  ...
  in this ledger. To allow PartiQL commands to run, you must create IAM permissions policies
  for specific table resources and PartiQL actions, in addition to the `SendCommand` API
  permission for the ledger. For information, see [Getting started with the standard permissions mode](https://docs.aws.amazon.com/qldb/latest/developerguide/getting-started-standard-mode.html)
  in the *Amazon QLDB Developer Guide*.
...
- `"DeletionProtection"`: Specifies whether the ledger is protected from being deleted by
  any user. If not defined during ledger creation, this feature is enabled (`true`) by
  default.
...
- `"KmsKey"`: The key in Key Management Service (KMS) to use for encryption of data at rest
  ...
  alias name, or alias ARN. When using an alias name, prefix it with `"alias/"`. To specify
  a key in a different Amazon Web Services account, you must use the key ARN or alias ARN.
...
function delete_ledger(Name; aws_config::AbstractAWSConfig=current_aws_config())
    return qldb("DELETE", "/ledgers/$(Name)"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function delete_ledger(
    ...
    return qldb(
        "DELETE", "/ledgers/$(Name)", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end

"""
    describe_journal_kinesis_stream(ledger_name, stream_id)
    describe_journal_kinesis_stream(ledger_name, stream_id, params::Dict{String,<:Any})
...
This action does not return any expired journal streams. For more information, see [Expiration for terminal streams](https://docs.aws.amazon.com/qldb/latest/developerguide/streams.create.html#streams.create.states.expiration)
in the *Amazon QLDB Developer Guide*.
...
This action does not return any expired export jobs. For more information, see [Export job expiration](https://docs.aws.amazon.com/qldb/latest/developerguide/export-journal.request.html#export-journal.request.expiration)
in the *Amazon QLDB Developer Guide*.
...
function describe_ledger(Name; aws_config::AbstractAWSConfig=current_aws_config())
    return qldb("GET", "/ledgers/$(Name)"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_ledger(
    ...
    return qldb(
        "GET", "/ledgers/$(Name)", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end

"""
    export_journal_to_s3(exclusive_end_time, inclusive_start_time, name, role_arn, s3_export_configuration)
    export_journal_to_s3(exclusive_end_time, inclusive_start_time, name, role_arn, s3_export_configuration, params::Dict{String,<:Any})
...
- `exclusive_end_time`: The exclusive end date and time for the range of journal contents to
  export.
...
- `"OutputFormat"`: The output format of your exported journal data. A journal export job
  can write the data objects in either the text or binary representation of [Amazon Ion](https://docs.aws.amazon.com/qldb/latest/developerguide/ion.html)
  format, or in [JSON Lines](https://jsonlines.org/) text format.

  Default: `ION_TEXT`

  In JSON Lines format, each journal block in an exported data object is a valid JSON object
  that is delimited by a newline. You can use this format to directly integrate JSON exports
  with analytics tools such as Amazon Athena and Glue because these services can parse
...
- `"DigestTipAddress"`: The latest block location covered by the digest for which to request
  a proof. An address is an Amazon Ion structure that has two fields: `strandId` and
  `sequenceNo`.
...
function get_digest(Name; aws_config::AbstractAWSConfig=current_aws_config())
    return qldb(
        "POST", "/ledgers/$(Name)/digest"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `block_address`: The block location of the document revision to be verified. An address is
...
"""
    list_journal_kinesis_streams_for_ledger(ledger_name)
    list_journal_kinesis_streams_for_ledger(ledger_name, params::Dict{String,<:Any})
...
This action returns a maximum of `MaxResults` items. It is paginated so that you can
retrieve all the items by calling `ListJournalKinesisStreamsForLedger` multiple times.
...
- `"max_results"`: The maximum number of results to return in a single
  `ListJournalKinesisStreamsForLedger` request. (The actual number of results returned might
  be fewer.)
...
function list_journal_kinesis_streams_for_ledger(
    LedgerName; aws_config::AbstractAWSConfig=current_aws_config()
)
    return qldb(
        "GET",
...
- `"max_results"`: The maximum number of results to return in a single
  `ListJournalS3Exports` request. (The actual number of results returned might be fewer.)
- `"next_token"`: A pagination token, indicating that you want to retrieve the next page of
  results. If you received a value for `NextToken` in the response from a previous
...
function list_journal_s3_exports(; aws_config::AbstractAWSConfig=current_aws_config())
    return qldb("GET", "/journal-s3-exports"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_journal_s3_exports(
    ...
    return qldb(
        "GET", "/journal-s3-exports", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"max_results"`: The maximum number of results to return in a single `ListLedgers`
...
function list_ledgers(; aws_config::AbstractAWSConfig=current_aws_config())
    return qldb("GET", "/ledgers"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_ledgers(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return qldb("GET", "/ledgers", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_tags_for_resource(
    ...
    return qldb("GET", "/tags/$(ResourceArn)"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
"""
    stream_journal_to_kinesis(inclusive_start_time, kinesis_configuration, ledger_name, role_arn, stream_name)
    stream_journal_to_kinesis(inclusive_start_time, kinesis_configuration, ledger_name, role_arn, stream_name, params::Dict{String,<:Any})
...
- `stream_name`: The name that you want to assign to the QLDB journal stream. User-defined
  ...
  names have the same naming constraints as ledger names, as defined in [Quotas in Amazon QLDB](https://docs.aws.amazon.com/qldb/latest/developerguide/limits.html#limits.naming)
  in the *Amazon QLDB Developer Guide*.
...
- `resource_arn`: The Amazon Resource Name (ARN) from which to remove the tags. For example:
...
function update_ledger(Name; aws_config::AbstractAWSConfig=current_aws_config())
    return qldb("PATCH", "/ledgers/$(Name)"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function update_ledger(
    ...
    return qldb(
        "PATCH", "/ledgers/$(Name)", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end

"""
    update_ledger_permissions_mode(name, permissions_mode)
    update_ledger_permissions_mode(name, permissions_mode, params::Dict{String,<:Any})

Updates the permissions mode of a ledger.

!!! important
    Before you switch to the `STANDARD` permissions mode, you must first create all required
    IAM policies and table tags to avoid disruption to your users. To learn more, see [Migrating to the standard permissions mode](https://docs.aws.amazon.com/qldb/latest/developerguide/ledger-management.basics.html#ledger-mgmt.basics.update-permissions.migrating)
    in the *Amazon QLDB Developer Guide*.
