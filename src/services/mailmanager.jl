...
function delete_relay(
    RelayId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return mailmanager(
        "DeleteRelay",
...
- `"PageSize"`: The maximum number of archive search jobs that are returned per call. You
  can use NextToken to obtain further pages of archives.
"""
function list_archive_searches(ArchiveId; aws_config::AbstractAWSConfig=current_aws_config())
    return mailmanager(
        "ListArchiveSearches",
        Dict{String,Any}("ArchiveId" => ArchiveId);
        aws_config=aws_config,
