...
- `ClientRequestToken`: A unique, case-sensitive identifier that you provide to ensure the
  idempotency of the operation. An idempotent operation completes no more than one time. This
  identifier is required only if you make a service request directly using an HTTP client. It
  is generated automatically if you use an AWS SDK or the AWS CLI.
- `InvitationId`: The unique identifier of the invitation that is sent to the member to
  join the network.
...
- `VotingPolicy`:  The voting rules used by the network to determine if a proposal is
  approved.
...
- `Tags`: Tags to assign to the network. Each tag consists of a key and optional value.
  ...
  request, with an overall maximum of 50 added to each resource. For more information about
  tags, see Tagging Resources in the Amazon Managed Blockchain Ethereum Developer Guide, or
...
Ethereum. Ethereum on Managed Blockchain is in preview release and is subject to change.
...
- `networkId`: The unique identifier of the network for the node. Ethereum public networks
...
- `ClientRequestToken`: A unique, case-sensitive identifier that you provide to ensure the
  ...
  identifier is required only if you make a service request directly using an HTTP client. It
  is generated automatically if you use an AWS SDK or the AWS CLI.
- `MemberId`: The unique identifier of the member that is creating the proposal. This
...
- `Tags`: Tags to assign to the proposal. Each tag consists of a key and optional value.
  When specifying tags during creation, you can specify multiple key-value pairs in a single
  request, with an overall maximum of 50 added to each resource. If the proposal is for a
  network invitation, the invitation inherits the tags added to the proposal. For more
...
# Required Parameters
- `networkId`: The unique identifier of the network that the node is on. Ethereum public
...
- `memberId`: The unique identifier of the member who owns the nodes to list. Applies only
...
# Required Parameters
- `resourceArn`: The Amazon Resource Name (ARN) of the resource. For more information about
  ARNs and their format, see Amazon Resource Names (ARNs) in the AWS General Reference.
...
- `Tags`: The tags to assign to the specified resource. Tag values can be empty, for
  ...
  with an overall maximum of 50 added to each resource.
- `resourceArn`: The Amazon Resource Name (ARN) of the resource. For more information about
  ARNs and their format, see Amazon Resource Names (ARNs) in the AWS General Reference.
...
# Required Parameters
- `memberId`: The unique identifier of the member.
- `networkId`: The unique identifier of the Managed Blockchain network to which the member
  belongs.
