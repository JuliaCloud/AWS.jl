...
- `tags_to_add`: The tags to add to objects created by the policy. Tags are strings in the
...
- `execution_role_arn`: The Amazon Resource Name (ARN) of the IAM role used to run the
  operations specified by the lifecycle policy.
- `policy_details`: The configuration of the lifecycle policy. You cannot update the policy
