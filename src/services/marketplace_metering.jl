...
- `UsageRecords`: The set of UsageRecords to submit. BatchMeterUsage accepts up to 25
...
- `UsageDimension`: It will be one of the fcp dimension name provided during the publishing
  of the product.
...
- `UsageAllocations`: The set of UsageAllocations to submit. The sum of all UsageAllocation
  quantities must equal the UsageQuantity of the MeterUsage request, and each UsageAllocation
...
# Required Parameters
- `RegistrationToken`: When a buyer visits your website during the registration process,
