...
- `role_arn`: The Amazon Resource Name (ARN) of the IAM role to associate with the Aurora DB
...
- `subscription_name`: The name of the RDS event notification subscription you want to add a
...
- `apply_action`: The pending maintenance action to apply to this resource.
  ...
  For more information about these actions, see [Maintenance actions for Amazon Aurora](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_UpgradeDBInstance.Maintenance.html#maintenance-actions-aurora)
  or [Maintenance actions for Amazon RDS](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_UpgradeDBInstance.Maintenance.html#maintenance-actions-rds).
...
- `resource_identifier`: The RDS Amazon Resource Name (ARN) of the resource that the pending
  maintenance action applies to. For information about creating an ARN, see [Constructing an RDS Amazon Resource Name (ARN)](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_Tagging.ARN.html#USER_Tagging.ARN.Constructing).
"""
...
- `backtrack_to`: The timestamp of the time to backtrack the DB cluster to, specified in ISO
  8601 format. For more information about ISO 8601, see the [ISO8601 Wikipedia page.](http://en.wikipedia.org/wiki/ISO_8601)
...
- `"UseEarliestTimeOnPointInTimeUnavailable"`: Specifies whether to backtrack the DB cluster
  to the earliest possible backtrack time when *BacktrackTo* is set to a timestamp earlier
  than the earliest backtrack time. When this parameter is disabled and *BacktrackTo* is set
...
!!! note
    ...
    cluster parameter group, which copies the default parameters and values for the
...
that case, the Amazon Web Services Region where you call the [`copy_dbcluster_snapshot`](@ref)
...
- `KmsKeyId` - The Amazon Web Services Key Management System (Amazon Web Services KMS) key
  identifier for the key to use to encrypt the copy of the DB cluster snapshot in the
...
- `SourceDBClusterSnapshotIdentifier` - The DB cluster snapshot identifier for the encrypted
  DB cluster snapshot to be copied. This identifier must be in the ARN format for the source
  Amazon Web Services Region and is the same value as the
...
For more information on copying encrypted Amazon Aurora DB cluster snapshots from one Amazon
...
- `source_dbcluster_snapshot_identifier`: The identifier of the DB cluster snapshot to copy.
  ...
  - If the source snapshot is in a different Amazon Web Services Region than the copy,
    specify a valid DB cluster snapshot ARN. You can also specify an ARN of a snapshot that
    is in a different account and a different Amazon Web Services Region. For more
    information, go to [Copying Snapshots Across Amazon Web Services Regions](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_CopySnapshot.html#USER_CopySnapshot.AcrossRegions)
    in the *Amazon Aurora User Guide*.
...
- `"KmsKeyId"`: The Amazon Web Services KMS key identifier for an encrypted DB cluster
  ...
  alias name for the Amazon Web Services KMS key.
...
- `"PreSignedUrl"`: When you are copying a DB cluster snapshot from one Amazon Web Services
  ...
  - `SourceDBClusterSnapshotIdentifier` - The DB cluster snapshot identifier for the
    ...
    Resource Name (ARN) format for the source Amazon Web Services Region. For example, if
    you are copying an encrypted DB cluster snapshot from the us-west-2 Amazon Web Services
...
Amazon Web Services Region where you call the [`copy_dbsnapshot`](@ref) operation is the
destination Amazon Web Services Region for the DB snapshot copy.
...
- `source_dbsnapshot_identifier`: The identifier for the source DB snapshot.
  ...
  If the source snapshot is in a different Amazon Web Services Region than the copy, specify
  a valid DB snapshot ARN. You can also specify an ARN of a snapshot that is in a different
  account and a different Amazon Web Services Region. For example, you might specify
...
- `"CopyTags"`: Specifies whether to copy all tags from the source DB snapshot to the target
  DB snapshot. By default, tags aren't copied.

- `"KmsKeyId"`: The Amazon Web Services KMS key identifier for an encrypted DB snapshot. The
  Amazon Web Services KMS key identifier is the key ARN, key ID, alias ARN, or alias name
  for the KMS key.
  ...
  specify a value for this parameter, then the copy of the DB snapshot is encrypted with the
  ...
  If you copy an encrypted snapshot to a different Amazon Web Services Region, then you must
  specify an Amazon Web Services KMS key identifier for the destination Amazon Web Services
  Region. KMS keys are specific to the Amazon Web Services Region that they are created in,
  and you can't use KMS keys from one Amazon Web Services Region in another Amazon Web
  Services Region.
...
- `"OptionGroupName"`: The name of an option group to associate with the copy of the
  ...
  uses Transparent Data Encryption for Oracle or Microsoft SQL Server, you must specify this
  option when copying across Amazon Web Services Regions. For more information, see [Option group considerations](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_CopySnapshot.html#USER_CopySnapshot.Options)
  in the *Amazon RDS User Guide*.
...
- `"PreSignedUrl"`: When you are copying a snapshot from one Amazon Web Services GovCloud
  ...
  You must specify this parameter when you copy an encrypted DB snapshot from another Amazon
  Web Services Region by using the Amazon RDS API. Don't specify `PreSignedUrl` when you are
...
- `"SnapshotAvailabilityZone"`: Specifies the name of the Availability Zone where RDS stores
  the DB snapshot. This value is valid only for snapshots that RDS stores on a Dedicated
  Local Zone.

- `"SnapshotTarget"`: Configures the location where RDS will store copied snapshots.
...
environment. The green environment is the staging environment, and it stays in sync with the
...
promote the green environment to be the new production environment. The switchover typically
...
- `"TargetAllocatedStorage"`: The amount of storage in gibibytes (GiB) to allocate for the
  green DB instance. You can choose to increase or decrease the allocated storage on the
  green DB instance.
...
- `"TargetDBClusterParameterGroupName"`: The DB cluster parameter group associated with the
  ...
  To test parameter changes, specify a DB cluster parameter group that is different from the
  one associated with the source DB cluster.
...
- `"TargetIops"`: The amount of Provisioned IOPS (input/output operations per second) to
  allocate for the green DB instance. For information about valid IOPS values, see [Amazon RDS DB instance storage](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_Storage.html)
  in the *Amazon RDS User Guide*.

  This setting doesn't apply to Amazon Aurora blue/green deployments.

- `"TargetStorageThroughput"`: The storage throughput value for the green DB instance.

  This setting applies only to the `gp3` storage type.

  This setting doesn't apply to Amazon Aurora blue/green deployments.

- `"TargetStorageType"`: The storage type to associate with the green DB instance.
...
- `engine_version`: The name of your custom engine version (CEV).

  For RDS Custom for Oracle, the name format is `19.*customized_string*`. For example, a
  valid CEV name is `19.my_cev1`.

  For RDS for SQL Server and RDS Custom for SQL Server, the name format is
  `major engine_version*.*minor_engine_version*.*customized_string*`. For example, a valid
  CEV name is `16.00.4215.2.my_cev1`.
...
- `"DatabaseInstallationFiles"`: The database installation files (ISO and EXE) uploaded to
  Amazon S3 for your database engine version to import to Amazon RDS.
...
- `"KMSKeyId"`: The Amazon Web Services KMS key identifier for an encrypted CEV. A symmetric
...
- `"UseAwsProvidedLatestImage"`: Specifies whether to use the latest service-provided Amazon
  Machine Image (AMI) for the CEV. If you specify `UseAwsProvidedLatestImage`, you can't
...
express configuration and create cluster in seconds. Express configuration provides a
cluster with a writer instance and feature specific values set to all other input parameters
of this API.
...
- `"AvailabilityZones"`: A list of Availability Zones (AZs) where you specifically want to
  ...
  For the first three DB instances that you create, RDS distributes each DB instance to a
  different AZ that you specify. For additional DB instances that you create, RDS randomly
  distributes them to the AZs that you specified. For example, if you create a DB cluster
  with one writer instance and three reader instances, RDS might distribute the writer
  instance to AZ 1, the first reader instance to AZ 2, the second reader instance to AZ 3,
  and the third reader instance to either AZ 1, AZ 2, or AZ 3.

  For more information, see [Availability Zones](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Concepts.RegionsAndAvailabilityZones.html#Concepts.RegionsAndAvailabilityZones.AvailabilityZones)
  and [High availability for Aurora DB instances](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Concepts.AuroraHighAvailability.html#Concepts.AuroraHighAvailability.Instances)
  in the *Amazon Aurora User Guide*.
...
- `"ClusterScalabilityType"`: Specifies the scalability mode of the Aurora DB cluster. When
  set to `limitless`, the cluster operates as an Aurora Limitless Database. When set to
...
- `"EnableGlobalWriteForwarding"`: Specifies whether to enable this DB cluster to forward
  ...
  For the primary DB cluster of an Aurora global database, this value is used immediately if
  the primary is demoted by a global cluster API operation, but it does nothing until then.
...
- `"EnableIAMDatabaseAuthentication"`: Specifies whether to enable mapping of Amazon Web
  ...
  in the *Amazon Aurora User Guide* or [IAM database authentication for MariaDB, MySQL, and PostgreSQL](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.IAMDBAuth.html)
  in the *Amazon RDS User Guide*.
...
- `"EngineLifecycleSupport"`: The life cycle type for this DB cluster.
  ...
  !!! note
      ...
      your DB cluster into Amazon RDS Extended Support. At the end of standard support, you
      can avoid charges for Extended Support by setting the value to
      `open-source-rds-extended-support-disabled`. In this case, creating the DB cluster
      will fail if the DB major version is past its end of standard support date.

  You can use this setting to enroll your DB cluster into Amazon RDS Extended Support. With
  RDS Extended Support, you can run the selected major engine version on your DB cluster
  past the end of standard support for that engine version. For more information, see the
  following sections:

  - Amazon Aurora - [Amazon RDS Extended Support with Amazon Aurora](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/extended-support.html)
    in the *Amazon Aurora User Guide*
  - Amazon RDS - [Amazon RDS Extended Support with Amazon RDS](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/extended-support.html)
    in the *Amazon RDS User Guide*
...
- `"EngineVersion"`: The version number of the database engine to use.
  ...
  - Aurora MySQL - see [Database engine updates for Amazon Aurora MySQL](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/AuroraMySQL.Updates.html)
    in the *Amazon Aurora User Guide*.
  - Aurora PostgreSQL - see [Amazon Aurora PostgreSQL releases and engine versions](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/AuroraPostgreSQL.Updates.20180305.html)
    in the *Amazon Aurora User Guide*.
...
- `"KmsKeyId"`: The Amazon Web Services KMS key identifier for an encrypted DB cluster.
  ...
  - If `ReplicationSourceIdentifier` identifies an encrypted source, then Amazon RDS uses
    the KMS key used to encrypt the source. Otherwise, Amazon RDS uses your default KMS key.
...
- `"MasterUserAuthenticationType"`: Specifies the authentication type for the master user.
  With IAM master user authentication, you can configure the master DB user with IAM
...
- `"MonitoringRoleArn"`: The Amazon Resource Name (ARN) for the IAM role that permits RDS to
...
- `"PerformanceInsightsKMSKeyId"`: The Amazon Web Services KMS key identifier for encryption
...
- `"PreferredMaintenanceWindow"`: The weekly time range during which system maintenance can
  ...
  The default is a 30-minute window selected at random from an 8-hour block of time for each
  Amazon Web Services Region, occurring on a random day of the week. To see the time blocks
  available, see [Adjusting the Preferred DB Cluster Maintenance Window](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_UpgradeDBInstance.Maintenance.html#AdjustingTheMaintenanceWindow.Aurora)
  in the *Amazon Aurora User Guide*.
...
- `"PubliclyAccessible"`: Specifies whether the DB cluster is publicly accessible.
  ...
  endpoint resolves to the private IP address. Access to the DB cluster is controlled by its
...
- `"TagSpecifications"`: Tags to assign to resources associated with the DB cluster.
...
- `"WithExpressConfiguration"`: Specifies to create an Aurora DB Cluster with express
  configuration in seconds. Express configuration provides a cluster with a writer instance
...
the DB instances in the DB cluster without failover for the new DB cluster parameter group
...
!!! important
    ...
    DB cluster parameter group is used as the default for a new DB cluster. This is
    ...
    or the [`describe_dbcluster_parameters`](@ref) operation to verify that your DB cluster
...
- `"AdditionalStorageVolumes"`: A list of additional storage volumes to create for the DB
  instance. You can create up to three additional storage volumes using the names
  `rdsdbdata2`, `rdsdbdata3`, and `rdsdbdata4`. Additional storage volumes are supported for
...
- `"AllocatedStorage"`: The amount of storage in gibibytes (GiB) to allocate for the DB
  ...
  - General Purpose (SSD) storage (gp2, gp3): Must be an integer from 40 to 65536 for RDS
...
- `"CharacterSetName"`: For supported engines, the character set (`CharacterSet`) to
  ...
  - Amazon Aurora - The character set is managed by the DB cluster. For more information,
    see `CreateDBCluster`.
  - RDS Custom - However, if you need to change the character set, you can change it on the
    database itself.
...
- `"DBName"`: The meaning of this parameter differs according to the database engine you
  ...
  database is created in the DB cluster.
  ...
  DB cluster is created. A database named `postgres` is always created. If this parameter is
  ...
  The Oracle System ID (SID) of the created RDS Custom DB instance. If you don't specify a
  value, the default value is `ORCL` for non-CDBs and `RDSCDB` for CDBs.
  ...
  that you don't add a database name. For more information, see [Additional considerations](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/db2-db-instance-prereqs.html#db2-prereqs-additional-considerations)
  in the *Amazon RDS User Guide*.
  ...
  The Oracle System ID (SID) of the created DB instance. If you don't specify a value, the
  default value is `ORCL`. You can't specify the string `null`, or any other reserved word,
  for `DBName`.
...
- `"DedicatedLogVolume"`: Indicates whether the DB instance has a dedicated log volume (DLV)
  enabled.
...
- `"Domain"`: The Active Directory directory ID to create the DB instance in. Currently, you
  can create only Db2, MySQL, Microsoft SQL Server, Oracle, and PostgreSQL DB instances in
...
- `"DomainDnsIps"`: The IPv4 DNS IP addresses of your primary and secondary Active Directory
  domain controllers.

  Constraints:

  - Two IP addresses must be provided. If there isn't a secondary domain controller, use the
    IP address of the primary domain controller for both entries in the list.
...
- `"EnableCustomerOwnedIp"`: Specifies whether to enable a customer-owned IP address (CoIP)
  ...
  through your on-premises network. For some use cases, a CoIP can provide lower latency for
  connections to the DB instance from outside of its virtual private cloud (VPC) on your
  local network.
...
- `"EnablePerformanceInsights"`: Specifies whether to enable Performance Insights for the DB
...
- `"EngineLifecycleSupport"`: The life cycle type for this DB instance.
  ...
  !!! note
      ...
      can avoid charges for Extended Support by setting the value to
      `open-source-rds-extended-support-disabled`. In this case, creating the DB instance
      will fail if the DB major version is past its end of standard support date.
  ...
  RDS Extended Support, you can run the selected major engine version on your DB instance
  past the end of standard support for that engine version. For more information, see [Amazon RDS Extended Support with Amazon RDS](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/extended-support.html)
  in the *Amazon RDS User Guide*.
...
- `"EngineVersion"`: The version number of the database engine to use.
  ...
  every Amazon Web Services Region.
  ...
  A valid CEV name is `19.my_cev1`. For more information, see [Creating an RDS Custom for Oracle DB instance](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/custom-creating.html#custom-creating.create)
  in the *Amazon RDS User Guide*.
...
- `"Iops"`: The amount of Provisioned IOPS (input/output operations per second) to initially
  allocate for the DB instance. For information about valid IOPS values, see [Amazon RDS DB instance storage](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_Storage.html)
  in the *Amazon RDS User Guide*.
...
- `"LicenseModel"`: The license model information for this DB instance.
  ...
  !!! note
      ...
      information, see [Amazon RDS for Db2 licensing options](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/db2-licensing.html)
      in the *Amazon RDS User Guide*.
...
- `"MultiAZ"`: Specifies whether the DB instance is a Multi-AZ deployment. You can't set the
...
- `"MultiTenant"`: Specifies whether to use the multi-tenant configuration or the single-
  ...
  - The DB engine that you specify in the request must support the multi-tenant
    configuration. If you attempt to enable the multi-tenant configuration on a DB engine
    that doesn't support it, the request fails.
...
- `"PreferredBackupWindow"`: The daily time range during which automated backups are created
  ...
  is a 30-minute window selected at random from an 8-hour block of time for each Amazon Web
  Services Region. For more information, see [Backup window](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_WorkingWithAutomatedBackups.html#USER_WorkingWithAutomatedBackups.BackupWindow)
  in the *Amazon RDS User Guide*.
...
- `"PubliclyAccessible"`: Specifies whether the DB instance is publicly accessible.
  ...
  endpoint resolves to the private IP address. Access to the DB instance is controlled by
  its security group settings.
  ...
  If `DBSubnetGroup` isn't specified, `PubliclyAccessible` defaults to `false` for Aurora
  instances and `true` for non-Aurora instances.
...
- `"StorageThroughput"`: The storage throughput value, in mebibyte per second (MiBps), for
  the DB instance.
...
cluster, use the [`create_dbinstance`](@ref) operation.

RDS creates read replicas with backups disabled. All other attributes (including DB security
groups and DB parameter groups) are inherited from the source DB instance or cluster, except
as specified.
...
- `"BackupTarget"`: The location where RDS stores automated backups and manual snapshots.
...
- `"DBInstanceClass"`: The compute and memory capacity of the read replica, for example
  ...
  or for all database engines. For the full list of DB instance classes, and availability
  for your engine, see [DB Instance Class](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Concepts.DBInstanceClass.html)
  in the *Amazon RDS User Guide*.

  Default: Inherits the value from the source DB instance.

- `"DBParameterGroupName"`: The name of the DB parameter group to associate with this read
  replica DB instance.

  For the Db2 DB engine, if your source DB instance uses the bring your own license (BYOL)
  model, then a custom parameter group must be associated with the replica. For a same
  Amazon Web Services Region replica, if you don't specify a custom parameter group, Amazon
  RDS associates the custom parameter group associated with the source DB instance. For a
  cross-Region replica, you must specify a custom parameter group. This custom parameter
  group must include your IBM Site ID and IBM Customer ID. For more information, see [IBM IDs for bring your own license (BYOL) for Db2](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/db2-licensing.html#db2-prereqs-ibm-info).

  For Single-AZ or Multi-AZ DB instance read replica instances, if you don't specify a value
  for `DBParameterGroupName`, then Amazon RDS uses the `DBParameterGroup` of the source DB
  instance for a same Region read replica, or the default `DBParameterGroup` for the
  specified DB engine for a cross-Region read replica.

  For Multi-AZ DB cluster same Region read replica instances, if you don't specify a value
  for `DBParameterGroupName`, then Amazon RDS uses the default `DBParameterGroup`.

  Specifying a parameter group for this operation is only supported for MySQL DB instances
  for cross-Region read replicas, for Multi-AZ DB cluster read replica instances, for Db2 DB
  instances, and for Oracle DB instances. It isn't supported for MySQL DB instances for same
...
- `"DatabaseInsightsMode"`: The mode of Database Insights to enable for the read replica.
...
- `"EnableCustomerOwnedIp"`: Specifies whether to enable a customer-owned IP address (CoIP)
  ...
  A *CoIP* provides local or external connectivity to resources in your Outpost subnets
  through your on-premises network. For some use cases, a CoIP can provide lower latency for
  connections to the read replica from outside of its virtual private cloud (VPC) on your
  local network.
...
- `"OptionGroupName"`: The option group to associate the DB instance with. If not specified,
  RDS uses the option group associated with the source DB instance or cluster.
...
- `"PreSignedUrl"`: When you are creating a read replica from one Amazon Web Services
  ...
  This setting applies only when replicating from a source DB *instance*. Source DB clusters
  aren't supported in Amazon Web Services GovCloud (US) Regions and China Amazon Web
  Services Regions.
  ...
  - `SourceDBInstanceIdentifier` - The DB instance identifier for the encrypted DB instance
    ...
    read replica from a DB instance in the us-west-2 Amazon Web Services Region, then your
...
- `"ReplicaMode"`: The open mode of the replica database.
  ...
  (SE). The main use case for standby replicas is cross-Region disaster recovery. Because it
  doesn't accept user connections, a standby replica can't serve a read-only workload.

  You can create a combination of standby and read-only DB replicas for the same primary DB
  instance. For more information, see [Working with replicas for Amazon RDS for Db2](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/db2-replication.html)
  in the *Amazon RDS User Guide*.
  ...
  Mounted DB replicas are included in Oracle Database Enterprise Edition. The main use case
  for mounted replicas is cross-Region disaster recovery. The primary database doesn't use
  ...
  instance. For more information, see [Working with read replicas for Amazon RDS for Oracle](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/oracle-read-replicas.html)
  in the *Amazon RDS User Guide*.

  For RDS Custom, you must specify this parameter and set it to `mounted`. The value won't
  be set by default. After replica creation, you can manage the open mode manually.
...
- `"SourceDBInstanceIdentifier"`: The identifier of the DB instance that will act as the
  source for the read replica. Each DB instance can have up to 15 read replicas, except for
  ...
  - Must be the identifier of an existing Db2, MariaDB, MySQL, Oracle, PostgreSQL, or SQL
    Server DB instance.
...
"""
    create_dbproxy(dbproxy_name, engine_family, role_arn, vpc_subnet_ids)
    create_dbproxy(dbproxy_name, engine_family, role_arn, vpc_subnet_ids, params::Dict{String,<:Any})
...
- `"DebugLogging"`: Specifies whether the proxy logs detailed connection and query
  information. When you enable `DebugLogging`, the proxy captures connection details and
  connection pool behavior from your queries. Debug logging increases CloudWatch costs and
  can impact proxy performance. Enable this option only when you need to troubleshoot
  connection or performance issues.

- `"DefaultAuthScheme"`: The default authentication scheme that the proxy uses for client
  connections to the proxy and connections from the proxy to the underlying database. Valid
  values are `NONE` and `IAM_AUTH`. When set to `IAM_AUTH`, the proxy uses end-to-end IAM
  authentication to connect to the database. If you don't specify `DefaultAuthScheme` or
  specify this parameter as `NONE`, you must specify the `Auth` option.

- `"EndpointNetworkType"`: The network type of the DB proxy endpoint. The network type
...
- `"TargetConnectionNetworkType"`: The network type that the proxy uses to connect to the
  target database. The network type determines the IP version that the proxy uses for
  ...
  - If you specify `IPV6`, the database must support dual-stack mode. RDS doesn't support
    IPv6-only databases.
...
cluster. You can also use DB proxy endpoints to access a DB proxy through a different VPC
...
- `"ComputeRedundancy"`: Specifies whether to create standby standby DB data access shard
  for the DB shard group. Valid values are the following:
...
- `dbsubnet_group_name`: The name for the DB subnet group. This value is stored as a
  ...
  - Must contain no more than 255 letters, numbers, periods, underscores, spaces, or
    hyphens.
  - Must not be default.
  - First character must be a letter.
...
provide a list of RDS sources (`SourceIds`) that triggers the events. You can also provide a
list of event categories (`EventCategories`) for events that you want to be notified of. For
...
- `"SourceType"`: The type of source that is generating the events. For example, if you want
  to be notified of events generated by a DB instance, you set this parameter to
  `db-instance`. For RDS Proxy events, specify `db-proxy`. If this value isn't specified,
...
- `global_cluster_identifier`: The cluster identifier for this global database cluster. This
  parameter is stored as a lowercase string.
...
- `"DatabaseName"`: The name for your database of up to 64 alphanumeric characters. If you
  ...
  - Can't be specified if `SourceDBClusterIdentifier` is specified. In this case, Amazon
    Aurora uses the database name from the source DB cluster.

- `"DeletionProtection"`: Specifies whether to enable deletion protection for the new global
  database cluster. The global database can't be deleted when deletion protection is
  enabled.
...
- `"EngineLifecycleSupport"`: The life cycle type for this global database cluster.
  ...
  !!! note
      ...
      `open-source-rds-extended-support-disabled`. In this case, creating the global cluster
      will fail if the DB major version is past its end of standard support date.
  ...
  see [Amazon RDS Extended Support with Amazon Aurora](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/extended-support.html)
  in the *Amazon Aurora User Guide*.
...
- `"DataFilter"`: Data filtering options for the integration. For more information, see [Data filtering for Aurora zero-ETL integrations with Amazon Redshift](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/zero-etl.filtering.html)
  or [Data filtering for Amazon RDS zero-ETL integrations with Amazon Redshift](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/zero-etl.filtering.html).
...
"""
    create_tenant_database(dbinstance_identifier, master_username, tenant_dbname)
    create_tenant_database(dbinstance_identifier, master_username, tenant_dbname, params::Dict{String,<:Any})
...
- `dbinstance_identifier`: The user-supplied DB instance identifier. RDS creates your tenant
...
!!! note
    ...
    integrated with Amazon Web Services CloudTrail. If you turn on data logging for Amazon
    RDS in CloudTrail, calls to the `DeleteCustomDbEngineVersion` event aren't logged.
...
- `"DeleteAutomatedBackups"`: Specifies whether to remove automated backups immediately
  ...
  remove automated backups immediately after the DB cluster is deleted, unless the Amazon
...
- `db_cluster_resource_id`: The identifier for the source DB cluster, which can't be changed
...
To delete a DB instance in this case, first use the [`promote_read_replica_dbcluster`](@ref)
operation to promote the DB cluster so that it's no longer a read replica. After the
promotion completes, use the [`delete_dbinstance`](@ref) operation to delete the final
...
- `"FinalDBSnapshotIdentifier"`: The `DBSnapshotIdentifier` of the new `DBSnapshot` created
  ...
  !!! note
      If you enable this parameter and also enable SkipFinalShapshot, the command results in
      an error.
...
- `"SkipFinalSnapshot"`: Specifies whether to skip the creation of a final DB snapshot
  ...
  instance is deleted. By default, skip isn't enabled, and the DB snapshot is created.
  ...
  When a DB instance is in a failure state and has a status of `failed`,
  `incompatible-restore`, or `incompatible-network`, RDS can delete the instance only if you
...
function delete_dbinstance_automated_backup(;
    ...
    return rds(
        "DeleteDBInstanceAutomatedBackup"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `dbshard_group_identifier`: The name of the DB shard group to delete.
"""
...
- `dbsubnet_group_name`: The name of the database subnet group to delete.
...
function describe_account_attributes(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeAccountAttributes"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_blue_green_deployments(;
    ...
    return rds("DescribeBlueGreenDeployments"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_blue_green_deployments(
    ...
    return rds(
        "DescribeBlueGreenDeployments", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_certificates(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeCertificates"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_certificates(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeCertificates", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
are returned for both the `DescribeDBClusterAutomatedBackups` and [`describe_dbclusters`](@ref)
operations.
...
- `"DBClusterIdentifier"`: (Optional) The user-supplied DB cluster identifier. If this
  ...
  information from the specific DB cluster's automated backup. This parameter isn't case-
  sensitive.
...
- `"Filters"`: A filter that specifies which resources to return based on status.
  ...
  - `db-cluster-id` - Accepts DB cluster identifiers and Amazon Resource Names (ARNs). The
    results list includes only information about the DB cluster automated backups identified
    by these ARNs.
  - `db-cluster-resource-id` - Accepts DB resource identifiers and Amazon Resource Names
    (ARNs). The results list includes only information about the DB cluster resources
...
function describe_dbcluster_automated_backups(;
    ...
    return rds(
        "DescribeDBClusterAutomatedBackups"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"BacktrackIdentifier"`: If specified, this value is the backtrack identifier of the
  ...
  - Must contain a valid universally unique identifier (UUID). For more information about
    UUIDs, see [Universally unique identifier](https://en.wikipedia.org/wiki/Universally_unique_identifier).
...
function describe_dbcluster_endpoints(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBClusterEndpoints"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbcluster_endpoints(
    ...
    return rds(
        "DescribeDBClusterEndpoints", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_dbcluster_parameter_groups(;
    ...
    return rds(
        "DescribeDBClusterParameterGroups"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"Filters"`: A filter that specifies one or more DB cluster parameters to describe.

  The only supported filter is `parameter-name`. The results list only includes information
  about the DB cluster parameters with these names.
...
function describe_dbcluster_snapshots(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBClusterSnapshots"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbcluster_snapshots(
    ...
    return rds(
        "DescribeDBClusterSnapshots", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"Filters"`: A filter that specifies one or more DB clusters to describe.
  ...
  - `clone-group-id` - Accepts clone group identifiers. The results list only includes
...
- `"IncludeShared"`: Specifies whether the output includes information about clusters shared
...
function describe_dbclusters(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBClusters"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_dbclusters(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeDBClusters", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- `"Filters"`: A filter that specifies one or more DB engine versions to describe.
  ...
  - `db-parameter-group-family` - Accepts parameter groups family names. The results list
    only includes information about the DB engine versions for these parameter group
    families.
  - `engine` - Accepts engine names. The results list only includes information about the DB
  ...
  - `engine-mode` - Accepts DB engine modes. The results list only includes information
    ...
    - `provisioned`
    - `serverless`
  - `engine-version` - Accepts engine versions. The results list only includes information
    about the DB engine versions for these engine versions.
  - `status` - Accepts engine version statuses. The results list only includes information
...
- `"ListSupportedCharacterSets"`: Specifies whether to list the supported character sets for
  each engine version.

  If this parameter is enabled and the requested engine supports the `CharacterSetName`
  parameter for `CreateDBInstance`, the response includes a list of supported character sets
...
function describe_dbengine_versions(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBEngineVersions"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbengine_versions(
    ...
    return rds(
        "DescribeDBEngineVersions", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
`DescribeDBInstanceAutomatedBackups` and [`describe_dbinstances`](@ref) operations.
...
- `"DBInstanceIdentifier"`: (Optional) The user-supplied instance identifier. If this
  ...
  returns information from the specific DB instance's automated backup. This parameter isn't
  case-sensitive.
...
- `"Filters"`: A filter that specifies which resources to return based on status.
  ...
  - `db-instance-id` - Accepts DB instance identifiers and Amazon Resource Names (ARNs). The
    results list includes only information about the DB instance automated backups
    identified by these ARNs.
  - `dbi-resource-id` - Accepts DB resource identifiers and Amazon Resource Names (ARNs).
    The results list includes only information about the DB instance resources identified by
    these ARNs.
...
function describe_dbinstance_automated_backups(;
    ...
    return rds(
        "DescribeDBInstanceAutomatedBackups"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"Filters"`: A filter that specifies one or more DB instances to describe.
  ...
  - `db-cluster-id` - Accepts DB cluster identifiers and DB cluster Amazon Resource Names
    (ARNs). The results list only includes information about the DB instances associated
  ...
  - `db-instance-id` - Accepts DB instance identifiers and DB instance Amazon Resource Names
    (ARNs). The results list only includes information about the DB instances identified by
    these ARNs.
...
function describe_dbinstances(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBInstances"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_dbinstances(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeDBInstances", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
"""
    describe_dbmajor_engine_versions()
    describe_dbmajor_engine_versions(params::Dict{String,<:Any})
...
- `"Engine"`: The database engine to return major version details for.
...
- `"MajorEngineVersion"`: A specific database major engine version to return details for.
...
- `"MaxRecords"`: The maximum number of records to include in the response. If more than the
  ...
  Default: 100
"""
function describe_dbmajor_engine_versions end

function describe_dbmajor_engine_versions(;
    aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeDBMajorEngineVersions"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbmajor_engine_versions(
    ...
    return rds(
        "DescribeDBMajorEngineVersions", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_dbparameter_groups(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBParameterGroups"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbparameter_groups(
    ...
    return rds(
        "DescribeDBParameterGroups", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"Filters"`: A filter that specifies one or more DB parameters to describe.
  ...
  The only supported filter is `parameter-name`. The results list only includes information
  about the DB parameters with these names.
...
- `"DBProxyName"`: The name of the DB proxy. If you omit this parameter, the output includes
...
function describe_dbproxies(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBProxies"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_dbproxies(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeDBProxies", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbproxy_endpoints(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBProxyEndpoints"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbproxy_endpoints(
    ...
    return rds(
        "DescribeDBProxyEndpoints", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"Filters"`: A filter that specifies one or more recommendations to describe.
  ...
  - `recommendation-id` - Accepts a list of recommendation identifiers. The results list
    only includes the recommendations whose identifier is one of the specified filter
    values.
  ...
  - `dbi-resource-id` - Accepts a list of database resource identifiers. The results list
    only includes the recommendations that generated for the specified databases.
  - `cluster-resource-id` - Accepts a list of cluster resource identifiers. The results list
    only includes the recommendations that generated for the specified clusters.
  - `pg-arn` - Accepts a list of parameter group ARNs. The results list only includes the
...
- `"LastUpdatedAfter"`: A filter to include only the recommendations that were updated after
  this specified time.
...
- `"Marker"`: An optional pagination token provided by a previous
  `DescribeDBRecommendations` request. If this parameter is specified, the response includes
  only records beyond the marker, up to the value specified by `MaxRecords`.
...
function describe_dbrecommendations(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBRecommendations"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbrecommendations(
    ...
    return rds(
        "DescribeDBRecommendations", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_dbsecurity_groups(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBSecurityGroups"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbsecurity_groups(
    ...
    return rds(
        "DescribeDBSecurityGroups", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"DBShardGroupIdentifier"`: The user-supplied DB shard group identifier. If this parameter
  is specified, information for only the specific DB shard group is returned. This parameter
  isn't case-sensitive.
...
function describe_dbshard_groups(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBShardGroups"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_dbshard_groups(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeDBShardGroups", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
snapshot that was taken from DB instance using the multi-tenant configuration, you restore
...
- `"Filters"`: A filter that specifies one or more tenant databases to describe.
  ...
  - `tenant-db-name` - Tenant database names. The results list only includes information
  ...
  - `dbi-resource-id` - DB instance resource identifiers. The results list only includes
    information about snapshots containing tenant databases contained within the DB
...
- `"SnapshotType"`: The type of DB snapshots to be returned. You can specify one of the
  ...
  - `automated` – All DB snapshots that have been automatically taken by Amazon RDS for my
    Amazon Web Services account.
...
function describe_dbsnapshot_tenant_databases(;
    ...
    return rds(
        "DescribeDBSnapshotTenantDatabases"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_dbsnapshots(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBSnapshots"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_dbsnapshots(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeDBSnapshots", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbsubnet_groups(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeDBSubnetGroups"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_dbsubnet_groups(
    ...
    return rds(
        "DescribeDBSubnetGroups", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"Filters"`: A filter that specifies one or more parameters to describe.
  ...
  The only supported filter is `parameter-name`. The results list only includes information
  about the parameters with these names.
...
function describe_event_categories(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeEventCategories"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_event_categories(
    ...
    return rds(
        "DescribeEventCategories", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_event_subscriptions(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeEventSubscriptions"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_event_subscriptions(
    ...
    return rds(
        "DescribeEventSubscriptions", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
snapshot, DB cluster snapshot group, or RDS Proxy can be obtained by providing the name as a
parameter.
...
function describe_events(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeEvents"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_events(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeEvents", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_export_tasks(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeExportTasks"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_export_tasks(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeExportTasks", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- `"Filters"`: A filter that specifies one or more global database clusters to describe.
  ...
  If used, the request returns information about any global cluster with at least one member
  (primary or secondary) in the specified Amazon Web Services Regions.
...
function describe_global_clusters(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeGlobalClusters"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_global_clusters(
    ...
    return rds(
        "DescribeGlobalClusters", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_integrations(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeIntegrations"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_integrations(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeIntegrations", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_option_groups(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeOptionGroups"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_option_groups(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeOptionGroups", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- `"EngineVersion"`: A filter to include only the available options for the specified engine
  version.

- `"Filters"`: This parameter isn't currently supported.

- `"LicenseModel"`: A filter to include only the available options for the specified license
  model.
...
- `"Vpc"`: Specifies whether to show only VPC or non-VPC offerings. RDS Custom supports only
  VPC offerings.
...
- `"Filters"`: A filter that specifies one or more resources to return pending maintenance
  ...
  - `db-cluster-id` - Accepts DB cluster identifiers and DB cluster Amazon Resource Names
    (ARNs). The results list only includes pending maintenance actions for the DB clusters
    identified by these ARNs.
...
function describe_pending_maintenance_actions(;
    ...
    return rds(
        "DescribePendingMaintenanceActions"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_reserved_dbinstances(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeReservedDBInstances"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_reserved_dbinstances(
    ...
    return rds(
        "DescribeReservedDBInstances", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_reserved_dbinstances_offerings(;
    ...
    return rds(
        "DescribeReservedDBInstancesOfferings"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_source_regions(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeSourceRegions"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_source_regions(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("DescribeSourceRegions", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_tenant_databases(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("DescribeTenantDatabases"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_tenant_databases(
    ...
    return rds(
        "DescribeTenantDatabases", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
!!! note
    ...
    `EnableHttpEndpoint` parameter of the [`modify_dbcluster`](@ref) operation.
...
!!! note
    ...
    publishing database logs to CloudWatch and then using the GetLogEvents operation. For
    more information, see [GetLogEvents](https://docs.aws.amazon.com/AmazonCloudWatchLogs/latest/APIReference/API_GetLogEvents.html)
...
- `"NumberOfLines"`: The number of lines to download. If the number of lines specified
  ...
  - You can download the log file in blocks of lines by specifying the size of the block
    ...
    parameter in your first request. Include the Marker value returned in the response as
    the Marker value for the next request, continuing until the AdditionalDataPending
...
- `"TargetDBInstanceIdentifier"`: The name of the DB instance to promote to the primary DB
  ...
  Specify the DB instance identifier for an Aurora Replica or a Multi-AZ readable standby in
...
!!! note
    ...
    database cluster, its intended use is for global database failover. To switch over a
    global database cluster, we recommend that you use the [`switchover_global_cluster`](@ref)
    operation instead.

How you use this operation depends on whether you are failing over or switching over your
global database cluster:

- Failing over - Specify the `AllowDataLoss` parameter and don't specify the `Switchover`
  parameter.
...
- *Failing over* - Use this operation to respond to an unplanned event, such as a Regional
  disaster in the primary Region. Failing over can result in a loss of write transaction
  data that wasn't replicated to the chosen secondary before the failover event occurred.
  However, the recovery process that promotes a DB instance on the chosen seconday DB
...
For more information about switching over an Amazon Aurora global database, see [Performing switchovers for Aurora global databases](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/aurora-global-database-disaster-recovery.html#aurora-global-database-disaster-recovery.managed-failover)
in the *Amazon Aurora User Guide*.
...
- `target_db_cluster_identifier`: The identifier of the secondary Aurora DB cluster that you
  want to promote to the primary for the global database cluster. Use the Amazon Resource
...
- `"AuditPolicyState"`: The audit policy state. When a policy is unlocked, it is read/write.
  When it is locked, it is read-only. You can edit your audit policy only when the activity
...
function modify_activity_stream(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("ModifyActivityStream"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function modify_activity_stream(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("ModifyActivityStream", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- You already migrated your applications to support the latest certificate authority (CA)
...
For more information about rotating your SSL/TLS certificate for Aurora DB engines, see [Rotating Your SSL/TLS Certificate](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/UsingWithRDS.SSL-certificate-rotation.html)
in the *Amazon Aurora User Guide*.
...
function modify_certificates(; aws_config::AbstractAWSConfig=current_aws_config())
    return rds("ModifyCertificates"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function modify_certificates(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rds("ModifyCertificates", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
cases, the capacity might not scale fast enough to meet a sudden change in workload, such as
a large number of new transactions. Call `ModifyCurrentDBClusterCapacity` to set the
capacity explicitly.
...
!!! note
    The MediaImport service that imports files from Amazon S3 to create CEVs isn't
    integrated with Amazon Web Services CloudTrail. If you turn on data logging for Amazon
    RDS in CloudTrail, calls to the `ModifyCustomDbEngineVersion` event aren't logged.
    However, you might see calls from the API gateway that accesses your Amazon S3 bucket.
...
- `"Status"`: The availability status to be assigned to the CEV. Valid values are as
  ...
  can't patch or create new instances with this CEV.
  ...
  example, you might change the status of your CEV from `available` to `inactive`, and from
  `inactive` back to `available`. To change the availability status of the CEV, it must not
  currently be in use by an RDS Custom instance, snapshot, or automated backup.
"""
...
- `"ApplyImmediately"`: Specifies whether the modifications in this request are
  asynchronously applied as soon as possible, regardless of the `PreferredMaintenanceWindow`
  setting for the DB cluster. If this parameter is disabled, changes to the DB cluster are
  applied during the next maintenance window.

  Most modifications can be applied immediately or during the next scheduled maintenance
  window. Some modifications, such as turning on deletion protection and changing the master
  password, are applied immediately—regardless of when you choose to apply them.
...
- `"DatabaseInsightsMode"`: Specifies the mode of Database Insights to enable for the DB
  cluster.
  ...
  `PerformanceInsightsEnabled` parameter to `true` to collect detailed database counter and
  per-query metrics.
...
- `"EnableLimitlessDatabase"`: Specifies whether to enable Aurora Limitless Database. You
  ...
  !!! note
      This setting is no longer used. Instead use the `ClusterScalabilityType` setting when
      you create your Aurora Limitless Database DB cluster.
...
- `"ManageMasterUserPassword"`: Specifies whether to manage the master user password with
  ...
  If the DB cluster doesn't manage the master user password with Amazon Web Services Secrets
...
- `"MasterUserAuthenticationType"`: Specifies the authentication type for the master user.
  With IAM master user authentication, you can change the master DB user to use IAM database
  authentication.
...
!!! important
    ...
    dynamic parameters are applied to the DB cluster immediately without a reboot. Changes
    to static parameters are applied only after the DB cluster is rebooted, which can be
    done using [`reboot_dbcluster`](@ref) operation. You can use the *Parameter Groups*
    option of the [Amazon RDS console](https://console.aws.amazon.com/rds/) or the [`describe_dbcluster_parameters`](@ref)
...
- `parameter`: A list of parameters in the DB cluster parameter group to modify.
  ...
  !!! note
      ...
      immediately to the DB clusters associated with the parameter group. When the
      application method is `pending-reboot`, changes to dynamic and static parameters are
      applied after a reboot without failover to the DB clusters associated with the
      parameter group.
...
the Amazon Web Services accounts that are authorized to restore the manual DB cluster
...
cluster snapshot, or whether a manual DB cluster snapshot is public or private, use the [`describe_dbcluster_snapshot_attributes`](@ref)
...
- `attribute_name`: The name of the DB cluster snapshot attribute to modify.
  ...
  To manage authorization for other Amazon Web Services accounts to copy or restore a manual
...
- `"ValuesToRemove"`: A list of DB cluster snapshot attributes to remove from the attribute
  ...
  To remove authorization for other Amazon Web Services accounts to copy or restore a manual
  DB cluster snapshot, set this list to include one or more Amazon Web Services account
  identifiers, or `all` to remove authorization for any Amazon Web Services account to copy
  or restore the DB cluster snapshot. If you specify `all`, an Amazon Web Services account
...
- `"AdditionalStorageVolumes"`: A list of additional storage volumes to modify or delete for
  the DB instance. You can create up to 3 additional storage volumes. Additional storage
  volumes are supported for RDS for Oracle and RDS for SQL Server DB instances only.

- `"AllocatedStorage"`: The new amount of storage in gibibytes (GiB) to allocate for the DB
  instance.

  For RDS for Db2, MariaDB, RDS for MySQL, RDS for Oracle, and RDS for PostgreSQL, the value
  supplied must be at least 10% greater than the current value. Values that are not at least
  10% greater than the existing value are rounded up so that they are 10% greater than the
current value.
  ...
  - When you increase the allocated storage for a DB instance that uses Provisioned IOPS
    (`gp3`, `io1`, or `io2` storage type), you must also specify the `Iops` parameter. You
...
- `"AutoMinorVersionUpgrade"`: Specifies whether minor version upgrades are applied
  ...
  For an RDS Custom DB instance, don't enable this setting. Otherwise, the operation returns
  an error.
...
- `"BackupRetentionPeriod"`: The number of days to retain automated backups. Setting this
  ...
  !!! note
      ...
      few seconds to a few minutes, depending on the size and class of your DB instance.

  These changes are applied during the next maintenance window unless the `ApplyImmediately`
  parameter is enabled for this request. If you change the parameter from one non-zero value
  to another non-zero value, the change is asynchronously applied as soon as possible.
...
- `"DBInstanceClass"`: The new compute and memory capacity of the DB instance, for example
  ...
  - If you are modifying the DB instance class and upgrading the engine version at the same
    time, the currently running engine version must be supported on the specified DB
    instance class. Otherwise, the operation returns an error. In this case, first run the
    operation to upgrade the engine version, and then run it again to modify the DB instance
    class.
...
- `"DBSubnetGroupName"`: The new DB subnet group for the DB instance. You can use this
  ...
  see [Working with a DB instance in a VPC](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_VPC.WorkingWithRDSInstanceinaVPC.html#USER_VPC.Non-VPC2VPC)
  in the *Amazon RDS User Guide*.
...
- `"EngineVersion"`: The version number of the database engine to upgrade to. Changing this
  ...
  If the instance that you're modifying is acting as a read replica, the engine version that
  you specify must be the same or higher than the version that the source DB instance or
  cluster is running.
  ...
  - If you are upgrading the engine version and modifying the DB instance class at the same
    time, the currently running engine version must be supported on the specified DB
    instance class. Otherwise, the operation returns an error. In this case, first run the
    operation to upgrade the engine version, and then run it again to modify the DB instance
    class.
...
- `"Iops"`: The new Provisioned IOPS (I/O operations per second) value for the RDS instance.
  ...
  If you choose to migrate your DB instance from using standard storage to Provisioned IOPS
  (io1), or from Provisioned IOPS to standard storage, the process can take time. The
  ...
  operations can take place for the instance, including modifying the instance, rebooting
  the instance, deleting the instance, creating a read replica for the instance, and
...
- `"ManageMasterUserPassword"`: Specifies whether to manage the master user password with
  ...
  - Can't specify for RDS for Oracle CDB instances in the multi-tenant configuration. Use
    `ModifyTenantDatabase` instead.
  - Can't specify the parameters `ManageMasterUserPassword` and `MultiTenant` in the same
    operation.
...
- `"MasterUserPassword"`: The new password for the master user.
  ...
  request, the `MasterUserPassword` element exists in the `PendingModifiedValues` element of
...
- `"MasterUserSecretKmsKeyId"`: The Amazon Web Services KMS key identifier to encrypt a
  ...
  If the DB instance already manages the master user password in Amazon Web Services Secrets
  Manager, you can't change the KMS key used to encrypt the secret.
  - You are turning on `ManageMasterUserPassword` to manage the master user password in
    Amazon Web Services Secrets Manager.
...
- `"MultiTenant"`: Specifies whether the to convert your DB instance from the single-tenant
  ...
  !!! important
      The conversion to the multi-tenant configuration is permanent and irreversible, so you
      can't later convert back to the single-tenant configuration. When you specify this
...
- `"PreferredBackupWindow"`: The daily time range during which automated backups are created
  ...
  Changing this parameter doesn't result in an outage and the change is asynchronously
  applied as soon as possible. The default is a 30-minute window selected at random from an
  8-hour block of time for each Amazon Web Services Region. For more information, see [Backup window](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_WorkingWithAutomatedBackups.html#USER_WorkingWithAutomatedBackups.BackupWindow)
in the *Amazon RDS User Guide*.
...
- `"PreferredMaintenanceWindow"`: The weekly time range during which system maintenance can
  ...
  reboot of the DB instance. If you change this window to the current time, there must be at
...
- `"ReplicaMode"`: The open mode of a replica database.
...
- `"RotateMasterUserPassword"`: Specifies whether to rotate the secret managed by Amazon Web
  ...
  This setting is valid only if the master user password is managed by RDS in Amazon Web
  Services Secrets Manager for the DB instance. The secret value contains the updated
  password.
...
- `"StorageType"`: The storage type to associate with the DB instance.
  ...
  factors such as database load, storage size, storage type (standard or Provisioned IOPS),
  amount of IOPS provisioned (if any), and the number of prior scale storage operations.
  Typical migration times are under 24 hours, but the process can take up to several days in
  some cases. During the migration, the DB instance is available for use, but might
  experience performance degradation. While the migration takes place, nightly backups for
  the instance are suspended. No other Amazon RDS operations can take place for the
  instance, including modifying the instance, rebooting the instance, deleting the instance,
...
!!! important
    ...
    parameter group. This allows Amazon RDS to fully complete the modify operation before
    the parameter group is used as the default for a new DB instance. This is especially
...
- `parameter`: An array of parameter names, values, and the application methods for the
  ...
  For more information on modifying DB parameters, see [Working with DB parameter groups](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_WorkingWithParamGroups.html)
  in the *Amazon RDS User Guide*.
...
- `dbproxy_endpoint_name`: The name of the DB proxy sociated with the DB proxy endpoint that
  you want to modify.
...
- `"NewDBProxyEndpointName"`: The new identifier for the `DBProxyEndpoint`. An identifier
  ...
  can't end with a hyphen or contain two consecutive hyphens.
- `"VpcSecurityGroupIds"`: The VPC security group IDs for the DB proxy endpoint. When the DB
  proxy endpoint uses a different VPC than the original proxy, you also specify a different
...
- `"ComputeRedundancy"`: Specifies whether to create standby DB shard groups for the DB
...
- `"EngineVersion"`: The engine version to upgrade the DB snapshot to.
  ...
  **MariaDB**
  ...
  For the list of engine versions that are available for upgrading a DB snapshot, see [Upgrading a MariaDB DB snapshot engine version](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/mariadb-upgrade-snapshot.html)
  in the *Amazon RDS User Guide.*
  ...
  **Oracle**
  ...
  - `21.0.0.0.ru-2025-04.rur-2025-04.r1` (supported for 21.0.0.0.ru-2022-01.rur-2022-01.r1,
    21.0.0.0.ru-2022-04.rur-2022-04.r1, 21.0.0.0.ru-2022-07.rur-2022-07.r1, 21.0.0.0.ru-
2022-10.rur-2022-10.r1, 21.0.0.0.ru-2023-01.rur-2023-01.r1 and 21.0.0.0.ru-2023-01.rur-2023-
01.r2 DB snapshots)
  - `19.0.0.0.ru-2025-04.rur-2025-04.r1` (supported for 19.0.0.0.ru-2019-07.rur-2019-07.r1,
  ...
  **PostgreSQL**
  ...
  For the list of engine versions that are available for upgrading a DB snapshot, see [Upgrading a PostgreSQL DB snapshot engine version](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_UpgradeDBSnapshot.PostgreSQL.html)
  in the *Amazon RDS User Guide.*
...
`all` to make the manual DB snapshot public, which means it can be copied or restored by all
...
To view which Amazon Web Services accounts have access to copy or restore a manual DB
snapshot, or whether a manual DB snapshot public or private, use the [`describe_dbsnapshot_attributes`](@ref)
API operation. The accounts are returned as values for the `restore` attribute.
...
- `"ValuesToRemove"`: A list of DB snapshot attributes to remove from the attribute
  ...
  `all` to remove authorization for any Amazon Web Services account to copy or restore the
  DB snapshot. If you specify `all`, an Amazon Web Services account whose account ID is
  explicitly added to the `restore` attribute can still copy or restore the manual DB
  snapshot.
"""
...
- `global_cluster_identifier`: The cluster identifier for the global cluster to modify. This
  parameter isn't case-sensitive.
...
- `"ApplyImmediately"`: Specifies whether to apply the change immediately or during the next
...
- `"MasterUserPassword"`: The new password for the master user of the specified tenant
  ...
  !!! note
      ...
      regain access to a tenant database user if the password is lost. This includes
      restoring privileges that might have been accidentally revoked.
...
- `"MasterUserSecretKmsKeyId"`: The Amazon Web Services KMS key identifier to encrypt a
  ...
  - You're turning on `ManageMasterUserPassword` to manage the master user password in
    Amazon Web Services Secrets Manager.

  If you're turning on `ManageMasterUserPassword` and don't specify
  `MasterUserSecretKmsKeyId`, then the `aws/secretsmanager` KMS key is used to encrypt the
  secret. If the secret is in a different Amazon Web Services account, then you can't use
  the `aws/secretsmanager` KMS key to encrypt the secret, and you must use a self-managed
  KMS key.
...
!!! note
    - Backup duration is a function of the amount of changes to the database since the
      ...
      recommend that you enable backups and complete at least one backup prior to promotion.
      In addition, a read replica cannot be promoted to a standalone instance when it is in
      the `backing-up` status. If you have enabled backups on your read replica, configure
      the automated backup window so that daily backups do not interfere with read replica
      promotion.
...
- `"PreferredBackupWindow"`: The daily time range during which automated backups are created
  ...
  The default is a 30-minute window selected at random from an 8-hour block of time for each
  Amazon Web Services Region. To see the time blocks available, see [Adjusting the Preferred Maintenance Window](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/AdjustingTheMaintenanceWindow.html)
  in the *Amazon RDS User Guide.*
...
If your DB instance is part of a Multi-AZ DB cluster, you can reboot the DB cluster with the [`reboot_dbcluster`](@ref)
operation.
...
if you make certain modifications, reboot the DB shard group for the changes to take effect.
...
- `role_arn`: The Amazon Resource Name (ARN) of the IAM role to disassociate from the Aurora
...
RDS must be authorized to access the Amazon S3 bucket and the data must be created using the
Percona XtraBackup utility as described in [Migrating Data from MySQL by Using an Amazon S3 Bucket](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/AuroraMySQL.Migrating.ExtMySQL.html#AuroraMySQL.Migrating.ExtMySQL.S3)
in the *Amazon Aurora User Guide*.

!!! note
    This operation only restores the DB cluster, not the DB instances for that DB cluster.
    You must invoke the [`create_dbinstance`](@ref) operation to create DB instances for the
    restored DB cluster, specifying the identifier of the restored DB cluster in
    `DBClusterIdentifier`. You can create DB instances only after the [`restore_dbcluster_from_s3`](@ref)
...
- `"CopyTagsToSnapshot"`: Specifies whether to copy all tags from the restored DB cluster to
...
- `"EngineLifecycleSupport"`: The life cycle type for this DB cluster.
  ...
  !!! note
      ...
      your DB cluster into Amazon RDS Extended Support. At the end of standard support, you
      can avoid charges for Extended Support by setting the value to
      `open-source-rds-extended-support-disabled`. In this case, RDS automatically upgrades
      your restored DB cluster to a higher engine version, if the major engine version is
...
- `"PreferredMaintenanceWindow"`: The weekly time range during which system maintenance can
  ...
  The default is a 30-minute window selected at random from an 8-hour block of time for each
  Amazon Web Services Region, occurring on a random day of the week. To see the time blocks
  available, see [Adjusting the Preferred Maintenance Window](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_UpgradeDBInstance.Maintenance.html#AdjustingTheMaintenanceWindow.Aurora)
  in the *Amazon Aurora User Guide*.
...
`EnableVPCNetworking` to `false` to disable the VPC network interface (ENI) for the cluster.
...
!!! note
    ...
    You must invoke the [`create_dbinstance`](@ref) operation to create DB instances for the
    restored DB cluster, specifying the identifier of the restored DB cluster in
    `DBClusterIdentifier`. You can create DB instances only after the [`restore_dbcluster_from_snapshot`](@ref)
    operation has completed and the DB cluster is available.
...
- `"DBClusterInstanceClass"`: The compute and memory capacity of the each DB instance in the
...
- `"EnableInternetAccessGateway"`: Specifies that the restored DB cluster should use
  internet-based connectivity through an internet access gateway. This allows clients to
  connect to the cluster over the internet without requiring a VPC.

  This parameter must be used together with `EnableVPCNetworking` set to `false`. When both
  parameters are specified, IAM database authentication is required. You must also specify
...
- `"EnableVPCNetworking"`: Specifies whether to enable VPC networking for the restored DB
  cluster. Set this parameter to `false` to create a cluster without the VPC network
  interface (ENI).

  This parameter must be used together with `EnableInternetAccessGateway`. When both
  parameters are specified, IAM database authentication is required. You must also specify
  `EnableIAMDatabaseAuthentication`.
...
- `"KmsKeyId"`: The Amazon Web Services KMS key identifier to use when restoring an
  ...
  - If the DB snapshot or DB cluster snapshot in `SnapshotIdentifier` isn't encrypted, then
    the restored DB cluster isn't encrypted.
...
- `"PubliclyAccessible"`: Specifies whether the DB cluster is publicly accessible.
  ...
  to the private IP address from within the DB cluster's virtual private cloud (VPC). It
  resolves to the public IP address from outside of the DB cluster's VPC. Access to the DB
  cluster is ultimately controlled by the security group it uses. That public access is not
  permitted if the security group assigned to the DB cluster doesn't permit it.
...
Unless the `RestoreType` is set to `copy-on-write`, the restore may occur in a different
Availability Zone (AZ) from the original DB cluster. The AZ where RDS restores the DB
...
!!! note
    ...
    DB cluster. You must invoke the [`create_dbinstance`](@ref) operation to create DB
    instances for the restored DB cluster, specifying the identifier of the restored DB
    cluster in `DBClusterIdentifier`. You can create DB instances only after the [`restore_dbcluster_to_point_in_time`](@ref)
    operation has completed and the DB cluster is available.
...
- `"EngineMode"`: The engine mode of the new cluster. Specify `provisioned` or `serverless`,
  ...
  must be an Aurora Serverless v1 cluster or an encrypted provisioned cluster. To create a
  full copy that is an Aurora Serverless v1 cluster, specify the engine mode `serverless`.
...
database restore point with most of the source's original configuration, including the
default security group and DB parameter group. By default, the new DB instance is created as
a Single-AZ deployment, except when the instance is a SQL Server instance that has an option
group associated with mirroring. In this case, the instance becomes a Multi-AZ deployment,
not a Single-AZ deployment.

If you want to replace your original DB instance with the new, restored DB instance, then
rename your original DB instance before you call the [`restore_dbinstance_from_dbsnapshot`](@ref)
...
of the DB instance as the `DBInstanceIdentifier` in the call to the [`restore_dbinstance_from_dbsnapshot`](@ref)
...
snapshot engine version, see [Upgrading a MySQL DB snapshot engine version](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/mysql-upgrade-snapshot.html).
...
- `"CopyTagsToSnapshot"`: Specifies whether to copy all tags from the restored DB instance
  ...
  In most cases, tags aren't copied by default. However, when you restore a DB instance from
  a DB snapshot, RDS checks whether you specify new tags. If yes, the new tags are added to
  the restored DB instance. If there are no new tags, RDS looks for the tags from the source
  DB instance for the DB snapshot, and then adds those tags to the restored DB instance.
...
- `"EngineLifecycleSupport"`: The life cycle type for this DB instance.
  ...
  !!! note
      ...
      can avoid charges for Extended Support by setting the value to
      `open-source-rds-extended-support-disabled`. In this case, RDS automatically upgrades
      your restored DB instance to a higher engine version, if the major engine version is
      past its end of standard support date.
...
- `"Iops"`: Specifies the amount of provisioned IOPS for the DB instance, expressed in I/O
  ...
  information, see [Amazon RDS Provisioned IOPS storage](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_Storage.html#USER_PIOPS)
  in the *Amazon RDS User Guide.*
...
- `"ManageMasterUserPassword"`: Specifies whether to manage the master user password with
  Amazon Web Services Secrets Manager in the restored DB instance.
...
DB instance running MySQL. For more information, see [Restoring a backup into an Amazon RDS for MySQL DB instance](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/MySQL.Procedural.Importing.html)
in the *Amazon RDS User Guide.*
...
- `"AdditionalStorageVolumes"`: A list of additional storage volumes to modify or delete for
  the DB instance. You can modify or delete up to three additional storage volumes using the
  names `rdsdbdata2`, `rdsdbdata3`, and `rdsdbdata4`. Additional storage volumes are
...
- `"EngineLifecycleSupport"`: The life cycle type for this DB instance.
  ...
  You can use this setting to enroll your DB instance into Amazon RDS Extended Support. With
  RDS Extended Support, you can run the selected major engine version on your DB instance
  past the end of standard support for that engine version. For more information, see [Amazon RDS Extended Support Amazon RDS](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/extended-support.html)
  in the *Amazon RDS User Guide*.
...
- `"PerformanceInsightsRetentionPeriod"`: The number of days to retain Performance Insights
  data. The default is 7 days. The following values are valid:
...
- `"AdditionalStorageVolumes"`: A list of additional storage volumes to restore to the DB
  instance. You can restore up to three additional storage volumes using the names
  `rdsdbdata2`, `rdsdbdata3`, and `rdsdbdata4`. Additional storage volumes are supported for
  RDS for Oracle and RDS for SQL Server DB instances only.
...
- `"AvailabilityZone"`: The Availability Zone (AZ) where the DB instance will be created.
  ...
  - You can't specify the `AvailabilityZone` parameter if the DB instance is a Multi-AZ
    deployment.
...
- `"UseLatestRestorableTime"`: Specifies whether the DB instance is restored from the latest
...
see [Monitoring Amazon Aurora with Database Activity Streams](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/DBActivityStreams.html)
...
the stop-db-cluster CLI command, or the [`stop_dbcluster`](@ref) operation.
...
stop-db-instance CLI command, or the [`stop_dbinstance`](@ref) operation.
...
`start-activity-stream` CLI command, or the [`start_activity_stream`](@ref) operation.
...
- `resource_arn`: The Amazon Resource Name (ARN) of the DB cluster for the database activity
...
restore if necessary. The instance restarts automatically after 7 days.
...
- `blue_green_deployment_identifier`: The resource ID of the blue/green deployment.
...
- `global_cluster_identifier`: The identifier of the global database cluster to switch over.
  ...
  - Must match the identifier of an existing global database cluster (Aurora global
    database).
