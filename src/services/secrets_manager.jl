...
To choose which secrets to retrieve, you can specify a list of secrets by name or ARN, or
you can use filters. If Secrets Manager encounters errors such as `AccessDeniedException`
while attempting to retrieve any of the secrets, you can see the errors in `Errors` in the
response.

Secrets Manager generates CloudTrail `GetSecretValue` log entries for each secret you
request when you call this action. Do not include sensitive information in request
parameters because it might be logged. For more information, see [Logging Secrets Manager events with CloudTrail](https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieve-ct-entries.html).
...
`kms:Decrypt` permissions for the keys. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
...
- `"Filters"`: The filters to choose which secrets to retrieve. You must include `Filters`
...
- `"SecretIdList"`: The ARN or names of the secrets to retrieve. You must include `Filters`
  or `SecretIdList`, but not both.
"""
batch_get_secret_value(; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("BatchGetSecretValue"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
batch_get_secret_value(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("BatchGetSecretValue", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
If you cancel a rotation in progress, it can leave the `VersionStage` labels in an
unexpected state. You might need to remove the staging label `AWSPENDING` from the
partially created version. You also need to determine whether to roll back to the previous
version of the secret by moving the staging label `AWSCURRENT` to the version that has
`AWSPENDING`. To determine which version has a specific staging label, call
<a>ListSecretVersionIds</a>. Then use <a>UpdateSecretVersionStage</a> to change staging
 ...
 **Required permissions: ** `secretsmanager:CancelRotateSecret`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
...
"""
cancel_rotate_secret(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("CancelRotateSecret", Dict{String, Any}("SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
cancel_rotate_secret(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("CancelRotateSecret", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)

"""
    create_secret(name)
    create_secret(name, params::Dict{String,<:Any})

Creates a new secret. A *secret* can be a password, a set of credentials such as a user
name and password, an OAuth token, or other secret information that you store in an
encrypted form in Secrets Manager. The secret also includes the connection information to
access a database or other service, which Secrets Manager doesn't encrypt. A secret in
Secrets Manager consists of both the protected secret data and the important information
needed to manage the secret.

For secrets that use *managed rotation*, you need to create the secret through the managing
service. For more information, see [Secrets Manager secrets managed by other Amazon Web Services services](https://docs.aws.amazon.com/secretsmanager/latest/userguide/service-linked-secrets.html).
...
secret, you must make sure the JSON you store in the `SecretString` matches the [JSON structure of a database secret](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_secret_json_structure.html).
...
the API, then you can't use `aws/secretsmanager` to encrypt the secret, and you must create
and use a customer managed KMS key.

Secrets Manager generates a CloudTrail log entry when you call this action. Do not include
sensitive information in request parameters except `SecretBinary` or `SecretString` because
...
secret, you also need `secretsmanager:TagResource`. To add replica Regions, you must also
have `secretsmanager:ReplicateSecretToRegions`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
...
being accessed or utilities having access to your command parameters. This is a concern if
the command includes the value of a secret. Learn how to [Mitigate the risks of using command-line tools to store Secrets Manager secrets](https://docs.aws.amazon.com/secretsmanager/latest/userguide/security_cli-exposure-risks.html).
...
- `"ClientRequestToken"`: If you include `SecretString` or `SecretBinary`, then Secrets
  ...
  random UUID for you and includes it as the value for this parameter in the request.If you
  ...
  generate a `ClientRequestToken` and include it in the request.
   ...
   <p>This value helps ensure idempotency. Secrets Manager uses this value to prevent the
  ...
  `ClientRequestToken` value isn't already associated with a version of the secret then a
  ...
  because you cannot modify an existing version. Instead, use <a>PutSecretValue</a> to
...
- `"KmsKeyId"`: The ARN, key ID, or alias of the KMS key that Secrets Manager uses to
  ...
  If the secret is in a different Amazon Web Services account from the credentials calling
  the API, then you can't use `aws/secretsmanager` to encrypt the secret, and you must
...
- `"SecretString"`: The text data to encrypt and store in this new version of the secret.
  ...
  stores the information as a JSON structure of key/value pairs that a Lambda rotation
  function can parse.
...
"""
create_secret(Name; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("CreateSecret", Dict{String, Any}("Name"=>Name, "ClientRequestToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_secret(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("CreateSecret", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name, "ClientRequestToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
a secret, use <a>PutResourcePolicy</a>.
...
- `secret_id`: The ARN or name of the secret to delete the attached resource-based policy
  ...
  For an ARN, we recommend that you specify a complete ARN rather than a partial ARN. See [Finding a secret from a partial ARN](https://docs.aws.amazon.com/secretsmanager/latest/userguide/troubleshoot.html#ARN_secretnamehyphen).
"""
delete_resource_policy(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("DeleteResourcePolicy", Dict{String, Any}("SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_resource_policy(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("DeleteResourcePolicy", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
delete the replicas using <a>RemoveRegionsFromReplication</a>, and then delete the primary
secret. When you delete a replica, it is deleted immediately.
...
window. For more information, see [ Monitor secrets scheduled for deletion](https://docs.aws.amazon.com/secretsmanager/latest/userguide/monitoring_cloudwatch_deleted-secrets.html).
...
At any time before recovery window ends, you can use <a>RestoreSecret</a> to remove the
...
first cancel the deletion with <a>RestoreSecret</a> and then you can retrieve the secret.

Secrets Manager generates a CloudTrail log entry when you call this action. Do not include
sensitive information in request parameters because it might be logged. For more
information, see [Logging Secrets Manager events with CloudTrail](https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieve-ct-entries.html).

 **Required permissions: ** `secretsmanager:DeleteSecret`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
...
"""
delete_secret(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("DeleteSecret", Dict{String, Any}("SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_secret(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("DeleteSecret", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
 ...
 **Required permissions: ** `secretsmanager:DescribeSecret`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
...
- `secret_id`: The ARN or name of the secret.
  ...
  For an ARN, we recommend that you specify a complete ARN rather than a partial ARN. See [Finding a secret from a partial ARN](https://docs.aws.amazon.com/secretsmanager/latest/userguide/troubleshoot.html#ARN_secretnamehyphen).
"""
describe_secret(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("DescribeSecret", Dict{String, Any}("SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_secret(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("DescribeSecret", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"ExcludePunctuation"`: Specifies whether to exclude the following punctuation characters
  from the password: `! " # $ % &amp; ' ( ) * + , - . / : ; &lt; = &gt; ? @ [ \ ] ^ _ ` { | } ~`. If you don't include this switch, the password can contain punctuation.
...
"""
get_random_password(; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("GetRandomPassword"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_random_password(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("GetRandomPassword", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
more information about permissions policies attached to a secret, see [Permissions policies attached to a secret](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access_resource-policies.html).

Secrets Manager generates a CloudTrail log entry when you call this action. Do not include
sensitive information in request parameters because it might be logged. For more
information, see [Logging Secrets Manager events with CloudTrail](https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieve-ct-entries.html).

 **Required permissions: ** `secretsmanager:GetResourcePolicy`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
...
- `secret_id`: The ARN or name of the secret to retrieve the attached resource-based policy
  ...
  For an ARN, we recommend that you specify a complete ARN rather than a partial ARN. See [Finding a secret from a partial ARN](https://docs.aws.amazon.com/secretsmanager/latest/userguide/troubleshoot.html#ARN_secretnamehyphen).
"""
get_resource_policy(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("GetResourcePolicy", Dict{String, Any}("SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_resource_policy(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("GetResourcePolicy", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
secrets improves speed and reduces your costs. For more information, see [Cache secrets for your applications](https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieving-secrets.html).

To retrieve the previous version of a secret, use `VersionStage` and specify AWSPREVIOUS.
To revert to the previous version of a secret, call [UpdateSecretVersionStage](https://docs.aws.amazon.com/cli/latest/reference/secretsmanager/update-secret-version-stage.html).
...
"""
get_secret_value(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("GetSecretValue", Dict{String, Any}("SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_secret_value(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("GetSecretValue", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
different versions of a secret. For more information, see [ Secrets Manager concepts: Versions](https://docs.aws.amazon.com/secretsmanager/latest/userguide/getting-started.html#term_version).
...
- `secret_id`: The ARN or name of the secret whose versions you want to list.
...
"""
list_secret_version_ids(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("ListSecretVersionIds", Dict{String, Any}("SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_secret_version_ids(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("ListSecretVersionIds", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
 ...
 **Required permissions: ** `secretsmanager:ListSecrets`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
...
"""
list_secrets(; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("ListSecrets"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_secrets(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("ListSecrets", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
optional. For more information, see [Authentication and access control for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html)
 ...
 **Required permissions: ** `secretsmanager:PutResourcePolicy`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).

# Arguments

- `resource_policy`: A JSON-formatted string for an Amazon Web Services resource-based
  policy. For example policies, see [Permissions policy examples](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access_examples.html).
- `secret_id`: The ARN or name of the secret to attach the resource-based policy.
...
"""
put_resource_policy(ResourcePolicy, SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("PutResourcePolicy", Dict{String, Any}("ResourcePolicy"=>ResourcePolicy, "SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
put_resource_policy(ResourcePolicy, SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("PutResourcePolicy", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourcePolicy"=>ResourcePolicy, "SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
`AWSCURRENT` to this version. If this operation creates the first version for the secret,
then Secrets Manager automatically attaches the staging label `AWSCURRENT` to it. If this
operation moves the staging label `AWSCURRENT` from another version to this version, then
Secrets Manager also automatically moves the staging label `AWSPREVIOUS` to the version
that `AWSCURRENT` was removed from.

This operation is idempotent. If you call this operation with a `ClientRequestToken` that
matches an existing version's VersionId, and you specify the same secret data, the
operation succeeds but does nothing. However, if the secret data is different, then the
operation fails because you can't modify an existing version; you can only create new ones.

Secrets Manager generates a CloudTrail log entry when you call this action. Do not include
sensitive information in request parameters except `SecretBinary`, `SecretString`, or
`RotationToken` because it might be logged. For more information, see [Logging Secrets Manager events with CloudTrail](https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieve-ct-entries.html).
...
- `"ClientRequestToken"`: A unique identifier for the new version of the secret. </p>
  ...
  `SecretBinary` values are different from those in the request, then the request fails
  because you can't modify a secret version. You can only create new versions to store new
  secret values.
This value becomes the `VersionId` of the new version.
- `"RotationToken"`: A unique identifier that indicates the source of the request. For
  cross-account rotation (when you rotate a secret in one account by using a Lambda
  rotation function in another account) and the Lambda rotation function assumes an IAM
  role to call Secrets Manager, Secrets Manager validates the identity with the rotation
  token. For more information, see [How rotation works](https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotating-secrets.html).
...
"""
put_secret_value(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("PutSecretValue", Dict{String, Any}("SecretId"=>SecretId, "ClientRequestToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
put_secret_value(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("PutSecretValue", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId, "ClientRequestToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `secret_id`: The ARN or name of the secret.
"""
remove_regions_from_replication(RemoveReplicaRegions, SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("RemoveRegionsFromReplication", Dict{String, Any}("RemoveReplicaRegions"=>RemoveReplicaRegions, "SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
remove_regions_from_replication(RemoveReplicaRegions, SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("RemoveRegionsFromReplication", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("RemoveReplicaRegions"=>RemoveReplicaRegions, "SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
`kms:Decrypt` permission to the key. To encrypt the replicated secret with a KMS key other
than `aws/secretsmanager`, you need `kms:GenerateDataKey` and `kms:Encrypt` to the key. For
...
- `"ForceOverwriteReplicaSecret"`: Specifies whether to overwrite a secret with the same
  name in the destination Region. By default, secrets aren't overwritten.
"""
replicate_secret_to_regions(AddReplicaRegions, SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("ReplicateSecretToRegions", Dict{String, Any}("AddReplicaRegions"=>AddReplicaRegions, "SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
replicate_secret_to_regions(AddReplicaRegions, SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("ReplicateSecretToRegions", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AddReplicaRegions"=>AddReplicaRegions, "SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)

"""
    restore_secret(secret_id)
    restore_secret(secret_id, params::Dict{String,<:Any})

Cancels the scheduled deletion of a secret by removing the `DeletedDate` time stamp. You
can access a secret again after it has been restored.

Secrets Manager generates a CloudTrail log entry when you call this action. Do not include
sensitive information in request parameters because it might be logged. For more
information, see [Logging Secrets Manager events with CloudTrail](https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieve-ct-entries.html).

 **Required permissions: ** `secretsmanager:RestoreSecret`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).

# Arguments

- `secret_id`: The ARN or name of the secret to restore.

  For an ARN, we recommend that you specify a complete ARN rather than a partial ARN. See [Finding a secret from a partial ARN](https://docs.aws.amazon.com/secretsmanager/latest/userguide/troubleshoot.html#ARN_secretnamehyphen).
"""
restore_secret(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("RestoreSecret", Dict{String, Any}("SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
restore_secret(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("RestoreSecret", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
in the *Secrets Manager User Guide*. If you include the configuration parameters, the
...
then any later invocation of `RotateSecret` assumes that a previous rotation request is
still in progress and returns an error. When rotation is unsuccessful, the `AWSPENDING`
staging label might be attached to an empty secret version. For more information, see [Troubleshoot rotation](https://docs.aws.amazon.com/secretsmanager/latest/userguide/troubleshoot_rotation.html)
 ...
 **Required permissions: ** `secretsmanager:RotateSecret`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
You also need `lambda:InvokeFunction` permissions on the rotation function. For more
...
- `secret_id`: The ARN or name of the secret to rotate.
...
- `"RotateImmediately"`: Specifies whether to rotate the secret immediately or wait until
  ...
  rotate the secret, Secrets Manager tests the rotation configuration by running the [ `testSecret` step](https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotate-secrets_lambda-functions.html#rotate-secrets_lambda-functions-code)
...
- `"RotationLambdaARN"`: For secrets that use a Lambda rotation function to rotate, the ARN
  ...
  For secrets that use *managed rotation*, omit this field. For more information, see [Managed rotation](https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotate-secrets_managed.html)
  in the *Secrets Manager User Guide*.
- `"RotationRules"`: A structure that defines the rotation configuration for this secret.
"""
rotate_secret(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("RotateSecret", Dict{String, Any}("SecretId"=>SecretId, "ClientRequestToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
rotate_secret(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("RotateSecret", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId, "ClientRequestToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
to a primary secret in the replica Region.
...
"""
stop_replication_to_replica(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("StopReplicationToReplica", Dict{String, Any}("SecretId"=>SecretId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
stop_replication_to_replica(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("StopReplicationToReplica", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
operation appends tags to the existing list of tags.
...
in the *Amazon Web Services General Reference guide*.</p>
...
Denied error.Secrets Manager generates a CloudTrail log entry when you call this action. Do
not include sensitive information in request parameters because it might be logged. For
more information, see [Logging Secrets Manager events with CloudTrail](https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieve-ct-entries.html).

 <p> **Required permissions: ** `secretsmanager:TagResource`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
...
- `tags`: The tags to attach to the secret as a JSON text string argument. Each element in
  the list consists of a `Key` and a `Value`.

  For storing multiple values, we recommend that you use a JSON text string argument and
  specify key/value pairs. For more information, see [Specifying parameter values for the Amazon Web Services CLI](https://docs.aws.amazon.com/cli/latest/userguide/cli-usage-parameters.html)
  in the Amazon Web Services CLI User Guide.
"""
tag_resource(SecretId, Tags; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("TagResource", Dict{String, Any}("SecretId"=>SecretId, "Tags"=>Tags); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
tag_resource(SecretId, Tags, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("TagResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId, "Tags"=>Tags), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
returned and the secret metadata is unchanged.</p>
...
error.Secrets Manager generates a CloudTrail log entry when you call this action. Do not
include sensitive information in request parameters because it might be logged. For more
information, see [Logging Secrets Manager events with CloudTrail](https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieve-ct-entries.html).

 <p> **Required permissions: ** `secretsmanager:UntagResource`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
...
- `tag_keys`: A list of tag key names to remove from the secret. You don't specify the
  ...
  in the Amazon Web Services CLI User Guide.
"""
untag_resource(SecretId, TagKeys; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("UntagResource", Dict{String, Any}("SecretId"=>SecretId, "TagKeys"=>TagKeys); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
untag_resource(SecretId, TagKeys, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("UntagResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId, "TagKeys"=>TagKeys), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
secret value, you can also use <a>PutSecretValue</a>.
...
secret in that service. See [Secrets Manager secrets managed by other Amazon Web Services services](https://docs.aws.amazon.com/secretsmanager/latest/userguide/service-linked-secrets.html).

We recommend you avoid calling `UpdateSecret` at a sustained rate of more than once every
10 minutes. When you call `UpdateSecret` to update the secret value, Secrets Manager
...
Manager automatically moves the staging label `AWSCURRENT` to the new version. Then it
...
`VersionId`, the operation results in an error. You can't modify an existing version, you
can only create a new version. To remove a version, remove all staging labels from it. See
 ...
 **Required permissions: ** `secretsmanager:UpdateSecret`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
...
`kms:Encrypt` permission to the new key, Secrets Manager does not re-encrypt existing
secret versions with the new key. For more information, see [ Secret encryption and decryption](https://docs.aws.amazon.com/secretsmanager/latest/userguide/security-encryption.html).
...
- `"KmsKeyId"`: The ARN, key ID, or alias of the KMS key that Secrets Manager uses to
  ...
  more information, see [About aliases](https://docs.aws.amazon.com/kms/latest/developerguide/alias-about.html).
...
- `"SecretString"`: The text data to encrypt and store in the new version of the secret. We
  ...
  logging the information in this field.
"""
update_secret(SecretId; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("UpdateSecret", Dict{String, Any}("SecretId"=>SecretId, "ClientRequestToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_secret(SecretId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("UpdateSecret", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId, "ClientRequestToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
`AWSPREVIOUS` to the version that `AWSCURRENT` was removed from.If this action results in
the last label being removed from a version, then the version is considered to be
...
information, see [Logging Secrets Manager events with CloudTrail](https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieve-ct-entries.html).
 ...
 <p> **Required permissions: ** `secretsmanager:UpdateSecretVersionStage`. For more
information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).
...
- `"RemoveFromVersionId"`: The ID of the version that the staging label is to be removed
  ...
  a different version, then you must include this parameter and specify the version that
  the label is to be removed from. If the label is attached and you either do not specify
  this parameter, or the version ID does not match, then the operation fails.
"""
update_secret_version_stage(SecretId, VersionStage; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("UpdateSecretVersionStage", Dict{String, Any}("SecretId"=>SecretId, "VersionStage"=>VersionStage); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_secret_version_stage(SecretId, VersionStage, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("UpdateSecretVersionStage", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SecretId"=>SecretId, "VersionStage"=>VersionStage), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
The API performs three checks when validating the policy:</p> - Sends a call to [Zelkova](https://aws.amazon.com/blogs/security/protect-sensitive-data-in-the-cloud-with-automated-reasoning-zelkova/),
an automated reasoning engine, to ensure your resource policy does not allow broad access
to your secret, for example policies that use a wildcard for the principal.
...
information, see [Logging Secrets Manager events with CloudTrail](https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieve-ct-entries.html).
 ...
 <p> **Required permissions: ** `secretsmanager:ValidateResourcePolicy` and
`secretsmanager:PutResourcePolicy`. For more information, see [ IAM policy actions for Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_iam-permissions.html#reference_iam-permissions_actions)
and [Authentication and access control in Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access.html).

# Arguments

- `resource_policy`: A JSON-formatted string that contains an Amazon Web Services resource-
  based policy. The policy in the string identifies who can access or manage this secret
...
- `"SecretId"`: The ARN or name of the secret with the resource-based policy you want to
  validate.
"""
validate_resource_policy(ResourcePolicy; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("ValidateResourcePolicy", Dict{String, Any}("ResourcePolicy"=>ResourcePolicy); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
validate_resource_policy(ResourcePolicy, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = secrets_manager("ValidateResourcePolicy", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourcePolicy"=>ResourcePolicy), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
