...
- `"MaxResults"`: The maximum number of results to return per API request. For example, you
  submit a `ListItems` request with `MaxResults` set at 500. Although 2,000 items match
...
- `"x-amz-upload-availability"`: Indicates the availability of an object while it is still
  ...
  after some initial buffering but before the object is uploaded completely. If the value
