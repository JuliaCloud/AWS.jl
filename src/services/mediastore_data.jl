...
function delete_object(Path; aws_config::AbstractAWSConfig=current_aws_config())
    return mediastore_data(
        "DELETE", "/$(Path)"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function delete_object(
    ...
    return mediastore_data(
        "DELETE", "/$(Path)", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_object(Path; aws_config::AbstractAWSConfig=current_aws_config())
    return mediastore_data("HEAD", "/$(Path)"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_object(
    ...
    return mediastore_data(
        "HEAD", "/$(Path)", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `path`: The path (including the file name) where the object is stored in the container.
  ...
  `premium/canada`, the service creates a `canada` subfolder in the `premium` folder. You
  then have two subfolders, `usa` and `canada`, in the `premium` folder.
...
- `"Range"`: The range bytes of an object to retrieve. For more information about the
  `Range` header, see [http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.35](http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.35).
...
function get_object(Path; aws_config::AbstractAWSConfig=current_aws_config())
    return mediastore_data("GET", "/$(Path)"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function get_object(
    ...
    return mediastore_data(
        "GET", "/$(Path)", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_items(; aws_config::AbstractAWSConfig=current_aws_config())
    return mediastore_data("GET", "/"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_items(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return mediastore_data("GET", "/", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- `"Cache-Control"`: An optional `CacheControl` header that allows the caller to control the
  object's cache behavior. Headers can be passed in as specified in the HTTP at [https://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.9](https://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.9).
...
- `"x-amz-storage-class"`: Indicates the storage class of a `Put` request. Defaults to high-
  performance temporal storage class, and objects are persisted into durable storage shortly
  after being received.

- `"x-amz-upload-availability"`: Indicates the availability of an object while it is still
  uploading. If the value is set to `streaming`, the object is available for downloading
  after some initial buffering but before the object is uploaded completely. If the value is
  set to `standard`, the object is available for downloading only when it is uploaded
