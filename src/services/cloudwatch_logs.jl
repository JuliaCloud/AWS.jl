...
- `"nextToken"`:
"""
function describe_delivery_destinations(; aws_config::AbstractAWSConfig=current_aws_config())
    return cloudwatch_logs(
...
- `"unmask"`: Specify true to display the log event fields with all sensitive data unmasked
  ...
  signed into an account with the logs:Unmask permission.
"""
function get_log_record(logRecordPointer; aws_config::AbstractAWSConfig=current_aws_config())
    return cloudwatch_logs(
        "GetLogRecord",
