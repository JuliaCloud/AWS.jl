...
- `output_uri`: The location of an Amazon Simple Storage Service (Amazon S3) bucket or
  directory to store the batch prediction results. The following substrings are not
  allowed in the `s3 key` portion of the `outputURI` field: ':', '//', '/./', '/../'.
...
[`create_batch_prediction`](@ref) operations.
...
- `rdsdata`: The data specification of an Amazon RDS `DataSource`:
  ...
  - ServiceRole - A role (DataPipelineDefaultRole) assumed by the AWS Data Pipeline
    service to monitor the progress of the copy task from Amazon RDS to Amazon S3. For
    more information, see [Role templates](https://docs.aws.amazon.com/datapipeline/latest/DeveloperGuide/dp-iam-roles.html)
    for data pipelines.
  - SecurityInfo - The security information to use to access an RDS DB instance. You need
    ...
    access to the Amazon RDS instance. Specify a [`SubnetId`, `SecurityGroupIds`] pair for a VPC-based RDS DB instance.
...
- `role_arn`: The role that Amazon ML assumes on behalf of the user to create and activate
  a data pipeline in the user's account and copy data using the `SelectSqlQuery` query
...
- `"ComputeStatistics"`: The compute statistics for a `DataSource`. The statistics are
  ...
  statistics internally during `MLModel` training. This parameter must be set to `true`
  if the <code/>DataSource<code/> needs to be used for `MLModel` training.
...
- `"Parameters"`: A list of the training parameters in the `MLModel`. The list is
  ...
  The value is a double that ranges from `0` to `MAX_DOUBLE`. The default is to not use
  L2 normalization. This parameter can't be used when `L1` is specified. Use this
  parameter sparingly.

- `"Recipe"`: The data recipe for creating the `MLModel`. You must specify either the
  recipe or its URI. If you don't specify a recipe or its URI, Amazon ML creates a
  default.
...
- `"Verbose"`: Specifies whether the [`get_mlmodel`](@ref) operation should return
  `Recipe`.
...
You can use the [`get_batch_prediction`](@ref) operation to view the contents of the
...
You can use the [`get_data_source`](@ref) operation to view the contents of the updated
data element.
