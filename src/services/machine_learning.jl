...
- `tags`: The key-value pairs to use to create tags. If you specify a key without specifying
...
You can poll for status updates by using the [`get_batch_prediction`](@ref) operation and
...
- `output_uri`: The location of an Amazon Simple Storage Service (Amazon S3) bucket or
  ...
  about how to set permissions, see the [Amazon Machine Learning Developer Guide](https://docs.aws.amazon.com/machine-learning/latest/dg).
...
`CreateEvaluation`, or [`create_batch_prediction`](@ref) operations.
...
includes an error message in the `Message` attribute of the [`get_data_source`](@ref)
operation response.
...
- `rdsdata`: The data specification of an Amazon RDS `DataSource`:
  ...
  - ResourceRole - A role (DataPipelineDefaultResourceRole) assumed by an EC2 instance to
    carry out the copy task from Amazon RDS to Amazon Simple Storage Service (Amazon S3).
    For more information, see [Role templates](https://docs.aws.amazon.com/datapipeline/latest/DeveloperGuide/dp-iam-roles.html)
    for data pipelines.
  ...
  - SecurityInfo - The security information to use to access an RDS DB instance. You need to
    ...
    the Amazon RDS instance. Specify a [`SubnetId`, `SecurityGroupIds`] pair for a VPC-based RDS DB instance.
...
- `role_arn`: The role that Amazon ML assumes on behalf of the user to create and activate a
...
references data that can be used to perform either `CreateMLModel`, `CreateEvaluation`, or [`create_batch_prediction`](@ref)
operations.
...
`PENDING` states can be used to perform only `CreateMLModel`, `CreateEvaluation`, or [`create_batch_prediction`](@ref)
operations.
...
`GetDataSource` for an existing datasource and copy the values to a `CreateDataSource` call.
Change the settings that you want to change and make sure that all required fields have the
appropriate values.
...
`CreateMLModel`, `CreateEvaluation`, or [`create_batch_prediction`](@ref) operations.
...
location, along with a schema that describes the data items by name and type. The same
schema must be used for all of the data files referenced by the `DataSource`.
...
- `data_spec`: The data specification of a `DataSource`:
...
`DataSource` for an `Evaluation` contains values for the `Target Variable`. The `Evaluation`
compares the predicted result for each observation to the actual outcome and provides a
summary so that you know how effective the `MLModel` functions on the test data. Evaluation
generates a relevant performance metric, such as BinaryAUC, RegressionRMSE or
...
`PENDING`. After the `Evaluation` is created and ready for use, Amazon ML sets the status to
`COMPLETED`.

You can use the [`get_evaluation`](@ref) operation to check progress of the evaluation
...
Learning (Amazon ML) immediately returns and sets the `MLModel` status to `PENDING`. After
the `MLModel` has been created and ready is for use, Amazon ML sets the status to
`COMPLETED`.

You can use the [`get_mlmodel`](@ref) operation to check the progress of the `MLModel`
during the creation operation.
...
`CreateDataSourceFromS3`, or [`create_data_source_from_redshift`](@ref) operations.
...
After using the [`delete_batch_prediction`](@ref) operation, you can use the [`get_batch_prediction`](@ref)
operation to verify that the status of the `BatchPrediction` changed to DELETED.

**Caution:** The result of the [`delete_batch_prediction`](@ref) operation is irreversible.
...
After using the [`delete_data_source`](@ref) operation, you can use the [`get_data_source`](@ref)
operation to verify that the status of the `DataSource` changed to DELETED.

**Caution:** The results of the [`delete_data_source`](@ref) operation are irreversible.
...
After invoking the [`delete_evaluation`](@ref) operation, you can use the [`get_evaluation`](@ref)
operation to verify that the status of the `Evaluation` changed to `DELETED`.

**Caution:** The results of the [`delete_evaluation`](@ref) operation are irreversible.
...
After using the [`delete_mlmodel`](@ref) operation, you can use the [`get_mlmodel`](@ref)
operation to verify that the status of the `MLModel` changed to DELETED.

**Caution:** The result of the [`delete_mlmodel`](@ref) operation is irreversible.
...
function describe_batch_predictions(; aws_config::AbstractAWSConfig=current_aws_config())
    return machine_learning(
        "DescribeBatchPredictions"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_batch_predictions(
    ...
    return machine_learning(
        "DescribeBatchPredictions", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_data_sources(; aws_config::AbstractAWSConfig=current_aws_config())
    return machine_learning(
        "DescribeDataSources"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_data_sources(
    ...
    return machine_learning(
        "DescribeDataSources", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_evaluations(; aws_config::AbstractAWSConfig=current_aws_config())
    return machine_learning(
        "DescribeEvaluations"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_evaluations(
    ...
    return machine_learning(
        "DescribeEvaluations", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"FilterVariable"`: Use one of the following variables to filter a list of `MLModel`:
  ...
  - `IAMUser` - Sets the search criteria to the user account that invoked the `MLModel`
    creation.
  - `TrainingDataSourceId` - Sets the search criteria to the `DataSource` used to train one
    or more `MLModel`.
...
function describe_mlmodels(; aws_config::AbstractAWSConfig=current_aws_config())
    return machine_learning("DescribeMLModels"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_mlmodels(
    ...
    return machine_learning(
        "DescribeMLModels", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
You can use the [`get_batch_prediction`](@ref) operation to view the contents of the updated
data element.
...
You can use the [`get_data_source`](@ref) operation to view the contents of the updated data
element.
...
You can use the [`get_evaluation`](@ref) operation to view the contents of the updated data
element.
...
You can use the [`get_mlmodel`](@ref) operation to view the contents of the updated data
element.
...
- `"ScoreThreshold"`: The `ScoreThreshold` used in binary classification `MLModel` that
  ...
  the `MLModel`, such as `true`. Output values less than the `ScoreThreshold` receive a
