...
- `sns_topic_name`: Specifies the name of the Amazon SNS topic defined for notification of
...
- `trail_name_list`: Specifies a list of trail names, trail ARNs, or both, of the trails to
...
- `start_time`: Optionally specifies, in UTC, the start of the time range to look up public
...
- `end_time`: Specifies that only events that occur before or at the specified time are
  ...
  returned.
- `event_category`: Specifies the event category. If you do not specify an event category,
...
- `lookup_attributes`: Contains a list of lookup attributes. Currently the list can contain
  only one item.
...
# Keyword Parameters
- `advanced_event_selectors`:  Specifies the settings for advanced event selectors. You can
...
- `event_selectors`: Specifies the settings for your event selectors. You can configure up
