...
- `FeatureTypes`: A list of the types of analysis to perform. Add TABLES to the list to
...
# Required Parameters
- `JobId`: A unique identifier for the text-detection job. The JobId is returned from
...
# Optional Parameters
- `ClientRequestToken`: The idempotent token that you use to identify the start request. If
  you use the same token with multiple StartDocumentAnalysis requests, the same JobId is
...
- `ClientRequestToken`: The idempotent token that's used to identify the start request. If
  you use the same token with multiple StartDocumentTextDetection requests, the same JobId is
  returned. Use ClientRequestToken to prevent the same job from being accidentally started
  more than once. For more information, see Calling Amazon Textract Asynchronous Operations.
- `JobTag`: An identifier that you specify that's included in the completion notification
  published to the Amazon SNS topic. For example, you can use JobTag to identify the type of
