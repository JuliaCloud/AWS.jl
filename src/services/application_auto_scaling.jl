...
- `resource_id`: The identifier of the resource associated with the scalable target. This
  ...
  - Custom resources are not supported with a resource type. This parameter must specify the
    ...
    unique identifier is defined by the service provider. More information is available in
    our [GitHub repository](https://github.com/aws/aws-auto-scaling-custom-resource).
...
- `"IncludeNotScaledActivities"`: Specifies whether to include activities that aren't scaled
  (*not scaled activities*) in the response. Not scaled activities are activities that
  aren't completed or started for various reasons, such as preventing infinite scaling
...
policies because conflicts between these policies can cause undesirable behavior. For
example, if the step scaling policy initiates a scale-in activity before the target tracking
policy is ready to scale in, the scale-in activity will not be blocked. After the scale-in
...
!!! note
    ...
    below the minimum capacity or above the maximum capacity, Application Auto Scaling
    adjusts the capacity of the scalable target to place it within these bounds, even if you
    don't include the `MinCapacity` or `max_capacity` request parameters.

# Arguments

- `resource_id`: The identifier of the resource that is associated with the scalable target.
