...
- `resource_ids`: The identifier of the resource associated with the scalable target. This
  string consists of the resource type and unique identifier.   ECS service - The resource
  type is service and the unique identifier is the cluster name and service name. Example:
  service/default/sample-webapp.   Spot Fleet - The resource type is spot-fleet-request and
  the unique identifier is the Spot Fleet request ID. Example:
...
- `max_results`: The maximum number of scalable targets. This value can be between 1 and
  ...
  operation returns up to 50 results and a NextToken value, if applicable.
- `next_token`: The token for the next set of results.
- `resource_id`: The identifier of the resource associated with the scaling activity. This
  string consists of the resource type and unique identifier.   ECS service - The resource
  type is service and the unique identifier is the cluster name and service name. Example:
  service/default/sample-webapp.   Spot Fleet - The resource type is spot-fleet-request and
  the unique identifier is the Spot Fleet request ID. Example:
...
# Keyword Parameters
- `max_results`: The maximum number of scheduled action results. This value can be between
...
# Keyword Parameters
- `policy_type`: The policy type. This parameter is required if you are creating a scaling
  ...
  Guide.
- `step_scaling_policy_configuration`: A step scaling policy. This parameter is required if
...
# Keyword Parameters
- `max_capacity`: The maximum value that you plan to scale out to. When a scaling policy is
...
- `min_capacity`: The minimum value that you plan to scale in to. When a scaling policy is
...
- `role_arn`: This parameter is required for services that do not support service-linked
  ...
  roles.
- `suspended_state`: An embedded object that contains attributes and attribute values that
