...
- `data_set_publication_date`: The date a data set was published. For daily data sets,
  provide a date with day-level granularity for the desired day. For monthly data sets
  except those with prefix disbursed_amount, provide a date with month-level granularity for
  the desired month (the day value will be ignored). For data sets with prefix
  disbursed_amount, provide a date with day-level granularity for the desired day. For these
  data sets we will look backwards in time over the range of 31 days until the first data
  set is found (the latest one).

- `data_set_type`: The desired data set type.

  - <strong>customer_subscriber_hourly_monthly_subscriptions</strong> <p>From 2017-09-15 to
  ...
  - <strong>daily_business_usage_by_instance_type</strong>From 2017-09-15 to present:
    Available daily by 24:00 UTC.
  - <strong>daily_business_fees</strong>From 2017-09-15 to present: Available daily by 24:00
    UTC.
  - <strong>daily_business_free_trial_conversions</strong>From 2017-09-15 to present:
    Available daily by 24:00 UTC.
  - <strong>daily_business_new_instances</strong>From 2017-09-15 to present: Available daily
    by 24:00 UTC.
  - <strong>daily_business_new_product_subscribers</strong>From 2017-09-15 to present:
    Available daily by 24:00 UTC.
  - <strong>daily_business_canceled_product_subscribers</strong>From 2017-09-15 to present:
    Available daily by 24:00 UTC.
  ...
  - <strong>monthly_revenue_flexible_payment_schedule</strong>From 2018-11-15 to present:
    Available monthly on the 15th day of the month by 24:00 UTC.
  - <strong>disbursed_amount_by_product</strong>From 2017-09-15 to present: Available every
    30 days by 24:00 UTC.
  - <strong>disbursed_amount_by_instance_hours</strong>From 2017-09-15 to present: Available
    every 30 days by 24:00 UTC.
  - <strong>disbursed_amount_by_customer_geo</strong>From 2017-09-15 to present: Available
    every 30 days by 24:00 UTC.
  - <strong>disbursed_amount_by_age_of_uncollected_funds</strong>From 2017-09-15 to present:
  ...
  - <strong>sales_compensation_billed_revenue</strong>From 2017-09-15 to present: Available
...
specified SNS topic once the data is available. Returns a unique request identifier that can
be used to correlate requests with notifications from the SNS topic. Data sets will be
published in comma-separated values (CSV) format with the file name {data_set_type}_YYYY-MM-
DD'T'HH-mm-ss'Z'.csv. If a file with the same name already exists (e.g. if the same data set
is requested twice), the original file will be overwritten by the new file. Requires a Role
with an attached permissions policy providing Allow permissions for the following actions:
...
- `"destinationS3Prefix"`: *This target has been deprecated.* (Optional) The desired S3
  ...
  For example, if given the bucket name "mybucket" and the prefix "myprefix/mydatasets", the
  output file "outputfile" would be published to
  "s3://mybucket/myprefix/mydatasets/outputfile". If the prefix directory structure does not
  exist, it will be created. If no prefix is provided, the data set will be published to the
  S3 bucket root.
"""
