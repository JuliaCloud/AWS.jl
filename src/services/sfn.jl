...
"""
function describe_execution(executionArn; aws_config::AbstractAWSConfig=current_aws_config())
    return sfn(
        "DescribeExecution",
...
- `"traceHeader"`: Passes the X-Ray trace header. The trace header can also be passed in
  the request payload.
"""
function start_execution(stateMachineArn; aws_config::AbstractAWSConfig=current_aws_config())
    return sfn(
        "StartExecution",
        Dict{String,Any}("stateMachineArn" => stateMachineArn);
        aws_config=aws_config,
