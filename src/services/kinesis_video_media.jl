...
`GetMedia` requests to this endpoint using the [--endpoint-url parameter](https://docs.aws.amazon.com/cli/latest/reference/).When
you put media data (fragments) on a stream, Kinesis Video Streams stores each incoming
...
The following limits apply when using the `GetMedia` API: - A client can call `GetMedia` up
to five times per second per stream.
 - Kinesis Video Streams sends media data at a rate of up to 25 megabytes per second (or
200 megabits per second) during a `GetMedia` session.
...
addition to what the HTTP status code provides.
 - `x-amz-RequestId` HTTP header – if you want to report an issue to AWS, the support team
...
"""
get_media(StartSelector; aws_config::AbstractAWSConfig=current_aws_config()) = kinesis_video_media("POST", "/getMedia", Dict{String, Any}("StartSelector"=>StartSelector); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_media(StartSelector, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = kinesis_video_media("POST", "/getMedia", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("StartSelector"=>StartSelector), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
