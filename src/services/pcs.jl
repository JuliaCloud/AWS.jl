...
"""
function get_cluster(clusterIdentifier; aws_config::AbstractAWSConfig=global_aws_config())
    return pcs(
        "GetCluster",
...
- `"nextToken"`: The value of nextToken is a unique pagination token for each page of
  ...
  returns an HTTP 400 InvalidToken error.
"""
function list_queues(clusterIdentifier; aws_config::AbstractAWSConfig=global_aws_config())
    return pcs(
        "ListQueues",
        Dict{String,Any}("clusterIdentifier" => clusterIdentifier);
        aws_config=aws_config,
