...
- `"dataDeletionPolicy"`: The data deletion policy for the data source. You can set the
  data deletion policy to:   DELETE: Deletes all underlying data belonging to the data source
  from the vector store upon deletion of a knowledge base or data source resource. Note that
  the vector store itself is not deleted, only the underlying data. This flag is ignored if
  an Amazon Web Services account is deleted.   RETAIN: Retains all underlying data in your
...
see Delete prompts from the Prompt management tool and Delete a version of a prompt from
the Prompt management tool in the Amazon Bedrock User Guide.
...
# Optional Parameters
Optional parameters can be passed as a `params::Dict{String,<:Any}`. Valid keys are:
- `"promptVersion"`: The version of the prompt to delete.
"""
...
Retrieves information about a prompt or a version of it. For more information, see View
information about prompts using Prompt management and View information about a version of
your prompt in the Amazon Bedrock User Guide.
...
prompt. For more information, see View information about prompts using Prompt management in
...
- `flow_identifier`: The unique identifier of the flow.
- `name`: The name of the flow alias.
- `routing_configuration`: Contains information about the version to which to map the alias.
