...
Specifies the extended metrics and custom metrics that you want a CloudWatch RUM app monitor
...
- `app_monitor_name`: The name of the CloudWatch RUM app monitor that is sending the
  metrics.
- `destination`: The type of destination that you want to view metrics for. Valid values are
...
code snippet to add to your web application. For more information, see [How do I find a code snippet that I've already generated?](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-RUM-find-code-snippet.html)
...
Deletes a destination for CloudWatch RUM extended metrics, so that the specified app monitor
stops sending extended metrics to that destination.

# Arguments

- `app_monitor_name`: The name of the app monitor that is sending metrics to the destination
...
- `destination`: The destination to send the metrics to. Valid values are `CloudWatch` and
  ...
  permission to write to the experiment.
- `metric_definition`: A structure that contains the new definition that you want to use for
  this metric.
