...
- `"cluster_identifier"`: The cluster identifier. This parameter is required when
  connecting to a cluster and authenticating using either Secrets Manager or temporary
  credentials.
...
function batch_execute_statement(
    Database, Sqls; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...
)
...
function describe_table(
    Database; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...
)
    params = amazonify(MAPPING, kwargs)
    return redshift_data(
        "DescribeTable",
...
- `"secret_arn"`: The name or ARN of the secret that enables access to the database. This
  parameter is required when authenticating using Secrets Manager.
- `"statement_name"`: The name of the SQL statement. You can name the SQL statement when
  you create it to identify the query.
- `"with_event"`: A value that indicates whether to send an event to the Amazon EventBridge
  event bus after the SQL statement runs.
"""
function execute_statement(
    Database, Sql; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...
)
    params = amazonify(MAPPING, kwargs)
    return redshift_data(
        "ExecuteStatement",
