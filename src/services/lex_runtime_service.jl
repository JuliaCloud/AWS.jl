...
"""
delete_session(botAlias, botName, userId; aws_config::AbstractAWSConfig=current_aws_config()) = lex_runtime_service("DELETE", "/bot/$(botName)/alias/$(botAlias)/user/$(userId)/session"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_session(botAlias, botName, userId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = lex_runtime_service("DELETE", "/bot/$(botName)/alias/$(botAlias)/user/$(userId)/session", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"checkpointLabelFilter"`: A string used to filter the intents returned in the
  ...
  string are returned.
"""
get_session(botAlias, botName, userId; aws_config::AbstractAWSConfig=current_aws_config()) = lex_runtime_service("GET", "/bot/$(botName)/alias/$(botAlias)/user/$(userId)/session/"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_session(botAlias, botName, userId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = lex_runtime_service("GET", "/bot/$(botName)/alias/$(botAlias)/user/$(userId)/session/", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
machine learning model that it built for the bot.
...
following example messages:  - For a user input "I would like a pizza," Amazon Lex might
return a response with a message eliciting slot data (for example, `PizzaSize`): "What size
...
response that you can use to enhance client behavior, such as displaying the appropriate
client user interface. Consider the following examples:  - If the message is to elicit slot
data, Amazon Lex returns the following context information:  <ul> <li> `x-amz-lex-dialog-
state` header set to `ElicitSlot`
...
current values
 </li> <li> If the message is a confirmation prompt, the `x-amz-lex-dialog-state` header is
set to `Confirmation` and the `x-amz-lex-slot-to-elicit` header is omitted.  </li> <li> If
the message is a clarification prompt configured for the intent, indicating that the user
intent is not understood, the `x-amz-dialog-state` header is set to `ElicitIntent` and the
`x-amz-slot-to-elicit` header is omitted.  </li> </ul> In addition, Amazon Lex also returns
your application-specific `sessionAttributes`. For more information, see [Managing Conversation Context](https://docs.aws.amazon.com/lex/latest/dg/context-mgmt.html).

# Arguments

- `content-_type`:  You pass this value as the `Content-Type` HTTP header.

   Indicates the audio format or text. The header value must start with one of the
  following prefixes:  - PCM format, audio data must be in little-endian byte order. <ul>
  <li>audio/l16; rate=16000; channels=1
...
- `user_id`: The ID of the client application user. Amazon Lex uses this to identify a
  user's conversation with your bot. At runtime, each request must contain the `userID`
  field.
  ...
  of the same bot. If you anticipate that a user will need to have conversation with two
...
- `"Accept"`:  You pass this value as the `Accept` HTTP header.
  ...
  `Accept` HTTP header value in the request.  - If the value is `text/plain; charset=utf-
  8`, Amazon Lex returns text in the response.
   - If the value begins with `audio/`, Amazon Lex returns speech in the response. Amazon
  Lex uses Amazon Polly to generate the speech (using the configuration you specified in
  the `Accept` header). For example, if you specify `audio/mpeg` as the value, Amazon Lex
...
- `"x-amz-lex-request-attributes"`: You pass this value as the `x-amz-lex-request-
  ...
  total size of the `requestAttributes` and `sessionAttributes` headers is limited to 12
  KB.
...
- `"x-amz-lex-session-attributes"`: You pass this value as the `x-amz-lex-session-
  attributes` HTTP header.
...
`responseCard` to display. Consider the following example messages:  - For a user input "I
would like a pizza", Amazon Lex might return a response with a message eliciting slot data
...
`intentName`, and `slots` fields in the response. Consider the following examples:  - If
the message is to elicit slot data, Amazon Lex returns the following context information:
<ul> <li> `dialogState` set to ElicitSlot
 - `intentName` set to the intent name in the current context
 - `slotToElicit` set to the slot name for which the `message` is eliciting information
 - `slots` set to a map of slots, configured for the intent, with currently known values
 </li> <li> If the message is a confirmation prompt, the `dialogState` is set to
ConfirmIntent and `SlotToElicit` is set to null.  </li> <li>If the message is a
clarification prompt (configured for the intent) that indicates that user intent is not
...
- `"activeContexts"`: A list of contexts active for the request. A context can be activated
  when a previous intent is fulfilled, or by including the context in the request,
...
- `"sessionAttributes"`: Application-specific information passed between Amazon Lex and a
  ...
  For more information, see [Setting Session Attributes](https://docs.aws.amazon.com/lex/latest/dg/context-mgmt.html#context-mgmt-session-attribs).
"""
post_text(botAlias, botName, inputText, userId; aws_config::AbstractAWSConfig=current_aws_config()) = lex_runtime_service("POST", "/bot/$(botName)/alias/$(botAlias)/user/$(userId)/text", Dict{String, Any}("inputText"=>inputText); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
post_text(botAlias, botName, inputText, userId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = lex_runtime_service("POST", "/bot/$(botName)/alias/$(botAlias)/user/$(userId)/text", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("inputText"=>inputText), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"recentIntentSummaryView"`: A summary of the recent intents for the bot. You can use the
  ...
  intents. You can also use it to remove or add intent summary objects to the list.
  ...
  `GetSession` request returns three intents in the summary view and you call `PutSession`
...
- `"sessionAttributes"`: Map of key/value pairs representing the session-specific context
  ...
  application.
"""
put_session(botAlias, botName, userId; aws_config::AbstractAWSConfig=current_aws_config()) = lex_runtime_service("POST", "/bot/$(botName)/alias/$(botAlias)/user/$(userId)/session"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
put_session(botAlias, botName, userId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = lex_runtime_service("POST", "/bot/$(botName)/alias/$(botAlias)/user/$(userId)/session", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
