...
information, see [Managing Conversation Context](https://docs.aws.amazon.com/lex/latest/dg/context-mgmt.html).
...
- `user_id`: The ID of the client application user. Amazon Lex uses this to identify a
  ...
  - The `userID` field must not contain any personally identifiable information of the user,
    for example, name, personal identification numbers, or other end user personal
    information.
  ...
  - A user can't have two independent conversations with two different versions of the same
    bot. For example, a user can't have a conversation with the PROD and BETA versions of
...
- `"x-amz-lex-active-contexts"`: A list of contexts active for the request. A context can be
...
- `"x-amz-lex-session-attributes"`: You pass this value as the
  ...
  total size of the `sessionAttributes` and `requestAttributes` headers is limited to 12 KB.
...
- If the message is a clarification prompt (configured for the intent) that indicates that
  user intent is not understood, the `dialogState` is set to ElicitIntent and `slotToElicit`
  is set to null.
...
- `"requestAttributes"`: Request-specific information passed between Amazon Lex and a client
  application.
...
- `"Accept"`: The message that Amazon Lex returns in the response can be either text or
  ...
  - If the value is `text/plain; charset=utf-8`, Amazon Lex returns text in the response.
  - If the value begins with `audio/`, Amazon Lex returns speech in the response. Amazon Lex
    uses Amazon Polly to generate the speech in the configuration that you specify. For
