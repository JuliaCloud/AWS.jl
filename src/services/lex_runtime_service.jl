...
The [`post_content`](@ref) operation supports audio input at 8kHz and 16kHz. You can use
...
- For a user input "I would like a pizza," Amazon Lex might return a response with a
  message eliciting slot data (for example, `PizzaSize`): "What size pizza would you
  like?".
...
- If the message is to elicit slot data, Amazon Lex returns the following context
  information:   - `x-amz-lex-dialog-state` header set to `ElicitSlot`
...
more information, see [Managing Conversation Context](https://docs.aws.amazon.com/lex/latest/dg/context-mgmt.html).
...
- `user_id`: The ID of the client application user. Amazon Lex uses this to identify a
  ...
  - A user can't have two independent conversations with two different versions of the
    ...
    conversation with two different versions, for example, while testing, include the bot
...
- `"Accept"`: You pass this value as the `Accept` HTTP header.
  ...
  The message Amazon Lex returns in the response can be either text or speech based on
  the `Accept` HTTP header value in the request.

  - If the value is `text/plain; charset=utf-8`, Amazon Lex returns text in the response.
  - If the value begins with `audio/`, Amazon Lex returns speech in the response. Amazon
    Lex uses Amazon Polly to generate the speech (using the configuration you specified
    in the `Accept` header). For example, if you specify `audio/mpeg` as the value,
...
- `"x-amz-lex-active-contexts"`: A list of contexts active for the request. A context can
  ...
  If you don't specify a list of contexts, Amazon Lex will use the current list of
  contexts for the session. If you specify an empty list, all contexts for the session
  are cleared.
- `"x-amz-lex-request-attributes"`: You pass this value as the
...
- `"x-amz-lex-session-attributes"`: You pass this value as the
  ...
  The total size of the `sessionAttributes` and `requestAttributes` headers is limited to
  12 KB.
...
- If the message is to elicit slot data, Amazon Lex returns the following context
  information:   - `dialogState` set to ElicitSlot
...
- `"Accept"`: The message that Amazon Lex returns in the response can be either text or
  ...
  - If the value is `text/plain; charset=utf-8`, Amazon Lex returns text in the response.
  - If the value begins with `audio/`, Amazon Lex returns speech in the response. Amazon
    Lex uses Amazon Polly to generate the speech in the configuration that you specify.
    For example, if you specify `audio/mpeg` as the value, Amazon Lex returns speech in
    the MPEG format.
