...
# Required Parameters
- `GraphArn`: The ARN of the behavior graph that the member account is accepting the
...
Creates a new behavior graph for the calling account, and sets that account as the master
account. This operation is called by the account that is enabling Detective. Before you try
to enable Detective, make sure that your account has been enrolled in Amazon GuardDuty for
at least 48 hours. If you do not meet this requirement, you cannot enable Detective. If you
do meet the GuardDuty prerequisite, then when you make the request to enable Detective, it
checks whether your data volume is within the Detective quota. If it exceeds the quota,
then you cannot enable Detective.  The operation also enables Detective for the calling
account in the currently selected Region. It returns the ARN of the new behavior graph.
CreateGraph triggers a process to create the corresponding data tables for the new behavior
graph. An account can only be the master account for one behavior graph within a Region. If
the same account calls CreateGraph with the same master account, it always returns the same
...
graph. This operation can only be called by the master account for a behavior graph.
CreateMembers verifies the accounts and then sends invitations to the verified accounts.
The request provides the behavior graph ARN and the list of accounts to invite. The
response separates the requested accounts into two lists:   The accounts that CreateMembers
was able to start the verification for. This list includes member accounts that are being
verified, that have passed verification and are being sent an invitation, and that have
failed verification.   The accounts that CreateMembers was unable to process. This list
includes accounts that were already invited to be member accounts in the behavior graph.
...
- `GraphArn`: The ARN of the behavior graph to invite the member accounts to contribute
  their data to.
...
called by the master account for a behavior graph.
...
can only be called by a Detective master account. That account cannot use DeleteMembers to
delete their own account from the behavior graph. To disable a behavior graph, the master
account uses the DeleteGraph API method.

# Required Parameters
- `AccountIds`: The list of AWS account identifiers for the member accounts to delete from
  the behavior graph.
...
# Required Parameters
- `GraphArn`: The ARN of the behavior graph to remove the member account from. The member
...
# Required Parameters
- `AccountIds`: The list of AWS account identifiers for the member account for which to
...
Returns the list of behavior graphs that the calling account is a master of. This operation
can only be called by a master account. Because an account can currently only be the master
of one behavior graph within a Region, the results always contain a single graph.
...
# Optional Parameters
- `MaxResults`: The maximum number of behavior graph invitations to return in the response.
  The total must be less than the overall limit on the number of results to return, which is
  currently 200.
- `NextToken`: For requests to retrieve the next page of results, the pagination token that
  was returned with the previous page of results. The initial request does not include a
  pagination token.
"""
...
# Required Parameters
- `GraphArn`: The ARN of the behavior graph for which to retrieve the list of member
  accounts.

# Optional Parameters
- `MaxResults`: The maximum number of member accounts to include in the response. The total
...
# Required Parameters
- `GraphArn`: The ARN of the behavior graph to reject the invitation to. The member
...
# Required Parameters
- `AccountId`: The account ID of the member account to try to enable. The account must be
