...
"""
GetScreenData(appId, screenId, workbookId; aws::AWSConfig=AWSConfig()) = honeycode("POST", "/screendata", Dict{String, Any}("appId"=>appId, "screenId"=>screenId, "workbookId"=>workbookId))
GetScreenData(appId, screenId, workbookId, args::AbstractDict{String, <:Any}; aws::AWSConfig=AWSConfig()) = honeycode("POST", "/screendata", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("appId"=>appId, "screenId"=>screenId, "workbookId"=>workbookId), args)); aws=aws)
...
- `variables`:  Variables are optional and are needed only if the screen requires them to render correctly. Variables are specified as a map where the key is the name of the variable as defined on the screen. The value is an object which currently has only one property, rawValue, which holds the value of the variable to be passed to the screen. 
"""
InvokeScreenAutomation(appId, automationId, screenId, workbookId; aws::AWSConfig=AWSConfig()) = honeycode("POST", "/workbooks/$(workbookId)/apps/$(appId)/screens/$(screenId)/automations/$(automationId)"; aws=aws)
InvokeScreenAutomation(appId, automationId, screenId, workbookId, args::AbstractDict{String, <:Any}; aws::AWSConfig=AWSConfig()) = honeycode("POST", "/workbooks/$(workbookId)/apps/$(appId)/screens/$(screenId)/automations/$(automationId)", args; aws=aws)
