...
- `Deployed`: Whether to display the deployed configuration (true) or include any pending
  changes (false). Defaults to false.
- `ExpressionNames`: Limits the DescribeExpressions response to the specified expressions.
...
# Required Parameters
- `DomainName`:
- `MultiAZ`: You expand an existing search domain to a second Availability Zone by setting
...
# Required Parameters
- `DomainEndpointOptions`: Whether to require that all requests to the domain arrive over
...
# Required Parameters
- `AccessPolicies`: The access rules you want to configure. These rules replace any
  existing rules.
- `DomainName`:
