...
function batch_get_traces(
    TraceIds; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...
)
...
function create_sampling_rule(
    SamplingRule; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...
)
    params = amazonify(MAPPING, kwargs)
    return xray(
        "POST",
...
- `"rule_arn"`: The ARN of the sampling rule. Specify a rule by either name or ARN, but not
  both.
- `"rule_name"`: The name of the sampling rule. Specify a rule by either name or ARN, but
  not both.
"""
...
- `"next_token"`: Specify the pagination token returned by a previous request to retrieve
...
# Optional Parameters
Optional parameters can be passed as a keyword argument. Valid keys are:
- `"group_arn"`: The Amazon Resource Name (ARN) of the group. Required if the GroupName
  isn't provided.
...
# Optional Parameters
Optional parameters can be passed as a keyword argument. Valid keys are:
- `"filter_expression"`: Specify a filter expression to retrieve trace summaries for
...
- `"time_range_type"`: A parameter to indicate whether to query trace summaries by TraceId
  or Event time.
"""
...
# Optional Parameters
Optional parameters can be passed as a keyword argument. Valid keys are:
- `"key_id"`: An Amazon Web Services KMS key in one of the following formats:    Alias -
...
function put_telemetry_records(
    TelemetryRecords; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...
)
    params = amazonify(MAPPING, kwargs)
    return xray(
        "POST",
...
function put_trace_segments(
    TraceSegmentDocuments; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...
)
    params = amazonify(MAPPING, kwargs)
    return xray(
        "POST",
...
- `"insights_configuration"`: The structure containing configurations related to insights.
  ...
  to disable insights for the group.   The NotifcationsEnabled boolean can be set to true to
...
function update_sampling_rule(
    SamplingRuleUpdate; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...
)
    params = amazonify(MAPPING, kwargs)
    return xray(
        "POST",
