...
!!! note
    - `AddPermission` generates a policy for you. You can use
      `[`set_queue_attributes`](@ref)` to upload your policy. For more information, see [Using Custom Policies with the Amazon SQS Access Policy Language](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-creating-custom-policies.html)
      in the *Amazon SQS Developer Guide*.
...
`[`change_message_visibility`](@ref).` The result of the action on each message is reported
individually in the response. You can send up to 10 `[`change_message_visibility`](@ref)`
...
`[`delete_message`](@ref).` The result of the action on each message is reported
...
`[`send_message`](@ref)` request might succeed, but after 60 seconds the queue and the
...
- `"AttributeNames"`:
  ...
  - `MessageGroupId` – Returns the value provided by the producer that calls the
    `[`send_message`](@ref)` action. Messages with the same `MessageGroupId` are returned
    in sequence.
