...
# Required Parameters
- `resourceArn`: The Amazon Resource Name (ARN) that identifies the AWS Elemental
...
- `encryption`: The type of encryption that will be used on the output associated with this
  entitlement.
- `entitlementStatus`: An indication of whether you want to enable the entitlement to allow
  access, or disable it to stop streaming content to the subscriber’s flow temporarily. If
  you don’t specify the entitlementStatus field in your request, MediaConnect leaves the
  value unchanged.
...
- `smoothingLatency`: The smoothing latency in milliseconds for RIST, RTP, and RTP-FEC
  streams.
- `streamId`: The stream ID that you want to use for this transport. This parameter applies
