...
Fargate use the `FARGATE` and `FARGATE_SPOT` capacity providers. These providers are
...
- `name`: The name of the capacity provider. Up to 255 characters are allowed. They include
  ...
  The name can't be prefixed with "`aws`", "`ecs`", or "`fargate`".
...
- `"tags"`: The metadata that you apply to the capacity provider to categorize and organize
  ...
  for either keys or values as it is reserved for Amazon Web Services use. You cannot edit
  or delete tag keys or values with this prefix. Tags with this prefix do not count against
  your tags per resource limit.
"""
create_capacity_provider(autoScalingGroupProvider, name; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("CreateCapacityProvider", Dict{String, Any}("autoScalingGroupProvider"=>autoScalingGroupProvider, "name"=>name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_capacity_provider(autoScalingGroupProvider, name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("CreateCapacityProvider", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("autoScalingGroupProvider"=>autoScalingGroupProvider, "name"=>name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
when you launch your first container instance. However, you can create your own cluster
with a unique name.
...
services on your behalf. However, if the user that makes the call doesn't have permissions
to create the service-linked role, it isn't created. For more information, see [Using service-linked roles for Amazon ECS](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/using-service-linked-roles.html)
...
- `"defaultCapacityProviderStrategy"`: The capacity provider strategy to set as the default
  for the cluster. After a default capacity provider strategy is set for a cluster, when
  you call the [CreateService](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_CreateService.html)
  or [RunTask](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_RunTask.html)
...
- `"serviceConnectDefaults"`: Use this parameter to set a default Service Connect
  namespace. After you set a default Service Connect namespace, any new services with
  Service Connect turned on that are created in the cluster are added as client services in
  the namespace. This setting only applies to new services that set the `enabled` parameter
  to `true` in the `ServiceConnectConfiguration`. You can set the namespace of each service
...
- `"tags"`: The metadata that you apply to the cluster to help you categorize and organize
  ...
  your tags per resource limit.
"""
create_cluster(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("CreateCluster"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_cluster(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("CreateCluster", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
number of tasks running in a service drops below the `desiredCount`, Amazon ECS runs
another copy of the task in the specified cluster. To update an existing service, use [UpdateService](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_UpdateService.html).</p>

!!! note
    On March 21, 2024, a change was made to resolve the task definition revision before
authorization. When a task definition revision is not specified, authorization will occur
using the latest revision of a task definition.In addition to maintaining the desired count
of tasks in your service, you can optionally run your service behind one or more load
balancers. The load balancers distribute traffic across the tasks that are associated with
the service. For more information, see [Service load balancing](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-load-balancing.html)
in the *Amazon Elastic Container Service Developer Guide*.

You can attach Amazon EBS volumes to Amazon ECS tasks by configuring the volume when
creating or updating a service. `volumeConfigurations` is only supported for REPLICA
...
strategy, or use Service Auto Scaling policies. For more information, see [Service scheduler concepts](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs_services.html)
in the *Amazon Elastic Container Service Developer Guide*.
...
task definition or by your desired count of a service. You can use [UpdateService](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_UpdateService.html).
...
during a deployment. Specifically, it represents it as a percentage of your desired number
of tasks (rounded up to the nearest integer). This happens when any of your container
instances are in the `DRAINING` state if the service contains tasks using the EC2 launch
type. Using this parameter, you can deploy without using additional cluster capacity. For
example, if you set your service to have desired number of four tasks and a minimum healthy
percent of 50%, the scheduler might stop two existing tasks to free up cluster capacity
before starting two new tasks. If they're in the `RUNNING` state, tasks for services that
don't use a load balancer are considered healthy . If they're in the `RUNNING` state and
reported as healthy by the load balancer, tasks for services that *do* use a load balancer
...
`RUNNING` or `PENDING` state during a deployment. Specifically, it represents it as a
...
tasks that use the EC2 launch type, the **minimum healthy percent** and **maximum percent**
values are used only to define the lower and upper limit on the number of the tasks in the
service that remain in the `RUNNING` state. This is while the container instances are in
the `DRAINING` state. If the tasks in the service use the Fargate launch type, the minimum
healthy percent and maximum percent values aren't used. This is the case even if they're
currently visible when describing your service.

When creating a service that uses the `EXTERNAL` deployment controller, you can specify
only parameters that aren't controlled at the task set level. The only required parameter
is the service name. You control your services using the [CreateTaskSet](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_CreateTaskSet.html).
...
information about task placement and task placement strategies, see [Amazon ECS task placement](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-placement.html)
in the *Amazon Elastic Container Service Developer Guide*
...
- `service_name`: The name of your service. Up to 255 letters (uppercase and lowercase),
  ...
  cluster, but you can have similarly named services in multiple clusters within a Region
...
- `"healthCheckGracePeriodSeconds"`: The period of time, in seconds, that the Amazon ECS
  ...
  balancer. If your service has a load balancer defined and you don't specify a health
  ...
  in the task definition health check parameters. For more information, see [Health check](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_HealthCheck.html).
...
- `"launchType"`: The infrastructure that you run your service on. For more information,
  ...
  The `FARGATE` launch type runs your tasks on Fargate On-Demand infrastructure.</p>

  !!! note
      Fargate Spot infrastructure is available for use but a capacity provider strategy
  must be used. For more information, see [Fargate capacity providers](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/fargate-capacity-providers.html)
  in the *Amazon ECS Developer Guide*.The `EC2` launch type runs your tasks on Amazon EC2
...
- `"loadBalancers"`: A load balancer object representing the load balancers to use with
  your service. For more information, see [Service load balancing](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-load-balancing.html)
  in the *Amazon Elastic Container Service Developer Guide*.
  ...
  that use multiple target groups. For more information, see [Using service-linked roles for Amazon ECS](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/using-service-linked-roles.html)
  in the *Amazon Elastic Container Service Developer Guide*.
  ...
  the load balancer. The container name must be as it appears in a container definition.
  The load balancer name parameter must be omitted. When a task from this service is placed
  on a container instance, the container instance and port combination is registered as a
...
- `"platformVersion"`: The platform version that your tasks in the service are running on.
  ...
  isn't specified, the `LATEST` platform version is used. For more information, see [Fargate platform versions](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/platform_versions.html)
  in the *Amazon Elastic Container Service Developer Guide*.
- `"propagateTags"`: Specifies whether to propagate the tags from the task definition to
  the task. If no value is specified, the tags aren't propagated. Tags can only be
  propagated to the task during task creation. To add tags to a task after task creation,
...
- `"role"`: The name or full Amazon Resource Name (ARN) of the IAM role that allows Amazon
  ...
  in the *Amazon Elastic Container Service Developer Guide*.If your specified role has a
  path other than `/`, then you must either specify the full role ARN (this is recommended)
  or prefix the role name with the path. For example, if a role with the name `bar` has a
...
- `"schedulingStrategy"`: The scheduling strategy to use for the service. For more
  ...
  scheduler strategy is required if the service uses the `CODE_DEPLOY` or `EXTERNAL`
...
"""
create_service(serviceName; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("CreateService", Dict{String, Any}("serviceName"=>serviceName); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_service(serviceName, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("CreateService", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("serviceName"=>serviceName), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
the `EXTERNAL` deployment controller type. For more information, see [Amazon ECS deployment types](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/deployment-types.html)
in the *Amazon Elastic Container Service Developer Guide*.

!!! note
    On March 21, 2024, a change was made to resolve the task definition revision before
authorization. When a task definition revision is not specified, authorization will occur
using the latest revision of a task definition.For information about the maximum number of
task sets and other quotas, see [Amazon ECS service quotas](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-quotas.html)
in the *Amazon Elastic Container Service Developer Guide*.
...
- `"tags"`: The metadata that you apply to the task set to help you categorize and organize
  them. Each tag consists of a key and an optional value. You define both. When a service
  ...
  your tags per resource limit.
"""
create_task_set(cluster, service, taskDefinition; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("CreateTaskSet", Dict{String, Any}("cluster"=>cluster, "service"=>service, "taskDefinition"=>taskDefinition); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_task_set(cluster, service, taskDefinition, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("CreateTaskSet", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("cluster"=>cluster, "service"=>service, "taskDefinition"=>taskDefinition), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `name`: The resource name to disable the account setting for. If `serviceLongArnFormat`
  is specified, the ARN for your Amazon ECS services is affected. If `taskLongArnFormat` is
...
- `"principalArn"`: The Amazon Resource Name (ARN) of the principal. It can be an user,
  ...
  authenticated user.
"""
delete_account_setting(name; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteAccountSetting", Dict{String, Any}("name"=>name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_account_setting(name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteAccountSetting", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("name"=>name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"cluster"`: The short name or full Amazon Resource Name (ARN) of the cluster that
  ...
  cluster is assumed.
"""
delete_attributes(attributes; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteAttributes", Dict{String, Any}("attributes"=>attributes); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_attributes(attributes, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteAttributes", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("attributes"=>attributes), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
or by deleting the cluster.Prior to a capacity provider being deleted, the capacity
provider must be removed from the capacity provider strategy from all services. The [UpdateService](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_UpdateService.html)
...
- `capacity_provider`: The short name or full Amazon Resource Name (ARN) of the capacity
  provider to delete.
"""
delete_capacity_provider(capacityProvider; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteCapacityProvider", Dict{String, Any}("capacityProvider"=>capacityProvider); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_capacity_provider(capacityProvider, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteCapacityProvider", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("capacityProvider"=>capacityProvider), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
delete_cluster(cluster; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteCluster", Dict{String, Any}("cluster"=>cluster); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_cluster(cluster, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteCluster", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("cluster"=>cluster), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
service status moves from `ACTIVE` to `DRAINING`, and the service is no longer visible in
the console or in the [ListServices](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_ListServices.html)
API operation. After all tasks have transitioned to either `STOPPING` or `STOPPED` status,
the service status moves from `DRAINING` to `INACTIVE`. Services in the `DRAINING` or
`INACTIVE` status can still be viewed with the [DescribeServices](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_DescribeServices.html)
...
- `"force"`: If `true`, allows you to delete a service even if it wasn't scaled down to
  zero tasks. It's only necessary to use this if the service uses the `REPLICA` scheduling
  strategy.
"""
delete_service(service; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteService", Dict{String, Any}("service"=>service); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_service(service, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteService", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("service"=>service), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
delete_task_definitions(taskDefinitions; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteTaskDefinitions", Dict{String, Any}("taskDefinitions"=>taskDefinitions); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_task_definitions(taskDefinitions, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteTaskDefinitions", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("taskDefinitions"=>taskDefinitions), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"force"`: If `true`, you can delete a task set even if it hasn't been scaled down to
  zero.
"""
delete_task_set(cluster, service, taskSet; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteTaskSet", Dict{String, Any}("cluster"=>cluster, "service"=>service, "taskSet"=>taskSet); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_task_set(cluster, service, taskSet, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeleteTaskSet", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("cluster"=>cluster, "service"=>service, "taskSet"=>taskSet), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"cluster"`: The short name or full Amazon Resource Name (ARN) of the cluster that hosts
  the container instance to deregister. If you do not specify a cluster, the default
  cluster is assumed.
...
"""
deregister_container_instance(containerInstance; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeregisterContainerInstance", Dict{String, Any}("containerInstance"=>containerInstance); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
deregister_container_instance(containerInstance, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeregisterContainerInstance", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("containerInstance"=>containerInstance), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
that you rely on `INACTIVE` task definitions persisting beyond the lifecycle of any
associated tasks and services.You must deregister a task definition revision before you
delete it. For more information, see [DeleteTaskDefinitions](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_DeleteTaskDefinitions.html).

# Arguments

- `task_definition`: The `family` and `revision` (`family:revision`) or full Amazon
  Resource Name (ARN) of the task definition to deregister. You must specify a `revision`.
"""
deregister_task_definition(taskDefinition; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeregisterTaskDefinition", Dict{String, Any}("taskDefinition"=>taskDefinition); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
deregister_task_definition(taskDefinition, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DeregisterTaskDefinition", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("taskDefinition"=>taskDefinition), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_capacity_providers(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeCapacityProviders"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_capacity_providers(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeCapacityProviders", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_clusters(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeClusters"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_clusters(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeClusters", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"include"`: Specifies whether you want to see the resource tags for the container
  ...
  `CONTAINER_INSTANCE_HEALTH` is specified, the container instance health is included in
  the response. If this field is omitted, tags and container instance health status aren't
  included in the response.
"""
describe_container_instances(containerInstances; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeContainerInstances", Dict{String, Any}("containerInstances"=>containerInstances); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_container_instances(containerInstances, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeContainerInstances", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("containerInstances"=>containerInstances), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"cluster"`: The short name or full Amazon Resource Name (ARN)the cluster that hosts the
  service to describe. If you do not specify a cluster, the default cluster is assumed.
  This parameter is required if the service or services you are describing were launched in
...
"""
describe_services(services; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeServices", Dict{String, Any}("services"=>services); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_services(services, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeServices", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("services"=>services), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"include"`: Determines whether to see the resource tags for the task definition. If
  ...
  tags aren't included in the response.
"""
describe_task_definition(taskDefinition; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeTaskDefinition", Dict{String, Any}("taskDefinition"=>taskDefinition); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_task_definition(taskDefinition, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeTaskDefinition", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("taskDefinition"=>taskDefinition), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_task_sets(cluster, service; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeTaskSets", Dict{String, Any}("cluster"=>cluster, "service"=>service); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_task_sets(cluster, service, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeTaskSets", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("cluster"=>cluster, "service"=>service), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_tasks(tasks; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeTasks", Dict{String, Any}("tasks"=>tasks); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_tasks(tasks, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DescribeTasks", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("tasks"=>tasks), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"containerInstance"`: The container instance ID or full ARN of the container instance.
  ...
  in the *Amazon ECS Developer Guide*.
"""
discover_poll_endpoint(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DiscoverPollEndpoint"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
discover_poll_endpoint(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("DiscoverPollEndpoint", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
execute_command(command, interactive, task; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ExecuteCommand", Dict{String, Any}("command"=>command, "interactive"=>interactive, "task"=>task); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
execute_command(command, interactive, task, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ExecuteCommand", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("command"=>command, "interactive"=>interactive, "task"=>task), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"tasks"`: A list of up to 100 task IDs or full ARN entries.
"""
get_task_protection(cluster; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("GetTaskProtection", Dict{String, Any}("cluster"=>cluster); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_task_protection(cluster, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("GetTaskProtection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("cluster"=>cluster), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"effectiveSettings"`: Determines whether to return the effective settings. If `true`,
  the account settings for the root user or the default setting for the `principalArn` are
...
- `"principalArn"`: The ARN of the principal, which can be a user, role, or the root user.
  If this field is omitted, the account settings are listed only for the authenticated
  user.
...
"""
list_account_settings(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListAccountSettings"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_account_settings(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListAccountSettings", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"nextToken"`: The `nextToken` value returned from a `ListAttributes` request indicating
  ...
  the next items in a list and not for other programmatic purposes.
"""
list_attributes(targetType; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListAttributes", Dict{String, Any}("targetType"=>targetType); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_attributes(targetType, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListAttributes", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("targetType"=>targetType), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"nextToken"`: The `nextToken` value returned from a `ListClusters` request indicating
  ...
  the next items in a list and not for other programmatic purposes.
"""
list_clusters(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListClusters"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_clusters(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListClusters", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
`filter` parameter. For more information, see [Cluster Query Language](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/cluster-query-language.html)
in the *Amazon Elastic Container Service Developer Guide*.
...
- `"maxResults"`: The maximum number of container instance results that
  ...
  `nextToken` response element. The remaining results of the initial request can be seen by
  sending another `ListContainerInstances` request with the returned `nextToken` value.
...
- `"nextToken"`: The `nextToken` value returned from a `ListContainerInstances` request
  ...
  needed. If `maxResults` was provided, it's possible the number of results to be fewer
  than `maxResults`.
...
"""
list_container_instances(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListContainerInstances"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_container_instances(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListContainerInstances", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"schedulingStrategy"`: The scheduling strategy to use when filtering the `ListServices`
  results.
"""
list_services(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListServices"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_services(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListServices", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
This list might include services in different clusters. In contrast, `ListServices` can
only list services in one cluster at a time. If you need to filter the list of services in
a single cluster by various parameters, use `ListServices`. For more information, see [Service Connect](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-connect.html)
in the *Amazon Elastic Container Service Developer Guide*.
...
- `"maxResults"`: The maximum number of service results that `ListServicesByNamespace`
  ...
  between 1 and 100. If this parameter isn't used, then `ListServicesByNamespace` returns
...
"""
list_services_by_namespace(namespace; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListServicesByNamespace", Dict{String, Any}("namespace"=>namespace); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_services_by_namespace(namespace, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListServicesByNamespace", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("namespace"=>namespace), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
list_tags_for_resource(resourceArn; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListTagsForResource", Dict{String, Any}("resourceArn"=>resourceArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_tags_for_resource(resourceArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListTagsForResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("resourceArn"=>resourceArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
list_task_definition_families(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListTaskDefinitionFamilies"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_task_definition_families(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListTaskDefinitionFamilies", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"status"`: The task definition status to filter the `ListTaskDefinitions` results with.
  ...
  `INACTIVE`, you can view task definitions that are `INACTIVE` as long as an active task
  or service still references them. If you paginate the resulting output, be sure to keep
  the `status` value constant in each subsequent request.
"""
list_task_definitions(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListTaskDefinitions"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_task_definitions(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListTaskDefinitions", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"cluster"`: The short name or full Amazon Resource Name (ARN) of the cluster to use when
  filtering the `ListTasks` results. If you do not specify a cluster, the default cluster
  is assumed.
...
- `"maxResults"`: The maximum number of task results that `ListTasks` returned in paginated
  ...
  then `ListTasks` returns up to 100 results and a `nextToken` value if applicable.
...
"""
list_tasks(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListTasks"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_tasks(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("ListTasks", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `name`: The Amazon ECS account setting name to modify.
  ...
  The following are the valid values for the account setting name. - `serviceLongArnFormat`
  - When modified, the Amazon Resource Name (ARN) and resource ID format of the resource
  type for a specified user, role, or the root user for an account is affected. The opt-in
  and opt-out account setting must be set for each Amazon ECS resource separately. The ARN
  and resource ID format of a resource is defined by the opt-in status of the user or role
  that created the resource. You must turn on this setting to use Amazon ECS features such
  as resource tagging.
  ...
  in status of the user or role that created the resource. You must turn on this setting to
  ...
  setting to use Amazon ECS features such as resource tagging.
   - `awsvpcTrunking` - When modified, the elastic network interface (ENI) limit for any
  new container instances that support the feature is changed. If `awsvpcTrunking` is
  turned on, any new container instances that support the feature are launched have the
  increased ENI limits available to them. For more information, see [Elastic Network Interface Trunking](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/container-instance-eni.html)
  in the *Amazon Elastic Container Service Developer Guide*.
   - `containerInsights` - When modified, the default setting indicating whether Amazon Web
  Services CloudWatch Container Insights is turned on for your clusters is changed. If
  `containerInsights` is turned on, any new clusters that are created will have Container
  Insights turned on unless you disable it during cluster creation. For more information,
  see [CloudWatch Container Insights](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/cloudwatch-container-insights.html)
  in the *Amazon Elastic Container Service Developer Guide*.
   - `dualStackIPv6` - When turned on, when using a VPC in dual stack mode, your tasks
  using the `awsvpc` network mode can have an IPv6 address assigned. For more information
  on using IPv6 with tasks launched on Amazon EC2 instances, see [Using a VPC in dual-stack mode](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-networking-awsvpc.html#task-networking-vpc-dual-stack).
  ...
  security or infrastructure update is needed for an Amazon ECS task hosted on Fargate, the
  ...
  as `ecsCreateCluster`. If tags are specified when you create a resource, Amazon Web
...
- `value`: The account setting value for the specified principal ARN. Accepted values are
  ...
  valid values: - `0` - Amazon Web Services sends the notification, and immediately retires
  the affected tasks.
...
- `"principalArn"`: The ARN of the principal, which can be a user, role, or the root user.
  ...
  the root user of the account unless a user or role explicitly overrides these settings.
  ...
  account settings set for them.
"""
put_account_setting(name, value; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("PutAccountSetting", Dict{String, Any}("name"=>name, "value"=>value); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
put_account_setting(name, value, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("PutAccountSetting", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("name"=>name, "value"=>value), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `value`: The account setting value for the specified principal ARN. Accepted values are
  ...
  retire the tasks.
"""
put_account_setting_default(name, value; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("PutAccountSettingDefault", Dict{String, Any}("name"=>name, "value"=>value); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
put_account_setting_default(name, value, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("PutAccountSettingDefault", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("name"=>name, "value"=>value), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"cluster"`: The short name or full Amazon Resource Name (ARN) of the cluster that
  ...
  cluster is assumed.
"""
put_attributes(attributes; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("PutAttributes", Dict{String, Any}("attributes"=>attributes); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
put_attributes(attributes, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("PutAttributes", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("attributes"=>attributes), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This action is only used by the Amazon ECS agent, and it is not intended for use
outside of the agent.Registers an EC2 instance into the specified cluster. This instance
...
- `"instanceIdentityDocumentSignature"`: The instance identity document signature for the
  ...
  from the instance: `curl http://169.254.169.254/latest/dynamic/instance-
  identity/signature/`
...
- `"tags"`: The metadata that you apply to the container instance to help you categorize
  and organize them. Each tag consists of a key and an optional value. You define both.
...
"""
register_container_instance(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("RegisterContainerInstance"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
register_container_instance(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("RegisterContainerInstance", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
You can specify a Docker networking mode for the containers in your task definition with
the `networkMode` parameter. If you specify the `awsvpc` network mode, the task is
allocated an elastic network interface, and you must specify a [NetworkConfiguration](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_NetworkConfiguration.html)
...
- `family`: You must specify a `family` for a task definition. You can use it track
  multiple versions of the same task definition. The `family` is used as a name for your
  task definition. Up to 255 letters (uppercase and lowercase), numbers, underscores, and
  hyphens are allowed.
...
- `"cpu"`: The number of CPU units used by the task. It can be expressed as an integer
  using CPU units (for example, `1024`) or as a string using vCPUs (for example, `1 vCPU`
  or `1 vcpu`) in a task definition. String values are converted to an integer indicating
  the CPU units when the task definition is registered.</p>

  !!! note
      Task-level CPU and memory parameters are ignored for Windows containers. We recommend
  specifying container-level resources for Windows containers.If you're using the EC2
  launch type, this field is optional. Supported values are between `128` CPU units
  (`0.125` vCPUs) and `10240` CPU units (`10` vCPUs). If you do not specify a value, the
 ...
 <p>This option requires Linux platform `1.4.0` or later.
...
- `"ipcMode"`: The IPC resource namespace to use for the containers in the task. The valid
  ...
  `host` IPC mode, IPC namespace related `systemControls` are not supported.
...
- `"memory"`: The amount of memory (in MiB) used by the task. It can be expressed as an
  integer using MiB (for example ,`1024`) or as a string using GB (for example, `1GB` or `1
  GB`) in a task definition. String values are converted to an integer indicating the MiB
  when the task definition is registered.</p>

  !!! note
      Task-level CPU and memory parameters are ignored for Windows containers. We recommend
  specifying container-level resources for Windows containers.If using the EC2 launch type,
...
- `"networkMode"`: The Docker networking mode to use for the containers in the task. The
  ...
  dynamic host port mappings. </p>

  !!! important
      When using the `host` network mode, you should not run containers using the root user
  (UID 0). It is considered best practice to use a non-root user.If the network mode is
  `awsvpc`, the task is allocated an elastic network interface, and you must specify a [NetworkConfiguration](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_NetworkConfiguration.html)
  value when you create a service or run a task with the task definition. For more
  information, see [Task Networking](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-networking.html)
  in the *Amazon Elastic Container Service Developer Guide*.

   <p>If the network mode is `host`, you cannot run multiple instantiations of the same
...
- `"pidMode"`: The process namespace to use for the containers in the task. The valid
  values are `host` or `task`. On Fargate for Linux containers, the only valid value is
  `task`. For example, monitoring sidecars might need `pidMode` to access information about
...
- `"proxyConfiguration"`: The configuration details for the App Mesh proxy.
  ...
  versions of the container agent and `ecs-init`. For more information, see [Amazon ECS-optimized AMI versions](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-ami-versions.html)
  in the *Amazon Elastic Container Service Developer Guide*.
- `"requiresCompatibilities"`: The task launch type that Amazon ECS validates the task
  definition against. A client exception is returned if the task definition doesn't
  validate against the compatibilities specified. If no value is specified, the parameter
...
- `"volumes"`: A list of volume definitions in JSON format that containers in your task
  might use.
"""
register_task_definition(containerDefinitions, family; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("RegisterTaskDefinition", Dict{String, Any}("containerDefinitions"=>containerDefinitions, "family"=>family); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
register_task_definition(containerDefinitions, family, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("RegisterTaskDefinition", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("containerDefinitions"=>containerDefinitions, "family"=>family), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)

"""
    run_task(task_definition)
    run_task(task_definition, params::Dict{String,<:Any})

Starts a new task using the specified task definition.</p>

!!! note
    On March 21, 2024, a change was made to resolve the task definition revision before
authorization. When a task definition revision is not specified, authorization will occur
using the latest revision of a task definition.You can allow Amazon ECS to place tasks for
you, or you can customize how Amazon ECS places tasks using placement constraints and
placement strategies. For more information, see [Scheduling Tasks](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/scheduling_tasks.html)
in the *Amazon Elastic Container Service Developer Guide*.
...
command you run that affects your Amazon ECS resources might not be immediately visible to
all subsequent commands you run. Keep this in mind when you carry out an API command that
immediately follows a previous API command.

 <p>To manage eventual consistency, you can do the following: - Confirm the state of the
resource before you run a command to modify it. Run the DescribeTasks command using an
...
- `task_definition`: The `family` and `revision` (`family:revision`) or full ARN of the
  task definition to run. If a `revision` isn't specified, the latest `ACTIVE` revision is
  used.
  ...
  specify revision 2, use `arn:aws:ecs:us-east-1:111122223333:task-
  definition/TaskFamilyName:2`.
...
- `"launchType"`: The infrastructure to run your standalone task on. For more information,
  ...
  specified, the `capacityProviderStrategy` parameter must be omitted.
   ...
   <p>When you use cluster auto scaling, you must specify `capacityProviderStrategy` and
  not `launchType`.
...
- `"startedBy"`: An optional tag specified when a task is started. For example, if you
  ...
  identifier for that job to your task with the `startedBy` parameter. You can then
  identify which tasks belong to that job by filtering the results of a [ListTasks](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_ListTasks.html)
...
- `"tags"`: The metadata that you apply to the task to help you categorize and organize
  ...
  or delete tag keys or values with this prefix. Tags with this prefix do not count against
  your tags per resource limit.
- `"volumeConfigurations"`: The details of the volume that was `configuredAtLaunch`. You
  can configure the size, volumeType, IOPS, throughput, snapshot and encryption in in [TaskManagedEBSVolumeConfiguration](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_TaskManagedEBSVolumeConfiguration.html).
  The `name` of the volume must match the `name` from the task definition.
"""
run_task(taskDefinition; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("RunTask", Dict{String, Any}("taskDefinition"=>taskDefinition, "clientToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
run_task(taskDefinition, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("RunTask", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("taskDefinition"=>taskDefinition, "clientToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    On March 21, 2024, a change was made to resolve the task definition revision before
authorization. When a task definition revision is not specified, authorization will occur
using the latest revision of a task definition.Starting April 15, 2023, Amazon Web Services
will not onboard new customers to Amazon Elastic Inference (EI), and will help current
customers migrate their workloads to options that offer better price and performance. After
April 15, 2023, new customers will not be able to launch instances with Amazon EI
accelerators in Amazon SageMaker, Amazon ECS, or Amazon EC2. However, customers who have
used Amazon EI at least once during the past 30-day period are considered current customers
...
- `container_instances`: The container instance IDs or full ARN entries for the container
  ...
  instances.
- `task_definition`: The `family` and `revision` (`family:revision`) or full ARN of the
  task definition to start. If a `revision` isn't specified, the latest `ACTIVE` revision
  is used.
...
- `"enableExecuteCommand"`: Whether or not the execute command functionality is turned on
  for the task. If `true`, this turns on the execute command functionality on all
  containers in the task.
- `"group"`: The name of the task group to associate with the task. The default value is
  the family name of the task definition (for example, family:my-family-name).
...
- `"overrides"`: A list of container overrides in JSON format that specify the name of a
  container in the specified task definition and the overrides it receives. You can
  override the default command for a container (that's specified in the task definition or
  Docker image) with a `command` override. You can also override existing environment
  variables (that are specified in the task definition or Docker image) on a container or
  add new environment variables to it with an `environment` override.
...
- `"tags"`: The metadata that you apply to the task to help you categorize and organize
  ...
  your tags per resource limit.
- `"volumeConfigurations"`: The details of the volume that was `configuredAtLaunch`. You
  can configure the size, volumeType, IOPS, throughput, snapshot and encryption in [TaskManagedEBSVolumeConfiguration](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_TaskManagedEBSVolumeConfiguration.html).
  The `name` of the volume must match the `name` from the task definition.
"""
start_task(containerInstances, taskDefinition; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("StartTask", Dict{String, Any}("containerInstances"=>containerInstances, "taskDefinition"=>taskDefinition); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_task(containerInstances, taskDefinition, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("StartTask", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("containerInstances"=>containerInstances, "taskDefinition"=>taskDefinition), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
sending a `CTRL_SHUTDOWN_EVENT`. For more information, see [Unable to react to graceful shutdown of (Windows) container #25982](https://github.com/moby/moby/issues/25982)
on GitHub.
...
"""
stop_task(task; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("StopTask", Dict{String, Any}("task"=>task); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
stop_task(task, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("StopTask", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("task"=>task), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
submit_attachment_state_changes(attachments; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("SubmitAttachmentStateChanges", Dict{String, Any}("attachments"=>attachments); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
submit_attachment_state_changes(attachments, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("SubmitAttachmentStateChanges", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("attachments"=>attachments), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"task"`: The task ID or full Amazon Resource Name (ARN) of the task that hosts the
  container.
"""
submit_container_state_change(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("SubmitContainerStateChange"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
submit_container_state_change(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("SubmitContainerStateChange", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
submit_task_state_change(; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("SubmitTaskStateChange"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
submit_task_state_change(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("SubmitTaskStateChange", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `tags`: The tags to add to the resource. A tag is an array of key-value pairs.
  ...
  your tags per resource limit.
"""
tag_resource(resourceArn, tags; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("TagResource", Dict{String, Any}("resourceArn"=>resourceArn, "tags"=>tags); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
tag_resource(resourceArn, tags, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("TagResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("resourceArn"=>resourceArn, "tags"=>tags), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
untag_resource(resourceArn, tagKeys; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UntagResource", Dict{String, Any}("resourceArn"=>resourceArn, "tagKeys"=>tagKeys); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
untag_resource(resourceArn, tagKeys, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UntagResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("resourceArn"=>resourceArn, "tagKeys"=>tagKeys), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
update_capacity_provider(autoScalingGroupProvider, name; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateCapacityProvider", Dict{String, Any}("autoScalingGroupProvider"=>autoScalingGroupProvider, "name"=>name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_capacity_provider(autoScalingGroupProvider, name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateCapacityProvider", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("autoScalingGroupProvider"=>autoScalingGroupProvider, "name"=>name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
update_cluster(cluster; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateCluster", Dict{String, Any}("cluster"=>cluster); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_cluster(cluster, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateCluster", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("cluster"=>cluster), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
update_cluster_settings(cluster, settings; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateClusterSettings", Dict{String, Any}("cluster"=>cluster, "settings"=>settings); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_cluster_settings(cluster, settings, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateClusterSettings", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("cluster"=>cluster, "settings"=>settings), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
`ecs-init` package. This updates the agent. For more information, see [Updating the Amazon ECS container agent](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/agent-update-ecs-ami.html)
in the *Amazon Elastic Container Service Developer Guide*.
...
agent version in your Windows clusters.The `UpdateContainerAgent` API requires an Amazon
ECS-optimized AMI or Amazon Linux AMI with the `ecs-init` service installed and running.
For help updating the Amazon ECS container agent on other operating systems, see [Manually updating the Amazon ECS container agent](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-agent-update.html#manually_update_agent)
in the *Amazon Elastic Container Service Developer Guide*.
...
- `"cluster"`: The short name or full Amazon Resource Name (ARN) of the cluster that your
  ...
  assumed.
"""
update_container_agent(containerInstance; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateContainerAgent", Dict{String, Any}("containerInstance"=>containerInstance); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_container_agent(containerInstance, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateContainerAgent", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("containerInstance"=>containerInstance), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
status. If the instance is in any other status, an error will be received.When you set a
container instance to `DRAINING`, Amazon ECS prevents new tasks from being scheduled for
placement on the container instance and replacement service tasks are started on other
container instances in the cluster if the resources are available. Service tasks on the
...
`minimumHealthyPercent` and `maximumPercent`. You can change the deployment configuration
of your service using [UpdateService](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_UpdateService.html).
...
`ACTIVE` status and once it has reached that status the Amazon ECS scheduler can begin
...
- `status`: The container instance state to update the container instance with. The only
  valid values for this action are `ACTIVE` and `DRAINING`. A container instance can only
  be updated to `DRAINING` status once it has reached an `ACTIVE` state. If a container
...
- `"cluster"`: The short name or full Amazon Resource Name (ARN) of the cluster that hosts
  ...
  assumed.
"""
update_container_instances_state(containerInstances, status; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateContainerInstancesState", Dict{String, Any}("containerInstances"=>containerInstances, "status"=>status); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_container_instances_state(containerInstances, status, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateContainerInstancesState", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("containerInstances"=>containerInstances, "status"=>status), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)

"""
    update_service(service)
    update_service(service, params::Dict{String,<:Any})

Modifies the parameters of a service.</p>

!!! note
    On March 21, 2024, a change was made to resolve the task definition revision before
authorization. When a task definition revision is not specified, authorization will occur
using the latest revision of a task definition.For services using the rolling update
(`ECS`) you can update the desired count, deployment configuration, network configuration,
load balancers, service registries, enable ECS managed tags option, propagate tags option,
task placement constraints and strategies, and task definition. When you update any of
...
doesn't trigger a new deployment. For more infomation on volumes, see [Amazon EBS volumes](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ebs-volumes.html#ebs-volume-types)
in the *Amazon Elastic Container Service Developer Guide*.
...
balancer, network configuration, platform version, or task definition need to be updated,
create a new task set For more information, see [CreateTaskSet](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_CreateTaskSet.html).
...
image/tag combination from your repository when they start.You can also update the
deployment configuration of a service. When a deployment is triggered by updating the task
definition of a service, the service scheduler uses the deployment configuration
parameters, `minimumHealthyPercent` and `maximumPercent`, to determine the deployment
strategy. - If `minimumHealthyPercent` is below 100%, the scheduler can ignore
`desiredCount` temporarily during a deployment. For example, if `desiredCount` is four
tasks, a minimum of 50% allows the scheduler to stop two existing tasks before starting two
new tasks. Tasks for services that don't use a load balancer are considered healthy if
they're in the `RUNNING` state. Tasks for services that use a load balancer are considered
...
this manner even though you can choose a different placement strategy. <ul> <li>Sort the
valid container instances by the fewest number of running tasks for this service in the
same Availability Zone as the instance. For example, if zone A has one running service task
and zones B and C each have zero, valid container instances in either zone B or C are
...
running tasks for this service.
 </li> </ul>When the service scheduler stops running tasks, it attempts to maintain balance
across the Availability Zones in your cluster using the following logic:  - Sort the
container instances by the largest number of running tasks for this service in the same
Availability Zone as the instance. For example, if zone A has one running service task and
zones B and C each have two, container instances in either zone B or C are considered
...
- `"forceNewDeployment"`: Determines whether to force a new deployment of the service. By
  default, deployments aren't forced. You can use this option to start a new deployment
...
- `"healthCheckGracePeriodSeconds"`: The period of time, in seconds, that the Amazon ECS
  ...
  2,147,483,647 seconds. During that time, the Amazon ECS service scheduler ignores the
  Elastic Load Balancing health check status. This grace period can prevent the ECS service
...
- `"loadBalancers"`: A list of Elastic Load Balancing load balancer objects. It contains
  the load balancer name, the container name, and the container port to access from the
  ...
  target groups by using ` [CreateDeployment](https://docs.aws.amazon.com/codedeploy/latest/APIReference/API_CreateDeployment.html)
...
- `"serviceRegistries"`: The details for the service discovery registries to assign to this
  service. For more information, see [Service Discovery](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-discovery.html).
...
- `"volumeConfigurations"`: The details of the volume that was `configuredAtLaunch`. You
  can configure the size, volumeType, IOPS, throughput, snapshot and encryption in [ServiceManagedEBSVolumeConfiguration](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_ServiceManagedEBSVolumeConfiguration.html).
...
"""
update_service(service; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateService", Dict{String, Any}("service"=>service); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_service(service, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateService", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("service"=>service), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `service`: The short name or full Amazon Resource Name (ARN) of the service that the task
  set exists in.
"""
update_service_primary_task_set(cluster, primaryTaskSet, service; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateServicePrimaryTaskSet", Dict{String, Any}("cluster"=>cluster, "primaryTaskSet"=>primaryTaskSet, "service"=>service); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_service_primary_task_set(cluster, primaryTaskSet, service, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateServicePrimaryTaskSet", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("cluster"=>cluster, "primaryTaskSet"=>primaryTaskSet, "service"=>service), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
property. The `expiresInMinutes` property is always reset when you invoke this operation
for a task that already has `protectionEnabled` set to `true`. You can keep extending the
...
- `"expiresInMinutes"`: If you set `protectionEnabled` to `true`, you can specify the
  ...
  (2 hours).
"""
update_task_protection(cluster, protectionEnabled, tasks; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateTaskProtection", Dict{String, Any}("cluster"=>cluster, "protectionEnabled"=>protectionEnabled, "tasks"=>tasks); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_task_protection(cluster, protectionEnabled, tasks, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateTaskProtection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("cluster"=>cluster, "protectionEnabled"=>protectionEnabled, "tasks"=>tasks), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
update_task_set(cluster, scale, service, taskSet; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateTaskSet", Dict{String, Any}("cluster"=>cluster, "scale"=>scale, "service"=>service, "taskSet"=>taskSet); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_task_set(cluster, scale, service, taskSet, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ecs("UpdateTaskSet", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("cluster"=>cluster, "scale"=>scale, "service"=>service, "taskSet"=>taskSet), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
