...
- `name`: The name of the data catalog to create. The catalog name must be unique for the
  ...
  - The catalog name allows special characters such as `_ , @ , \\ , -`. These characters
  ...
  - The catalog name has a theoretical limit of 128 characters. However, since we use it to
    create other resources that allow less characters and we prepend a prefix to it, the
    actual catalog name limit for `FEDERATED` catalog is 64 - 23 = 41 characters.

- `type`: The type of data catalog to create: `LAMBDA` for a federated catalog, `GLUE` for
  an Glue Data Catalog, and `HIVE` for an external Apache Hive metastore. `FEDERATED` is a
...
- `"Parameters"`: Specifies the Lambda function or functions to use for creating the data
  ...
  - The `FEDERATED` data catalog type uses one of the following parameters, but not both.
    Use `connection-arn` for an existing Glue connection. Use `connection-type` and
    `connection-properties` to specify the configuration setting for a new connection.
    - `connection-arn:*<glue_connection_arn_to_reuse>*`
    - `lambda-role-arn` (optional): The execution role to use for the Lambda function. If
      not provided, one is created.
...
- `"Tags"`: A list of comma separated tags to add to the data catalog that is created. All
  the resources that are created by the `CreateDataCatalog` API operation with `FEDERATED`
  type will have the tag `federated_athena_datacatalog="true"`. This includes the CFN Stack,
...
- `"DeleteCatalogOnly"`: Deletes the Athena Data Catalog. You can only use this with the
  `FEDERATED` catalogs. You usually perform this before registering the connector with Glue
  Data Catalog. After deletion, you will have to manage the Glue Connection and Lambda
  function.
"""
...
Athena query results location in Amazon S3. For more information, see [Working with query results, recent queries, and output files](https://docs.aws.amazon.com/athena/latest/ug/querying.html)
in the *Amazon Athena User Guide*. This request does not execute the query but returns
results. Use [`start_query_execution`](@ref) to run a query.
...
- `"QueryResultType"`: When you set this to `DATA_ROWS` or empty, `GetQueryResults` returns
  the query results in rows. If set to `DATA_MANIFEST`, it returns the manifest file in
  rows. Only the query types `CREATE TABLE AS SELECT`, `UNLOAD`, and `INSERT` can generate a
  manifest file. If you use `DATA_MANIFEST` for other query types, the query will fail.
"""
...
be available immediately after a query completes or, in some cases, may not be returned. The
non-timeline statistics are also not included when a query has row-level filters defined in
Lake Formation.
...
"""
    get_resource_dashboard(resource_arn)
    get_resource_dashboard(resource_arn, params::Dict{String,<:Any})

Gets the Live UI/Persistence UI for a session.

# Arguments

- `resource_arn`: The The Amazon Resource Name (ARN) for a session.
"""
function get_resource_dashboard end

function get_resource_dashboard(
...
"""
    get_session_endpoint(session_id)
    get_session_endpoint(session_id, params::Dict{String,<:Any})
...
- `session_id`: The session ID.
"""
function get_session_endpoint end

function get_session_endpoint(SessionId; aws_config::AbstractAWSConfig=current_aws_config())
    return athena(
        "GetSessionEndpoint",
        Dict{String,Any}("SessionId" => SessionId);
        aws_config,
...
that can be imported is 10 megabytes. If an `ipynb` file with the same name already exists
in the workgroup, throws an error.
...
function list_application_dpusizes(; aws_config::AbstractAWSConfig=current_aws_config())
    return athena("ListApplicationDPUSizes"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_application_dpusizes(
    ...
    return athena(
        "ListApplicationDPUSizes", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_capacity_reservations(; aws_config::AbstractAWSConfig=current_aws_config())
    return athena("ListCapacityReservations"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_capacity_reservations(
    ...
    return athena(
        "ListCapacityReservations", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_data_catalogs(; aws_config::AbstractAWSConfig=current_aws_config())
    return athena("ListDataCatalogs"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_data_catalogs(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return athena("ListDataCatalogs", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_engine_versions(; aws_config::AbstractAWSConfig=current_aws_config())
    return athena("ListEngineVersions"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_engine_versions(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return athena("ListEngineVersions", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_named_queries(; aws_config::AbstractAWSConfig=current_aws_config())
    return athena("ListNamedQueries"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_named_queries(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return athena("ListNamedQueries", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_query_executions(; aws_config::AbstractAWSConfig=current_aws_config())
    return athena("ListQueryExecutions"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_query_executions(
    ...
    return athena(
        "ListQueryExecutions", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_work_groups(; aws_config::AbstractAWSConfig=current_aws_config())
    return athena("ListWorkGroups"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_work_groups(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return athena("ListWorkGroups", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- `"ClientRequestToken"`: A unique case-sensitive string used to ensure the request to
  create the query is idempotent (executes only once). If another `StartQueryExecution`
...
- `"EngineConfiguration"`: The engine configuration for the workgroup, which includes the
  minimum/maximum number of Data Processing Units (DPU) that queries should use when running
  in provisioned capacity. If not specified, Athena uses default values (Default value for
  min is 4 and for max is Minimum of 124 and allocated DPUs).

  To specify minimum and maximum DPU values for Capacity Reservations queries, the workgroup
  containing `EngineConfiguration` should have the following values: The name of the
  `Classifications` should be `athena-query-engine-properties`, with the only allowed
...
- `"ExecutionRole"`: The ARN of the execution role used to access user resources for Spark
  sessions and Identity Center enabled workgroups. This property applies only to Spark
  enabled workgroups and Identity Center enabled workgroups.

- `"MonitoringConfiguration"`: Contains the configuration settings for managed log
  persistence, delivering logs to Amazon S3 buckets, Amazon CloudWatch log groups etc.
...
!!! note
    ...
    charged for a calculation that cannot be cancelled, consider terminating the session in
    which the calculation is running.
...
Terminates an active session. A `TerminateSession` call on a session that is already
inactive (for example, in a `FAILED`, `TERMINATED` or `TERMINATING` state) succeeds but has
no effect. Calculations running in the session when `TerminateSession` is called are
