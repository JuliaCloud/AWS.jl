...
not valid. The AssociateNode API call can be integrated into Auto Scaling configurations,
AWS Cloudformation templates, or the user data of a server's instance.

# Arguments

- `engine_attributes`: Engine attributes used for associating the node.
  ...
  default only one organization named `default` can exist.
   - `CHEF_NODE_PUBLIC_KEY`: A PEM-formatted public key. This key is required for the `chef-
  client` agent to access the Chef API.

  ## Attributes accepted in a AssociateNode request for Puppet

  - `PUPPET_NODE_CSR`: A PEM-formatted certificate-signing request (CSR) that is created by
  the node.
- `node_name`: The name of the node.
- `server_name`: The name of the server with which to associate the node.
"""
function associate_node end
...
An `InvalidStateException` is thrown when the server is not in any of the following states:
...
- `"Tags"`: A map that contains tag keys and tag values to attach to an AWS OpsWorks-CM
  server backup. - The key cannot be empty.
...
(10). A `ResourceAlreadyExistsException` is thrown when a server with the same name already
exists in the account. A `ResourceNotFoundException` is thrown when you specify a backup ID
that is not valid or is for a backup that does not exist. A `ValidationException` is thrown
...
your security group rules to allow access from known IP addresses and address ranges only.
To edit security group rules, open Security Groups in the navigation pane of the EC2
management console.

To specify your own domain for a server, and provide your own self-signed or CA-signed
certificate and private key, specify values for `CustomDomain`, `CustomCertificate`, and
`CustomPrivateKey`.

# Arguments

- `engine`:  The configuration management engine to use. Valid values include
...
- `instance_profile_arn`:  The ARN of the instance profile that your Amazon EC2 instances
  ...
  template, located at https://s3.amazonaws.com/opsworks-cm-us-east-1-prod-default-
  assets/misc/opsworks-cm-roles.yaml. This template creates a CloudFormation stack that
...
- `service_role_arn`:  The service role that the AWS OpsWorks CM service backend uses to
  ...
  the service role for you, if you are using the AWS CLI or API commands, run the service-
  role-creation.yaml AWS CloudFormation template, located at
  https://s3.amazonaws.com/opsworks-cm-us-east-1-prod-default-assets/misc/opsworks-cm-
  roles.yaml. This template creates a CloudFormation stack that includes the service role
...
- `"CustomCertificate"`: A PEM-formatted HTTPS certificate. The value can be be a single,
  ...
  custom certificate, or the full certificate chain.
   - The certificate must be a valid X509 certificate, or a certificate chain in PEM
  format.
   - The certificate must be valid at the time of upload. A certificate can't be used
  before its validity period begins (the certificate's `NotBefore` date), or after it
...
- `"CustomDomain"`: An optional public endpoint of a server, such as `https://aws.my-
  company.com`. To access the server, create a CNAME DNS record in your preferred DNS
  service that points the custom domain to the endpoint that is generated when the server
  is created (the value of the CreateServer Endpoint attribute). You cannot access the
  server by using the generated `Endpoint` value if the server is using a custom domain. If
...
- `"EngineAttributes"`: Optional engine attributes on a specified server.
  ...
  key is required to access the Chef API. When no CHEF_AUTOMATE_PIVOTAL_KEY is set, a
...
- `"EngineVersion"`:  The major release version of the engine that you want to use. For a
  Chef server, the valid value for EngineVersion is currently `2`. For a Puppet server,
  valid values are `2019` or `2017`.
- `"KeyPair"`:  The Amazon EC2 key pair to set for the instance. This parameter is
  optional; if desired, you may specify this parameter to connect to your instances by
  using SSH.
- `"PreferredBackupWindow"`:  The start time for a one-hour period during which AWS
  ...
  enabled. Valid values must be specified in one of the following formats: </p> - `HH:MM`
  for daily backups
   ...
   **Example:** `08:00`, which represents a daily start time of 08:00 UTC.
   ...
   <p> **Example:** `Mon:08:00`, which represents a start time of every Monday at 08:00
  UTC. (8:00 a.m.)
- `"PreferredMaintenanceWindow"`:  The start time for a one-hour period each week during
  which AWS OpsWorks CM performs maintenance on the instance. Valid values must be
  specified in the following format: `DDD:HH:MM`. `MM` must be specified as `00`. The
  specified time is in coordinated universal time (UTC). The default value is a random one-
  hour period on Tuesday, Wednesday, or Friday. See `TimeWindowDefinition` for more
  information.
...
not specify a BackupId or ServerName, the command returns all backups.
...
- `"NextToken"`: NextToken is a string that is returned in some command responses. It
  indicates that not all entries have been returned, and that you must run at least one
...
"""
function describe_node_association_status end
...
A node can can only be disassociated from a server that is in a `HEALTHY` state. Otherwise,
an `InvalidStateException` is thrown. A `ResourceNotFoundException` is thrown when the
server does not exist. A `ValidationException` is raised when parameters of the request are
not valid.
...
- `"EngineAttributes"`: Engine attributes that are used for disassociating the node. No
  ...
  default only one organization named `default` can exist.
"""
function disassociate_node end
...
can export user data that you can use in EC2 to associate nodes with a server.
...
- `export_attribute_name`: The name of the export attribute. Currently, the supported
  export attribute is `Userdata`. This exports a user data script that includes parameters
  and values provided in the `InputAttributes` list.
...
- `"InputAttributes"`: The list of engine attributes. The list type is `EngineAttribute`.
  An `EngineAttribute` list item is a pair that includes an attribute name and its value.
  For the `Userdata` ExportAttributeName, the following are supported engine attribute
  names. - **RunList** In Chef, a list of roles or recipes that are run in the specified
...
"""
function export_server_engine_attribute end
...
- `resource_arn`: The Amazon Resource Number (ARN) of an AWS OpsWorks for Chef Automate or
  ...
  example, `arn:aws:opsworks-cm:us-west-2:123456789012:server/test-owcm-server/EXAMPLE-66b0-
  4196-8274-d1a2bEXAMPLE`.
...
`UNHEALTHY`, or `TERMINATED` state. When you run RestoreServer, the server's EC2 instance
is deleted, and a new EC2 instance is configured. RestoreServer maintains the existing
server endpoint, so configuration management of the server's client devices (nodes) should
continue to work.

Restoring from a backup is performed by creating a new EC2 instance. If restoration is
successful, and the server is in a `HEALTHY` state, AWS OpsWorks CM switches traffic over
to the new instance. After restoration is finished, the old EC2 instance is maintained in a
...
- `"InstanceType"`:  The type of instance to restore. Valid values must be specified in the
  following format: `^([cm][34]|t2).*` For example, `m5.large`. Valid values are `m5.large`, `r5.xlarge`, and `r5.2xlarge`. If you do not specify this parameter, RestoreServer uses the instance type from the specified backup.
...
"""
function start_maintenance end
...
- `resource_arn`: The Amazon Resource Number (ARN) of a resource to which you want to apply
  tags. For example, `arn:aws:opsworks-cm:us-west-2:123456789012:server/test-owcm-
...
- `tags`: A map that contains tag keys and tag values to attach to AWS OpsWorks-CM servers
  or backups. - The key cannot be empty.
   - The key can be a maximum of 127 characters, and can contain only Unicode letters,
  numbers, or separators, or the following special characters: `+ - = . _ : /`
   - The value can be a maximum 255 characters, and contain only Unicode letters, numbers,
  or separators, or the following special characters: `+ - = . _ : /`
...
- `resource_arn`: The Amazon Resource Number (ARN) of a resource from which you want to
  remove tags. For example, `arn:aws:opsworks-cm:us-west-2:123456789012:server/test-owcm-
  server/EXAMPLE-66b0-4196-8274-d1a2bEXAMPLE`.
...
- `"DisableAutomatedBackup"`: Setting DisableAutomatedBackup to `true` disables automated
...
"""
function update_server_engine_attributes end
