...
443. If a KeyName is present, AWS OpsWorks enables SSH access. SSH is also open to the world
on TCP port 22.
...
- `service_role_arn`: The service role that the AWS OpsWorks CM service backend uses to work
  with your account. Although the AWS OpsWorks management console typically creates the
  service role for you, if you are using the AWS CLI or API commands, run the service-role-
  creation.yaml AWS CloudFormation template, located at https://s3.amazonaws.com/opsworks-
  cm-us-east-1-prod-default-assets/misc/opsworks-cm-roles.yaml. This template creates a
  CloudFormation stack that includes the service role and instance profile that you need.
...
- `"CustomDomain"`: An optional public endpoint of a server, such as
  ...
  the server is created (the value of the CreateServer Endpoint attribute). You cannot
...
- `"PreferredMaintenanceWindow"`: The start time for a one-hour period each week during
  ...
  in the following format: `DDD:HH:MM`. `MM` must be specified as `00`. The specified time
  is in coordinated universal time (UTC). The default value is a random one-hour period on
  Tuesday, Wednesday, or Friday. See `TimeWindowDefinition` for more information.
...
- `backup_id`: The ID of the backup to delete. Run the DescribeBackups command to get a list
...
- `"NextToken"`: NextToken is a string that is returned in some command responses. It
  ...
  there are no more results, the response object's `nextToken` parameter value is `null`.
  Setting a `nextToken` value that was not returned in your previous results causes an
...
A `ResourceNotFoundException` is thrown when no recent association or disassociation request
with the specified token is found, or when the server does not exist. A
`ValidationException` is raised when parameters of the request are not valid.

# Arguments

- `node_association_status_token`: The token returned in either the AssociateNodeResponse or
...
- `export_attribute_name`: The name of the export attribute. Currently, the supported export
  attribute is `Userdata`. This exports a user data script that includes parameters and
...
- `"NextToken"`: NextToken is a string that is returned in some command responses. It
  indicates that not all entries have been returned, and that you must run at least one more
  request to get remaining items. To get remaining results, call `ListTagsForResource`
  again, and assign the token from the previous results as the value of the `nextToken`
  parameter. If there are no more results, the response object's `nextToken` parameter value
  is `null`. Setting a `nextToken` value that was not returned in your previous results
...
endpoint, so configuration management of the server's client devices (nodes) should continue
to work.
...
- `"EngineAttributes"`: Engine attributes that are specific to the server on which you want
  ...
  - `CHEF_MAJOR_UPGRADE`: If a Chef Automate server is eligible for upgrade to Chef Automate
    2, add this engine attribute to a `StartMaintenance` request and set the value to `true`
to upgrade the server to Chef Automate 2. For more information, see [Upgrade an AWS OpsWorks for Chef Automate Server to Chef Automate 2](https://docs.aws.amazon.com/opsworks/latest/userguide/opscm-a2upgrade.html).
"""
...
This operation can only be called for servers in `HEALTHY` or `UNHEALTHY` states. Otherwise,
an `InvalidStateException` is raised. A `ResourceNotFoundException` is thrown when the
server does not exist. A `ValidationException` is raised when parameters of the request are
not valid.
