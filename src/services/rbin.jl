...
- **Tag-level retention rules** - These retention rules use resource tags to identify the
  resources to protect. For each retention rule, you specify one or more tag key and value
  pairs. Resources (of the specified type) that have at least one of these tag key and value
  pairs are automatically retained in the Recycle Bin upon deletion. Use this type of
  retention rule to protect specific resources in your account based on their tags.
- **Region-level retention rules** - These retention rules, by default, apply to all of the
  resources (of the specified type) in the Region, even if the resources are not tagged.
  However, you can specify exclusion tags to exclude resources that have specific tags. Use
  this type of retention rule to protect all resources of a specific type in a Region.

For more information, see [Create Recycle Bin retention rules](https://docs.aws.amazon.com/ebs/latest/userguide/recycle-bin.html)
...
- `retention_period`: Information about the retention period for which the retention rule is
  to retain resources.
...
- `resource_type`: The resource type retained by the retention rule. Only retention rules
  that retain the specified resource type are listed. Currently, only EBS volumes, EBS
...
"""
    lock_rule(identifier, lock_configuration)
    lock_rule(identifier, lock_configuration, params::Dict{String,<:Any})
...
function lock_rule(
    Identifier, LockConfiguration; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rbin(
        "PATCH",
...
- `resource_arn`: The Amazon Resource Name (ARN) of the retention rule.
- `tag_keys`: The tag keys of the tags to unassign. All tags that have the specified tag key
  are unassigned.
"""
