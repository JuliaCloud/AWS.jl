...
Makes a copy of an existing snapshot.</p>
...
use an IAM policy to control who has the ability to use the `CopySnapshot` operation. For
more information about using IAM to control the use of ElastiCache operations, see [Exporting Snapshots](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/backups-exporting.html)
and [Authentication &amp; Access Control](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/IAM.html).You
could receive the following error messages.
...
Bucket.
 ...
 <p> **Solution:** Add View Permissions on the bucket. For more information, see [Step 2: Grant ElastiCache Access to Your Amazon S3 Bucket](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/backups-exporting.html#backups-exporting-grant-access)
in the ElastiCache User Guide.
...
- `"TargetBucket"`: The Amazon S3 bucket to which the snapshot is exported. This parameter
  ...
  permissions to this S3 bucket. For more information, see [Step 2: Grant ElastiCache Access to Your Amazon S3 Bucket](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/backups-exporting.html#backups-exporting-grant-access)
...
- `"CacheNodeType"`: The compute and memory capacity of the nodes in the node group
  ...
  to their equivalent previous generation counterparts.</p> - General purpose: <ul>
  ...
  **M6g node types** (available only for Redis OSS engine version 5.0.6 onward and for
  ...
  `cache.m3.2xlarge`
   </li> <li>Compute optimized: - Previous generation: (not recommended. Existing clusters
  are still supported but creation of new clusters is not supported for these types.)

   **C1 node types:** `cache.c1.xlarge`
   </li> <li>Memory optimized: - Current generation:
  ...
  **R6g node types** (available only for Redis OSS engine version 5.0.6 onward and for
   ...
   **M2 node types:** `cache.m2.xlarge`, `cache.m2.2xlarge`, `cache.m2.4xlarge`
   ...
   <p> **R3 node types:** `cache.r3.large`, `cache.r3.xlarge`, `cache.r3.2xlarge`,
  `cache.r3.4xlarge`, `cache.r3.8xlarge`
   </li> </ul> **Additional node type info**  - All current generation instance types are
...
- `"CacheParameterGroupName"`: The name of the parameter group to associate with this
  ...
  engine is used. You cannot use any parameter group which has `cluster-enabled='yes'` when
  creating a cluster.
...
- `"PreferredAvailabilityZones"`: A list of the Availability Zones in which cache nodes are
  ...
  This option is only supported on Memcached.</p>
  ...
  The number of Availability Zones listed must equal the value of `NumCacheNodes`.If you
  want all the nodes in the same Availability Zone, use `PreferredAvailabilityZone`
  instead, or repeat the Availability Zone multiple times in the list.

 <p>Default: System chosen Availability Zones.
...
- `"SnapshotArns"`: A single-element string list containing an Amazon Resource Name (ARN)
  ...
  !!! note
      This parameter is only valid if the `Engine` parameter is `redis`.Example of an
...
- `"SnapshotRetentionLimit"`: The number of days for which ElastiCache retains automatic
  ...
  !!! note
      This parameter is only valid if the `Engine` parameter is `redis`.Default: 0 (i.e.,
...
can change the values of specific parameters. For more information, see: - [ModifyCacheParameterGroup](https://docs.aws.amazon.com/AmazonElastiCache/latest/APIReference/API_ModifyCacheParameterGroup.html)
...
- `replication_group_id`: The replication group identifier. This parameter is stored as a
  ...
  Constraints: - A name must contain from 1 to 40 alphanumeric characters or hyphens.
   - The first character must be a letter.
   - A name cannot end with a hyphen or contain two consecutive hyphens.
...
- `"CacheParameterGroupName"`: The name of the parameter group to associate with this
  ...
  want to use a default parameter group, we recommend that you specify the parameter group
  by name.  - To create a Redis OSS (cluster mode disabled) replication group, use
...
- `"NodeGroupConfiguration"`: A list of node group (shard) configuration options. Each node
  ...
  a Redis OSS (cluster mode enabled) cluster from a S3 rdb file. You must configure each
  node group (shard) using this parameter because you must specify the slots for each node
  group.
- `"NotificationTopicArn"`: The Amazon Resource Name (ARN) of the Amazon Simple
  Notification Service (SNS) topic to which notifications are sent.
...
- `"PreferredCacheClusterAZs"`: A list of EC2 Availability Zones in which the replication
  ...
  order in which clusters are allocated. The primary cluster is created in the first AZ in
  the list.

  This parameter is not used if there is more than one node group (shard). You should use
  `NodeGroupConfiguration` instead.</p>
...
- `"SnapshotArns"`: A list of Amazon Resource Names (ARN) that uniquely identify the Redis
  ...
  by the parameter *NumNodeGroups* or the number of node groups configured by
...
- `"NewReplicaCount"`: The number of read replica nodes you want at the completion of this
  ...
  mode disabled) <ul> <li>If Multi-AZ is enabled: 1
   - If Multi-AZ is not enabled: 0
   </li> <li>Redis OSS (cluster mode enabled): 0 (though you will not be able to failover
  to a replica if your primary node fails) </li> </ul>
...
This operation is not valid for: - Redis OSS (cluster mode enabled) clusters
...
Deleting a Global datastore is a two-step process: </p> - First, you must
<a>DisassociateGlobalReplicationGroup</a> to remove the secondary clusters in the Global
datastore.
...
- `"AZMode"`: Specifies whether the new nodes in this Memcached cluster are all created in
  ...
  This option is only supported for Memcached clusters.</p>
...
- `"AuthToken"`: Reserved parameter. The password used to access a password protected
  ...
  constraints: - Must be only printable ASCII characters
   - Must be at least 16 characters and no more than 128 characters in length
...
- `"AuthTokenUpdateStrategy"`: Specifies the strategy to use to update the AUTH token. This
  ...
  default, if no update strategy is provided
...
- `"NewAvailabilityZones"`:
  ...
  !!! note
      This option is only supported on Memcached clusters.</p>The list of Availability
  Zones where the new Memcached cache nodes are created.
  ...
  Scenarios: - **Scenario 1:** You have 3 active nodes and wish to add 2 nodes. Specify
  ...
  **Availability Zone Considerations** section of [Cache Node Considerations for Memcached](https://docs.aws.amazon.com/AmazonElastiCache/latest/mem-ug/CacheNodes.SupportedTypes.html).

   <p> **Impact of new add/remove requests upon pending requests**  - Scenario-1 <ul>
  <li>Pending Action: Delete
   - New Request: Delete
   - Result: The new delete, pending or immediate, replaces the pending delete.
   </li> <li>Scenario-2 - Pending Action: Delete
   - New Request: Create
   - Result: The new create, pending or immediate, replaces the pending delete.
   </li> <li>Scenario-3 - Pending Action: Create
   - New Request: Delete
   - Result: The new delete, pending or immediate, replaces the pending create.
   </li> <li>Scenario-4 - Pending Action: Create
   - New Request: Create
   - Result: The new create is added to the pending create.

  !!! important
      **Important:** If the new create request is **Apply Immediately - Yes**, all creates
  are performed immediately. If the new create request is **Apply Immediately - No**, all
  creates are pending.
 </li> </ul>
...
Modifies the settings for a replication group. This is limited to Redis OSS 7 and newer. - [Scaling for Amazon ElastiCache (Redis OSS) (cluster mode enabled)](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/scaling-redis-cluster-mode-enabled.html)
in the ElastiCache User Guide
...
- `"AuthToken"`: Reserved parameter. The password used to access a password protected
  ...
  parameter. Password constraints: - Must be only printable ASCII characters
   - Must be at least 16 characters and no more than 128 characters in length
   - Cannot contain any of the following characters: '/', '"', or '@', '%'
   For more information, see AUTH password at [AUTH](http://redis.io/commands/AUTH).
- `"AuthTokenUpdateStrategy"`: Specifies the strategy to use to update the AUTH token. This
...
large-scale operational events, Amazon may block this API. </p>
...
(cluster mode enabled) replication group, the first node replacement must complete before a
...
automatic failover related events, listed here in order of occurrance: <li>Replication
group message: `Test Failover API called for node group &lt;node-group-id&gt;`
 - Cache cluster message: `Failover from primary node &lt;primary-node-id&gt; to replica
node &lt;node-id&gt; completed`
 - Replication group message: `Failover from primary node &lt;primary-node-id&gt; to
replica node &lt;node-id&gt; completed`
 - Cache cluster message: `Recovering cache nodes &lt;node-id&gt;`
 - Cache cluster message: `Finished recovery for cache nodes &lt;node-id&gt;`For more
information see: <ul> - [Viewing ElastiCache Events](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/ECEvents.Viewing.html)
in the *ElastiCache User Guide*
 - [DescribeEvents](https://docs.aws.amazon.com/AmazonElastiCache/latest/APIReference/API_DescribeEvents.html)
in the ElastiCache API Reference
 </li> </ul>Also see, [Testing Multi-AZ ](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/AutoFailover.html#auto-failover-test)
in the *ElastiCache User Guide*.
