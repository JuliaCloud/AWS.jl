 ...
 **Bangladesh** </p> - You must specify the tax registration certificate document in the
`taxRegistrationDocuments` field of the `VerificationDetails` object.
 **Brazil**  - You must complete the tax registration process in the [Payment preferences](https://console.aws.amazon.com/billing/home#/paymentpreferences/paymentmethods)
 ...
 **Georgia**  - The valid `personType` values are `Physical Person` and `Business`.
 **Kenya**  - You must specify the `personType` in the `kenyaAdditionalInfo` field of the
...
object.
 **Malaysia**  - If you use this operation to set a tax registration number (TRN) in
Malaysia, only resellers with a valid sales and service tax (SST) number are required to
provide tax registration information.
 - By using this API operation to set a TRN in Malaysia, Amazon Web Services will regard
you as self-declaring that you're an authorized business reseller registered with the Royal
...
IT service - 9907101676
 ...
 <p>Digital services and electronic medium - 9907121690
 **Nepal**  - The sector valid values are `Business` and `Individual`.
 **Saudi Arabia**  - For `address`, you must specify `addressLine3`.
 **South Korea**  - You must specify the `certifiedEmailId` and `legalName` in the
 ...
 **Spain**  - You must specify the `registrationType` in the `spainAdditionalInfo` field of
 ...
 **Turkey**  - You must specify the `sector` in the `taxRegistrationEntry` object.
 - If your `sector` is `Business`, `Individual`, or `Government`: <ul> <li>Specify the
...
**Public institutions**
 </li> <li>If your `sector` is `Business` and you're subject to KDV tax, you must specify
your industry in the `industries` field. </li> <li>For `address`, you must specify
`districtOrCounty`. </li> </ul> **Ukraine**  - The sector valid values are `Business` and
`Individual`.
