...
!!! note
    ...
    - If you are working with an AWS SDK, use the QLDB driver. The driver provides a high-
      level abstraction layer above this *QLDB Session* data plane and manages `SendCommand`
      API calls for you. For information and a list of supported programming languages, see [Getting started with the driver](https://docs.aws.amazon.com/qldb/latest/developerguide/getting-started-driver.html)
...
function send_command(; aws_config::AbstractAWSConfig=current_aws_config())
    return qldb_session("SendCommand"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function send_command(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return qldb_session("SendCommand", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
