...
- `query_type`: The type of query to run. The following are the three types of queries that
  ...
  - `OVERALL_TRAFFIC_SUGGESTIONS_DETAILS`: Provides TTFB, using a 30-day weighted average,
    for each top location, for a proposed Amazon Web Services location. Must provide an
    Amazon Web Services location to search.
  - `ROUTING_SUGGESTIONS`: Provides the predicted average round-trip time (RTT) from an IP
    prefix toward an Amazon Web Services location for a DNS resolver. The RTT is calculated
