...
- `api_caching_behavior`: Caching behavior. - **FULL_REQUEST_CACHING**: All requests are
  fully cached.
...
- `type`: The cache instance type. Valid values are </p> - `SMALL`
   - `MEDIUM`
   - `LARGE`
   - `XLARGE`
   - `LARGE_2X`
  ...
  is deprecated, and the generic identifiers above should be used.
   ...
   <p>The following legacy instance types are available, but their use is discouraged: -
...
- `"healthMetricsConfig"`: Controls how cache health metrics will be emitted to CloudWatch.
  ...
  dropped because the throughput exceeded the aggregated bandwidth limit. This is useful
...
- `"kind"`: The resolver type. - **UNIT**: A UNIT resolver type. A UNIT resolver is the
  default resolver type. You can use a UNIT resolver to run a GraphQL query against a
  single data source.
   - **PIPELINE**: A PIPELINE resolver type. You can use a PIPELINE resolver to invoke a
  series of `Function` objects in a serial manner. You can use a pipeline resolver to run a
...
When creating an environmental variable, it must follow the constraints below: - Both
JavaScript and VTL templates support environmental variables.
...
- `environment_variables`: The list of environmental variables to add to the API.
  ...
  `environmentVariables` payload as a list in the format `{"key1":"value1","key2":"value2",
  …}`. Note that each call of the `PutGraphqlApiEnvironmentVariables` action will result in
  the overwriting of the existing environmental variable list of that API. This means the
