...
operation still succeeds, even if no job is canceled. These jobs must be terminated with the [`terminate_job`](@ref)
operation.
...
capacity in your managed compute environment. You can optionally set a maximum price so that
...
find the Amazon ECS cluster that's associated with it. Then, launch your container instances
...
- `"serviceRole"`: The full Amazon Resource Name (ARN) of the IAM role that allows Batch to
  ...
  !!! important
      ...
      Batch service-linked role doesn't exist in your account, and no role is specified
  ...
  information, see [Friendly names and paths](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_identifiers.html#identifiers-friendly-names)
...
- `"nextToken"`: The `nextToken` value returned from a previous paginated
  `DescribeComputeEnvironments` request where `maxResults` was used and the results exceeded
...
- `"jobDefinitions"`: A list of up to 100 job definitions. Each entry in the list can either
...
- `"maxResults"`: The maximum number of results returned by `DescribeJobQueues` in paginated
  output. When this parameter is used, `DescribeJobQueues` only returns `maxResults` results
...
- `"nextToken"`: The `nextToken` value returned from a previous paginated
  `DescribeJobQueues` request where `maxResults` was used and the results exceeded the value
...
- `"filters"`: The filter to apply to the query. Only one filter can be used at a time. When
  ...
  for the filter is the job definition name, the results include all the jobs that used any
  revision of that job definition name. If the value ends with an asterisk (*), the filter
  matches any job definition name that begins with the string before the '*'. For example,
  `jd1` matches only `jd1`, and `jd1*` matches both `jd1` and `jd1A`. The version of the job
  definition that's used doesn't affect the sort order. When the `JOB_DEFINITION` filter is
...
- `"maxResults"`: The maximum number of results returned by `ListJobs` in a paginated
  ...
  - If neither parameter is used, then `ListJobs` returns up to 1000 results (jobs that are
    in the `RUNNING` status) and a `nextToken` value, if applicable.
...
- `"maxResults"`: The maximum number of results that's returned by `ListSchedulingPolicies`
  ...
  request with the returned `nextToken` value. This value can be between 1 and 100. If this
  parameter isn't used, `ListSchedulingPolicies` returns up to 100 results and a `nextToken`
  value if applicable.
...
- `"nodeProperties"`: An object with properties specific to multi-node parallel jobs. If you
  specify node properties for a job, it becomes a multi-node parallel job. For more
...
!!! important
    ...
    This is because, after 14 days, Fargate resources might become unavailable and job might
    be terminated.
...
- `"containerOverrides"`: An object with properties that override the defaults for the job
  ...
  is specified in the job definition or the Docker image, with a `command` override. You can
...
- `"dependsOn"`: A list of dependencies for the job. A job can depend upon a maximum of 20
  ...
  jobs so that each child array job completes sequentially, starting at index 0. You can
  also specify an `N_TO_N` type dependency with a job ID for array jobs. In that case, each
...
- `"priority"`: The priority of the job queue. Job queues with a higher priority (or a
  ...
  job queue with a priority value of `1`. All of the compute environments must be either EC2
  (`EC2` or `SPOT`) or Fargate (`FARGATE` or `FARGATE_SPOT`). EC2 and Fargate compute
  environments can't be mixed.
