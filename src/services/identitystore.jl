...
- `IdentityStoreId`: The globally unique identifier for the identity store, such as
...
- `Filters`: A list of Filter objects, which is used in the ListUsers and ListGroups
  request.
- `MaxResults`: The maximum number of results to be returned per request, which is used in
  the ListUsers and ListGroups request to specify how many results to return in one page. The
  length limit is 50 characters.
- `NextToken`: The pagination token used for the ListUsers and ListGroups APIs. This value
  is generated by the identity store service and is returned in the API response if the total
  results are more than the size of one page, and when this token is used in the API request
