...
# Required Parameters
- `RecipeVersions`: An array of version identifiers, for the recipe versions to be deleted.
  You can specify numeric versions (X.Y) or LATEST_WORKING. LATEST_PUBLISHED is not supported.
...
- `OutputLocation`:
- `RoleArn`: The Amazon Resource Name (ARN) of the AWS Identity and Access Management (IAM)
...
- `JobSample`: Sample configuration for profile jobs only. Determines the number of rows on
  which the profile job will be executed. If a JobSample value is not provided, the default
  value will be used. The default value is CUSTOM_ROWS for the mode parameter and 20000 for
  the size parameter.
...
- `MaxCapacity`: The maximum number of nodes that DataBrew can consume when the job
  processes data.
- `MaxRetries`: The maximum number of times to retry the job after a job run fails.
- `ProjectName`: Either the name of an existing project, or a combination of a recipe and a
...
# Required Parameters
- `CronExpression`: The date or dates and time or times when the jobs are to be run. For
...
- `recipeVersion`: The version of the recipe to be deleted. You can specify a numeric
...
# Optional Parameters
- `datasetName`: The name of a dataset. Using this parameter indicates to return only those
  jobs that act on the specified dataset.
- `maxResults`: The maximum number of results to return in this request.
- `nextToken`: A token generated by DataBrew that specifies where to continue pagination if
  a previous request was truncated. To get the next set of pages, pass in the NextToken value
  from the response object of the previous page call.
- `projectName`: The name of a project. Using this parameter indicates to return only those
...
# Required Parameters
- `ResourceArn`: The Amazon Resource Name (ARN) string that uniquely identifies the
  DataBrew resource.
...
# Optional Parameters
- `AssumeControl`: A value that, if true, enables you to take control of a session, even if
...
# Required Parameters
- `ResourceArn`: The DataBrew resource to which tags should be added. The value for this
...
# Required Parameters
- `ResourceArn`: A DataBrew resource from which you want to remove a tag or tags. The value
...
- `EncryptionMode`: The encryption mode for the job, which can be one of the following:
  ...
  encryption with keys managed by Amazon S3.
- `JobSample`: Sample configuration for Profile Jobs only. Determines the number of rows on
  which the Profile job will be executed. If a JobSample value is not provided for profile
