...
- `"clientToken"`: A unique, case-sensitive identifier that you provide to ensure the
  ...
  once. With an idempotent request, if the original request completes successfully,
  subsequent retries with the same client token returns the result from the original
  successful request.
...
- `identity_provider_details`: The identity provider details. The following list describes
  ...
  - For Google and Login with Amazon:   - `client_id`
    - `client_secret`
    - `authorize_scopes`
...
- `"authenticationType"`: The type of authentication integration points used when signing
  ...
  integration), plus user and group access to your web portal, can be configured in the
  IAM Identity Center.
- `"clientToken"`: A unique, case-sensitive identifier that you provide to ensure the
  idempotency of the request. Idempotency ensures that an API request completes only
  once. With an idempotent request, if the original request completes successfully,
