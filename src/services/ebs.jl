...
# Required Parameters
- `blockIndex`: The block index of the block from which to get data. Obtain the BlockIndex
  by running the ListChangedBlocks or ListSnapshotBlocks operations.
- `blockToken`: The block token of the block from which to get data. Obtain the BlockToken
  by running the ListChangedBlocks or ListSnapshotBlocks operations.
...
- `startingBlockIndex`: The block index from which the comparison should start. The list in
  the response will start from this block index or the next valid block index in the
  snapshots.
"""
...
# Required Parameters
- `BlockData`: The data to write to the block. The block data is not signed as part of the
  ...
  Cloud User Guide.
- `blockIndex`: The block index of the block in which to write the data. A block index is a
...
- `snapshotId`: The ID of the snapshot.
- `x-amz-Checksum`: A Base64-encoded SHA256 checksum of the data. Only SHA256 checksums are
  supported.
- `x-amz-Checksum-Algorithm`: The algorithm used to generate the checksum. Currently, the
  only supported algorithm is SHA256.
- `x-amz-Data-Length`: The size of the data to write to the block, in bytes. Currently, the
  only supported size is 524288. Valid values: 524288
...
- `Encrypted`: Indicates whether to encrypt the snapshot. To create an encrypted snapshot,
  ...
  encrypted using the CMK specified using the KmsKeyArn parameter. If no value is specified
  for KmsKeyArn, the default CMK for your account is used. If no default CMK has been
  specified for your account, the AWS managed CMK is used. To set a default CMK for your
...
- `ParentSnapshotId`: The ID of the parent snapshot. If there is no parent snapshot, or if
  you are creating the first snapshot for an on-premises volume, omit this parameter. If your
  account is enabled for encryption by default, you cannot use an unencrypted snapshot as a
  parent snapshot. You must first create an encrypted copy of the parent snapshot using
  CopySnapshot.
