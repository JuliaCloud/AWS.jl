...
- `structured`: search specific fields, construct compound queries using Boolean operators,
  and use advanced features such as term boosting and proximity searching.
...
- `q`: Specifies the search criteria for the request. How you specify the search criteria
  depends on the query parser used for the request and the parser options specified in
...
- `"expr"`: Defines one or more numeric expressions that can be used to sort results or
  ...
  expressions, see [Writing Expressions](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/configuring-expressions.html#writing-expressions)
  in the *Amazon CloudSearch Developer Guide*.
- `"facet"`: Specifies one or more fields for which to get facet information, and options
  ...
  - `buckets` specifies an array of the facet values or ranges to count. Ranges are
    ...
    more information, see [Searching for a Range of Values](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching-ranges.html)
    in the *Amazon CloudSearch Developer Guide*. Buckets are returned in the order they
    are specified in the request. The `sort` and `size` options are not valid if you
    specify `buckets`.
  ...
  - `sort` specifies how you want to sort the facets in the results: `bucket` or `count`.
    ...
    descending order). To retrieve facet counts for particular values or ranges of
...
- `"fq"`: Specifies a structured query that filters the results of a search without
  ...
  in the `query` parameter. Specifying a filter controls only which matching documents
...
- `"highlight"`: Retrieves highlights for matches in the specified `text` or `text-array`
  ...
  - `pre_tag`: specifies the string to prepend to an occurrence of a search term. The
    default for HTML highlights is `&amp;lt;em&amp;gt;`. The default for text highlights
    is `*`.
...
- `"partial"`: Enables partial results to be returned if one or more index partitions are
  ...
  errors. When you enable partial results, Amazon CloudSearch returns whatever results
  are available and includes the percentage of documents searched in the search results
  (percent-searched). This enables you to more gracefully degrade your users' search
  experience. For example, rather than displaying no results, you could display the
  partial results and a message indicating that the results might be incomplete due to a
...
- `"q.options"`: Configures options for the query parser specified in the `queryParser`
  ...
  - `defaultOperator`: The default operator used to combine individual terms in the
    ...
    specify a percentage that represents the percentage of terms in the search string
    (rounded down) that must match, rather than a default operator. A value of `0%` is
    the equivalent to OR, and a value of `100%` is equivalent to AND. The percentage must
    be specified as a value in the range 0-100 followed by the percent (%) symbol. For
    example, `defaultOperator: 50%`. Valid values: `and`, `or`, a percentage in the range
    0%-100% (`dismax`). Default: `and` (`simple`, `structured`, `lucene`) or `100`
    (`dismax`). Valid for: `simple`, `structured`, `lucene`, and `dismax`.
  - `fields`: An array of the fields to search when no fields are specified in a search.
    ...
    text-array fields are searched. You can specify a weight for each field to control
    the relative importance of each field when Amazon CloudSearch calculates relevance
    scores. To specify a field weight, append a caret (`^`) symbol and the weight to the
    field name. For example, to boost the importance of the `title` field over the
    `description` field you could specify: `"fields":["title^5","description"]`. Valid values: The name of any configured field and an optional numeric value greater than zero. Default: All `text` and `text-array` fields. Valid for: `simple`, `structured`, `lucene`, and `dismax`.
- `operators`: An array of the operators or special characters you want to disable for the simple query parser. If you disable the `and`, `or`, or `not` operators, the corresponding operators (`+`, `|`, `-`)
    ...
    `prefix` disables the wildcard operator (`*`) and disabling `phrase` disables the
    ability to search for phrases by enclosing phrases in double quotes. Disabling
    precedence disables the ability to control order of precedence using parentheses.
    Disabling `near` disables the ability to use the ~ operator to perform a sloppy
    phrase search. Disabling the `fuzzy` operator disables the ability to use the ~
    operator to perform a fuzzy search. `escape` disables the ability to use a backslash
    (`\\`) to escape special characters within the search string. Disabling whitespace is
    an advanced option that prevents the parser from tokenizing on whitespace, which can
    be useful for Vietnamese. (It prevents Vietnamese words from being split
    incorrectly.) For example, you could disable all operators other than the phrase
    operator to support just simple term and phrase queries: `"operators":["and","not","or", "prefix"]`. Valid values: `and`, `escape`, `fuzzy`, `near`, `not`, `or`, `phrase`, `precedence`, `prefix`, `whitespace`. Default: All operators and special characters are enabled. Valid for: `simple`.
...
- `explicitPhraseSlop`: An integer value that specifies how much a match can deviate from the search phrase when the phrase is enclosed in double quotes in the search string. (Phrases that exceed this proximity distance are not considered a match.)
  ...
  - `tieBreaker`: When a term in the search string is found in a document's field, a
    ...
    compared to other documents. If the term occurs in multiple fields within a document,
    by default only the highest scoring field contributes to the document's overall
    score. You can specify a `tieBreaker` value to enable the matches in lower-scoring
    fields to contribute to the document's score. That way, if two documents have the
    same max field score for a particular term, the score for the document that has
    matches in more fields will be higher. The formula for calculating the score with a
    tieBreaker is
...
- `"q.parser"`: Specifies which query parser to use to process the request. If
  ...
  - `simple`: perform simple searches of `text` and `text-array` fields. By default, the
    ...
    enclose the phrase in double quotes. For more information, see [Searching for Text](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching-text.html)
    in the *Amazon CloudSearch Developer Guide*.
  - `structured`: perform advanced searches by combining multiple expressions to define
    the search criteria. You can also search within particular fields, search for values
    and ranges of values, and use advanced options such as term boosting, `matchall`, and
...
- `"return"`: Specifies the field and expression values to include in the response.
  ...
  document IDs for the matching documents, specify `_no_fields`. To retrieve the
...
- `"sort"`: Specifies the fields or custom expressions to use to sort the search results.
  ...
  specify the sort direction (`asc` or `desc`) for each field; for example,
  `year desc,title asc`. To use a field to sort results, the field must be sort-enabled
  in the domain configuration. Array type fields cannot be used for sorting. If no `sort`
  parameter is specified, results are sorted by their default relevance scores in
  descending order: `_score desc`. You can also sort by document ID (`_id asc`) and
