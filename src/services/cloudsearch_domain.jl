...
query parsers:</p> - `simple`: search all `text` and `text-array` fields for the specified
...
in the *Amazon CloudSearch Developer Guide*.
 ...
 <p>The endpoint for submitting `Search` requests is domain-specific. You submit search
...
- `q`: Specifies the search criteria for the request. How you specify the search criteria
  ...
  For more information about specifying search criteria, see [Searching Your Data](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching.html)
  in the *Amazon CloudSearch Developer Guide*.
...
- `"cursor"`: Retrieves a cursor value you can use to page through large result sets. Use
  ...
  specify either the `cursor` or `start` parameter in a request; they are mutually
  exclusive. To get the first cursor, set the cursor value to `initial`. In subsequent
...
- `"expr"`: Defines one or more numeric expressions that can be used to sort results or
  ...
  You specify the expressions in JSON using the form `{"EXPRESSIONNAME":"EXPRESSION"}`. You
  can define and use multiple expressions in a search request. For example:

  ` {"expression1":"_score*rating", "expression2":"(1/rank)*year"} `

  For information about the variables, operators, and functions you can use in expressions,
  see [Writing Expressions](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/configuring-expressions.html#writing-expressions)
  in the *Amazon CloudSearch Developer Guide*.
- `"facet"`: Specifies one or more fields for which to get facet information, and options
  that control how the facet information is returned. Each specified field must be facet-
  enabled in the domain configuration. The fields and options are specified in JSON using
  the form
  `{"FIELD":{"OPTION":VALUE,"OPTION:"STRING"},"FIELD":{"OPTION":VALUE,"OPTION":"STRING"}}`.

  You can specify the following faceting options:</p> - `buckets` specifies an array of the
  facet values or ranges to count. Ranges are specified using the same syntax that you use
  to search for a range of values. For more information, see [ Searching for a Range of Values](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching-ranges.html)
  in the *Amazon CloudSearch Developer Guide*. Buckets are returned in the order they are
  ...
  Amazon CloudSearch returns counts for the top 10. The `size` parameter is only valid when
  you specify the `sort` option; it cannot be used in conjunction with `buckets`.
  ...
  descending order). To retrieve facet counts for particular values or ranges of values,
  use the `buckets` option instead of `sort`.
...
` {"year":{"sort":"bucket"}} `
 ...
 <p>For more information, see [Getting and Using Facet Information](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/faceting.html) in the *Amazon CloudSearch Developer Guide*.
- `"fq"`: Specifies a structured query that filters the results of a search without
  ...
  with the `query` parameter to filter the documents that match the constraints specified
  in the `query` parameter. Specifying a filter controls only which matching documents are
  ...
  For more information about using filters, see [Filtering Matching Documents](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/filtering-results.html)
  in the *Amazon CloudSearch Developer Guide*.
- `"highlight"`: Retrieves highlights for matches in the specified `text` or `text-array`
  ...
  You can specify the following highlight options:</p> - `format`: specifies the format of
  the data in the text field: `text` or `html`. When data is returned as HTML, all non-
  ...
  HTML and the first match is highlighted with emphasis tags: `&amp;lt;em&gt;search-
  term&amp;lt;/em&amp;gt;`.

  For example, the following request retrieves highlights for the `actors` and `title`
  fields.

   <p> `{ "actors": {}, "title": {"format": "text","max_phrases": 2,"pre_tag":
  "**","post_tag": "**"} }`
...
- `"q.options"`: Configures options for the query parser specified in the `queryParser`
  ...
  percentage that represents the percentage of terms in the search string (rounded down)
  that must match, rather than a default operator. A value of `0%` is the equivalent to OR,
  and a value of `100%` is equivalent to AND. The percentage must be specified as a value
  in the range 0-100 followed by the percent (%) symbol. For example, `defaultOperator:
  50%`. Valid values: `and`, `or`, a percentage in the range 0%-100% (`dismax`). Default:
  `and` (`simple`, `structured`, `lucene`) or `100` (`dismax`). Valid for: `simple`,
  ...
  could specify: `"fields":["title^5","description"]`. Valid values: The name of any configured field and an optional numeric value greater than zero. Default: All `text` and `text-array` fields. Valid for: `simple`, `structured`, `lucene`, and `dismax`.
 - `operators`: An array of the operators or special characters you want to disable for the simple query parser. If you disable the `and`, `or`, or `not` operators, the corresponding operators (`+`, `|`, `-`)
  have no special meaning and are dropped from the search string. Similarly, disabling
  `prefix` disables the wildcard operator (`*`) and disabling `phrase` disables the ability
  to search for phrases by enclosing phrases in double quotes. Disabling precedence
  ...
  disable all operators other than the phrase operator to support just simple term and
  phrase queries: `"operators":["and","not","or", "prefix"]`. Valid values: `and`, `escape`, `fuzzy`, `near`, `not`, `or`, `phrase`, `precedence`, `prefix`, `whitespace`. Default: All operators and special characters are enabled. Valid for: `simple`.
 - `phraseFields`: An array of the `text` or `text-array` fields you want to use for phrase searches. When the terms in the search string appear in close proximity within a field, the field scores higher. You can specify a weight for each field to boost that score. The `phraseSlop` option controls how much the matches can deviate from the search string and still be boosted. To specify a field weight, append a caret (`^`)
  symbol and the weight to the field name. For example, to boost phrase matches in the
  `title` field over the `abstract` field, you could specify: `"phraseFields":["title^3", "plot"]` Valid values: The name of any `text` or `text-array` field and an optional numeric value greater than zero. Default: No fields. If you don't specify any fields with `phraseFields`, proximity scoring is disabled even if `phraseSlop` is specified. Valid for: `dismax`.
 ...
 - `explicitPhraseSlop`: An integer value that specifies how much a match can deviate from the search phrase when the phrase is enclosed in double quotes in the search string. (Phrases that exceed this proximity distance are not considered a match.)
  ...
  document's score. That way, if two documents have the same max field score for a
  ...
  (tieBreaker) * (sum of the scores for the rest of the matching fields)`. Set `tieBreaker`
  to 0 to disregard all but the highest scoring field (pure max): `"tieBreaker":0`. Set to
  1 to sum the scores from all fields (pure sum): `"tieBreaker":1`. Valid values: 0.0 to
  1.0. Default: 0.0. Valid for: `dismax`.
- `"q.parser"`: Specifies which query parser to use to process the request. If
  `queryParser` is not specified, Amazon CloudSearch uses the `simple` query parser.

  Amazon CloudSearch supports four query parsers: - `simple`: perform simple searches of
  `text` and `text-array` fields. By default, the `simple` query parser searches all `text`
  and `text-array` fields. You can specify which fields to search by with the
  `queryOptions` parameter. If you prefix a search term with a plus sign (+) documents must
  contain the term to be considered a match. (This is the default, unless you configure the
  default operator with the `queryOptions` parameter.) You can use the `-` (NOT), `|` (OR),
  and `*` (wildcard) operators to exclude particular terms, find results that match any of
  the specified terms, or search for a prefix. To search for a phrase rather than
  individual terms, enclose the phrase in double quotes. For more information, see [Searching for Text](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching-text.html)
  in the *Amazon CloudSearch Developer Guide*.
  ...
  ranges of values, and use advanced options such as term boosting, `matchall`, and `near`.
  For more information, see [Constructing Compound Queries](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching-compound-queries.html)
  in the *Amazon CloudSearch Developer Guide*.
  ...
  defined by the DisMax query parser. For more information, see [DisMax Query Parser Syntax](http://wiki.apache.org/solr/DisMaxQParserPlugin#Query_Syntax).
...
- `"sort"`: Specifies the fields or custom expressions to use to sort the search results.
  ...
  To use a field to sort results, the field must be sort-enabled in the domain
  configuration. Array type fields cannot be used for sorting. If no `sort` parameter is
  specified, results are sorted by their default relevance scores in descending order:
  `_score desc`. You can also sort by document ID (`_id asc`) and version (`_version
  desc`).
...
- `"start"`: Specifies the offset of the first search hit you want to return. Note that the
  result set is zero-based; the first result is at index 0. You can specify either the
  `start` or `cursor` parameter in a request, they are mutually exclusive.

  For more information, see [Paginating Results](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/paginating-results.html)
  in the *Amazon CloudSearch Developer Guide*.
- `"stats"`: Specifies one or more fields for which to get statistics information. Each
  specified field must be facet-enabled in the domain configuration. The fields are
  specified in JSON using the form: `{"FIELD-A":{},"FIELD-B":{}}`There are currently no
  options supported for statistics.
"""
search(q; aws_config::AbstractAWSConfig=current_aws_config()) = cloudsearch_domain("GET", "/2013-01-01/search?format=sdk&pretty=true", Dict{String, Any}("q"=>q); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
search(q, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = cloudsearch_domain("GET", "/2013-01-01/search?format=sdk&pretty=true", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("q"=>q), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
For more information about configuring suggesters and retrieving suggestions, see [Getting Suggestions](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/getting-suggestions.html)
in the *Amazon CloudSearch Developer Guide*.
...
"""
suggest(q, suggester; aws_config::AbstractAWSConfig=current_aws_config()) = cloudsearch_domain("GET", "/2013-01-01/suggest?format=sdk&pretty=true", Dict{String, Any}("q"=>q, "suggester"=>suggester); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
suggest(q, suggester, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = cloudsearch_domain("GET", "/2013-01-01/suggest?format=sdk&pretty=true", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("q"=>q, "suggester"=>suggester), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
For more information about formatting your data for Amazon CloudSearch, see [Preparing Your Data](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/preparing-data.html)
in the *Amazon CloudSearch Developer Guide*. For more information about uploading data for
indexing, see [Uploading Data](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/uploading-data.html)
in the *Amazon CloudSearch Developer Guide*.

# Arguments

- `content-_type`: The format of the batch you are uploading. Amazon CloudSearch supports
  two document batch formats: - application/json
 - application/xml
- `documents`: A batch of documents formatted in JSON or HTML.
"""
upload_documents(Content_Type, documents; aws_config::AbstractAWSConfig=current_aws_config()) = cloudsearch_domain("POST", "/2013-01-01/documents/batch?format=sdk", Dict{String, Any}("documents"=>documents, "headers"=>Dict{String, Any}("Content-Type"=>Content_Type)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
