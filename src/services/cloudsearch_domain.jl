...
The endpoint for submitting `Search` requests is domain-specific. You submit search requests
to a domain's search endpoint. To get the search endpoint for your domain, use the Amazon
...
- `"facet"`: Specifies one or more fields for which to get facet information, and options
  ...
  - `buckets` specifies an array of the facet values or ranges to count. Ranges are
    ...
    information, see [Searching for a Range of Values](http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching-ranges.html)
  ...
  To sort facets by facet count, use the `count` option. For example, the following request
  sets the `sort` option to `count` to sort the facet values by facet count, with the facet
  values that have the most matching documents listed first. Setting the `size` option to 3
...
- `"fq"`: Specifies a structured query that filters the results of a search without
  ...
  `query` parameter. Specifying a filter controls only which matching documents are included
  in the results, it has no effect on how they are scored and sorted. The `filterQuery`
...
- `"q.options"`: Configures options for the query parser specified in the `queryParser`
  ...
  - `defaultOperator`: The default operator used to combine individual terms in the search
    ...
    that must match, rather than a default operator. A value of `0%` is the equivalent to
    OR, and a value of `100%` is equivalent to AND. The percentage must be specified as a
    ...
    (`dismax`). Default: `and` (`simple`, `structured`, `lucene`) or `100` (`dismax`). Valid
    for: `simple`, `structured`, `lucene`, and `dismax`.
  - `fields`: An array of the fields to search when no fields are specified in a search. If
    no fields are specified in a search and this option is not specified, all text and text-
    array fields are searched. You can specify a weight for each field to control the
    relative importance of each field when Amazon CloudSearch calculates relevance scores.
    ...
    and `text-array` fields. Valid for: `simple`, `structured`, `lucene`, and `dismax`.
  - `operators`: An array of the operators or special characters you want to disable for the
    ...
    disabling `phrase` disables the ability to search for phrases by enclosing phrases in
    double quotes. Disabling precedence disables the ability to control order of precedence
    using parentheses. Disabling `near` disables the ability to use the ~ operator to
    perform a sloppy phrase search. Disabling the `fuzzy` operator disables the ability to
    use the ~ operator to perform a fuzzy search. `escape` disables the ability to use a
    backslash (`\\`) to escape special characters within the search string. Disabling
    whitespace is an advanced option that prevents the parser from tokenizing on whitespace,
    which can be useful for Vietnamese. (It prevents Vietnamese words from being split
  ...
  - `phraseFields`: An array of the `text` or `text-array` fields you want to use for phrase
    ...
    the field scores higher. You can specify a weight for each field to boost that score.
    The `phraseSlop` option controls how much the matches can deviate from the search string
    and still be boosted. To specify a field weight, append a caret (`^`) symbol and the
    weight to the field name. For example, to boost phrase matches in the `title` field over
    the `abstract` field, you could specify: `"phraseFields":["title^3", "plot"]` Valid
    values: The name of any `text` or `text-array` field and an optional numeric value
    greater than zero. Default: No fields. If you don't specify any fields with
    `phraseFields`, proximity scoring is disabled even if `phraseSlop` is specified. Valid
    for: `dismax`.
  ...
  - `explicitPhraseSlop`: An integer value that specifies how much a match can deviate from
    the search phrase when the phrase is enclosed in double quotes in the search string.
    (Phrases that exceed this proximity distance are not considered a match.) For example,
    to specify a slop of three for dismax phrase queries, you would specify
  ...
  - `tieBreaker`: When a term in the search string is found in a document's field, a score
    ...
    other documents. If the term occurs in multiple fields within a document, by default
...
- `"q.parser"`: Specifies which query parser to use to process the request. If `queryParser`
  ...
  - `simple`: perform simple searches of `text` and `text-array` fields. By default, the
    ...
    fields to search by with the `queryOptions` parameter. If you prefix a search term with
    a plus sign (+) documents must contain the term to be considered a match. (This is the
    default, unless you configure the default operator with the `queryOptions` parameter.)
    You can use the `-` (NOT), `|` (OR), and `*` (wildcard) operators to exclude particular
    terms, find results that match any of the specified terms, or search for a prefix. To
    search for a phrase rather than individual terms, enclose the phrase in double quotes.
  ...
  - `structured`: perform advanced searches by combining multiple expressions to define the
    search criteria. You can also search within particular fields, search for values and
...
- `"return"`: Specifies the field and expression values to include in the response. Multiple
  ...
  response includes all return enabled fields (`_all_fields`). To return only the document
  IDs for the matching documents, specify `_no_fields`. To retrieve the relevance score
...
- `"sort"`: Specifies the fields or custom expressions to use to sort the search results.
  ...
  `_score desc`. You can also sort by document ID (`_id asc`) and version (`_version desc`).
...
`DescribeDomains` action. A domain's endpoints are also displayed on the domain dashboard in
