...
Firehose stream is `CREATING`. After the Firehose stream is created, its status is `ACTIVE`
and it now accepts data. If the Firehose stream creation fails, the status transitions to
`CREATING_FAILED`. Attempts to send data to a delivery stream that is not in the `ACTIVE`
state cause an exception. To check the state of a Firehose stream, use [`describe_delivery_stream`](@ref).
...
A Firehose stream can be configured to receive records directly from providers using [`put_record`](@ref)
or [`put_record_batch`](@ref), or it can be configured to use an existing Kinesis stream as
its source. To specify a Kinesis data stream as input, set the `DeliveryStreamType`
parameter to `KinesisStreamAsSource`, and provide the Kinesis stream Amazon Resource Name
...
invoke [`start_delivery_stream_encryption`](@ref) to turn on SSE for an existing Firehose
...
- An Amazon Redshift destination requires an S3 bucket as intermediate location. Firehose
  first delivers data to Amazon S3 and then uses `COPY` syntax to load data into an Amazon
...
- `delivery_stream_name`: The name of the Firehose stream. This name must be unique per
  Amazon Web Services account in the same Amazon Web Services Region. If the Firehose
  streams are in different accounts or different Regions, you can have multiple Firehose
...
- `"DatabaseSourceConfiguration"`: The top level object for configuring streams with
...
- `"DeliveryStreamType"`: The Firehose stream type. This parameter can be one of the
  following values:
...
- `"ElasticsearchDestinationConfiguration"`: The destination in Amazon OpenSearch Service.
...
- `"KinesisStreamSourceConfiguration"`: When a Kinesis data stream is used as the source for
  the Firehose stream, a `KinesisStreamSourceConfiguration` containing the Kinesis data
...
- `"Tags"`: A set of tags to assign to the Firehose stream. A tag is a key-value pair that
  ...
  can help you distinguish the Firehose stream. For more information about tags, see [Using Cost Allocation Tags](https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/cost-alloc-tags.html)
  ...
  new Firehose streams with IAM resource tags will fail with an `AccessDeniedException` such
  as following.
...
`DELETING`, `CREATING_FAILED`, or `DELETING_FAILED`. You can't delete a Firehose stream that
is in the `CREATING` state. To check the state of a Firehose stream, use [`describe_delivery_stream`](@ref).
...
- `delivery_stream_name`: The name of the Firehose stream.

# Optional Parameters

Optional parameters can be passed as a `params::Dict{String,<:Any}`. Valid keys are:

- `"AllowForceDelete"`: Set this to true if you want to delete the Firehose stream even if
...
stream is created, call `DescribeDeliveryStream` to see whether the Firehose stream is
...
- `"ExclusiveStartDestinationId"`: The ID of the destination to start returning the
  destination information. Firehose supports one destination per Firehose stream.
...
value of `HasMoreDeliveryStreams` in the output. If there are more Firehose streams to list,
you can request them by calling this operation again and setting the
`ExclusiveStartDeliveryStreamName` parameter to the name of the last Firehose stream
...
- `"DeliveryStreamType"`: The Firehose stream type. This can be one of the following values:
  ...
  This parameter is optional. If this parameter is omitted, Firehose streams of all types
  are returned.

- `"ExclusiveStartDeliveryStreamName"`: The list of Firehose streams returned by this call
  to `ListDeliveryStreams` will start with the Firehose stream whose name comes
...
- `delivery_stream_name`: The name of the Firehose stream whose tags you want to list.
...
- `"Limit"`: The number of tags to return. If this number is less than the total number of
  tags associated with the Firehose stream, `HasMoreTags` is set to `true` in the response.
...
Firehose stream, use [`put_record_batch`](@ref). Applications using these operations are
referred to as producers.

By default, each Firehose stream can take in up to 2,000 transactions per second, 5,000
records per second, or 5 MB per second. If you use [`put_record`](@ref) and [`put_record_batch`](@ref),
the limits are an aggregate across these two operations for each Firehose stream. For more
information about limits and how to request an increase, see [Amazon Firehose Limits](https://docs.aws.amazon.com/firehose/latest/dev/limits.html).
...
You must specify the name of the Firehose stream and the data record when using [`put_record`](@ref).
...
length is less than 1000 KiB. If you include more than 500 records, the request succeeds but
the record de-aggregation doesn't work as expected and transformation lambda is invoked with
the complete base64 encoded data blob instead of de-aggregated base64 decoded records.
...
to each record. Producer applications can use this ID for purposes such as auditability and
investigation.

If the [`put_record`](@ref) operation throws a `ServiceUnavailableException`, the API is
automatically reinvoked (retried) 3 times. If the exception persists, it is possible that
the throughput limits have been exceeded for the Firehose stream.
...
Firehose stream as it tries to send the records to the destination. If the destination is
...
records into a Firehose stream, use [`put_record`](@ref). Applications using these
...
Each [`put_record_batch`](@ref) request supports up to 500 records. Each record in the
request can be as large as 1,000 KB (before base64 encoding), up to a limit of 4 MB for the
entire request. These limits cannot be changed.
...
`FailedPutCount`, and an array of responses, `RequestResponses`. Even if the [`put_record_batch`](@ref)
...
Data records sent to Firehose are stored for 24 hours from the time they are added to a
Firehose stream as it attempts to send the records to the destination. If the destination is
unreachable for more than 24 hours, the data is no longer available.
...
encryption status of a Firehose stream is the `Status` property in
`DeliveryStreamEncryptionConfiguration`. If the operation fails, the encryption status
changes to `ENABLING_FAILED`. You can continue to read and write data to your Firehose
...
to the Firehose stream are encrypted. To find out whether a record or a batch of records was
encrypted, check the response elements `PutRecordOutput\$Encrypted` and
`PutRecordBatchOutput\$Encrypted`, respectively.

To check the encryption status of a Firehose stream, use [`describe_delivery_stream`](@ref).
...
If the encryption status of your Firehose stream is `ENABLING_FAILED`, you can invoke this
...
have a combined limit of 25 calls per Firehose stream per 24 hours. For example, you reach
...
- `delivery_stream_name`: The name of the Firehose stream for which you want to enable
...
to the Firehose stream are no longer subject to encryption. To find out whether a record or
...
To check the encryption state of a Firehose stream, use [`describe_delivery_stream`](@ref).
...
- `delivery_stream_name`: The name of the Firehose stream for which you want to disable
  server-side encryption (SSE).
"""
...
Adds or updates tags for the specified Firehose stream. A tag is a key-value pair that you
...
information that can help you distinguish the Firehose stream. For more information about
...
- `delivery_stream_name`: The name of the Firehose stream to which you want to add the tags.
...
updated, so data writes to the Firehose stream can continue during this process. The updated
...
successfully, the version ID is updated, and can be retrieved using [`describe_delivery_stream`](@ref).
...
- `"ElasticsearchDestinationUpdate"`: Describes an update for a destination in Amazon
  OpenSearch Service.
