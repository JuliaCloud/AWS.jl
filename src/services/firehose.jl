...
records into a delivery stream, use [`put_record_batch`](@ref). Applications using these
...
the limits are an aggregate across these two operations for each delivery stream. For more
...
records into a delivery stream, use [`put_record`](@ref). Applications using these
operations are referred to as producers.
