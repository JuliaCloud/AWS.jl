...
- `sessionState`: Sets the state of the session with the user. You can use this to set the
...
# Optional Parameters
- `ResponseContentType`: The message that Amazon Lex returns in the response can be either
  text or speech depending on the value of this parameter.    If the value is text/plain;
...
# Required Parameters
- `Content-Type`: Indicates the format for audio input or that the content is text. The
...
- `x-amz-lex-session-state`: Sets the state of the session with the user. You can use this
  to set the current intent, attributes, context, and dialog action. Use the dialog action to
  determine the next step that Amazon Lex should use in the conversation with the user.
"""
