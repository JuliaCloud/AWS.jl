...
- `"NextToken"`: If NextToken is returned by a previous response, there are more results
  ...
  unchanged.
"""
function describe_fleet_advisor_schemas(; aws_config::AbstractAWSConfig=current_aws_config())
    return database_migration_service(
...
- `"MaxRecords"`:  The maximum number of records to include in the response. If more
  ...
  Constraints: Minimum 20, maximum 100.
"""
function describe_replication_instances(; aws_config::AbstractAWSConfig=current_aws_config())
    return database_migration_service(
...
"""
function run_fleet_advisor_lsa_analysis(; aws_config::AbstractAWSConfig=current_aws_config())
    return database_migration_service(
