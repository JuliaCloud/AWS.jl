...
validation if `Status` is `PUBLISHED`. Note that the `\$SAVED` alias' content will always
be updated, but the `\$LATEST` alias' content will only be updated if `Status` is
`PUBLISHED`.
