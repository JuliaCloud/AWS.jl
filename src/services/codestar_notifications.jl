...
- `EventTypeIds`: A list of event types associated with this notification rule. For a list
...
- `ClientRequestToken`: A unique, client-generated idempotency token that, when provided in
  a request, ensures the request cannot be repeated with a changed parameter. If a request
