...
# Keyword Parameters
- `default_value`:
- `split_charge_rules`:  The split charge rules used to allocate your charges between your
...
- `next_page_token`: The token to retrieve the next set of results. Amazon Web Services
  ...
  page size.
- `total_impact`: Filters anomaly results by the total impact field on the anomaly object.
...
- `granularity`: The granularity of the Amazon Web Services cost data for the reservation.
  Valid values are MONTHLY and DAILY. If GroupBy is set, Granularity can't be set. If
  Granularity isn't set, the response object doesn't include Granularity, either MONTHLY or
  DAILY. The GetReservationCoverage operation supports only DAILY and MONTHLY granularities.
...
- `max_results`: The maximum number of objects that you returned for this request. If more
...
- `filter`:
- `lookback_period_in_days`: The number of previous days that you want Amazon Web Services
...
- `group_by`: You can group the data using the attributes INSTANCE_FAMILY, REGION, or
  SERVICE.
- `max_results`: The number of items to be returned in a response. The default is 20, with
...
- `next_token`: The token to retrieve the next set of results. Amazon Web Services provides
