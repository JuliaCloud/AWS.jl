...
copy). You can have up to five backup copy requests in progress to a single destination
Region per account.

You can use cross-Region backup copies for cross-Region disaster recovery. You can
periodically take backups and copy them to another Region so that in the event of a disaster
in the primary Region, you can restore from backup and recover availability quickly in the
other Region. You can make cross-Region copies only within your Amazon Web Services
partition. A partition is a grouping of Regions. Amazon Web Services currently has three
partitions: `aws` (Standard Regions), `aws-cn` (China Regions), and `aws-us-gov` (Amazon Web
...
- `"Options"`: Confirms that you want to delete data on the destination volume that wasn’t
  ...
  you can’t copy the snapshot.

  - `DELETE_INTERMEDIATE_SNAPSHOTS` - Deletes snapshots on the destination volume that
    aren’t on the source volume.
  - `DELETE_CLONED_VOLUMES` - Deletes snapshot clones on the destination volume that aren't
...
By using the idempotent operation, you can retry a [`create_backup`](@ref) operation without
the risk of creating an extra backup. This approach can be useful when an initial call fails
in a way that makes it unclear whether a backup was created. If you use the same client
...
about linking a data repository to your file system, see [Linking your file system to an S3 bucket](https://docs.aws.amazon.com/fsx/latest/LustreGuide/create-dra-linked-data-repo.html).
...
- `"ImportedFileChunkSize"`: For files imported from a data repository, this value
  ...
  single physical disk. The maximum number of disks that a single file can be striped across
...
- `"S3"`: The configuration for an Amazon S3 data repository linked to an Amazon FSx Lustre
  ...
  events (new, changed, or deleted files or directories) are automatically imported from the
...
- `report`: Defines whether or not Amazon FSx provides a CompletionReport once the task has
  ...
  see [Working with Task Completion Reports](https://docs.aws.amazon.com/fsx/latest/LustreGuide/task-completion-report.html).
...
- `type`: Specifies the type of data repository task to create.
  ...
  - `IMPORT_METADATA_FROM_REPOSITORY` tasks import metadata changes from a linked S3 bucket
    to your Amazon FSx for Lustre file system.
...
- `"Paths"`: A list of paths for the data repository task to use when the task is processed.
  If a path that you provide isn't valid, the task fails. If you don't provide paths, the
  default behavior is to export all files to S3 (for export tasks), import all files from S3
  (for import tasks), or release all exported files that meet the last accessed time
  criteria (for release tasks).

  - For export tasks, the list contains paths on the FSx for Lustre file system from which
    the files are exported to the Amazon S3 bucket. The default path is the file system root
    directory. The paths you provide need to be relative to the mount point of the file
    system. If the mount point is `/mnt/fsx` and `/mnt/fsx/path1` is a directory or file on
  ...
  - For release tasks, the list contains directory or file paths on the FSx for Lustre file
    ...
    release all exported files in the file system, specify a forward slash (/) as the path.

    !!! note
        A file must also meet the last accessed time criteria specified in for the file to
        be released.

- `"ReleaseConfiguration"`: The configuration that specifies the last accessed time criteria
  for files that will be released from an Amazon FSx for Lustre file system.
...
uses to ensure idempotent creation. If a cache with the specified client request token
exists and the parameters match, `CreateFileCache` returns the description of the existing
cache. If a cache with the specified client request token exists and the parameters don't
...
- `file_cache_type_version`: Sets the Lustre version for the cache that you're creating,
  which must be `2.12`.
- `storage_capacity`: The storage capacity of the cache in gibibytes (GiB). Valid values are
...
- `"DataRepositoryAssociations"`: A list of up to 8 configurations for data repository
  ...
  - All configurations on the list must be of the same data repository type, either all S3
    or all NFS. A cache can't link to different data repository types at the same time.
...
- `"SecurityGroupIds"`: A list of IDs specifying the security groups to apply to all network
  interfaces created for Amazon File Cache access. This list isn't returned in later
...
client request token has no effect. By using the idempotent operation, you can retry a [`create_file_system`](@ref)
operation without the risk of creating an extra file system. This approach can be useful
when an initial call fails in a way that makes it unclear whether a file system was created.
...
By using the idempotent operation, you can retry a `CreateFileSystemFromBackup` call without
the risk of creating an extra file system. This approach can be useful when an initial call
fails in a way that makes it unclear whether a file system was created. Examples are if a
...
!!! note
    ...
    still `CREATING`. You can check the file-system creation status by calling the [DescribeFileSystems](https://docs.aws.amazon.com/fsx/latest/APIReference/API_DescribeFileSystems.html)
    operation, which returns the file system state along with other information.
...
- `"StorageType"`: Sets the storage type for the Windows, OpenZFS, or Lustre file system
  ...
  !!! note
      ...
      used SSD storage if the original SSD file system had a storage capacity of at least
      2000 GiB.
...
- `"ActiveDirectoryConfiguration"`: Describes the self-managed Microsoft Active Directory to
  which you want to join the SVM. Joining an Active Directory provides user authentication
  and access control for SMB clients, including Microsoft Windows and macOS clients
  accessing the file system.

- `"ClientRequestToken"`:

- `"RootVolumeSecurityStyle"`: The security style of the root volume of the SVM. Specify one
  ...
  - `NTFS` if the file system is managed by a Microsoft Windows administrator, the majority
    of users are SMB clients, and an application accessing the data uses a Microsoft Windows
...
- `"SvmAdminPassword"`: The password to use when managing the SVM using the NetApp ONTAP CLI
  or REST API. If you do not specify a password, you can still use the file system's
...
data repository association unlinks the file system from the Amazon S3 bucket. When deleting
a data repository association, you have the option of deleting the data in the file system
that corresponds to the data repository association. Data repository associations are
supported on all FSx for Lustre 2.12 and 2.15 file systems, excluding `scratch_1` deployment
type.
...
The [`delete_file_cache`](@ref) operation returns while the cache has the `DELETING` status.
You can check the cache deletion status by calling the [DescribeFileCaches](https://docs.aws.amazon.com/fsx/latest/APIReference/API_DescribeFileCaches.html)
...
operation, which returns a list of file systems in your account. If you pass the file system
ID for a deleted file system, the [`describe_file_systems`](@ref) operation returns a
...
for that backup. Otherwise, it returns all backups owned by your Amazon Web Services account
...
function describe_backups(; aws_config::AbstractAWSConfig=current_aws_config())
    return fsx("DescribeBackups"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_backups(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return fsx("DescribeBackups", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
File Cache resources and all FSx for Lustre 2.12 and 2,15 file systems, excluding
...
optional `MaxResults` parameter to limit the number of data repository associations returned
in a response. If more data repository associations remain, a `NextToken` value is returned
in the response. In this case, send a later request with the `next_token` request parameter
...
- `"AssociationIds"`: IDs of the data repository associations whose descriptions you want to
  retrieve (String).
- `"Filters"`:
- `"MaxResults"`: The maximum number of resources to return in the response. This value must
...
function describe_data_repository_associations(;
    ...
    return fsx(
        "DescribeDataRepositoryAssociations"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_data_repository_tasks(;
    ...
    return fsx("DescribeDataRepositoryTasks"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_data_repository_tasks(
    ...
    return fsx(
        "DescribeDataRepositoryTasks", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_file_caches(; aws_config::AbstractAWSConfig=current_aws_config())
    return fsx("DescribeFileCaches"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_file_caches(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return fsx("DescribeFileCaches", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
send a later request with the `next_token` request parameter set to the value of `NextToken`
...
function describe_file_systems(; aws_config::AbstractAWSConfig=current_aws_config())
    return fsx("DescribeFileSystems"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_file_systems(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return fsx("DescribeFileSystems", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
ONTAP Multi-AZ file systems in subnets that are shared by a virtual private cloud (VPC)
owner. For more information, see [Creating FSx for ONTAP file systems in shared subnets](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/creating-file-systems.html#fsxn-vpc-shared-subnets).
"""
...
function describe_shared_vpc_configuration(;
    ...
    return fsx(
        "DescribeSharedVpcConfiguration"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_snapshots(; aws_config::AbstractAWSConfig=current_aws_config())
    return fsx("DescribeSnapshots"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_snapshots(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return fsx("DescribeSnapshots", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function describe_storage_virtual_machines(;
    ...
    return fsx(
        "DescribeStorageVirtualMachines"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function describe_volumes(; aws_config::AbstractAWSConfig=current_aws_config())
    return fsx("DescribeVolumes"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
from an Amazon FSx for Windows File Server file system. If you attempt to disassociate a DNS
alias that is not associated with the file system, Amazon FSx responds with an HTTP status
code 400 (Bad Request). For more information, see [Working with DNS Aliases](https://docs.aws.amazon.com/fsx/latest/WindowsGuide/managing-dns-aliases.html).
...
the response. In this case, send a later request with the `next_token` request parameter set
...
- The implementation might return fewer than `MaxResults` file system descriptions while
  still including a `NextToken` value.
- The order of tags returned in the response of one `ListTagsForResource` call and the order
...
File Server file system, use this action to initiate the process of Amazon FSx attempting to
...
- `resource_arn`: The Amazon Resource Name (ARN) of the Amazon FSx resource that you want to
  tag.
...
- `"StorageCapacity"`: Use this parameter to increase the storage capacity of an FSx for
  ...
  - For `SCRATCH_2`, `PERSISTENT_1`, and `PERSISTENT_2 SSD` deployment types, valid values
    are in multiples of 2400 GiB. The value must be greater than the current storage
    capacity.
...
- `"ActiveDirectoryConfiguration"`: Specifies updates to an SVM's Microsoft Active Directory
  (AD) configuration.
- `"ClientRequestToken"`:
