...
- `"ResourcePolicy"`: An Amazon Web Services resource-based policy document in JSON format
  ...
  a table, the policy creation is strongly consistent. The maximum size supported for a
  resource-based policy document is 20 KB. DynamoDB counts whitespaces when calculating the
  size of a policy against this limit. You can’t request an increase for this limit. For a
  full list of all considerations that you should keep in mind while attaching a
...
you can assume the policy will start getting applied in the authorization of requests to
...
If you specify an ExpectedRevisionId which doesn't match the current policy's RevisionId,
...
- `policy`: An Amazon Web Services resource-based policy document in JSON format. The
  ...
  all considerations that you should keep in mind while attaching a resource-based policy,
...
- `"ExpectedRevisionId"`: A string value that you can use to conditionally update your
  ...
  put a policy when no policy exists for the resource, specify NO_POLICY for the revision ID.
"""
put_resource_policy(
...
tables Version 2019.11.21 you can use UpdateTable instead.   Although you can use
...
asynchronous operation; while it's executing, the table status changes from ACTIVE to
UPDATING. While it's UPDATING, you can't issue another UpdateTable request. When the table
