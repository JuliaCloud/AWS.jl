...
The [`batch_write_item`](@ref) operation puts or deletes multiple items in one or more
tables. A single call to `BatchWriteItem` can transmit up to 16MB of data over the network,
...
this distinction, see [Naming Rules and Data Types](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.NamingRulesDataTypes.html).

!!! note
    `BatchWriteItem` cannot update items. If you perform a [`batch_write_item`](@ref)
...
conditions on individual put and delete requests, and `BatchWriteItem` does not return
...
!!! note
    ...
    request immediately after running the `DeleteResourcePolicy` request, DynamoDB might
    still return the deleted policy. This is because the policy for your resource might not
    have been deleted yet. Wait for a few seconds, and then try the `GetResourcePolicy`
    request again.
...
function describe_endpoints(; aws_config::AbstractAWSConfig=current_aws_config())
    return dynamodb("DescribeEndpoints"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
extremely rapidly, but the only quota that applies is that the aggregate provisioned
...
function describe_limits(; aws_config::AbstractAWSConfig=current_aws_config())
    return dynamodb("DescribeLimits"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function describe_limits(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return dynamodb("DescribeLimits", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
!!! note
    ...
    uses an eventually consistent query, and the metadata for your table might not be
    available at that moment. Wait for a few seconds, and then try the `DescribeTable`
    request again.
...
!!! note
    ...
    cannot mix both in one transaction. The EXISTS function is an exception and can be used
    to check the condition of specific attributes of the item in a similar manner to
...
- If you issue a `GetResourcePolicy` request immediately after a `CreateTable` request,
  which includes a resource-based policy, DynamoDB might return a
...
function list_backups(; aws_config::AbstractAWSConfig=current_aws_config())
    return dynamodb("ListBackups"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_backups(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return dynamodb("ListBackups", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_contributor_insights(; aws_config::AbstractAWSConfig=current_aws_config())
    return dynamodb("ListContributorInsights"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_contributor_insights(
    ...
    return dynamodb(
        "ListContributorInsights", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_exports(; aws_config::AbstractAWSConfig=current_aws_config())
    return dynamodb("ListExports"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_exports(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return dynamodb("ListExports", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_global_tables(; aws_config::AbstractAWSConfig=current_aws_config())
    return dynamodb("ListGlobalTables"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_global_tables(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return dynamodb("ListGlobalTables", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_imports(; aws_config::AbstractAWSConfig=current_aws_config())
    return dynamodb("ListImports"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_imports(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return dynamodb("ListImports", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_tables(; aws_config::AbstractAWSConfig=current_aws_config())
    return dynamodb("ListTables"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_tables(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return dynamodb("ListTables", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
!!! note
    ...
    previous policy, if there was one, or return the `PolicyNotFoundException`. This is
...
!!! note
    ...
    consumed will be the same whether you request all of the attributes (the default
...
a table or on a local secondary index, you can set the `ConsistentRead` parameter to `true`
and obtain a strongly consistent result. Global secondary indexes support eventually
...
use a `FilterExpression` in the scan request, then `Count` is the same as `ScannedCount`.

!!! note
    `Count` and `ScannedCount` only return the count of items specific to a single scan
    request and, unless the table is less than 1MB, do not represent the total number of
    items in the table.
...
table or secondary index, applications can request a parallel [`scan`](@ref) operation by
providing the `Segment` and `TotalSegments` parameters. For more information, see [Parallel Scan](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Scan.html#Scan.ParallelScan)
in the *Amazon DynamoDB Developer Guide*.
...
changes at the time the scan iterates through each item in the table. If you require a
strongly consistent read of each item as the scan iterates through the items in the table,
you can set the `ConsistentRead` parameter to true. Strong consistency only relates to the
...
You can also return the item's attribute values in the same [`update_item`](@ref) operation
using the `ReturnValues` parameter.
...
- `"UpdateExpression"`: An expression that defines one or more attributes to be updated, the
  ...
  example, if the attribute value was the set `[a,b,c]` and the `DELETE` action specifies
  ...
  For more information on update expressions, see [Modifying Items and Attributes](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.Modifying.html)
  in the *Amazon DynamoDB Developer Guide*.
"""
...
request. When the table returns to the `ACTIVE` state, the [`update_table`](@ref) operation
is complete.
