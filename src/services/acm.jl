...
In addition to `keyType`, you can also filter by the `CertificateStatuses`, `keyUsage`, and `extendedKeyUsage` attributes on the certificate. For more information, see [`filters`](@ref).
