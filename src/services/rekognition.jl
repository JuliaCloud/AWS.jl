...
function create_project_version(
    OutputConfig, ProjectArn, VersionName; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rekognition(
...
- `"SortBy"`: Sort to use for celebrities returned in Celebrities field. Specify ID to sort
  ...
  recognized.
"""
function get_celebrity_recognition(JobId; aws_config::AbstractAWSConfig=current_aws_config())
    return rekognition(
...
- `"QualityFilter"`: A filter that specifies a quality bar for how much filtering is done
  ...
  with version 3 of the face model or higher.
"""
function index_faces(CollectionId, Image; aws_config::AbstractAWSConfig=current_aws_config())
    return rekognition(
        "IndexFaces",
...
- `"SourceRefContains"`: If specified, ListDatasetEntries only returns JSON Lines where the
  ...
  folder.
"""
function list_dataset_entries(DatasetArn; aws_config::AbstractAWSConfig=current_aws_config())
    return rekognition(
        "ListDatasetEntries",
        Dict{String,Any}("DatasetArn" => DatasetArn);
        aws_config=aws_config,
...
function start_media_analysis_job(
    Input, OperationsConfig, OutputConfig; aws_config::AbstractAWSConfig=current_aws_config()
)
    return rekognition(
        "StartMediaAnalysisJob",
        Dict{String,Any}(
