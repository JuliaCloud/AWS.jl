...
This ensures that the `FaceIds` are associated with the right UserID. The value ranges from
...
different `UserID`, isn't found in the collection, doesn’t meet the `UserMatchThreshold`,
or there are already 100 faces associated with the `UserID`, it will be returned as part of
an array of `UnsuccessfulFaceAssociations.`

The `UserStatus` reflects the status of an operation which updates a UserID representation
with a list of given faces. The `UserStatus` can be:  - ACTIVE - All associations or
disassociations of FaceID(s) for a UserID are complete.
...
- `"ClientRequestToken"`: Idempotent token used to identify the request to
  `AssociateFaces`. If you use the same token with multiple `AssociateFaces` requests, the
...
"""
associate_faces(CollectionId, FaceIds, UserId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("AssociateFaces", Dict{String, Any}("CollectionId"=>CollectionId, "FaceIds"=>FaceIds, "UserId"=>UserId, "ClientRequestToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
associate_faces(CollectionId, FaceIds, UserId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("AssociateFaces", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId, "FaceIds"=>FaceIds, "UserId"=>UserId, "ClientRequestToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
negative is an incorrect prediction that a face in the target image has a low similarity
confidence score when compared to the face in the source image. To reduce the probability
of false negatives, we recommend that you compare the target image against multiple source
images. If you plan to use `CompareFaces` to make a decision that impacts an individual's
rights, privacy, or access to services, we recommend that you pass the result to a human
for review and further validation before taking action.You pass the input and target images
either as base64-encoded image bytes or as references to images in an Amazon S3 bucket. If
you use the AWS CLI to call Amazon Rekognition operations, passing image bytes isn't
...
facial landmarks, pose details (pitch, roll, and yaw), quality (brightness and sharpness),
...
The `QualityFilter` input parameter allows you to filter out detected faces that don’t meet
a required quality bar. The quality bar is based on a variety of common use cases. Use
`QualityFilter` to set the quality bar by specifying `LOW`, `MEDIUM`, or `HIGH`. If you do
...
persist.For an example, see Comparing Faces in Images in the Amazon Rekognition Developer
Guide.

 <p>This operation requires permissions to perform the `rekognition:CompareFaces` action.
...
- `source_image`: The input image as base64-encoded bytes or an S3 object. If you use the
  ...
  If you are using an AWS SDK to call Amazon Rekognition, you might not need to base64-
...
- `"QualityFilter"`: A filter that specifies a quality bar for how much filtering is done
  to identify faces. Filtered faces aren't compared. If you specify `AUTO`, Amazon
  ...
  variety of common use cases. Low-quality detections can occur for a number of reasons.
  Some examples are an object that's misidentified as a face, a face that's too blurry, or
  a face with a pose that's too extreme to use. If you specify `NONE`, no filtering is
...
"""
compare_faces(SourceImage, TargetImage; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CompareFaces", Dict{String, Any}("SourceImage"=>SourceImage, "TargetImage"=>TargetImage); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
compare_faces(SourceImage, TargetImage, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CompareFaces", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SourceImage"=>SourceImage, "TargetImage"=>TargetImage), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Copies a version of
an Amazon Rekognition Custom Labels model from a source project to a destination project.
The source and destination projects can be in different AWS accounts but must be in the
same AWS Region. You can't copy a model to another AWS service.

To copy a model version to a different AWS account, you need to create a resource-based
policy known as a *project policy*. You attach the project policy to the source project by
calling <a>PutProjectPolicy</a>. The project policy gives permission to copy the model
version from a trusting AWS account to a trusted account.

For more information creating and attaching a project policy, see Attaching a project
policy (SDK) in the *Amazon Rekognition Custom Labels Developer Guide*.
...
To copy a model, the destination project, source project, and source model version must
already exist.Copying a model version takes a while to complete. To get the current status,
call <a>DescribeProjectVersions</a> and check the value of `Status` in the
<a>ProjectVersionDescription</a> object. The copy operation has finished when the value of
`Status` is `COPYING_COMPLETED`.

 <p>This operation requires permissions to perform the `rekognition:CopyProjectVersion`
action.
...
- `"KmsKeyId"`: The identifier for your AWS Key Management Service key (AWS KMS key). You
  can supply the Amazon Resource Name (ARN) of your KMS key, the ID of your KMS key, an
  alias for your KMS key, or an alias ARN. The key is used to encrypt training results and
...
"""
copy_project_version(DestinationProjectArn, OutputConfig, SourceProjectArn, SourceProjectVersionArn, VersionName; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CopyProjectVersion", Dict{String, Any}("DestinationProjectArn"=>DestinationProjectArn, "OutputConfig"=>OutputConfig, "SourceProjectArn"=>SourceProjectArn, "SourceProjectVersionArn"=>SourceProjectVersionArn, "VersionName"=>VersionName); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
copy_project_version(DestinationProjectArn, OutputConfig, SourceProjectArn, SourceProjectVersionArn, VersionName, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CopyProjectVersion", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DestinationProjectArn"=>DestinationProjectArn, "OutputConfig"=>OutputConfig, "SourceProjectArn"=>SourceProjectArn, "SourceProjectVersionArn"=>SourceProjectVersionArn, "VersionName"=>VersionName), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
collection. Then, a user can search the collection for faces in the user-specific
container.
...
!!! note
    Collection names are case-sensitive.This operation requires permissions to perform the
`rekognition:CreateCollection` action. If you want to tag your collection, you also require
...
"""
create_collection(CollectionId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateCollection", Dict{String, Any}("CollectionId"=>CollectionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_collection(CollectionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateCollection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Creates a new
Amazon Rekognition Custom Labels dataset. You can create a dataset by using an Amazon
Sagemaker format manifest file or by copying an existing Amazon Rekognition Custom Labels
dataset.
...
To check if any non-terminal errors occurred, call <a>ListDatasetEntries</a> and check for
...
For more information, see Creating dataset in the *Amazon Rekognition Custom Labels
Developer Guide*.

 <p>This operation requires permissions to perform the `rekognition:CreateDataset` action.
...
"""
create_dataset(DatasetType, ProjectArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateDataset", Dict{String, Any}("DatasetType"=>DatasetType, "ProjectArn"=>ProjectArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_dataset(DatasetType, ProjectArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateDataset", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DatasetType"=>DatasetType, "ProjectArn"=>ProjectArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)

"""
    create_face_liveness_session()
    create_face_liveness_session(params::Dict{String,<:Any})

This API operation initiates a Face Liveness session. It returns a `SessionId`, which you
can use to start streaming Face Liveness video and get the results for a Face Liveness
session.
...
`GetFaceLivenessSessionResults` is called. This number is between 0 and 4. By default, it
...
"""
create_face_liveness_session(; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateFaceLivenessSession"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_face_liveness_session(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateFaceLivenessSession", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
create_project(ProjectName; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateProject", Dict{String, Any}("ProjectName"=>ProjectName); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_project(ProjectName, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateProject", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ProjectName"=>ProjectName), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
status by calling <a>DescribeProjectVersions</a>. Training completed successfully if the
value of the `Status` field is `TRAINING_COMPLETED`. Once training has successfully
completed, call <a>DescribeProjectVersions</a> to get the training results and evaluate the
model.
...
!!! note
    *The following applies only to projects with Amazon Rekognition Custom Labels as the
chosen feature:*
...
longer train a model version for the project by specifying manifest files.
 ...
 <p>Instead of training with a project without associated datasets, we recommend that you
use the manifest files to create training and test datasets for the project. <p/>

# Arguments

- `output_config`: The Amazon S3 bucket location to store the results of training. The
  bucket can be any S3 bucket in your AWS account. You need `s3:PutObject` permission on
  the bucket.
...
- `"KmsKeyId"`: The identifier for your AWS Key Management Service key (AWS KMS key). You
  can supply the Amazon Resource Name (ARN) of your KMS key, the ID of your KMS key, an
  alias for your KMS key, or an alias ARN. The key is used to encrypt training images, test
  images, and manifest files copied into the service for the project version. Your source
  images are unaffected. The key is also used to encrypt training results and manifest
...
- `"TestingData"`: Specifies an external manifest that the service uses to test the project
  version. If you specify `TestingData` you must also specify `TrainingData`. The project
  must not have any associated datasets.
- `"TrainingData"`: Specifies an external manifest that the services uses to train the
  project version. If you specify `TrainingData` you must also specify `TestingData`. The
...
"""
create_project_version(OutputConfig, ProjectArn, VersionName; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateProjectVersion", Dict{String, Any}("OutputConfig"=>OutputConfig, "ProjectArn"=>ProjectArn, "VersionName"=>VersionName); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_project_version(OutputConfig, ProjectArn, VersionName, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateProjectVersion", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("OutputConfig"=>OutputConfig, "ProjectArn"=>ProjectArn, "VersionName"=>VersionName), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
faces and detecting labels.</p> - If you are creating a stream processor for detecting
faces, you provide as input a Kinesis video stream (`Input`) and a Kinesis data stream
(`Output`) stream for receiving the output. You must use the `FaceSearch` option in
`Settings`, specifying the collection that contains the faces you want to recognize. After
you have finished analyzing a streaming video, use <a>StopStreamProcessor</a> to stop
processing.
 - If you are creating a stream processor to detect labels, you provide as input a Kinesis
video stream (`Input`), Amazon S3 bucket information (`Output`), and an Amazon SNS topic
ARN (`NotificationChannel`). You can also provide a KMS key ID to encrypt the data sent to
your Amazon S3 bucket. You specify what you want to detect by using the `ConnectedHome`
option in settings, and selecting one of the following: `PERSON`, `PET`, `PACKAGE`, `ALL`
You can also specify where in the frame you want Amazon Rekognition to monitor with
`RegionsOfInterest`. When you run the <a>StartStreamProcessor</a> operation on a label
...
<a>StartStreamProcessor</a> with the `Name` field.

 <p>This operation requires permissions to perform the `rekognition:CreateStreamProcessor`
...
- `input`: Kinesis video stream stream that provides the source streaming video. If you are
  using the AWS CLI, the parameter name is `StreamProcessorInput`. This is required for
  both face search and label detection stream processors.
- `name`: An identifier you assign to the stream processor. You can use `Name` to manage
  the stream processor. For example, you can get the current status of the stream processor
  by calling <a>DescribeStreamProcessor</a>. `Name` is idempotent. This is required for
  both face search and label detection stream processors.
- `output`: Kinesis data stream stream or Amazon S3 bucket location to which Amazon
  ...
  name is `StreamProcessorOutput`. This must be a <a>S3Destination</a> of an Amazon S3
...
- `role_arn`: The Amazon Resource Number (ARN) of the IAM role that allows access to the
  stream processor. The IAM role provides Rekognition read permissions for a Kinesis
  stream. It also provides write permissions to an Amazon S3 bucket and Amazon Simple
  Notification Service topic for a label detection stream processor. This is required for
  both face search and label detection stream processors.
...
- `"DataSharingPreference"`:  Shows whether you are sharing data with Rekognition to
  improve model performance. You can choose this option at the account level or on a per-
  stream basis. Note that if you opt out at the account level this setting is ignored on
  individual streams.
...
- `"Tags"`:  A set of tags (key-value pairs) that you want to attach to the stream
  processor.
"""
create_stream_processor(Input, Name, Output, RoleArn, Settings; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateStreamProcessor", Dict{String, Any}("Input"=>Input, "Name"=>Name, "Output"=>Output, "RoleArn"=>RoleArn, "Settings"=>Settings); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
provided `UserId` will alias the system generated UUID to make the `UserId` more user
friendly.

Uses a `ClientToken`, an idempotency token that ensures a call to `CreateUser` completes
only once. If the value is not supplied, the AWS SDK generates an idempotency token for the
...
- `"ClientRequestToken"`: Idempotent token used to identify the request to `CreateUser`. If
  you use the same token with multiple `CreateUser` requests, the same response is
  returned. Use ClientRequestToken to prevent the same request from being processed more
  than once.
"""
create_user(CollectionId, UserId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateUser", Dict{String, Any}("CollectionId"=>CollectionId, "UserId"=>UserId, "ClientRequestToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_user(CollectionId, UserId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("CreateUser", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId, "UserId"=>UserId, "ClientRequestToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
delete_collection(CollectionId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteCollection", Dict{String, Any}("CollectionId"=>CollectionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_collection(CollectionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteCollection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Deletes an existing
Amazon Rekognition Custom Labels dataset. Deleting a dataset might take while. Use
<a>DescribeDataset</a> to check the current status. The dataset is still deleting if the
...
dataset is updating (`Status` = `UPDATE_IN_PROGRESS`).
 ...
 <p>This operation requires permissions to perform the `rekognition:DeleteDataset` action.
...
- `dataset_arn`:  The ARN of the Amazon Rekognition Custom Labels dataset that you want to
  delete.
"""
delete_dataset(DatasetArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteDataset", Dict{String, Any}("DatasetArn"=>DatasetArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_dataset(DatasetArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteDataset", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DatasetArn"=>DatasetArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
delete_faces(CollectionId, FaceIds; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteFaces", Dict{String, Any}("CollectionId"=>CollectionId, "FaceIds"=>FaceIds); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_faces(CollectionId, FaceIds, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteFaces", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId, "FaceIds"=>FaceIds), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
<a>DescribeProjects</a>. The project is deleted when the project no longer appears in the
...
"""
delete_project(ProjectArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteProject", Dict{String, Any}("ProjectArn"=>ProjectArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_project(ProjectArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteProject", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ProjectArn"=>ProjectArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
To attach a project policy to a project, call <a>PutProjectPolicy</a>.
 ...
 <p>This operation requires permissions to perform the `rekognition:DeleteProjectPolicy`
action.
...
"""
delete_project_policy(PolicyName, ProjectArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteProjectPolicy", Dict{String, Any}("PolicyName"=>PolicyName, "ProjectArn"=>ProjectArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_project_policy(PolicyName, ProjectArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteProjectPolicy", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("PolicyName"=>PolicyName, "ProjectArn"=>ProjectArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
You can't delete a project version if it is running or if it is training. To check the
status of a project version, use the Status field returned from
<a>DescribeProjectVersions</a>. To stop a project version call <a>StopProjectVersion</a>.
...
- `project_version_arn`: The Amazon Resource Name (ARN) of the project version that you
  want to delete.
"""
delete_project_version(ProjectVersionArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteProjectVersion", Dict{String, Any}("ProjectVersionArn"=>ProjectVersionArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_project_version(ProjectVersionArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteProjectVersion", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ProjectVersionArn"=>ProjectVersionArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
create the stream processor with <a>CreateStreamProcessor</a>. You might not be able to use
...
"""
delete_stream_processor(Name; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteStreamProcessor", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_stream_processor(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteStreamProcessor", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
`ResourceNotFoundException` will be thrown. If the action is successful with a 200
response, an empty HTTP body is returned.
...
- `"ClientRequestToken"`: Idempotent token used to identify the request to `DeleteUser`. If
  you use the same token with multiple `DeleteUser `requests, the same response is
  returned. Use ClientRequestToken to prevent the same request from being processed more
  than once.
"""
delete_user(CollectionId, UserId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteUser", Dict{String, Any}("CollectionId"=>CollectionId, "UserId"=>UserId, "ClientRequestToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_user(CollectionId, UserId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DeleteUser", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId, "UserId"=>UserId, "ClientRequestToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_collection(CollectionId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DescribeCollection", Dict{String, Any}("CollectionId"=>CollectionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_collection(CollectionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DescribeCollection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p> Describes an
Amazon Rekognition Custom Labels dataset. You can get information such as the current
status of a dataset and statistics about the images and labels in a dataset.

 <p>This operation requires permissions to perform the `rekognition:DescribeDataset` action.

# Arguments

- `dataset_arn`:  The Amazon Resource Name (ARN) of the dataset that you want to describe.
"""
describe_dataset(DatasetArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DescribeDataset", Dict{String, Any}("DatasetArn"=>DatasetArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_dataset(DatasetArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DescribeDataset", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DatasetArn"=>DatasetArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
model or adapter versions in `ProjectVersionArns`. If you don't specify a value,
...
- `"NextToken"`: If the previous response was incomplete (because there is more results to
  retrieve), Amazon Rekognition returns a pagination token in the response. You can use
...
- `"VersionNames"`: A list of model or project version names that you want to describe. You
  can add up to 10 model or project version names to the list. If you don't specify a
  value, all project version descriptions are returned. A version name is part of a project
  version ARN. For example, `my-model.2020-01-21T09.10.15` is the version name in the
  following ARN. `arn:aws:rekognition:us-east-1:123456789012:project/getting-
  started/version/*my-model.2020-01-21T09.10.15*/1234567890123`.
"""
describe_project_versions(ProjectArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DescribeProjectVersions", Dict{String, Any}("ProjectArn"=>ProjectArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_project_versions(ProjectArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DescribeProjectVersions", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ProjectArn"=>ProjectArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"ProjectNames"`: A list of the projects that you want Rekognition to describe. If you
  ...
  AWS account.
"""
describe_projects(; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DescribeProjects"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_projects(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DescribeProjects", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)

"""
    describe_stream_processor(name)
    describe_stream_processor(name, params::Dict{String,<:Any})

Provides information about a stream processor created by <a>CreateStreamProcessor</a>. You
can get information about the input and output streams, the input parameters for the face
...
"""
describe_stream_processor(Name; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DescribeStreamProcessor", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_stream_processor(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DescribeStreamProcessor", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Detects custom
...
`DetectCustomLabels` are also returned as a percentage. You can use `MinConfidence` to
change the precision and recall or your model. For more information, see *Analyzing an
...
This operation requires permissions to perform the `rekognition:DetectCustomLabels` action.
 ...
 <p>For more information, see *Analyzing an image* in the Amazon Rekognition Custom Labels
Developer Guide.
...
- `project_version_arn`: The ARN of the model version that you want to use. Only models
  associated with Custom Labels projects accepted by the operation. If a provided ARN
  refers to a model version associated with a project for a different feature type, then an
  InvalidParameterException is returned.
...
- `"MinConfidence"`: Specifies the minimum confidence level for the labels to return.
  `DetectCustomLabels` doesn't return any labels with a confidence value that's lower than
  this specified value. If you specify a value of 0, `DetectCustomLabels` returns all
  labels, regardless of the assumed threshold applied to each label. If you don't specify a
  value for `MinConfidence`, `DetectCustomLabels` returns labels based on the assumed
  threshold of each label.
"""
detect_custom_labels(Image, ProjectVersionArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectCustomLabels", Dict{String, Any}("Image"=>Image, "ProjectVersionArn"=>ProjectVersionArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
detect_custom_labels(Image, ProjectVersionArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectCustomLabels", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Image"=>Image, "ProjectVersionArn"=>ProjectVersionArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
such as facial landmarks (for example, coordinates of eye and mouth), pose, presence of
...
data.This operation requires permissions to perform the `rekognition:DetectFaces` action.
...
- `"Attributes"`: An array of facial attributes you want to be returned. A `DEFAULT` subset
  ...
  the default list) - by using [`"DEFAULT", "FACE_OCCLUDED"`] or just [`"FACE_OCCLUDED"`]. You can request for all facial attributes by using [`"ALL"]`. Requesting more attributes may increase response time.
...
"""
detect_faces(Image; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectFaces", Dict{String, Any}("Image"=>Image); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
detect_faces(Image, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectFaces", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Image"=>Image), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
when calling the DetectLabels API. Including `GENERAL_LABELS` will ensure the response
includes the labels detected in the input image, while including `IMAGE_PROPERTIES `will
ensure the response includes information about the image quality and color.

When using `GENERAL_LABELS` and/or `IMAGE_PROPERTIES` you can provide filtering criteria to
the Settings parameter. You can filter with sets of individual labels or with label
categories. You can specify inclusive filters, exclusive filters, or a combination of
inclusive and exclusive filters. For more information on filtering see [Detecting Labels in an Image](https://docs.aws.amazon.com/rekognition/latest/dg/labels-detect-labels-image.html).
...
the following types of information about labels:</p> - Name - The name of the detected
label.
...
object, describing the location of the label on the input image. It also includes the
...
ImageProperties structure: - Quality - Information about the Sharpness, Brightness, and
Contrast of the input image, scored between 0 to 100. Image quality is returned for the
...
along with information about that label. In the following example, suppose the input image
has a lighthouse, the sea, and a rock. The response includes all three labels, one for each
...
details that the <a>DetectFaces</a> operation provides.This is a stateless API operation
that doesn't return any data.

 <p>This operation requires permissions to perform the `rekognition:DetectLabels` action.

# Arguments

- `image`: The input image as base64-encoded bytes or an S3 object. If you use the AWS CLI
  to call Amazon Rekognition operations, passing image bytes is not supported. Images
...
"""
detect_labels(Image; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectLabels", Dict{String, Any}("Image"=>Image); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
detect_labels(Image, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectLabels", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Image"=>Image), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
detect_moderation_labels(Image; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectModerationLabels", Dict{String, Any}("Image"=>Image); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
detect_moderation_labels(Image, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectModerationLabels", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Image"=>Image), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
Rekognition can detect the following types of PPE.</p> - Face cover
 - Hand cover
 - Head cover
You pass the input image as base64-encoded image bytes or as a reference to an image in an
...
also returns a bounding box (<a>BoundingBox</a>) for each detected person and each detected
item of PPE.
...
"""
detect_protective_equipment(Image; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectProtectiveEquipment", Dict{String, Any}("Image"=>Image); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
detect_protective_equipment(Image, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectProtectiveEquipment", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Image"=>Image), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
The `DetectText` operation returns text in an array of <a>TextDetection</a> elements,
...
represent the end of a line. If a sentence spans multiple lines, the `DetectText` operation
...
"""
detect_text(Image; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectText", Dict{String, Any}("Image"=>Image); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
detect_text(Image, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DetectText", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Image"=>Image), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
Removes the association between a `Face` supplied in an array of `FaceIds` and the User. If
the User is not present already, then a `ResourceNotFound` exception is thrown. If
successful, an array of faces that are disassociated from the User is returned. If a given
face is already disassociated from the given UserID, it will be ignored and not be returned
in the response. If a given face is already associated with a different User or not found
in the collection it will be returned as part of `UnsuccessfulDisassociations`. You can
...
- `"ClientRequestToken"`: Idempotent token used to identify the request to
  `DisassociateFaces`. If you use the same token with multiple `DisassociateFaces`
...
"""
disassociate_faces(CollectionId, FaceIds, UserId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DisassociateFaces", Dict{String, Any}("CollectionId"=>CollectionId, "FaceIds"=>FaceIds, "UserId"=>UserId, "ClientRequestToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
disassociate_faces(CollectionId, FaceIds, UserId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DisassociateFaces", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId, "FaceIds"=>FaceIds, "UserId"=>UserId, "ClientRequestToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Distributes the
entries (images) in a training dataset across the training dataset and the test dataset for
a project. `DistributeDatasetEntries` moves 20% of the training dataset images to the test
...
dataset. The training dataset must contain the images that you want to split. The test
dataset must be empty. The datasets must belong to the same project. To create training and
test datasets for a project, call <a>CreateDataset</a>.

Distributing a dataset takes a while to complete. To check the status call
`DescribeDataset`. The operation is complete when the `Status` field for the training
dataset and the test dataset is `UPDATE_COMPLETE`. If the dataset split fails, the value of
...
"""
distribute_dataset_entries(Datasets; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DistributeDatasetEntries", Dict{String, Any}("Datasets"=>Datasets); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
distribute_dataset_entries(Datasets, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("DistributeDatasetEntries", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Datasets"=>Datasets), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
get_celebrity_info(Id; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetCelebrityInfo", Dict{String, Any}("Id"=>Id); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_celebrity_info(Id, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetCelebrityInfo", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Id"=>Id), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
call to <a>StartCelebrityRecognition</a> which returns a job identifier (`JobId`).
...
call to `StartCelebrityRecognition`. To get the results of the celebrity recognition
analysis, first check that the status value published to the Amazon SNS topic is
`SUCCEEDED`. If so, call `GetCelebrityDetection` and pass the job identifier (`JobId`) from
...
in an array (`Celebrities`) of <a>CelebrityRecognition</a> objects. Each
`CelebrityRecognition` contains information about the celebrity in a <a>CelebrityDetail</a>
object and the time, `Timestamp`, the celebrity was detected. This <a>CelebrityDetail</a>
object stores information about the detected celebrity's face attributes, a face bounding
box, known gender, the celebrity's name, and a confidence estimate.</p>

!!! note
    `GetCelebrityRecognition` only returns the default facial attributes (`BoundingBox`,
`Confidence`, `Landmarks`, `Pose`, and `Quality`). The `BoundingBox` field only applies to
the detected face instance. The other facial attributes listed in the `Face` object of the
...
No information is returned for faces not recognized as celebrities.
 ...
 <p>Use MaxResults parameter to limit the number of labels returned. If there are more
...
results, call `GetCelebrityDetection` and populate the `NextToken` request parameter with
the token value returned from the previous call to `GetCelebrityRecognition`.
...
- `"SortBy"`: Sort to use for celebrities returned in `Celebrities` field. Specify `ID` to
  sort by the celebrity identifier, specify `TIMESTAMP` to sort by the time the celebrity
  was recognized.
"""
get_celebrity_recognition(JobId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetCelebrityRecognition", Dict{String, Any}("JobId"=>JobId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_celebrity_recognition(JobId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetCelebrityRecognition", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("JobId"=>JobId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
moderation labels in Amazon Rekognition, see [Using the image and video moderation APIs](https://docs.aws.amazon.com/rekognition/latest/dg/moderation.html#moderation-api).

Amazon Rekognition Video inappropriate or offensive content detection in a stored video is
an asynchronous operation. You start analysis by calling <a>StartContentModeration</a>
which returns a job identifier (`JobId`). When analysis finishes, Amazon Rekognition Video
publishes a completion status to the Amazon Simple Notification Service topic registered in
the initial call to `StartContentModeration`. To get the results of the content analysis,
...
to limit the number of labels returned in a single call to `GetContentModeration`. If there
...
page of results, call `GetContentModeration` and populate the `NextToken` request parameter
with the value of `NextToken` returned from the previous call to `GetContentModeration`.
...
"""
get_content_moderation(JobId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetContentModeration", Dict{String, Any}("JobId"=>JobId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_content_moderation(JobId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetContentModeration", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("JobId"=>JobId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
detection by calling <a>StartFaceDetection</a> which returns a job identifier (`JobId`).
When the face detection operation finishes, Amazon Rekognition Video publishes a completion
status to the Amazon Simple Notification Service topic registered in the initial call to
`StartFaceDetection`. To get the results of the face detection operation, first check that
the status value published to the Amazon SNS topic is `SUCCEEDED`. If so, call
<a>GetFaceDetection</a> and pass the job identifier (`JobId`) from the initial call to
`StartFaceDetection`.
...
than specified in `MaxResults`, the value of `NextToken` in the operation response contains
a pagination token for getting the next set of results. To get the next page of results,
call `GetFaceDetection` and populate the `NextToken` request parameter with the token value
...
"""
get_face_detection(JobId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetFaceDetection", Dict{String, Any}("JobId"=>JobId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_face_detection(JobId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetFaceDetection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("JobId"=>JobId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
get_face_liveness_session_results(SessionId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetFaceLivenessSessionResults", Dict{String, Any}("SessionId"=>SessionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_face_liveness_session_results(SessionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetFaceLivenessSessionResults", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SessionId"=>SessionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
<a>StartFaceSearch</a>. The search returns faces in a collection that match the faces of
persons detected in a video. It also includes the time(s) that faces are matched in the
video.

Face search in a video is an asynchronous operation. You start face search by calling to
<a>StartFaceSearch</a> which returns a job identifier (`JobId`). When the search operation
finishes, Amazon Rekognition Video publishes a completion status to the Amazon Simple
Notification Service topic registered in the initial call to `StartFaceSearch`. To get the
search results, first check that the status value published to the Amazon SNS topic is
`SUCCEEDED`. If so, call `GetFaceSearch` and pass the job identifier (`JobId`) from the
initial call to `StartFaceSearch`.

For more information, see Searching Faces in a Collection in the Amazon Rekognition
Developer Guide.

The search results are retured in an array, `Persons`, of <a>PersonMatch</a> objects.
Each`PersonMatch` element contains details about the matching faces in the input
collection, person information (facial attributes, bounding boxes, and person identifer)
...
the `Face` object of the following response syntax are not returned. For more information,
see FaceDetail in the Amazon Rekognition Developer Guide.By default, the `Persons` array is
sorted by the time, in milliseconds from the start of the video, persons are matched. You
...
- `"NextToken"`: If the previous response was incomplete (because there is more search
  results to retrieve), Amazon Rekognition Video returns a pagination token in the
  response. You can use this pagination token to retrieve the next set of search results.
...
"""
get_face_search(JobId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetFaceSearch", Dict{String, Any}("JobId"=>JobId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_face_search(JobId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetFaceSearch", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("JobId"=>JobId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
returns a job identifier (`JobId`). When the label detection operation finishes, Amazon
Rekognition publishes a completion status to the Amazon Simple Notification Service topic
registered in the initial call to `StartlabelDetection`.

To get the results of the label detection operation, first check that the status value
published to the Amazon SNS topic is `SUCCEEDED`. If so, call <a>GetLabelDetection</a> and
...
`SortBy` input parameter. If there is no `NAME` specified, the default sort is by
timestamp.
...
The returned Labels array may include the following attributes:</p> - Name - The name of
the detected label.
 - Confidence - The level of confidence in the label assigned to a detected object.
...
label car. The label car has two parent labels: Vehicle (its parent) and Transportation
(its grandparent). The response includes the all ancestors for a label, where every
ancestor is a unique label. In the previous example, Car, Vehicle, and Transportation are
...
detected. For aggregation by `SEGMENTS`, the `StartTimestampMillis`, `EndTimestampMillis`,
and `DurationMillis` structures are what define a segment. Although the “Timestamp”
structure is still returned with each label, its value is set to be the same as
...
detected instances isn’t returned.

The version of the label model used for the detection is also returned.

 **Note `DominantColors` isn't returned for `Instances`, although it is shown as part of
the response in the sample seen below.**

Use `MaxResults` parameter to limit the number of labels returned. If there are more
results than specified in `MaxResults`, the value of `NextToken` in the operation response
contains a pagination token for getting the next set of results. To get the next page of
results, call `GetlabelDetection` and populate the `NextToken` request parameter with the
token value returned from the previous call to `GetLabelDetection`.

 <p>If you are retrieving results while using the Amazon Simple Notification Service, note
...
- `"SortBy"`: Sort to use for elements in the `Labels` array. Use `TIMESTAMP` to sort array
  elements by the time labels are detected. Use `NAME` to alphabetically group elements for
...
"""
get_label_detection(JobId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetLabelDetection", Dict{String, Any}("JobId"=>JobId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_label_detection(JobId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetLabelDetection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("JobId"=>JobId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `job_id`: Unique identifier for the media analysis job for which you want to retrieve
  results.
"""
get_media_analysis_job(JobId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetMediaAnalysisJob", Dict{String, Any}("JobId"=>JobId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_media_analysis_job(JobId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetMediaAnalysisJob", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("JobId"=>JobId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
published to the Amazon SNS topic is `SUCCEEDED`. If so, call <a>GetPersonTracking</a> and
...
For more information, see FaceDetail in the Amazon Rekognition Developer Guide.By default,
the array is sorted by the time(s) a person's path is tracked in the video. You can sort by
tracked persons by specifying `INDEX` for the `SortBy` input parameter.

 <p>Use the `MaxResults` parameter to limit the number of items returned. If there are more
results than specified in `MaxResults`, the value of `NextToken` in the operation response
contains a pagination token for getting the next set of results. To get the next page of
results, call `GetPersonTracking` and populate the `NextToken` request parameter with the
token value returned from the previous call to `GetPersonTracking`.

# Arguments

- `job_id`: The identifier for a job that tracks persons in a video. You get the `JobId`
...
- `"SortBy"`: Sort to use for elements in the `Persons` array. Use `TIMESTAMP` to sort
  array elements by the time persons are detected. Use `INDEX` to sort by the tracked
...
"""
get_person_tracking(JobId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetPersonTracking", Dict{String, Any}("JobId"=>JobId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_person_tracking(JobId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetPersonTracking", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("JobId"=>JobId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
segment detection by calling <a>StartSegmentDetection</a> which returns a job identifier
(`JobId`). When the segment detection operation finishes, Amazon Rekognition publishes a
completion status to the Amazon Simple Notification Service topic registered in the initial
call to `StartSegmentDetection`. To get the results of the segment detection operation,
first check that the status value published to the Amazon SNS topic is `SUCCEEDED`. if so,
call `GetSegmentDetection` and pass the job identifier (`JobId`) from the initial call of
`StartSegmentDetection`.

 `GetSegmentDetection` returns detected segments in an array (`Segments`) of
<a>SegmentDetection</a> objects. `Segments` is sorted by the segment types specified in the
...
are more results than specified in `MaxResults`, the value of `NextToken` in the operation
response contains a pagination token for getting the next set of results. To get the next
page of results, call `GetSegmentDetection` and populate the `NextToken` request parameter
with the token value returned from the previous call to `GetSegmentDetection`.
...
"""
get_segment_detection(JobId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetSegmentDetection", Dict{String, Any}("JobId"=>JobId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_segment_detection(JobId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetSegmentDetection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("JobId"=>JobId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
detection by calling <a>StartTextDetection</a> which returns a job identifier (`JobId`)
When the text detection operation finishes, Amazon Rekognition publishes a completion
status to the Amazon Simple Notification Service topic registered in the initial call to
`StartTextDetection`. To get the results of the text detection operation, first check that
the status value published to the Amazon SNS topic is `SUCCEEDED`. if so, call
...
results than specified in `MaxResults`, the value of `NextToken` in the operation response
contains a pagination token for getting the next set of results. To get the next page of
results, call `GetTextDetection` and populate the `NextToken` request parameter with the
...
"""
get_text_detection(JobId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetTextDetection", Dict{String, Any}("JobId"=>JobId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_text_detection(JobId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("GetTextDetection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("JobId"=>JobId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
operations using the <a>SearchFaces</a> and <a>SearchFacesByImage</a> operations.
...
To determine which version of the model you're using, call <a>DescribeCollection</a> and
...
<a>ListFaces</a> operation, the response returns the external ID. You can use this external
image ID to create a client-side index to associate the faces with each image. You can then
...
The `QualityFilter` input parameter allows you to filter out detected faces that don’t meet
a required quality bar. The quality bar is based on a variety of common use cases. By
...
<a>DescribeCollection</a>.Information about faces detected in an image, but not indexed, is
returned in an array of <a>UnindexedFace</a> objects, `UnindexedFaces`. Faces aren't
indexed for reasons such as: - The number of faces detected exceeds the value of the
...
faces, `FaceRecords`. This includes:  - The bounding box, `BoundingBox`, of the detected
face.
...
detectionAttributes parameter, Amazon Rekognition returns detailed facial attributes, such
as facial landmarks (for example, location of eye and mouth), facial occlusion, and other
facial attributes.
...
passing image bytes isn't supported. The image must be formatted as a PNG or JPEG file.
 ...
 <p>This operation requires permissions to perform the `rekognition:IndexFaces` action.
...
- `"DetectionAttributes"`: An array of facial attributes you want to be returned. A
  `DEFAULT` subset of facial attributes - `BoundingBox`, `Confidence`, `Pose`, `Quality`,
  and `Landmarks` - will always be returned. You can request for specific facial attributes
  (in addition to the default list) - by using `["DEFAULT", "FACE_OCCLUDED"]` or just `["FACE_OCCLUDED"]`. You can request for all facial attributes by using `["ALL"]`. Requesting more attributes may increase response time.

If you provide both, `["ALL", "DEFAULT"]`, the service uses a logical AND operator to determine which attributes to return (in this case, all attributes).
- `"ExternalImageId"`: The ID you want to assign to all the faces detected in the image.
- `"MaxFaces"`: The maximum number of faces to index. The value of `MaxFaces` must be
  greater than or equal to 1. `IndexFaces` returns no more than 100 detected faces in an
...
- `"QualityFilter"`: A filter that specifies a quality bar for how much filtering is done
  to identify faces. Filtered faces aren't indexed. If you specify `AUTO`, Amazon
  Rekognition chooses the quality bar. If you specify `LOW`, `MEDIUM`, or `HIGH`, filtering
  removes all faces that don’t meet the chosen quality bar. The default value is `AUTO`.
  The quality bar is based on a variety of common use cases. Low-quality detections can
  occur for a number of reasons. Some examples are an object that's misidentified as a
  face, a face that's too blurry, or a face with a pose that's too extreme to use. If you
  specify `NONE`, no filtering is performed.

  To use quality filtering, the collection you are using must be associated with version 3
  of the face model or higher.
"""
index_faces(CollectionId, Image; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("IndexFaces", Dict{String, Any}("CollectionId"=>CollectionId, "Image"=>Image); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
index_faces(CollectionId, Image, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("IndexFaces", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId, "Image"=>Image), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
list_collections(; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListCollections"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_collections(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListCollections", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p> Lists the entries
...
You can filter the response in variety of ways, such as choosing which labels to return and
returning JSON Lines created after a specific date.

 <p>This operation requires permissions to perform the `rekognition:ListDatasetEntries`
action.
...
- `"Labeled"`:  Specify `true` to get only the JSON Lines where the image is labeled.
  Specify `false` to get only the JSON Lines where the image isn't labeled. If you don't
...
- `"SourceRefContains"`: If specified, `ListDatasetEntries` only returns JSON Lines where
  the value of `SourceRefContains` is part of the `source-ref` field. The `source-ref`
  field contains the Amazon S3 location of the image. You can use `SouceRefContains` for
  tasks such as getting the JSON Line for a single image, or gettting JSON Lines for all
  images within a specific folder.
"""
list_dataset_entries(DatasetArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListDatasetEntries", Dict{String, Any}("DatasetArn"=>DatasetArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_dataset_entries(DatasetArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListDatasetEntries", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DatasetArn"=>DatasetArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Lists the labels in
a dataset. Amazon Rekognition Custom Labels uses labels to describe images. For more
information, see [Labeling images](https://docs.aws.amazon.com/rekognition/latest/customlabels-dg/md-labeling-images.html).


 <p> Lists the labels in a dataset. Amazon Rekognition Custom Labels uses labels to
describe images. For more information, see Labeling images in the *Amazon Rekognition
...
- `"NextToken"`: If the previous response was incomplete (because there is more results to
  ...
  You can use this pagination token to retrieve the next set of results.
"""
list_dataset_labels(DatasetArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListDatasetLabels", Dict{String, Any}("DatasetArn"=>DatasetArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_dataset_labels(DatasetArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListDatasetLabels", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DatasetArn"=>DatasetArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"UserId"`: An array of user IDs to filter results with when listing faces in a
  collection.
"""
list_faces(CollectionId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListFaces", Dict{String, Any}("CollectionId"=>CollectionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_faces(CollectionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListFaces", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
list_media_analysis_jobs(; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListMediaAnalysisJobs"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_media_analysis_jobs(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListMediaAnalysisJobs", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Gets a list of the
project policies attached to a project.

To attach a project policy to a project, call <a>PutProjectPolicy</a>. To remove a project
policy from a project, call <a>DeleteProjectPolicy</a>.

 <p>This operation requires permissions to perform the `rekognition:ListProjectPolicies`
action.
...
- `"NextToken"`: If the previous response was incomplete (because there is more results to
  ...
  You can use this pagination token to retrieve the next set of results.
"""
list_project_policies(ProjectArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListProjectPolicies", Dict{String, Any}("ProjectArn"=>ProjectArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_project_policies(ProjectArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListProjectPolicies", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ProjectArn"=>ProjectArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: If the previous response was incomplete (because there are more stream
  ...
  processors.
"""
list_stream_processors(; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListStreamProcessors"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_stream_processors(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListStreamProcessors", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
list_tags_for_resource(ResourceArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListTagsForResource", Dict{String, Any}("ResourceArn"=>ResourceArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_tags_for_resource(ResourceArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListTagsForResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourceArn"=>ResourceArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)

"""
    list_users(collection_id)
    list_users(collection_id, params::Dict{String,<:Any})

Returns metadata of the User such as `UserID` in the specified collection. Anonymous User
(to reserve faces without any identity) is not returned as part of this request. The
results are sorted by system generated primary key ID. If the response is truncated,
`NextToken` is returned in the response that can be used in the subsequent request to
...
"""
list_users(CollectionId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListUsers", Dict{String, Any}("CollectionId"=>CollectionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_users(CollectionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("ListUsers", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Attaches a project
...
attach multiple project policies to a project. You can also update an existing project
policy by specifying the policy revision ID of the existing policy.

To remove a project policy from a project, call <a>DeleteProjectPolicy</a>. To get a list
of project policies attached to a project, call <a>ListProjectPolicies</a>.

You copy a model version by calling <a>CopyProjectVersion</a>.

 <p>This operation requires permissions to perform the `rekognition:PutProjectPolicy`
action.

# Arguments

- `policy_document`: A resource policy to add to the model. The policy is a JSON structure
  that contains one or more statements that define the policy. The policy must follow the
  IAM syntax. For more information about the contents of a JSON policy document, see [IAM JSON policy reference](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies.html).
...
"""
put_project_policy(PolicyDocument, PolicyName, ProjectArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("PutProjectPolicy", Dict{String, Any}("PolicyDocument"=>PolicyDocument, "PolicyName"=>PolicyName, "ProjectArn"=>ProjectArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
put_project_policy(PolicyDocument, PolicyName, ProjectArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("PutProjectPolicy", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("PolicyDocument"=>PolicyDocument, "PolicyName"=>PolicyName, "ProjectArn"=>ProjectArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
celebrities in the `CelebrityFaces` array and any unrecognized faces in the
`UnrecognizedFaces` array. `RecognizeCelebrities` doesn't return celebrities whose faces
...
- `image`: The input image as base64-encoded bytes or an S3 object. If you use the AWS CLI
  ...
  the Amazon Rekognition developer guide.
"""
recognize_celebrities(Image; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("RecognizeCelebrities", Dict{String, Any}("Image"=>Image); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
recognize_celebrities(Image, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("RecognizeCelebrities", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Image"=>Image), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
to. You get a face ID when you add a face to the collection using the <a>IndexFaces</a>
...
operation. The operation response returns an array of faces that match, ordered by
similarity score with the highest similarity first. More specifically, it is an array of
metadata for each face match that is found. Along with the metadata, the response also
includes a `confidence` value for each face match, indicating the confidence that the
specific face matches the input face.

For an example, see Searching for a face using its face ID in the Amazon Rekognition
Developer Guide.

 <p>This operation requires permissions to perform the `rekognition:SearchFaces` action.
...
- `"FaceMatchThreshold"`: Optional value specifying the minimum confidence in the face
  match to return. For example, don't return any matches where confidence in matches is
...
"""
search_faces(CollectionId, FaceId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("SearchFaces", Dict{String, Any}("CollectionId"=>CollectionId, "FaceId"=>FaceId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
search_faces(CollectionId, FaceId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("SearchFaces", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId, "FaceId"=>FaceId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
to make face crops, which then you can pass in to the `SearchFacesByImage` operation.You
...
bounding box (and a confidence level that the bounding box contains a face) of the face
...
Guide.
 ...
 <p>The `QualityFilter` input parameter allows you to filter out detected faces that don’t
meet a required quality bar. The quality bar is based on a variety of common use cases. Use
`QualityFilter` to set the quality bar for filtering by specifying `LOW`, `MEDIUM`, or
`HIGH`. If you do not want to filter detected faces, specify `NONE`. The default value is
`NONE`.

!!! note
    To use quality filtering, you need a collection associated with version 3 of the face
model or higher. To get the version of the face model associated with a collection, call
<a>DescribeCollection</a>.This operation requires permissions to perform the
...
- `"FaceMatchThreshold"`: (Optional) Specifies the minimum confidence in the face match to
  return. For example, don't return any matches where confidence in matches is less than
...
- `"QualityFilter"`: A filter that specifies a quality bar for how much filtering is done
  ...
  `HIGH`, filtering removes all faces that don’t meet the chosen quality bar. The quality
  ...
  of the face model or higher.
"""
search_faces_by_image(CollectionId, Image; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("SearchFacesByImage", Dict{String, Any}("CollectionId"=>CollectionId, "Image"=>Image); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
search_faces_by_image(CollectionId, Image, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("SearchFacesByImage", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId, "Image"=>Image), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)

"""
    search_users(collection_id)
    search_users(collection_id, params::Dict{String,<:Any})

Searches for UserIDs within a collection based on a `FaceId` or `UserId`. This API can be
used to find the closest UserID (with a highest similarity) to associate a face. The
request must be provided with either `FaceId` or `UserId`. The operation returns an array
of UserID that match the `FaceId` or `UserId`, ordered by similarity score with the highest
similarity first.
...
"""
search_users(CollectionId; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("SearchUsers", Dict{String, Any}("CollectionId"=>CollectionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
search_users(CollectionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("SearchUsers", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
returned in an array of `UnsearchedFace` objects. If no valid face is detected in the
image, the response will contain an empty `UserMatches` list and no `SearchedFace` object.
...
- `"QualityFilter"`: A filter that specifies a quality bar for how much filtering is done
  to identify faces. Filtered faces aren't searched for in the collection. The default
  value is NONE.
- `"UserMatchThreshold"`: Specifies the minimum confidence in the UserID match to return.
  Default value is 80.
"""
search_users_by_image(CollectionId, Image; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("SearchUsersByImage", Dict{String, Any}("CollectionId"=>CollectionId, "Image"=>Image); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
search_users_by_image(CollectionId, Image, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("SearchUsersByImage", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId, "Image"=>Image), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
results of the analysis. When celebrity recognition analysis is finished, Amazon
Rekognition Video publishes a completion status to the Amazon Simple Notification Service
topic that you specify in `NotificationChannel`. To get the results of the celebrity
recognition analysis, first check that the status value published to the Amazon SNS topic
is `SUCCEEDED`. If so, call <a>GetCelebrityRecognition</a> and pass the job identifier
...
- `"ClientRequestToken"`: Idempotent token used to identify the start request. If you use
  ...
  returned. Use `ClientRequestToken` to prevent the same job from being accidently started
  more than once.
- `"JobTag"`: An identifier you specify that's returned in the completion notification
  that's published to your Amazon Simple Notification Service topic. For example, you can
  use `JobTag` to group related jobs and identify them in the completion notification.
...
"""
start_celebrity_recognition(Video; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartCelebrityRecognition", Dict{String, Any}("Video"=>Video); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_celebrity_recognition(Video, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartCelebrityRecognition", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Video"=>Video), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
results of the analysis. When content analysis is finished, Amazon Rekognition Video
publishes a completion status to the Amazon Simple Notification Service topic that you
specify in `NotificationChannel`.

To get the results of the content analysis, first check that the status value published to
the Amazon SNS topic is `SUCCEEDED`. If so, call <a>GetContentModeration</a> and pass the
...
- `"ClientRequestToken"`: Idempotent token used to identify the start request. If you use
  the same token with multiple `StartContentModeration` requests, the same `JobId` is
  returned. Use `ClientRequestToken` to prevent the same job from being accidently started
  more than once.
- `"JobTag"`: An identifier you specify that's returned in the completion notification
  that's published to your Amazon Simple Notification Service topic. For example, you can
...
- `"MinConfidence"`: Specifies the minimum confidence that Amazon Rekognition must have in
  ...
  confidence. 100 is the highest confidence. Amazon Rekognition doesn't return any
  moderated content labels with a confidence level lower than this specified value. If you
  don't specify `MinConfidence`, `GetContentModeration` returns labels with confidence
  values greater than or equal to 50 percent.
- `"NotificationChannel"`: The Amazon SNS topic ARN that you want Amazon Rekognition Video
  to publish the completion status of the content analysis to. The Amazon SNS topic must
  have a topic name that begins with *AmazonRekognition* if you are using the
  AmazonRekognitionServiceRole permissions policy to access the topic.
"""
start_content_moderation(Video; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartContentModeration", Dict{String, Any}("Video"=>Video); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_content_moderation(Video, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartContentModeration", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Video"=>Video), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
<a>Video</a> to specify the bucket name and the filename of the video. `StartFaceDetection`
...
the Amazon SNS topic is `SUCCEEDED`. If so, call <a>GetFaceDetection</a> and pass the job
...
- `"ClientRequestToken"`: Idempotent token used to identify the start request. If you use
  the same token with multiple `StartFaceDetection` requests, the same `JobId` is returned.
...
"""
start_face_detection(Video; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartFaceDetection", Dict{String, Any}("Video"=>Video); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_face_detection(Video, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartFaceDetection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Video"=>Video), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
`SUCCEEDED`. If so, call <a>GetFaceSearch</a> and pass the job identifier (`JobId`) from
the initial call to `StartFaceSearch`. For more information, see [Searching stored videos for faces](https://docs.aws.amazon.com/rekognition/latest/dg/procedure-person-search-videos.html).
...
- `"ClientRequestToken"`: Idempotent token used to identify the start request. If you use
  the same token with multiple `StartFaceSearch` requests, the same `JobId` is returned.
  Use `ClientRequestToken` to prevent the same job from being accidently started more than
  once.
...
- `"JobTag"`: An identifier you specify that's returned in the completion notification
  ...
  use `JobTag` to group related jobs and identify them in the completion notification.
- `"NotificationChannel"`: The ARN of the Amazon SNS topic to which you want Amazon
  Rekognition Video to publish the completion status of the search. The Amazon SNS topic
  must have a topic name that begins with *AmazonRekognition* if you are using the
  AmazonRekognitionServiceRole permissions policy to access the topic.
"""
start_face_search(CollectionId, Video; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartFaceSearch", Dict{String, Any}("CollectionId"=>CollectionId, "Video"=>Video); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_face_search(CollectionId, Video, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartFaceSearch", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CollectionId"=>CollectionId, "Video"=>Video), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
(`JobId`) which you use to get the results of the operation. When label detection is
finished, Amazon Rekognition Video publishes a completion status to the Amazon Simple
...
allows you to provide filtering criteria to the `Settings` parameter. You can filter with
sets of individual labels or with label categories. You can specify inclusive filters,
...
- `"ClientRequestToken"`: Idempotent token used to identify the start request. If you use
  the same token with multiple `StartLabelDetection` requests, the same `JobId` is
  returned. Use `ClientRequestToken` to prevent the same job from being accidently started
  more than once.
...
- `"MinConfidence"`: Specifies the minimum confidence that Amazon Rekognition Video must
  ...
  Rekognition is that a label is correctly identified.0 is the lowest confidence. 100 is
  the highest confidence. Amazon Rekognition Video doesn't return any labels with a
  confidence level lower than this specified value.

  If you don't specify `MinConfidence`, the operation returns labels and bounding boxes (if
...
"""
start_label_detection(Video; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartLabelDetection", Dict{String, Any}("Video"=>Video); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_label_detection(Video, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartLabelDetection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Video"=>Video), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"ClientRequestToken"`: Idempotency token used to prevent the accidental creation of
  ...
  `StartMediaAnalysisJobRequest` requests, the same response is returned. Use
...
bucket. Use <a>Video</a> to specify the bucket name and the filename of the video.
`StartPersonTracking` returns a job identifier (`JobId`) which you use to get the results
of the operation. When label detection is finished, Amazon Rekognition publishes a
...
- `"ClientRequestToken"`: Idempotent token used to identify the start request. If you use
  the same token with multiple `StartPersonTracking` requests, the same `JobId` is
  returned. Use `ClientRequestToken` to prevent the same job from being accidently started
  more than once.
- `"JobTag"`: An identifier you specify that's returned in the completion notification
  that's published to your Amazon Simple Notification Service topic. For example, you can
...
- `"NotificationChannel"`: The Amazon SNS topic ARN you want Amazon Rekognition Video to
  ...
  AmazonRekognitionServiceRole permissions policy.
"""
start_person_tracking(Video; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartPersonTracking", Dict{String, Any}("Video"=>Video); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_person_tracking(Video, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartPersonTracking", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Video"=>Video), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Starts the running
of the version of a model. Starting a model takes a while to complete. To check the current
...
<a>DetectCustomLabels</a>.

!!! note
    You are charged for the amount of time that the model is running. To stop a running
model, call <a>StopProjectVersion</a>.This operation requires permissions to perform the
...
- `"MaxInferenceUnits"`: The maximum number of inference units to use for auto-scaling the
  ...
  the model.
"""
start_project_version(MinInferenceUnits, ProjectVersionArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartProjectVersion", Dict{String, Any}("MinInferenceUnits"=>MinInferenceUnits, "ProjectVersionArn"=>ProjectVersionArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
<a>Video</a> to specify the bucket name and the filename of the video.
`StartSegmentDetection` returns a job identifier (`JobId`) which you use to get the results
of the operation. When segment detection is finished, Amazon Rekognition Video publishes a
completion status to the Amazon Simple Notification Service topic that you specify in
`NotificationChannel`.

You can use the `Filters` (<a>StartSegmentDetectionFilters</a>) input parameter to specify
the minimum detection confidence returned in the response. Within `Filters`, use
`ShotFilter` (<a>StartShotDetectionFilter</a>) to filter detected shots. Use
`TechnicalCueFilter` (<a>StartTechnicalCueDetectionFilter</a>) to filter technical cues.

To get the results of the segment detection operation, first check that the status value
published to the Amazon SNS topic is `SUCCEEDED`. if so, call <a>GetSegmentDetection</a>
and pass the job identifier (`JobId`) from the initial call to `StartSegmentDetection`.
...
- `"ClientRequestToken"`: Idempotent token used to identify the start request. If you use
  the same token with multiple `StartSegmentDetection` requests, the same `JobId` is
  returned. Use `ClientRequestToken` to prevent the same job from being accidently started
  more than once.
...
- `"NotificationChannel"`: The ARN of the Amazon SNS topic to which you want Amazon
  ...
  *AmazonRekognition* if you are using the AmazonRekognitionServiceRole permissions policy
  to access the topic.
"""
start_segment_detection(SegmentTypes, Video; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartSegmentDetection", Dict{String, Any}("SegmentTypes"=>SegmentTypes, "Video"=>Video); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_segment_detection(SegmentTypes, Video, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartSegmentDetection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SegmentTypes"=>SegmentTypes, "Video"=>Video), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
<a>CreateStreamProcessor</a>. To tell `StartStreamProcessor` which stream processor to
start, use the value of the `Name` field specified in the call to `CreateStreamProcessor`.

If you are using a label detection stream processor to detect labels, you need to provide a
`Start selector` and a `Stop selector` to determine the length of the stream processing
time.
...
- `"StopSelector"`:  Specifies when to stop processing the stream. You can specify a
  ...
  to start a face search stream processor.
"""
start_stream_processor(Name; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartStreamProcessor", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_stream_processor(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartStreamProcessor", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
<a>Video</a> to specify the bucket name and the filename of the video. `StartTextDetection`
...
published to the Amazon SNS topic is `SUCCEEDED`. if so, call <a>GetTextDetection</a> and
pass the job identifier (`JobId`) from the initial call to `StartTextDetection`.
...
- `"ClientRequestToken"`: Idempotent token used to identify the start request. If you use
  the same token with multiple `StartTextDetection` requests, the same `JobId` is returned.
  Use `ClientRequestToken` to prevent the same job from being accidentaly started more than
  once.
...
- `"JobTag"`: An identifier returned in the completion status published by your Amazon
  Simple Notification Service topic. For example, you can use `JobTag` to group related
  jobs and identify them in the completion notification.
- `"NotificationChannel"`:
"""
start_text_detection(Video; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartTextDetection", Dict{String, Any}("Video"=>Video); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_text_detection(Video, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StartTextDetection", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Video"=>Video), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Stops a running
model. The operation might take a while to complete. To check the current status, call
<a>DescribeProjectVersions</a>. Only applies to Custom Labels projects.

 <p>This operation requires permissions to perform the `rekognition:StopProjectVersion`
action.
...
- `project_version_arn`: The Amazon Resource Name (ARN) of the model version that you want
  ...
  action.
"""
stop_project_version(ProjectVersionArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StopProjectVersion", Dict{String, Any}("ProjectVersionArn"=>ProjectVersionArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
stop_project_version(ProjectVersionArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StopProjectVersion", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ProjectVersionArn"=>ProjectVersionArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
stop_stream_processor(Name; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StopStreamProcessor", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
stop_stream_processor(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("StopStreamProcessor", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
Custom Labels model. For more information, see [Tagging AWS Resources](https://docs.aws.amazon.com/general/latest/gr/aws_tagging.html).
...
"""
tag_resource(ResourceArn, Tags; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("TagResource", Dict{String, Any}("ResourceArn"=>ResourceArn, "Tags"=>Tags); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
tag_resource(ResourceArn, Tags, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("TagResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourceArn"=>ResourceArn, "Tags"=>Tags), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
untag_resource(ResourceArn, TagKeys; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("UntagResource", Dict{String, Any}("ResourceArn"=>ResourceArn, "TagKeys"=>TagKeys); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
untag_resource(ResourceArn, TagKeys, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("UntagResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourceArn"=>ResourceArn, "TagKeys"=>TagKeys), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    This operation applies only to Amazon Rekognition Custom Labels.</p>Adds or updates one
or more entries (images) in a dataset. An entry is a JSON Line which contains the
information for a single image, including the image location, assigned labels, and object
location bounding boxes. For more information, see Image-Level labels in manifest files and
Object localization in manifest files in the *Amazon Rekognition Custom Labels Developer
Guide*.

If the `source-ref` field in the JSON line references an existing image, the existing image
in the dataset is updated. If `source-ref` field doesn't reference an existing image, the
image is added as a new image to the dataset.

You specify the changes that you want to make in the `Changes` input parameter. There isn't
a limit to the number JSON Lines that you can change, but the size of `Changes` must be
less than 5MB.

 `UpdateDatasetEntries` returns immediatly, but the dataset update might take a while to
complete. Use <a>DescribeDataset</a> to check the current status. The dataset updated
successfully if the value of `Status` is `UPDATE_COMPLETE`.

To check if any non-terminal errors occured, call <a>ListDatasetEntries</a> and check for
the presence of `errors` lists in the JSON Lines.

Dataset update fails if a terminal error occurs (`Status` = `UPDATE_FAILED`). Currently,
you can't access the terminal error information from the Amazon Rekognition Custom Labels
SDK.

 <p>This operation requires permissions to perform the `rekognition:UpdateDatasetEntries`
action.
...
"""
update_dataset_entries(Changes, DatasetArn; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("UpdateDatasetEntries", Dict{String, Any}("Changes"=>Changes, "DatasetArn"=>DatasetArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_dataset_entries(Changes, DatasetArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("UpdateDatasetEntries", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Changes"=>Changes, "DatasetArn"=>DatasetArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"DataSharingPreferenceForUpdate"`:  Shows whether you are sharing data with Rekognition
  to improve model performance. You can choose this option at the account level or on a per-
  stream basis. Note that if you opt out at the account level this setting is ignored on
  individual streams.
...
- `"RegionsOfInterestForUpdate"`:  Specifies locations in the frames where Amazon
  Rekognition checks for objects or people. This is an optional parameter for label
...
- `"SettingsForUpdate"`:  The stream processor settings that you want to update. Label
  ...
  confidence.
"""
update_stream_processor(Name; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("UpdateStreamProcessor", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_stream_processor(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = rekognition("UpdateStreamProcessor", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
