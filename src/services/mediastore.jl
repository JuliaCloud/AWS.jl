...
# Required Parameters
- `ContainerName`: The name of the container that is associated with the metric policy that
  you want to delete.
...
# Required Parameters
- `ContainerName`: The name of the container that the object lifecycle policy is assigned
  to.
...
- `MaxResults`: Enter the maximum number of containers in the response. Use from 1 to 255
  characters.
- `NextToken`: Only if you used MaxResults in the first command, enter the token (which was
  included in the previous response) to obtain the next set of containers. This token is
...
# Required Parameters
- `ContainerName`: The name of the container that you want to assign the object lifecycle
  policy to.
...
- `MetricPolicy`: The metric policy that you want to associate with the container. In the
...
# Required Parameters
- `Resource`: The Amazon Resource Name (ARN) for the container.
- `TagKeys`: A comma-separated list of keys for tags that you want to remove from the
