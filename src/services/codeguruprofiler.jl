...
- `"frameMetrics"`: The details of the metrics that are used to request a time series of
  values. The metric includes the name of the frame, the aggregation type to calculate
  the metric value for the frame, and the thread states to use to get the count for the
...
- `"startTime"`: The start time of the time period for the frame metrics used to return the
  time series values. This is specified using the ISO 8601 format. For example, 2020-06-
...
- `"targetResolution"`: The requested resolution of time steps for the returned time series
  ...
  resolution after the target resolution. There are 3 valid values.
...
- `profiling_group_name`: The name of the profiling group for which the configured agent is
...
- `"fleetInstanceId"`: A universally unique identifier (UUID) for a profiling instance. For
  example, if the profiling instance is an Amazon EC2 instance, it is the instance ID. If
...
- `"metadata"`: Metadata captured about the compute platform the agent is running on. It
  ...
  - `LAMBDA_FUNCTION_ARN` - The Amazon Resource Name (ARN) that is used to invoke a
    Lambda function.
  - `LAMBDA_MEMORY_LIMIT_IN_MB` - The memory allocated to a Lambda function.
  - `LAMBDA_REMAINING_TIME_IN_MILLISECONDS` - The time in milliseconds before execution
...
- `"computePlatform"`: The compute platform of the profiling group. Use `AWSLambda` if your
  ...
  platform that is not AWS Lambda, such an Amazon EC2 instance, an on-premises server, or
  a different platform. If not specified, `Default` is used.
...
- `"dailyReportsOnly"`: A `Boolean` value indicating whether to only return reports from
  daily profiles. If set to `True`, only analysis data from daily profiles is returned.
  If set to `False`, analysis data is returned from smaller time windows (for example,
  one hour).
...
- `"nextToken"`: The `nextToken` value returned from a previous paginated
  `ListProfileTimes` request where `maxResults` was used and the results exceeded the
...
- `"maxResults"`: The maximum number of profiling groups results returned by
  ...
  by sending another `ListProfilingGroups` request with the returned `nextToken` value.
- `"nextToken"`: The `nextToken` value returned from a previous paginated
...
- `"profileToken"`: Amazon CodeGuru Profiler uses this universally unique identifier (UUID)
  to prevent the accidental submission of duplicate profiling data if there are failures
  and retries.
"""
...
- `action_group`: Specifies an action group that contains permissions to add to a profiling
...
- `resource_arn`: The Amazon Resource Name (ARN) of the resource that the tags are added to.
