...
- `"metadata"`: Metadata captured about the compute platform the agent is running on. It
  ...
  - `LAMBDA_TIME_GAP_BETWEEN_INVOKES_IN_MILLISECONDS` - The time in milliseconds between two
...
- `client_token`: Amazon CodeGuru Profiler uses this universally unique identifier (UUID) to
  prevent the accidental creation of duplicate profiling groups if there are failures and
  retries.
...
- `"maxResults"`: The maximum number of results returned by
  ...
  `GetFindingsReportAccountSummary` only returns `maxResults` results in a single page along
  with a `nextToken` response element. The remaining results of the initial request can be
  seen by sending another `GetFindingsReportAccountSummary` request with the returned
  `nextToken` value.
...
- `action_group`: Specifies an action group that contains permissions to add to a profiling
  group resource. One action group is supported, `agentPermissions`, which grants permission
  to perform actions required by the profiling agent, `ConfigureAgent` and
