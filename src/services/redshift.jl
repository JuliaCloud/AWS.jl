...
# Required Parameters
- `ReservedNodeId`: A string representing the node identifier of the DC1 Reserved Node to
  be exchanged.
- `TargetReservedNodeOfferingId`: The unique identifier of the DC2 Reserved Node offering
  to be used for the exchange. You can obtain the value for the parameter by calling
...
# Required Parameters
- `ClusterSecurityGroupName`: The name of the security group to which the ingress rule is
  added.
...
# Required Parameters
- `AccountWithRestoreAccess`: The identifier of the AWS customer account authorized to
...
# Required Parameters
- `DeleteClusterSnapshotMessage`: A list of identifiers for the snapshots that you want to
  delete.
...
# Required Parameters
- `ClusterIdentifier`: The unique identifier for the cluster that you want to cancel a
...
# Required Parameters
- `SourceSnapshotIdentifier`: The identifier for the source snapshot. Constraints:   Must
...
- `SourceSnapshotClusterIdentifier`: The identifier of the cluster the source snapshot was
...
# Required Parameters
- `ClusterIdentifier`: A unique identifier for the cluster. You use this identifier to
  ...
  myexamplecluster
- `MasterUserPassword`: The password associated with the master user account for the
...
- `MasterUsername`: The user name associated with the master user account for the cluster
  ...
  Developer Guide.
- `NodeType`: The node type to be provisioned for the cluster. For information about node
...
- `ClusterType`: The type of the cluster. When cluster type is specified as    single-node,
  the NumberOfNodes parameter is not required.    multi-node, the NumberOfNodes parameter is
  required.   Valid Values: multi-node | single-node  Default: multi-node
...
- `IamRoles`: A list of AWS Identity and Access Management (IAM) roles that can be used by
...
- `ManualSnapshotRetentionPeriod`: The default number of days to retain a manual snapshot.
  ...
  retention period of existing snapshots. The value must be either -1 or an integer between 1
  and 3,653.
...
- `ParameterGroupFamily`: The Amazon Redshift engine version to which the cluster parameter
  group applies. The cluster engine version determines the set of parameters. To get a list
  of valid parameter group family names, you can call DescribeClusterParameterGroups. By
  default, Amazon Redshift returns a list of all the parameter groups that are owned by your
  AWS account, including the default parameter groups for each Amazon Redshift engine
...
- `ParameterGroupName`: The name of the cluster parameter group. Constraints:   Must be 1
...
# Required Parameters
- `ClusterSecurityGroupName`: The name for the security group. Amazon Redshift stores the
...
- `SnapshotIdentifier`: A unique identifier for the snapshot that you are requesting. This
...
# Required Parameters
- `ClusterSubnetGroupName`: The name for the subnet group. Amazon Redshift stores the value
  as a lowercase string. Constraints:   Must contain no more than 255 alphanumeric characters
  or hyphens.   Must not be \"Default\".   Must be unique for all subnet groups that are
  created by your AWS account.   Example: examplesubnetgroup
- `Description`: A description for the subnet group.
- `SubnetIdentifier`: An array of VPC subnet IDs. A maximum of 20 subnets can be modified
  in a single request.
...
# Required Parameters
- `SnsTopicArn`: The Amazon Resource Name (ARN) of the Amazon SNS topic used to transmit
  the event notifications. The ARN is created by Amazon SNS when you create a topic and
  subscribe to it.
- `SubscriptionName`: The name of the event subscription to be created. Constraints:
  Cannot be null, empty, or blank.   Must contain from 1 to 255 alphanumeric characters or
  hyphens.   First character must be a letter.   Cannot end with a hyphen or contain two
  consecutive hyphens.
...
- `EventCategories`: Specifies the Amazon Redshift event categories to be published by the
...
- `SourceIds`: A list of one or more identifiers of Amazon Redshift source objects. All of
...
- `SourceType`: The type of source that will be generating the events. For example, if you
...
# Required Parameters
- `HsmClientCertificateIdentifier`: The identifier to be assigned to the new HSM client
...
- `HsmConfigurationIdentifier`: The identifier to be assigned to the new Amazon Redshift
  HSM configuration.
- `HsmIpAddress`: The IP address that the Amazon Redshift cluster must use to access the
  HSM.
- `HsmPartitionName`: The name of the partition in the HSM where the Amazon Redshift
  clusters will store their database encryption keys.
- `HsmPartitionPassword`: The password required to access the HSM partition.
- `HsmServerPublicCertificate`: The HSMs public certificate file. When using Cloud HSM, the
...
# Required Parameters
- `IamRole`: The IAM role to assume to run the target action. For more information about
  this parameter, see ScheduledAction.
- `Schedule`: The schedule in at( ) or cron( ) format. For more information about this
  parameter, see ScheduledAction.
- `ScheduledActionName`: The name of the scheduled action. The name must be unique within
  an account. For more information about this parameter, see ScheduledAction.
- `TargetAction`: A JSON format string of the Amazon Redshift API operation with input
...
# Required Parameters
- `SnapshotCopyGrantName`: The name of the snapshot copy grant. This name must be unique in
  the region for the AWS account. Constraints:   Must contain from 1 to 63 alphanumeric
...
- `DryRun`:
- `NextInvocations`:
- `ScheduleDefinitions`: The definition of the snapshot schedule. The definition is made up
...
# Required Parameters
- `ResourceName`: The Amazon Resource Name (ARN) to which you want to add the tag or tags.
...
- `LimitType`: The type of limit. Depending on the feature type, this can be based on a
...
# Required Parameters
- `ClusterIdentifier`: The identifier of the cluster to be deleted. Constraints:   Must
...
- `SkipFinalClusterSnapshot`: Determines whether a final snapshot of the cluster is created
  before Amazon Redshift deletes the cluster. If true, a final cluster snapshot is not
  created. If false, a final cluster snapshot is created before the cluster is deleted.   The
  FinalClusterSnapshotIdentifier parameter must be specified if SkipFinalClusterSnapshot is
  false.  Default: false
"""
...
# Required Parameters
- `ParameterGroupName`: The name of the parameter group to be deleted. Constraints:   Must
  be the name of an existing cluster parameter group.   Cannot delete a default cluster
  parameter group.
...
# Required Parameters
- `HsmClientCertificateIdentifier`: The identifier of the HSM client certificate to be
  deleted.
...
# Required Parameters
- `HsmConfigurationIdentifier`: The identifier of the Amazon Redshift HSM configuration to
  be deleted.
...
# Required Parameters
- `ResourceName`: The Amazon Resource Name (ARN) from which you want to remove the tag or
...
- `MaxRecords`: The maximum number of response records to return in each call. If the
  ...
  20, maximum 100.
- `ParameterGroupName`: The name of a specific parameter group for which to return details.
  By default, details about all parameter groups and the default parameter group are returned.
- `TagKeys`: A tag key or keys for which you want to return all matching cluster parameter
...
# Optional Parameters
- `ClusterExists`: A value that indicates whether to return snapshots only for an existing
...
- `SortingEntities`:
- `StartTime`: A value that requests only snapshots created at or after the specified time.
  The time value is specified in ISO 8601 format. For more information about ISO 8601, go to
  the ISO8601 Wikipedia page.  Example: 2012-07-16T18:00:00Z
- `TagKeys`: A tag key or keys for which you want to return all matching cluster snapshots
...
- `MaxRecords`: The maximum number of response records to return in each call. If the
  ...
  retrying the command with the returned marker value.  Default: 100  Constraints: minimum
  20, maximum 100.
- `TagKeys`: A tag key or keys for which you want to return all matching clusters that are
...
# Optional Parameters
- `SourceType`: The source type, such as cluster or parameter group, to which the described
  event categories apply. Valid values: cluster, cluster-snapshot, cluster-parameter-group,
...
- `SubscriptionName`: The name of the Amazon Redshift event notification subscription to be
  described.
- `TagKeys`: A tag key or keys for which you want to return all matching event notification
  subscriptions that are associated with the specified key or keys. For example, suppose that
  you have subscriptions that are tagged with keys called owner and environment. If you
  specify both of these tag keys in the request, Amazon Redshift returns a response with the
  subscriptions that have either or both of these tag keys associated with them.
...
- `SourceIdentifier`: The identifier of the event source for which events will be returned.
  ...
  Constraints: If SourceIdentifier is supplied, SourceType must also be provided.   Specify a
  cluster identifier when SourceType is cluster.   Specify a cluster security group name when
  SourceType is cluster-security-group.   Specify a cluster parameter group name when
  SourceType is cluster-parameter-group.   Specify a cluster snapshot identifier when
...
# Optional Parameters
- `HsmClientCertificateIdentifier`: The identifier of a specific HSM client certificate for
  which you want information. If no identifier is specified, information is returned for all
...
- `MaxRecords`: The maximum number of response records to return in each call. If the
  ...
  retrying the command with the returned marker value.  Default: 100  Constraints: minimum
  20, maximum 100.
- `TagKeys`: A tag key or keys for which you want to return all matching HSM configurations
  that are associated with the specified key or keys. For example, suppose that you have HSM
  configurations that are tagged with keys called owner and environment. If you specify both
  of these tag keys in the request, Amazon Redshift returns a response with the HSM
...
# Required Parameters
- `ActionType`: The action type to evaluate for possible node configurations. Specify
...
- `MaxRecords`: The maximum number of response records to return in each call. If the
  ...
  100, maximum 500.
- `OwnerAccount`: The AWS customer account used to create or copy the snapshot. Required if
...
- `MaxRecords`: The maximum number of response records to return in each call. If the
  ...
  retrying the command with the returned marker value.  Default: 100  Constraints: minimum
  20, maximum 100.
- `NodeType`: The node type filter value. Specify this parameter to show only the available
...
- `EndTime`: The end time in UTC of the scheduled action to retrieve. Only active scheduled
...
- `TagKeys`: A tag key or keys for which you want to return all matching resources that are
...
- `MaxRecords`: The maximum number or response records to return in each call. If the
  ...
  returned in a marker field of the response. You can retrieve the next set of records by
  retrying the command with the returned marker value.
- `ResourceName`: The Amazon Resource Name (ARN) for which you want to describe the tag or
  tags. For example, arn:aws:redshift:us-east-2:123456789:cluster:t1.
...
- `TagValues`: A tag value or values for which you want to return all matching usage limit
...
- `ClusterIdentifier`: The unique identifier of the source cluster that you want to disable
  copying of snapshots to a destination region. Constraints: Must be the valid name of an
...
# Optional Parameters
- `S3KeyPrefix`: The prefix applied to the log file names. Constraints:   Cannot exceed 512
  characters   Cannot contain spaces( ), double quotes (\"), single quotes ('), a backslash
  (), or control characters. The hexadecimal codes for invalid characters are:    x00 to x20
...
- `SnapshotCopyGrantName`: The name of the snapshot copy grant to use when snapshots of an
...
# Required Parameters
- `ClusterIdentifier`: The unique identifier of the cluster that contains the database for
  which your are requesting credentials. This parameter is case sensitive.
- `DbUser`: The name of a database user. If a user name matching DbUser exists in the
...
# Optional Parameters
- `AutoCreate`: Create a database user with the name specified for the user named in DbUser
  if one does not exist.
- `DbGroups`: A list of the names of existing database groups that the user named in DbUser
  will join for the current session, in addition to any group memberships for an existing
  user. If not specified, a new user is added only to PUBLIC. Database group name constraints
    Must be 1 to 64 alphanumeric characters or hyphens   Must contain only lowercase letters,
  numbers, underscore, plus sign, period (dot), at symbol (@), or hyphen.   First character
  must be a letter.   Must not contain a colon ( : ) or slash ( / ).    Cannot be a reserved
  word. A list of reserved words can be found in Reserved Words in the Amazon Redshift
  Database Developer Guide.
- `DbName`: The name of a database that DbUser is authorized to log on to. If DbName is not
  specified, DbUser can log on to any existing database. Constraints:   Must be 1 to 64
...
# Required Parameters
- `ReservedNodeId`: A string representing the node identifier for the DC1 Reserved Node to
  be exchanged.
...
# Required Parameters
- `ClusterIdentifier`: The unique identifier of the cluster to be modified. Example:
  examplecluster
...
- `ClusterVersion`: The new version number of the Amazon Redshift engine to upgrade to. For
  major version upgrades, if a non-default cluster parameter group is currently in use, a new
  cluster parameter group in the cluster parameter group family for the new version must be
  specified. The new cluster parameter group can be the default for that cluster parameter
...
# Required Parameters
- `ClusterIdentifier`: The unique identifier of a cluster whose database revision you want
  to modify.  Example: examplecluster
- `RevisionTarget`: The identifier of the database revision. You can retrieve this value
...
# Optional Parameters
- `AddIamRoles`: Zero or more IAM roles to associate with the cluster. The roles must be in
  their Amazon Resource Name (ARN) format. You can associate up to 10 IAM roles with a single
  cluster in a single request.
- `RemoveIamRoles`: Zero or more IAM roles in ARN format to disassociate from the cluster.
  You can disassociate up to 10 IAM roles from a single cluster in a single request.
"""
...
# Required Parameters
- `ClusterIdentifier`: A unique identifier for the cluster whose snapshot schedule you want
  to modify.

# Optional Parameters
- `DisassociateSchedule`: A boolean to indicate whether to remove the assoiciation between
  the cluster and the schedule.
- `ScheduleIdentifier`: A unique alphanumeric identifier for the schedule that you want to
...
# Required Parameters
- `SubscriptionName`: The name of the modified Amazon Redshift event notification
  subscription.
...
# Required Parameters
- `ClusterIdentifier`: The unique identifier of the cluster for which you want to change
...
# Required Parameters
- `ScheduleDefinition`: An updated list of schedule definitions. A schedule definition is
...
# Optional Parameters
- `ScheduleDefinitions`: An updated list of schedule definitions. A schedule definition is
  made up of schedule expressions, for example, \"cron(30 12 *)\" or \"rate(12 hours)\".
"""
...
- `NumberOfNodes`: The new number of nodes for the cluster. If not specified, the cluster's
...
# Required Parameters
- `ClusterIdentifier`: The identifier of the cluster that will be created from restoring
  ...
  an AWS account.
- `SnapshotIdentifier`: The name of the snapshot from which to create the new cluster. This
...
- `MaintenanceTrackName`: The name of the maintenance track for the restored cluster. When
  you take a snapshot, the snapshot inherits the MaintenanceTrack value from the cluster. The
  snapshot might be on a different track than the cluster that was the source for the
  snapshot. For example, suppose that you take a snapshot of a cluster that is on the current
...
# Required Parameters
- `ClusterIdentifier`: The identifier of the Amazon Redshift cluster to restore the table
  to.
...
# Required Parameters
- `ClusterSecurityGroupName`: The name of the security Group from which to revoke the
  ingress rule.
...
- `EC2SecurityGroupName`: The name of the EC2 Security Group whose access is to be revoked.
  If EC2SecurityGroupName is specified, EC2SecurityGroupOwnerId must also be provided and
...
# Required Parameters
- `AccountWithRestoreAccess`: The identifier of the AWS customer account that can no longer
