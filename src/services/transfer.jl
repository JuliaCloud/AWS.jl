...
- `EndpointDetails`: The virtual private cloud (VPC) endpoint settings that are configured
  for your server. When you host your endpoint within your VPC, you can make it accessible
  only to resources within your VPC, or you can attach Elastic IPs and make it accessible to
  clients over the internet. Your VPC's default security groups are automatically assigned to
  your endpoint.
- `EndpointType`: The type of VPC endpoint that you want your server to connect to. You can
...
- `LoggingRole`: Allows the service to write your users' activity to your Amazon CloudWatch
...
- `ServerId`: A system-assigned unique identifier for a server instance. This is the
  specific server that you added your user to.
- `UserName`: A unique string that identifies a user and is associated with a as specified
...
# Optional Parameters
- `HomeDirectory`: The landing directory (folder) for a user when they log in to the server
  using the client. An example is  your-Amazon-S3-bucket-name&gt;/home/username .
- `HomeDirectoryMappings`: Logical directory mappings that specify what Amazon S3 paths and
  keys should be visible to your user and how you want to make them visible. You will need to
  specify the \"Entry\" and \"Target\" pair, where Entry shows how the path is made visible
  and Target is the actual Amazon S3 path. If you only specify a target, it will be displayed
  as is. You will need to also make sure that your IAM role provides access to paths in
  Target. The following is an example.  '[ \"/bucket2/documentation\", { \"Entry\":
...
- `PosixProfile`:
- `SshPublicKeyBody`: The public portion of the Secure Shell (SSH) key used to authenticate
...
# Required Parameters
- `ServerId`: A system-assigned unique identifier for a file transfer protocol-enabled
...
# Required Parameters
- `ServerId`: A system-assigned unique identifier for a server instance that has the user
  assigned to it.
...
- `UserName`: The name of the user assigned to one or more servers. User names are part of
...
- `NextToken`: When additional results are obtained from the ListSecurityPolicies command,
  a NextToken parameter is returned in the output. You can then pass the NextToken parameter
...
# Optional Parameters
- `MaxResults`: Specifies the number of servers to return as a response to the ListServers
  query.
...
- `NextToken`: When you request additional results from the ListTagsForResource operation,
  a NextToken parameter is returned in the input. You can then pass in a subsequent command
...
# Required Parameters
- `ServerId`: A system-assigned unique identifier for a server that has users assigned to
  it.
...
# Required Parameters
- `ServerId`: A system-assigned identifier for a specific server. That server's user
...
- `TagKeys`: TagKeys are key-value pairs assigned to ARNs that can be used to group and
...
# Required Parameters
- `ServerId`: A system-assigned unique identifier for a server instance that the user
...
- `Certificate`: The Amazon Resource Name (ARN) of the AWS Certificate Manager (ACM)
  ...
  with FQDN or IP address specified and information about the issuer.
- `EndpointDetails`: The virtual private cloud (VPC) endpoint settings that are configured
  for your server. With a VPC endpoint, you can restrict access to your server to resources
  only within your VPC. To control incoming internet traffic, you will need to associate one
  or more Elastic IP addresses with your server's endpoint.
...
- `ServerId`: A system-assigned unique identifier for a server instance that the user
  account is assigned to.
- `UserName`: A unique string that identifies a user and is associated with a server as
...
# Optional Parameters
- `HomeDirectory`: Specifies the landing directory (folder) for a user when they log in to
  the server using their file transfer protocol client. An example is
...
- `HomeDirectoryType`: The type of landing directory (folder) you want your users' home
  ...
  it LOGICAL, you will need to provide mappings in the HomeDirectoryMappings for how you want
  to make Amazon S3 paths visible to your users.
- `Policy`: Allows you to supply a scope-down policy for your user so you can use the same
  IAM role across multiple users. The policy scopes down user access to portions of your
