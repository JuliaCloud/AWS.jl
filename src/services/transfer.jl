...
- `external_id`: A unique identifier that is required to identify specific groups within
  your directory. The users of the group that you associate have access to your Amazon S3
  or Amazon EFS resources over the enabled protocols using Transfer Family. If you know the
...
In most cases, you can use this value instead of the session policy to lock down your user to the designated home directory ("`chroot`").
  To do this, you can set `Entry` to `/` and set `Target` to the `HomeDirectory` parameter
  value.
...
- `"Policy"`: A session policy for your user so that you can use the same Identity and
  ...
  For an example of a session policy, see [Example session policy](https://docs.aws.amazon.com/transfer/latest/userguide/session-policy.html).
   ...
   <p>For more information, see [AssumeRole](https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html)
...
- `as2_id`: The `As2Id` is the *AS2-name*, as defined in the [RFC 4130](https://datatracker.ietf.org/doc/html/rfc4130).
  ...
  to the partner using the `StartFileTransfer` API operation. This ID cannot include spaces.
...
- `"EndpointDetails"`: The virtual private cloud (VPC) endpoint settings that are
  configured for your server. When you host your endpoint within your VPC, you can make
  your endpoint accessible only to resources within your VPC, or you can attach Elastic IP
...
- `"EndpointType"`: The type of endpoint that you want your server to use. You can choose
  ...
  resources only within your VPC or choose to make it internet facing by attaching Elastic
  IP addresses directly to it.</p>
  ...
  For more information, see https://docs.aws.amazon.com/transfer/latest/userguide/create-
  server-in-vpc.html#deprecate-vpc-endpoint.

   <p>It is recommended that you use `VPC` as the `EndpointType`. With this endpoint type,
...
- `"HostKey"`: The RSA, ECDSA, or ED25519 private key to use for your SFTP-enabled server.
  ...
  to a new server, don't update the host key. Accidentally changing a server's host key can
  be disruptive.For more information, see [Manage host keys for your SFTP-enabled server](https://docs.aws.amazon.com/transfer/latest/userguide/edit-server-config.html#configuring-servers-change-host-key)
...
- `"PreAuthenticationLoginBanner"`: Specifies a string to display when users connect to a
  server. This string is displayed before the user authenticates. For example, the
...
- `"ProtocolDetails"`: The protocol settings that are configured for your server. - To
  indicate passive mode (for FTP and FTPS protocols), use the `PassiveIp` parameter. Enter
...
- `"Protocols"`: Specifies the file transfer protocol or protocols over which your file
  ...
  are: - `SFTP` (Secure Shell (SSH) File Transfer Protocol): File transfer over SSH
   - `FTPS` (File Transfer Protocol Secure): File transfer with TLS encryption
   - `FTP` (File Transfer Protocol): Unencrypted file transfer
   - `AS2` (Applicability Statement 2): used for transporting structured business-to-
  business data
  ...
  and the `IdentityProviderType` must be either `AWS_DIRECTORY_SERVICE`, `AWS_LAMBDA`, or
  `API_GATEWAY`.
...
In most cases, you can use this value instead of the session policy to lock your user down to the designated home directory ("`chroot`").
  To do this, you can set `Entry` to `/` and set `Target` to the value the user should see
...
- `"SshPublicKeyBody"`: The public portion of the Secure Shell (SSH) key used to
  ...
  Transfer Family accepts RSA, ECDSA, and ED25519 keys. - For RSA keys, the key type is
  `ssh-rsa`.
...
- `steps`: Specifies the details for the steps that are in the specified workflow.
  ...
  specify either the Amazon S3 bucket and key, or the Amazon EFS file system ID and path.
...
!!! note
 ...
 <p> If you provide an ID for an execution that is not in progress, or if the execution
...
- `"PrivateKey"`: - For the CLI, provide a file path for a private key in URI format.For
  example, `--private-key file://encryption-key.pem`. Alternatively, you can provide the
  raw content of the private key file.
   - For the SDK, specify the raw content of a private key file. For example, `--private-
...
Returns a list of the agreements for the server that's identified by the `ServerId` that
you supply. If you want to limit the results to a certain number, supply a value for the
...
- `"NextToken"`:  `ListExecutions` returns the `NextToken` parameter in the output. You can
  ...
  you might only want to list first 10. If so, call the API by specifying the `max-
  results`:
...
execution of a custom step of a workflow. You must include those with their callback as
...
output file contains the following information: - `filePath`: the complete path of a remote
file, relative to the directory of the listing request for your SFTP connector on the
remote server.
...
increase the value provided in the optional `max-items` input attribute to be able to list
more items (up to the maximum allowed list size of 10,000 items).
...
server. - For an AS2 connector, you specify the `ConnectorId` and one or more
...
specify the following items: <ul> <li>If you are transferring file from a partner's SFTP
server to Amazon Web Services storage, you specify one or more `RetrieveFilePaths` to
identify the files you want to transfer, and a `LocalDirectoryPath` to specify the
destination folder.
...
- `"RemoteDirectoryPath"`: For an outbound transfer, the `RemoteDirectoryPath` specifies
  the destination for one or more files that are transferred to the partner's SFTP server.
  If you don't specify a `RemoteDirectoryPath`, the destination for transferred files is
...
- `"SendFilePaths"`: One or more source paths for the Amazon S3 storage. Each string
  represents a source file path for one outbound file transfer. For example, ` *DOC-EXAMPLE-
  BUCKET*/*myfile.txt* `.
...
server, such as server and user properties, are not affected by stopping your server.</p>

!!! note
    Stopping the server does not reduce or impact your file transfer protocol endpoint
billing; you must delete the server to stop being billed.The state of `STOPPING` indicates
that the server is in an intermediate state, either not fully able to respond, or not fully
offline. The values of `STOP_FAILED` can indicate an error condition.

 <p>No response is returned from this call.
...
Note the following:</p> - You cannot use `TestIdentityProvider` if the
`IdentityProviderType` of your server is `SERVICE_MANAGED`.
 ...
 - `TestIdentityProvider` can test the password operation for a custom Identity Provider
that handles keys and passwords.
 - If you provide any incorrect values for any parameters, the `Response` field is empty.
 - If you provide a server ID for a server that uses service-managed users, you get an
error:
...
operation: s-*server-ID* not configured for external auth `
 - If you enter a Server ID for the `--server-id` parameter that does not identify an
actual Transfer server, you receive the following error:

 `An error occurred (ResourceNotFoundException) when calling the TestIdentityProvider
operation: Unknown server`.

 <p>It is possible your sever is in a different region. You can specify a region by adding
the following: `--region region-code`, such as `--region us-east-2` to specify a server in
**US East (Ohio)**.
...
- `"ServerProtocol"`: The type of file transfer protocol to be tested.
  ...
  The available protocols are: - Secure Shell (SSH) File Transfer Protocol (SFTP)
...
- `"BaseDirectory"`: To change the landing directory (folder) for files that are
  transferred, provide the bucket folder that you want to use; for example, `/*DOC-EXAMPLE-
  BUCKET*/*home*/*mydirectory* `.
...
that the user can't access anything outside of their home directory. The programmatic way
to configure this behavior is to update the user. Set their `HomeDirectoryType` to
`LOGICAL`, and specify `HomeDirectoryMappings` with `Entry` as root (`/`) and `Target` as
...
selected.
 ...
 ` aws transfer update-user --server-id &lt;server-id&gt; --user-name admin-user --home-
directory-type LOGICAL --home-directory-mappings "[{\"Entry\":\"/\", \"Target\":\"/test/admin-user\"}]"`
...
- `user_name`: A unique string that identifies a user and is associated with a server as
  ...
  hyphen '-', period '.', and at sign '@'. The user name can't start with a hyphen, period,
  or at sign.
