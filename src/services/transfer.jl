...
- `external_id`: A unique identifier that is required to identify specific groups within
  ...
  or Amazon EFS resources over the enabled protocols using Transfer Family. If you know
  the group name, you can view the SID values by running the following command using
  Windows PowerShell.
...
- `role`: The Amazon Resource Name (ARN) of the Identity and Access Management (IAM) role
  ...
  The policies attached to this role determine the level of access that you want to
  provide your users when transferring files into and out of your Amazon S3 bucket or
  Amazon EFS file system. The IAM role should also contain a trust relationship that
...
- `"HomeDirectoryMappings"`: Logical directory mappings that specify what Amazon S3 or
  ...
  visible. You must specify the `Entry` and `Target` pair, where `Entry` shows how the
  path is made visible and `Target` is the actual Amazon S3 or Amazon EFS path. If you
  only specify a target, it is displayed as is. You also must ensure that your Identity
  and Access Management (IAM) role provides access to paths in `Target`. This value can
  be set only when `HomeDirectoryType` is set to *LOGICAL*.
...
- `"HomeDirectoryType"`: The type of landing directory (folder) that you want your users'
  ...
  `HomeDirectoryMappings` for how you want to make Amazon S3 or Amazon EFS paths visible
  to your users.
...
- `"Policy"`: A session policy for your user so that you can use the same Identity and
  ...
  !!! note
      ...
      the Amazon Resource Name (ARN) of the policy. You save the policy as a JSON blob
...
- `access_role`: Connectors are used to send files using either the AS2 or SFTP protocol.
  ...
  **For AS2 connectors**
  ...
  paths in the request parameter, `SendFilePaths`. We use the file’s parent directory
  (for example, for `--send-file-paths /bucket/dir/file.txt`, parent directory is
  `/bucket/dir/`) to temporarily store a processed AS2 message file, store the MDN when
  we receive them from the partner, and write a final JSON file containing relevant
  metadata of the transmission. So, the `AccessRole` needs to provide read and write
  access to the parent directory of the file location used in the `StartFileTransfer`
  ...
  in Secrets Manager, then the role also needs the `kms:Decrypt` permission for that key.

  **For SFTP connectors**

  Make sure that the access role provides read and write access to the parent directory
  of the file location that's used in the `StartFileTransfer` request. Additionally, make
  sure that the role provides `secretsmanager:GetSecretValue` permission to Secrets
  Manager.
...
- `"Domain"`: The domain of the storage system that is used for file transfers. There are
  two domains available: Amazon Simple Storage Service (Amazon S3) and Amazon Elastic
...
- `"EndpointDetails"`: The virtual private cloud (VPC) endpoint settings that are
  ...
  your endpoint accessible only to resources within your VPC, or you can attach Elastic
  IP addresses and make your endpoint accessible to clients over the internet. Your VPC's
...
- `"EndpointType"`: The type of endpoint that you want your server to use. You can choose
  ...
  resources only within your VPC or choose to make it internet facing by attaching
...
- `"HostKey"`: The RSA, ECDSA, or ED25519 private key to use for your SFTP-enabled server.
  You can add multiple host keys, in case you want to rotate keys, or have a set of
...
- `"IdentityProviderDetails"`: Required when `IdentityProviderType` is set to
  `AWS_DIRECTORY_SERVICE`, `Amazon Web Services_LAMBDA` or `API_GATEWAY`. Accepts an
...
- `"IdentityProviderType"`: The mode of authentication for a server. The default value is
  ...
  Service for Microsoft Active Directory or Microsoft Active Directory in your on-
  premises environment or in Amazon Web Services using AD Connector. This option also
  requires you to provide a Directory ID by using the `IdentityProviderDetails`
  parameter.
...
- `"ProtocolDetails"`: The protocol settings that are configured for your server.
  ...
  - To indicate passive mode (for FTP and FTPS protocols), use the `PassiveIp` parameter.
    Enter a single dotted-quad IPv4 address, such as the external IP address of a
...
- `"Protocols"`: Specifies the file transfer protocol or protocols over which your file
  ...
  !!! note
      ...
      - If `Protocol` is set only to `SFTP`, the `EndpointType` can be set to `PUBLIC`
        and the `IdentityProviderType` can be set any of the supported identity types:
...
- `"StructuredLogDestinations"`: Specifies the log groups to which your server logs are
  ...
  effect turn off structured logging, by providing an empty value for this parameter in
  an `update-server` call. For example:
...
- `user_name`: A unique string that identifies a user and is associated with a `ServerId`.
  ...
  following are valid characters: a-z, A-Z, 0-9, underscore '_', hyphen '-', period '.',
...
- `"PosixProfile"`: Specifies the full POSIX identity, including user ID (`Uid`), group ID
  ...
  access to your Amazon EFS file systems. The POSIX permissions that are set on files and
...
- `steps`: Specifies the details for the steps that are in the specified workflow.
  ...
  For file location, you specify either the Amazon S3 bucket and key, or the Amazon EFS
...
- `"OnExceptionSteps"`: Specifies the steps (actions) to take if errors are encountered
  ...
  !!! note
      For custom steps, the Lambda function needs to send `FAILURE` to the call back API
      to kick off the exception steps. Additionally, if the Lambda does not send
...
- `truncated`: a flag indicating whether the list output contains all of the items
  contained in the remote directory or not. If your `Truncated` output value is true, you
  can increase the value provided in the optional `max-items` input attribute to be able to
  list more items (up to the maximum allowed list size of 10,000 items).
...
- For an SFTP connector, the file transfer can be either outbound or inbound. In both
  cases, you specify the `ConnectorId`. Depending on the direction of the transfer, you
  also specify the following items:   - If you are transferring file from a partner's SFTP
...
- `"RemoteDirectoryPath"`: For an outbound transfer, the `RemoteDirectoryPath` specifies
  the destination for one or more files that are transferred to the partner's SFTP
  server. If you don't specify a `RemoteDirectoryPath`, the destination for transferred
...
- `"PosixProfile"`: Specifies the full POSIX identity, including user ID (`Uid`), group ID
  (`Gid`), and any secondary groups IDs (`SecondaryGids`), that controls your users'
  access to your Amazon Elastic File Systems (Amazon EFS). The POSIX permissions that are
  set on files and directories in your file system determines the level of access your
