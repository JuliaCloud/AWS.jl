...
- `tracking_id`: The ID of your action interaction event tracker. When you create an Action
  interactions dataset, Amazon Personalize creates an action interaction event tracker
  for you. For more information, see [Action interaction event tracker ID](https://docs.aws.amazon.com/personalize/latest/dg/action-interaction-tracker-id.html).
"""
...
- `session_id`: The session ID associated with the user's visit. Your application generates
  ...
  Personalize uses the sessionId to associate events with the user before they log in.
