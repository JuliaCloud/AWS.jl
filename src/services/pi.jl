...
- `service_type`: The Amazon Web Services service for which Performance Insights will return
...
- `end_time`: The date and time specifying the end of the requested time series query range.
  The value specified is *exclusive*. Thus, the command returns data points less than (but
...
- `start_time`: The date and time specifying the beginning of the requested time series
  ...
  The value specified is *inclusive*. Thus, the command returns data points equal to or
...
- `"MaxResults"`: The maximum number of items to return in the response. If more items exist
  than the specified `MaxResults` value, a pagination token is included in the response so
  that the remaining results can be retrieved.
- `"NextToken"`: An optional pagination token provided by a previous request. If this
  parameter is specified, the response includes only records beyond the token, up to the
