...
# Required Parameters
- `EndTime`: The date and time specifying the end of the requested time series data. The
...
- `GroupBy`: A specification for how to aggregate the data points from a query result. You
...
- `Metric`: The name of a Performance Insights metric to be measured. Valid values for
  ...
  db.load.avg only.
- `ServiceType`: The AWS service for which Performance Insights will return metrics. The
  only valid value for ServiceType is RDS.
- `StartTime`: The date and time specifying the beginning of the requested time series
...
- `PartitionBy`: For each dimension specified in GroupBy, specify a secondary dimension to
...
- `MetricQueries`: An array of one or more queries to perform. Each query must specify a
  Performance Insights metric, and can optionally specify aggregation and filtering criteria.
- `ServiceType`: The AWS service for which Performance Insights returns metrics. The only
