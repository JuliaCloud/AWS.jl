...
- `"page_size"`: Used to specify the number of items that should be returned in the
  response.
"""
...
- `"caller_id"`: The phone number that appears on recipients' devices when they receive the
  message.
- `"configuration_set_name"`: The name of the configuration set that you want to use to
  send the message.
- `"content"`:
- `"destination_phone_number"`: The phone number that you want to send the voice message to.
- `"origination_phone_number"`: The phone number that Amazon Pinpoint should use to send
  the voice message. This isn't necessarily the phone number that appears on recipients'
  devices when they receive the message, because you can specify a CallerId parameter in the
  request.
"""
