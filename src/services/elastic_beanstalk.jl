...
- `"GroupName"`: The name of the group to which the target environments belong. Specify a
  ...
  with a + (plus) character. See [Environment Manifest (env.yaml)](https://docs.aws.amazon.com/elasticbeanstalk/latest/dg/environment-cfg-manifest.html)
  for details.
...
- `"EnvironmentName"`: A unique name for the environment.
  ...
  numbers, and hyphens. It can't start or end with a hyphen. This name must be unique within
  a region in your account. If the specified name already exists in the region, Elastic
...
- `"SolutionStackName"`: The name of an Elastic Beanstalk solution stack (platform version)
  ...
  solution stacks, see [Elastic Beanstalk Supported Platforms](https://docs.aws.amazon.com/elasticbeanstalk/latest/platforms/platforms-supported.html)
...
- `"Tier"`: Specifies the tier to use in creating this environment. The environment tier
  ...
  application that handles HTTP(S) requests or a web application that handles background-
  processing tasks.
...
while the update is in progress or if the update fails. The `DeploymentStatus` for the draft
...
- `"TemplateName"`: The name of the configuration template to describe.
  ...
  If you specify both, AWS Elastic Beanstalk returns an `InvalidParameterCombination` error.
...
- `"TerminateResources"`: Indicates whether the associated AWS resources should shut down
  ...
  - `false`: AWS Elastic Beanstalk resource management is removed from the environment, but
...
!!! note
    If a property (for example, `description`) is not provided, the value remains unchanged.
    To clear properties, specify an empty string.
...
- `application_name`: The name of the application associated with the configuration template
  to update.
...
Controls permission to call `UpdateTagsForResource` and pass a list of tag keys to remove in
