...
- `"filterValues"`: The values to use when filtering recommendations. For each placeholder
  ...
  expressions that use an `EXCLUDE` element to exclude actions, you can omit the `filter-
  values`. In this case, Amazon Personalize doesn't use that portion of the expression to
...
- `"filterValues"`: The values to use when filtering recommendations. For each placeholder
  ...
  values for all parameters that are defined in the expression. For filters with
  expressions that use an `EXCLUDE` element to exclude items, you can omit the `filter-
  values`.In this case, Amazon Personalize doesn't use that portion of the expression to
  filter recommendations.
...
required, `itemId` not used
 - RELATED_ITEMS - `itemId` required, `userId` not used


!!! note
    Campaigns that are backed by a solution created using a recipe of type
PERSONALIZED_RANKING use the API. For recommenders, the recommender's ARN is required and
the required item and user input depends on the use case (domain-based recipe) backing the
recommender. For information on use case requirements see [Choosing recommender use cases](https://docs.aws.amazon.com/personalize/latest/dg/domain-use-cases.html).
...
- `"metadataColumns"`: If you enabled metadata in recommendations when you created or
  ...
  column names from your Items dataset. The maximum number of columns you can provide is
  10.
