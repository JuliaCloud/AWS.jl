...
see [Monitoring Kinesis Video Streams](http://docs.aws.amazon.com/kinesisvideostreams/latest/dg/monitoring.html).
...
3. Provide the URL (containing the encrypted session token) for the MPEG-DASH manifest to a
...
- **GetMP4MediaFragment:** Retrieves MP4 media fragments. These fragments contain the
  ...
  !!! note
      ...
      not change between fragments in a session. This also means that the session fails if
      the fragments in a stream change from having only video to having both audio and
      video.
...
For pricing information, see [Amazon Kinesis Video Streams Pricing](https://aws.amazon.com/kinesis/video-streams/pricing/)
and [Amazon Web Services Pricing](https://aws.amazon.com/pricing/). Charges for both HLS
...
- `"MaxManifestFragmentResults"`: The maximum number of fragments that are returned in the
  ...
  streams with 1-second fragments, and more than 2 1/2 hours of video on streams with 10-
  second fragments.
...
- `"PlaybackMode"`: Whether to retrieve live, live replay, or archived, on-demand data.
  ...
  - **`LIVE`**: For sessions of this type, the MPEG-DASH manifest is continually updated
    ...
    retrieve a new manifest on a one-second interval. When this type of session is played in
    a media player, the user interface typically displays a "live" notification, with no
  ...
  - **`LIVE_REPLAY`**: For sessions of this type, the MPEG-DASH manifest is updated
    ...
    ingested, fragments are added as the duration of the next fragment elapses. For example,
    if the fragments in the session are two seconds long, then a new fragment is added to
    the manifest every two seconds. This mode is useful to be able to start playback from
    when an event is detected and continue live streaming media that has not yet been
  ...
  multiple fragments with the same start timestamp, the fragment that has the larger
...
function get_dashstreaming_session_url(; aws_config::AbstractAWSConfig=current_aws_config())
    return kinesis_video_archived_media(
        "POST", "/getDASHStreamingSessionURL"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
2. Retrieve the HLS URL using `GetHLSStreamingSessionURL`. Kinesis Video Streams creates an
   ...
   encrypted session token) for the session's HLS *master playlist* (the root resource
   ...
   stream, time range, and format. No other media data (such as frames outside the requested
   window or alternate bitrates) is made available.
3. Provide the URL (containing the encrypted session token) for the HLS master playlist to a
...
- **GetMP4MediaFragment:** Retrieves MP4 media fragments. These fragments contain the
  ...
  !!! note
      ...
      supported. After the first media fragment is made available in a streaming session,
      fragments can contain CPD changes for each track. Therefore, the fragments in a
      session can have a different resolution, bit rate, or other information in the CPD
      without interrupting playback. However, any change made in the track number or track
      codec format can return an error when those different media fragments are loaded. For
      example, streaming will fail if the fragments in the stream change from having only
      video to having both audio and video, or if an AAC audio track is changed to an ALAW
...
- `"ContainerFormat"`: Specifies which format should be used for packaging the media.
  Specifying the `FRAGMENTED_MP4` container format packages the media into MP4 fragments
  (fMP4 or CMAF). This is the recommended packaging because there is minimal packaging
  overhead. The other container format option is `MPEG_TS`. HLS has supported MPEG TS chunks
  since it was released and is sometimes the only supported packaging on older HLS players.
  MPEG TS typically has a 5-25 percent packaging overhead. This means MPEG TS typically
...
- `"DiscontinuityMode"`: Specifies when flags marking discontinuities between fragments are
  ...
  - `ALWAYS`: a discontinuity marker is placed between every fragment in the HLS media
    ...
    not accurate.
  - `NEVER`: no discontinuity markers are placed anywhere. It is recommended to use a value
    of `NEVER` to ensure the media player timeline most accurately maps to the producer
    timestamps.
...
- `"MaxMediaPlaylistFragmentResults"`: The maximum number of fragments that are returned in
  ...
  streams with 1-second fragments, and more than 13 hours of video on streams with 10-second
  fragments.
...
- `"PlaybackMode"`: Whether to retrieve live, live replay, or archived, on-demand data.
  ...
  - **`LIVE`**: For sessions of this type, the HLS media playlist is continually updated
    with the latest fragments as they become available. We recommend that the media player
    retrieve a new playlist on a one-second interval. When this type of session is played in
    a media player, the user interface typically displays a "live" notification, with no
    scrubber control for choosing the position in the playback window to display.
  ...
  - **`LIVE_REPLAY`**: For sessions of this type, the HLS media playlist is updated
    ...
    ingested, fragments are added as the duration of the next fragment elapses. For example,
    if the fragments in the session are two seconds long, then a new fragment is added to
    the media playlist every two seconds. This mode is useful to be able to start playback
  ...
  - **`ON_DEMAND`**: For sessions of this type, the HLS media playlist contains all the
    ...
    typically displays a scrubber control for choosing the position in the playback window
    to display.
...
function get_hlsstreaming_session_url(; aws_config::AbstractAWSConfig=current_aws_config())
    return kinesis_video_archived_media(
        "POST", "/getHLSStreamingSessionURL"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `start_timestamp`: The starting point from which the images should be generated. This
  `StartTimestamp` must be within an inclusive range of timestamps for an image to be
  returned.
...
- `"HeightPixels"`: The height of the output image that is used in conjunction with the
  ...
  the `WidthPixels` ratio. If neither parameter is provided, the original image size will be
  returned.

- `"MaxResults"`: The maximum number of images to be returned by the API.

  !!! note
      The default limit is 25 images per API response. Providing a `MaxResults` greater than
      this value will result in a page size of 25. Any additional results will be paginated.
...
- `"StreamName"`: The name of the stream from which to retrieve the images. You must specify
  either the `StreamName` or the `StreamARN`.

- `"WidthPixels"`: The width of the output image that is used in conjunction with the
  `HeightPixels` parameter. When both `WidthPixels` and `HeightPixels` parameters are
  provided, the image will be stretched to fit the specified aspect ratio. If only the
...
from a request to `ListFragments`. However, results are typically available in less than one
second.
...
function list_fragments(; aws_config::AbstractAWSConfig=current_aws_config())
    return kinesis_video_archived_media(
        "POST", "/listFragments"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_fragments(
    ...
    return kinesis_video_archived_media(
        "POST", "/listFragments", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
