...
"""
    batch_get_command_executions(command_execution_ids, sandbox_id)
    batch_get_command_executions(command_execution_ids, sandbox_id, params::Dict{String,<:Any})

Gets information about the command executions.

# Arguments

- `command_execution_ids`: A comma separated list of `commandExecutionIds`.
- `sandbox_id`: A `sandboxId` or `sandboxArn`.
"""
function batch_get_command_executions end

function batch_get_command_executions(
    commandExecutionIds, sandboxId; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild(
        "BatchGetCommandExecutions",
        Dict{String,Any}(
            "commandExecutionIds" => commandExecutionIds, "sandboxId" => sandboxId
        );
        aws_config,
        feature_set=SERVICE_FEATURE_SET,
    )
...
"""
    batch_get_sandboxes(ids)
    batch_get_sandboxes(ids, params::Dict{String,<:Any})
...
"""
function batch_get_sandboxes end

function batch_get_sandboxes(ids; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild(
        "BatchGetSandboxes",
        Dict{String,Any}("ids" => ids);
        aws_config,
...
- `compute_type`: Information about the compute resources the compute fleet uses. Available
  ...
  - `ATTRIBUTE_BASED_COMPUTE`: Specify the amount of vCPUs, memory, disk space, and the type
    ...
    !!! note
        ...
        `computeConfiguration`. CodeBuild will select the cheapest instance that satisfies
        your specified attributes. For more information, see [Reserved capacity environment types](https://docs.aws.amazon.com/codebuild/latest/userguide/build-env-ref-compute-types.html#environment-reserved-capacity.types)
        in the *CodeBuild User Guide*.

  - `CUSTOM_INSTANCE_TYPE`: Specify the instance type for your compute fleet. For a list of
    supported instance types, see [Supported instance families](https://docs.aws.amazon.com/codebuild/latest/userguide/build-env-ref-compute-types.html#environment-reserved-capacity.instance-types)
    in the *CodeBuild User Guide*.
  ...
  - For environment type `ARM_CONTAINER`, you can use up to 4 GiB memory and 2 vCPUs on ARM-
...
- `environment_type`: The environment type of the compute fleet.
  ...
  - The environment type `ARM_CONTAINER` is available only in regions US East (N. Virginia),
    US East (Ohio), US West (Oregon), EU (Ireland), Asia Pacific (Mumbai), Asia Pacific
    (Tokyo), Asia Pacific (Singapore), Asia Pacific (Sydney), EU (Frankfurt), and South
    America (São Paulo).
  - The environment type `ARM_EC2` is available only in regions US East (N. Virginia), US
  ...
  - The environment type `WINDOWS_EC2` is available only in regions US East (N. Virginia),
...
- `"computeConfiguration"`: The compute configuration of the compute fleet. This is only
  required if `computeType` is set to `ATTRIBUTE_BASED_COMPUTE` or `CUSTOM_INSTANCE_TYPE`.
...
- `"autoRetryLimit"`: The maximum number of additional automatic retries after a failed
  build. For example, if the auto-retry limit is set to 2, CodeBuild will call the
...
- `"fileSystemLocations"`: An array of `ProjectFileSystemLocation` objects for a CodeBuild
  ...
  `location`, `mountOptions`, `mountPoint`, and `type` of a file system created using Amazon
  Elastic File System.
...
!!! important
    If you enable webhooks for an CodeBuild project, and the project is used as a build step
    in CodePipeline, then two identical builds are created for each commit. One build is
    triggered through webhooks, and one through CodePipeline. Because billing is on a per-
    build basis, you are billed for both builds. Therefore, if you are using CodePipeline,
    we recommend that you disable webhooks in CodeBuild. In the CodeBuild console, clear the
...
- `"buildType"`: Specifies the type of build this webhook will trigger.
  ...
  !!! note
      ...
      Buildkite runner builds, see [Tutorial: Configure a CodeBuild-hosted Buildkite runner](https://docs.aws.amazon.com/codebuild/latest/userguide/sample-runner-buildkite.html)
      in the *CodeBuild user guide*.
...
- `"pullRequestBuildPolicy"`: A PullRequestBuildPolicy object that defines comment-based
  approval requirements for triggering builds on pull requests. This policy helps control
...
- `"maxResults"`: The maximum number of paginated test cases returned per response. Use
  `nextToken` to iterate pages in the list of returned `TestCase` objects. The default value
  is 100.

- `"nextToken"`: During a previous call, the maximum number of items that can be returned is
  the value specified in `maxResults`. If there more items in the list, then a unique string
...
function list_build_batches(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("ListBuildBatches"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_build_batches(
    ...
    return codebuild(
        "ListBuildBatches", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_build_batches_for_project(;
    ...
    return codebuild(
        "ListBuildBatchesForProject"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_build_batches_for_project(
    ...
    return codebuild(
        "ListBuildBatchesForProject", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_builds(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("ListBuilds"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_builds(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild("ListBuilds", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
"""
    list_command_executions_for_sandbox(sandbox_id)
    list_command_executions_for_sandbox(sandbox_id, params::Dict{String,<:Any})
...
- `"nextToken"`: The next token, if any, to get paginated results. You will get this value
  from previous execution of list sandboxes.
- `"sortOrder"`: The order in which sandbox records should be retrieved.
"""
function list_command_executions_for_sandbox end

function list_command_executions_for_sandbox(
    sandboxId; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild(
        "ListCommandExecutionsForSandbox",
        Dict{String,Any}("sandboxId" => sandboxId);
        aws_config,
        feature_set=SERVICE_FEATURE_SET,
    )
end
...
function list_command_executions_for_sandbox(
    ...
    return codebuild(
        "ListCommandExecutionsForSandbox",
        Dict{String,Any}(
            mergewith(_merge, Dict{String,Any}("sandboxId" => sandboxId), params)
        );
        aws_config,
        feature_set=SERVICE_FEATURE_SET,
    )
...
function list_curated_environment_images(;
    ...
    return codebuild(
        "ListCuratedEnvironmentImages"; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_curated_environment_images(
    ...
    return codebuild(
        "ListCuratedEnvironmentImages", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_fleets(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("ListFleets"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_fleets(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild("ListFleets", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_projects(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("ListProjects"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_projects(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild("ListProjects", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- `"sortOrder"`: Used to specify the order to sort the list of returned report groups. Valid
...
function list_report_groups(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("ListReportGroups"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_report_groups(
    ...
    return codebuild(
        "ListReportGroups", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_reports(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("ListReports"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_reports(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild("ListReports", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
"""
    list_sandboxes()
    list_sandboxes(params::Dict{String,<:Any})
...
- `"sortOrder"`: The order in which sandbox records should be retrieved.
"""
function list_sandboxes end

function list_sandboxes(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("ListSandboxes"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function list_sandboxes(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild("ListSandboxes", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end

"""
    list_sandboxes_for_project(project_name)
    list_sandboxes_for_project(project_name, params::Dict{String,<:Any})
...
- `"sortOrder"`: The order in which sandbox records should be retrieved.
"""
function list_sandboxes_for_project end

function list_sandboxes_for_project(
    projectName; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild(
...
- `"sortBy"`: The criterion to be used to list build projects shared with the current Amazon
...
function list_shared_projects(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("ListSharedProjects"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_shared_projects(
    ...
    return codebuild(
        "ListSharedProjects", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_shared_report_groups(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("ListSharedReportGroups"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_shared_report_groups(
    ...
    return codebuild(
        "ListSharedReportGroups", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
function list_source_credentials(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("ListSourceCredentials"; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function list_source_credentials(
    ...
    return codebuild(
        "ListSourceCredentials", params; aws_config, feature_set=SERVICE_FEATURE_SET
    )
end
...
- `"idempotencyToken"`: A unique, case sensitive identifier you provide to ensure the
  idempotency of the `RetryBuild` request. The token is included in the `RetryBuild` request
  and is valid for five minutes. If you repeat the `RetryBuild` request with the same token,
...
function retry_build(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("RetryBuild"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function retry_build(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild("RetryBuild", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- `"idempotencyToken"`: A unique, case sensitive identifier you provide to ensure the
  ...
  `RetryBuildBatch` request with the same token, but change a parameter, CodeBuild returns a
...
function retry_build_batch(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("RetryBuildBatch"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function retry_build_batch(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild("RetryBuildBatch", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- `"autoRetryLimitOverride"`: The maximum number of additional automatic retries after a
  failed build. For example, if the auto-retry limit is set to 2, CodeBuild will call the
  `RetryBuild` API to automatically retry your build for up to 2 additional times.
...
- `"buildspecOverride"`: A buildspec file declaration that overrides the latest one defined
  ...
  example, `arn:aws:s3:::my-codebuild-sample2/buildspec.yml`). If this value is not provided
  or is set to an empty string, the source code must contain a buildspec file in its root
  directory. For more information, see [Buildspec File Name and Storage Location](https://docs.aws.amazon.com/codebuild/latest/userguide/build-spec-ref.html#build-spec-ref-name-storage).
  ...
  !!! note
      ...
      set this parameter can override the default settings. Moreover, we encourage that you
      use a trustworthy buildspec location like a file in your source repository or a Amazon
      S3 bucket. Alternatively, you can restrict overrides to the buildspec by using a
      condition key: [Prevent unauthorized modifications to project buildspec](https://docs.aws.amazon.com/codebuild/latest/userguide/action-context-keys.html#action-context-keys-example-overridebuildspec.html).
...
- `"imageOverride"`: The name of an image for this build that overrides the one specified in
  the build project.
...
- `"imagePullCredentialsTypeOverride"`: The type of credentials CodeBuild uses to pull
  ...
  repository policy to trust CodeBuild's service principal.
...
- `"logsConfigOverride"`: Log settings for this build that override the log settings defined
...
- `"reportBuildStatusOverride"`: Set to true to report to your source provider the status of
  a build's start and completion. If you use this option with a source provider other than
  GitHub, GitHub Enterprise, GitLab, GitLab Self Managed, or Bitbucket, an
  `invalidInputException` is thrown.

  To be able to report the build status to the source provider, the user associated with the
  source provider must have write access to the repo. If the user does not have write
...
- `"sourceLocationOverride"`: A location that overrides, for this build, the source location
...
- `"sourceVersion"`: The version of the build input to be built, for this build only. If not
  ...
  of the source code you want to build. If a pull request ID is specified, it must use the
  format `pr/pull-request-ID` (for example `pr/25`). If a branch name is specified, the
  branch's HEAD commit ID is used. If not specified, the default branch's HEAD commit ID is
  used.
  ...
  The commit ID, branch name, or tag name that corresponds to the version of the source code
  you want to build. If a branch name is specified, the branch's HEAD commit ID is used. If
...
- `"certificateOverride"`: The name of a certificate for this batch build that overrides the
...
- `"idempotencyToken"`: A unique, case sensitive identifier you provide to ensure the
  ...
  `StartBuildBatch` request with the same token, but change a parameter, CodeBuild returns a
  parameter mismatch error.
...
"""
    start_command_execution(command, sandbox_id)
    start_command_execution(command, sandbox_id, params::Dict{String,<:Any})
...
function start_command_execution(
    command, sandboxId; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild(
        "StartCommandExecution",
        Dict{String,Any}("command" => command, "sandboxId" => sandboxId);
        aws_config,
        feature_set=SERVICE_FEATURE_SET,
    )
end
...
"""
    start_sandbox()
    start_sandbox(params::Dict{String,<:Any})
...
"""
function start_sandbox end

function start_sandbox(; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild("StartSandbox"; aws_config, feature_set=SERVICE_FEATURE_SET)
end

function start_sandbox(
    params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild("StartSandbox", params; aws_config, feature_set=SERVICE_FEATURE_SET)
end

"""
    start_sandbox_connection(sandbox_id)
    start_sandbox_connection(sandbox_id, params::Dict{String,<:Any})
...
- `sandbox_id`: A `sandboxId` or `sandboxArn`.
"""
function start_sandbox_connection end

function start_sandbox_connection(
    sandboxId; aws_config::AbstractAWSConfig=current_aws_config()
)
    return codebuild(
...
"""
    stop_sandbox(id)
    stop_sandbox(id, params::Dict{String,<:Any})
...
"""
function stop_sandbox end

function stop_sandbox(id; aws_config::AbstractAWSConfig=current_aws_config())
    return codebuild(
        "StopSandbox",
        Dict{String,Any}("id" => id);
        aws_config,
...
- `"computeType"`: Information about the compute resources the compute fleet uses. Available
  values include:
...
- `"logsConfig"`: Information about logs for the build project. A project can create logs in
...
!!! important
    ...
    - All build logs and artifacts are available to the public. Environment variables,
      ...
      and artifacts. You must be careful about what information is output to the build logs.
      Some best practice are:
      - Do not store sensitive values in environment variables. We recommend that you use an
    ...
    - A malicious user can use public builds to distribute malicious artifacts. We recommend
      ...
      change. We also recommend that you validate any artifacts with their checksums to make
