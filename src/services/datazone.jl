...
- `"clientToken"`: A unique, case-sensitive identifier that is provided to ensure the
  idempotency of the request.
- `"skipDeletionCheck"`: Optional flag to delete all child entities within the domain
"""
...
# Optional Parameters
Optional parameters can be passed as a `params::Dict{String,<:Any}`. Valid keys are:
- `"skipDeletionCheck"`: Optional flag to asynchronously delete child entities within the
  project
"""
delete_project(
    domainIdentifier, identifier; aws_config::AbstractAWSConfig=global_aws_config()
