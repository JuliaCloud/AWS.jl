...
# Required Parameters
- `ProductCode`: Product code is used to uniquely identify a product in AWS Marketplace.
...
- `MaxResults`: The maximum number of items to retrieve from the GetEntitlements operation.
  For pagination, use the NextToken field in subsequent calls to GetEntitlements.
- `NextToken`: For paginated calls to GetEntitlements, pass the NextToken from the previous
