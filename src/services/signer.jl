...
- `nextToken`: String for specifying the next set of paginated results to return. After you
...
- `signingMaterial`: The AWS Certificate Manager certificate that will be used to sign code
...
# Required Parameters
- `profileName`: A human-readable name for the signing profile with permissions to be
  removed.
...
# Required Parameters
- `effectiveTime`: A timestamp for when revocation of a Signing Profile should become
...
# Required Parameters
- `clientRequestToken`: String that identifies the signing request. All calls after the
