...
- `target_configurations`: The configuration of the target Convertible Reserved Instance to
...
# Keyword Parameters
- `address`: [EC2-VPC] The Elastic IP address to recover or an IPv4 address from an address
  pool.
...
- `domain`: Indicates whether the Elastic IP address is for use with instances in a VPC or
  instances in EC2-Classic. Default: If the Region supports EC2-Classic, the default is
...
- `instance_family`: Specifies the instance family to be supported by the Dedicated Hosts.
...
- `ipv6_prefix_count`: The number of IPv6 prefixes that Amazon Web Services automatically
  ...
  option.
- `ipv6_prefixes`: One or more IPv6 prefixes assigned to the network interface. You cannot
...
- `ipv4_prefix_count`: The number of IPv4 prefixes that Amazon Web Services automatically
  ...
  option.
- `ipv4_prefixes`: One or more IPv4 prefixes assigned to the network interface. You cannot
  use this option if you use the Ipv4PrefixCount option.
- `private_ip_addresses`: One or more IP addresses to be assigned as a secondary private IP
...
- `private_ip_address`: [EC2-VPC] The primary or secondary private IP address to associate
...
- `public_ip`: [EC2-Classic] The Elastic IP address to associate with the instance. This is
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `network_card_index`: The index of the network card. Some instance types support multiple
...
- `access_group_id`: The ID of the group to grant access to, for example, the Active
  ...
  or not specified.
- `authorize_all_groups`: Indicates whether to grant access to all clients. Specify true to
...
- `ip_protocol`: Not supported. Use a set of IP permissions to specify the protocol name or
  number.
- `source_security_group_name`: Not supported. Use a set of IP permissions to specify a
  destination security group.
- `source_security_group_owner_id`: Not supported. Use a set of IP permissions to specify a
  destination security group.
...
# Keyword Parameters
- `cidr_ip`: The IPv4 address range, in CIDR format. You can't specify this parameter when
...
- `group_name`: [EC2-Classic, default VPC] The name of the security group. You must specify
  either the security group ID or the security group name in the request.
- `ip_permissions`: The sets of IP permissions.
- `ip_protocol`: The IP protocol name (tcp, udp, icmp) or number (see Protocol Numbers). To
  specify icmpv6, use a set of IP permissions. [VPC only] Use -1 to specify all protocols. If
  you specify -1 or a protocol other than tcp, udp, or icmp, traffic on all ports is allowed,
  regardless of any ports you specify. Alternatively, use a set of IP permissions to specify
  multiple rules and a description for the rule.
- `source_security_group_name`: [EC2-Classic, default VPC] The name of the source security
...
- `source_security_group_owner_id`: [nondefault VPC] The Amazon Web Services account ID for
...
- `destination_outpost_arn`: The Amazon Resource Name (ARN) of the Outpost to which to copy
...
- `kms_key_id`: The identifier of the symmetric Key Management Service (KMS) KMS key to use
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `encrypted`: To encrypt a copy of an unencrypted snapshot if encryption by default is not
  enabled, enable encryption using this parameter. Otherwise, omit this parameter. Encrypted
  snapshots are encrypted, even if you omit this parameter and encryption by default is not
  enabled. You cannot set this parameter to false. For more information, see Amazon EBS
...
- `ebs_optimized`: Indicates whether the Capacity Reservation supports EBS-optimized
  ...
  instance.
- `end_date`: The date and time at which the Capacity Reservation expires. When a Capacity
...
- `tenancy`: Indicates the tenancy of the Capacity Reservation. A Capacity Reservation can
  have one of the following tenancy settings:    default - The Capacity Reservation is
...
- `tag_specifications`: The tags to assign to the Capacity Reservation Fleet. The tags are
...
- `dns_servers`: Information about the DNS servers to be used for DNS resolution. A Client
...
- `split_tunnel`: Indicates whether split-tunnel is enabled on the Client VPN endpoint. By
...
- `vpn_port`: The port number to assign to the Client VPN endpoint for TCP and UDP traffic.
...
- `replace_unhealthy_instances`: Indicates whether EC2 Fleet should replace unhealthy Spot
...
- `type`: The fleet type. The default value is maintain.    maintain - The EC2 Fleet places
  an asynchronous request for your desired capacity, and continues to maintain your desired
  Spot capacity by replenishing interrupted Spot Instances.    request - The EC2 Fleet places
  an asynchronous one-time request for your desired capacity, but does submit Spot requests
  in alternative capacity pools if Spot capacity is unavailable, and does not maintain Spot
  capacity if Spot Instances are interrupted.    instant - The EC2 Fleet places a synchronous
  one-time request for your desired capacity, and returns errors for any instances that could
  not be launched.   For more information, see EC2 Fleet request types in the Amazon EC2 User
  Guide.
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `no_reboot`: By default, Amazon EC2 attempts to shut down and reboot the instance before
...
- `tag_specifications`: The tags to apply to the AMI and snapshots on creation. You can tag
...
- `cron_expression`: The cron expression for the event window, for example, * 0-4,20-23 * *
  1,5. If you specify a cron expression, you can't specify a time range. Constraints:   Only
  hour and day of the week values are supported.   For day of the week values, you can
...
- `source_version`: The version number of the launch template version on which to base the
...
# Keyword Parameters
- `allocation_id`: [Public NAT gateways only] The allocation ID of an Elastic IP address to
  associate with the NAT gateway. You cannot specify an Elastic IP address with a private NAT
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `icmp_type_code`: ICMP protocol: The ICMP or ICMPv6 type and code. Required if specifying
  protocol 1 (ICMP) or protocol 58 (ICMPv6) with an IPv6 CIDR block.
- `ipv6_cidr_block`: The IPv6 network range to allow or deny, in CIDR notation (for example
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `source_ip`: The IP address of the Amazon Web Services resource that is the source of the
  path.
...
- `partition_count`: The number of partitions. Valid only when Strategy is set to partition.
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `snapshot_id`: The ID of the snapshot from which to restore the replacement root volume.
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `name`: The name for the restored AMI. The name must be unique for AMIs in the Region for
  this account. If you do not provide a name, the new AMI gets the same name as the original
  AMI.
- `tag_specifications`: The tags to apply to the AMI and snapshots on restoration. You can
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `egress_only_internet_gateway_id`: [IPv6 traffic only] The ID of an egress-only internet
  gateway.
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `outpost_arn`: The Amazon Resource Name (ARN) of the Outpost on which to create the local
  snapshots.   To create snapshots from an instance in a Region, omit this parameter. The
  snapshots are created in the same Region as the instance.   To create snapshots from an
...
# Keyword Parameters
- `availability_zone`: The Availability Zone or Local Zone for the subnet. Default: Amazon
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `packet_length`: The number of bytes in each packet to mirror. These are bytes after the
...
- `network_load_balancer_arn`: The Amazon Resource Name (ARN) of the Network Load Balancer
...
- `encrypted`: Indicates whether the volume should be encrypted. The effect of setting the
  ...
  information, see Encryption by default in the Amazon Elastic Compute Cloud User Guide.
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `instance_tenancy`: The tenancy options for instances launched into the VPC. For default,
  instances are launched with shared tenancy by default. You can launch instances with any
  tenancy into a shared tenancy VPC. For dedicated, instances are launched as dedicated
...
- `private_dns_name`: (Interface endpoint configuration) The private DNS name to assign to
...
- `peer_region`: The Region code for the accepter VPC, if the accepter VPC is located in a
...
- `vpn_gateway_id`: The ID of the virtual private gateway. If you specify a virtual private
...
# Keyword Parameters
- `amazon_side_asn`: A private Autonomous System Number (ASN) for the Amazon side of a BGP
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `force`: Specify true to remove the permission even if the network interface is attached
  to an instance.
"""
...
- `group_name`: [EC2-Classic, default VPC] The name of the security group. You can specify
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: The filters.    group-name - For Availability Zones, use the Region name. For
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: The filters.    bundle-id - The ID of the bundle task.    error-code - If the
...
# Keyword Parameters
- `capacity_reservation_fleet_ids`: The IDs of the Capacity Reservation Fleets to describe.
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    state - The state of the Fleet (submitted | modifying
...
# Keyword Parameters
- `dhcp_options_ids`: The IDs of one or more DHCP options sets. Default: Describes all your
  DHCP options sets.
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    dhcp-options-id - The ID of a DHCP options set.    key
  - The key for one of the options (for example, domain-name).    value - The value for one
  of the options.    owner-id - The ID of the Amazon Web Services account that owns the DHCP
  options set.    tag:&lt;key&gt; - The key/value combination of a tag assigned to the
  resource. Use the tag key in the filter name and the tag value as the filter value. For
  example, to find all resources that have a tag with the key Owner and the value TeamA,
  specify tag:Owner for the filter name and TeamA for the filter value.    tag-key - The key
  of a tag assigned to the resource. Use this filter to find all resources assigned a tag
...
- `filters`: The filters.    availability-zone - The Availability Zone in which the Elastic
  Graphics accelerator resides.    elastic-gpu-health - The status of the Elastic Graphics
  accelerator (OK | IMPAIRED).    elastic-gpu-state - The state of the Elastic Graphics
  accelerator (ATTACHED).    elastic-gpu-type - The type of Elastic Graphics accelerator; for
  example, eg1.medium.    instance-id - The ID of the instance to which the Elastic Graphics
...
- `max_results`: The maximum number of results to return in a single call. Specify a value
...
- `filter`: The filters.    instance-family - The instance family of the offering (for
  ...
  AllUpfront).
- `max_duration`: This is the maximum duration of the reservation to purchase, specified in
  seconds. Reservations are available in one-year and three-year terms. The number of seconds
  specified must be the number of seconds in a year (365x24x60x60) times one of the supported
...
# Keyword Parameters
- `filter`: The filters.    auto-placement - Whether auto-placement is enabled or disabled
  (on | off).    availability-zone - The Availability Zone of the host.    client-token - The
  idempotency token that you provided when you allocated the host.    host-reservation-id -
  The ID of the reservation assigned to this host.    instance-type - The instance type size
  that the Dedicated Host is configured to support.    state - The allocation state of the
  Dedicated Host (available | under-assessment | permanent-failure | released |
  released-permanent-failure).    tag-key - The key of a tag assigned to the resource. Use
...
- `filters`: The filters.    instance-id - The ID of the instance.    state - The state of
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `executable_users`: Scopes the images by users with explicit launch permissions. Specify
  an Amazon Web Services account ID, self (the sender of the request), or all (public AMIs).
...
- `filters`: The filters.    architecture - The image architecture (i386 | x86_64 | arm64).
  ...
  gp2 | gp3 | sc1 | st1 | standard).    block-device-mapping.encrypted - A Boolean that
  indicates whether the Amazon EBS volume is encrypted.    description - The description of
  the image (provided during image creation).    ena-support - A Boolean that indicates
  whether enhanced networking with ENA is enabled.    hypervisor - The hypervisor type (ovm |
  xen).    image-id - The ID of the image.    image-type - The image type (machine | kernel |
  ramdisk).    is-public - A Boolean that indicates whether the image is public.    kernel-id
  - The kernel ID.    manifest-location - The location of the image manifest.    name - The
  name of the AMI (provided during image creation).    owner-alias - The owner alias (amazon
  | aws-marketplace). The valid aliases are defined in an Amazon-maintained list. This is not
  the Amazon Web Services account alias that can be set using the IAM console. We recommend
  that you use the Owner request parameter instead of this filter.    owner-id - The Amazon
  Web Services account ID of the owner. We recommend that you use the Owner request parameter
  instead of this filter.    platform - The platform. To only list Windows-based AMIs, use
  windows.    product-code - The product code.    product-code.type - The type of the product
  code (marketplace).    ramdisk-id - The RAM disk ID.    root-device-name - The device name
  of the root device volume (for example, /dev/sda1).    root-device-type - The type of the
  root device volume (ebs | instance-store).    state - The state of the image (available |
  pending | failed).    state-reason-code - The reason code for the state change.
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    dedicated-host-id - The event windows associated with
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    attachment.state - The current state of the attachment
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    create-time - The time the launch template version was
  created.    ebs-optimized - A boolean that indicates whether the instance is optimized for
  Amazon EBS I/O.    iam-instance-profile - The ARN of the IAM instance profile.    image-id
  - The ID of the AMI.    instance-type - The instance type.    is-default-version - A
  boolean that indicates whether the launch template version is the default version.
  kernel-id - The kernel ID.    ram-disk-id - The RAM disk ID.
- `launch_template_id`: The ID of the launch template. To describe one or more versions of
...
- `launch_template_name`: The name of the launch template. To describe one or more versions
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    association.association-id - The ID of an association
...
- `max_results`: The maximum number of results to return with a single call. To retrieve
  the remaining results, make another call with the returned nextToken value.
- `network_acl_ids`: One or more network ACL IDs. Default: Describes all your network ACLs.
- `next_token`: The token for the next page of results.
"""
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: The filters. The following are possible values:   PathFound - A Boolean value
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: The filters. The following are possible values:   Destination - The ID of the
...
- `offering_type`: The Reserved Instance offering type. If you are using tools that predate
  the 2011-11-01 API version, you only have access to the Medium Utilization Reserved
  Instance offering type.
- `reserved_instances_ids`: One or more Reserved Instance IDs. Default: Describes all your
...
- `max_duration`: The maximum duration (in seconds) to filter when searching for offerings.
...
- `min_duration`: The minimum duration (in seconds) to filter when searching for offerings.
...
- `offering_type`: The Reserved Instance offering type. If you are using tools that predate
  the 2011-11-01 API version, you only have access to the Medium Utilization Reserved
  Instance offering type.
- `product_description`: The Reserved Instance product platform description. Instances that
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    association.route-table-association-id - The ID of an
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: The filters. If using multiple filters for rules, the results include security
  groups for which any combination of rules - not necessarily a single rule - match all
  filters.    description - The description of the security group.
...
- `group_ids`: The IDs of the security groups. Required for security groups in a nondefault
...
- `owner_ids`: Scopes the results to snapshots with the specified owners. You can specify a
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    availability-zone-group - The Availability Zone group.
...
- `max_results`: The maximum number of results to return in a single call. Specify a value
  ...
  NextToken value.
- `next_token`: The token to request the next set of results. This value is null when there
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `end_time`: The date and time, up to the current date, from which to stop retrieving the
...
- `image_ids`: The AMI IDs for which to show progress. Up to 20 AMI IDs can be included in
  a request.
- `max_results`: The maximum number of results to return in a single call. To retrieve the
  remaining results, make another call with the returned NextToken value. This value can be
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters. The possible values are:    association.state - The state
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters. The possible values are:    options.protocol - The tunnel
  protocol (gre).    state - The state of the attachment (initiating | initiatingRequest |
  pendingAcceptance | rollingBack | pending | available | modifying | deleting | deleted |
  failed | rejected | rejecting | failing).    transit-gateway-attachment-id - The ID of the
  Connect attachment.    transit-gateway-id - The ID of the transit gateway.
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters. The possible values are:    transit-gateway-attachment-id
  - The ID of the transit gateway attachment.    local-owner-id - The ID of your Amazon Web
  Services account.    remote-owner-id - The ID of the Amazon Web Services account in the
  remote Region that owns the transit gateway.    state - The state of the peering
  attachment. Valid values are available | deleted | deleting | failed | failing |
  initiatingRequest | modifying | pendingAcceptance | pending | rollingBack | rejected |
  rejecting).    tag:&lt;key&gt; - The key/value combination of a tag assigned to the
  resource. Use the tag key in the filter name and the tag value as the filter value. For
  example, to find all resources that have a tag with the key Owner and the value TeamA,
  specify tag:Owner for the filter name and TeamA for the filter value.    tag-key - The key
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: The filters.    modification-state - The current modification state (modifying
  | optimizing | completed | failed).    original-iops - The original IOPS rate of the
  volume.    original-size - The original size of the volume, in GiB.    original-volume-type
  ...
  false).    start-time - The modification start time.    target-iops - The target IOPS rate
  of the volume.    target-size - The target size of the volume, in GiB.
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    connection-notification-arn - The ARN of the SNS topic
  for the notification.    connection-notification-id - The ID of the notification.
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    service-name - The name of the service.    service-id
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters.    accepter-vpc-info.cidr-block - The IPv4 CIDR block of
...
- `max_results`: The maximum number of results to return with a single call. To retrieve
  the remaining results, make another call with the returned nextToken value.
- `next_token`: The token for the next page of results.
- `vpc_peering_connection_ids`: One or more VPC peering connection IDs. Default: Describes
...
- `force`: Forces detachment if the previous detachment attempt did not occur cleanly (for
  ...
  resort to detach a volume from a failed instance. The instance won't have an opportunity to
...
- `target_capacity_unit_type`: The unit for the target capacity. Default: units (translates
...
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `filters`: One or more filters. The possible values are:    attachment.resource-id - The
...
- `kms_key_id`: An identifier for the symmetric KMS key to use when creating the encrypted
...
- `usage_operation`: The usage operation value. For more information, see Licensing options
...
- `remove_end_date`: Indicates whether to remove the end date from the Capacity Reservation
...
- `connection_log_options`: Information about the client connection logging options. If you
  ...
  connection requests   Client connection termination time
- `description`: A brief description of the Client VPN endpoint.
- `dns_servers`: Information about the DNS servers to be used by Client VPN connections. A
...
- `operation_type`: The operation type. This parameter can be used only when the Attribute
...
- `user_groups`: The user groups. This parameter can be used only when the Attribute
  parameter is launchPermission.
- `user_ids`: The Amazon Web Services account IDs. This parameter can be used only when the
...
# Keyword Parameters
- `attribute`: The name of the attribute.
- `block_device_mappings`: Modifies the DeleteOnTermination attribute for volumes that are
...
- `ena_support`: Set to true to enable enhanced networking with ENA for the instance. This
...
- `groups`: [EC2-VPC] Replaces the security groups of the instance with the specified
  ...
  security group name.
- `instance_initiated_shutdown_behavior`: Specifies whether an instance stops or terminates
  when you initiate shutdown from the instance (using the operating system command for system
  shutdown).
- `instance_type`: Changes the instance type to the specified value. For more information,
...
- `kernel`: Changes the instance's kernel to the specified value. We recommend that you use
...
- `user_data`: Changes the instance's user data to the specified value. If you are using an
  Amazon Web Services SDK or command line tool, base64-encoding is performed for you, and you
...
# Keyword Parameters
- `client_token`: A unique, case-sensitive token that you provide to ensure idempotency of
...
- `group_name`: The name of the placement group in which to place the instance. For spread
...
# Keyword Parameters
- `client_token`: A unique, case-sensitive token you provide to ensure idempotency of your
...
- `attribute`: The snapshot attribute to modify. Only volume creation permissions can be
  modified.
- `create_volume_permission`: A JSON representation of the snapshot attribute modification.
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
...
# Keyword Parameters
- `context`: Reserved.
- `excess_capacity_termination_policy`: Indicates whether running Spot Instances should be
...
- `packet_length`: The number of bytes in each packet to mirror. These are bytes after the
  ...
  packet.
- `remove_fields`: The properties that you want to remove from the Traffic Mirror session.
...
- `session_number`: The session number determines the order in which sessions are evaluated
  ...
  the one that mirrors the packets. Valid values are 1-32766.
- `traffic_mirror_filter_id`: The ID of the Traffic Mirror filter.
- `traffic_mirror_target_id`: The Traffic Mirror target. The target must be in the same VPC
...
- `throughput`: The target throughput of the volume, in MiB/s. This parameter is valid only
  for gp3 volumes. The maximum value is 1,000. Default: The existing value is retained if the
...
- `volume_type`: The target EBS volume type of the volume. For more information, see Amazon
  EBS volume types in the Amazon Elastic Compute Cloud User Guide. Default: The existing type
  is retained.
"""
...
# Keyword Parameters
- `auto_enable_io`: Indicates whether the volume should be auto-enabled for I/O operations.
- `dry_run`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
"""
...
# Keyword Parameters
- `add_route_table_ids`: (Gateway endpoint) One or more route tables IDs to associate with
  the endpoint.
- `add_security_group_ids`: (Interface endpoint) One or more security group IDs to
  associate with the network interface.
- `add_subnet_ids`: (Interface and Gateway Load Balancer endpoints) One or more subnet IDs
...
- `remove_route_table_ids`: (Gateway endpoint) One or more route table IDs to disassociate
  from the endpoint.
- `remove_security_group_ids`: (Interface endpoint) One or more security group IDs to
  disassociate from the network interface.
- `remove_subnet_ids`: (Interface endpoint) One or more subnets IDs in which to remove the
  endpoint.
...
- `publicly_advertisable`: (IPv6 only) Indicate whether the address range will be publicly
...
- `purchase_time`: The time at which to purchase the Reserved Instance, in UTC format (for
...
- `billing_products`: The billing product codes. Your account must be authorized to specify
...
- `boot_mode`: The boot mode of the AMI. For more information, see Boot modes in the Amazon
  Elastic Compute Cloud User Guide.
...
- `image_location`: The full path to your AMI manifest in Amazon S3 storage. The specified
...
- `local_gateway_id`: The ID of the local gateway.
- `local_target`: Specifies whether to reset the local route to its default target (local).
- `nat_gateway_id`: [IPv4 traffic only] The ID of a NAT gateway.
- `network_interface_id`: The ID of a network interface.
- `transit_gateway_id`: The ID of a transit gateway.
- `vpc_endpoint_id`: The ID of a VPC endpoint. Supported for Gateway Load Balancer
...
- `launch_specification`: The launch specification.
- `spot_price`: The maximum price per hour that you are willing to pay for a Spot Instance.
...
- `valid_until`: The end date of the request, in UTC format (YYYY-MM-DDTHH:MM:SSZ).   For a
  persistent request, the request remains active until the ValidUntil date and time is
...
# Keyword Parameters
- `cidr_ip`: The CIDR IP address range. You can't specify this parameter when specifying a
...
- `source_security_group_owner_id`: [EC2-Classic] The Amazon Web Services account ID of the
...
- `elastic_inference_accelerators`: An elastic inference accelerator to associate with the
...
- `hibernation_options`: Indicates whether an instance is enabled for hibernation. For more
...
- `iam_instance_profile`: The name or Amazon Resource Name (ARN) of an IAM instance profile.
- `image_id`: The ID of the AMI. An AMI ID is required to launch an instance and must be
  specified here or in a launch template.
- `instance_initiated_shutdown_behavior`: Indicates whether an instance stops or terminates
  when you initiate shutdown from the instance (using the operating system command for system
...
- `ipv6_address_count`: [EC2-VPC] The number of IPv6 addresses to associate with the
  ...
  the same request.
- `ipv6_addresses`: [EC2-VPC] The IPv6 addresses from the range of the subnet to associate
...
- `kernel_id`: The ID of the kernel.  We recommend that you use PV-GRUB instead of kernels
...
- `subnet_id`: [EC2-VPC] The ID of the subnet to launch the instance into. If you specify a
  network interface, you must specify any subnets as part of the network interface.
- `tag_specifications`: The tags to apply to the resources during launch. You can only tag
...
- `ip_permissions`: The IP permissions for the security group rule. You must specify either
  the IP permissions or the description.
- `security_group_rule_descriptions`: The description for the egress security group rules.
