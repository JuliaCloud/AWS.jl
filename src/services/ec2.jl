...
# Required Parameters
- `ReservedInstanceId`: The IDs of the Convertible Reserved Instances to exchange for
...
- `autoPlacement`: Indicates whether the host accepts any untargeted instance launches that
  match its instance type configuration, or if it only accepts Host tenancy instance launches
  that specify its unique host ID. For more information, see  Understanding auto-placement
...
- `SecurityGroupId`: The IDs of the security groups to apply to the associated target
...
# Optional Parameters
- `allowReassignment`: Indicates whether to allow an IP address that is already assigned to
  another network interface or instance to be reassigned to the specified network interface.
...
- `InstanceId`: The ID of the instance. The instance must have exactly one attached network
  interface. For EC2-VPC, you can specify either the instance ID or the network interface ID,
  but not both. For EC2-Classic, you must specify an instance ID and the instance must be in
  the running state.
- `PublicIp`: [EC2-Classic] The Elastic IP address to associate with the instance. This is
...
- `dryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `networkInterfaceId`: [EC2-VPC] The ID of the network interface. If the instance has more
  than one network interface, you must specify a network interface ID. For EC2-VPC, you can
...
# Required Parameters
- `DhcpOptionsId`: The ID of the DHCP options set, or default to associate no DHCP options
  with the VPC.
...
# Required Parameters
- `ipv6CidrBlock`: The IPv6 CIDR block for your subnet. The subnet must have a /64 prefix
  length.
...
- `SubnetIds`: The IDs of the subnets to associate with the transit gateway multicast
  domain.
- `TransitGatewayAttachmentId`: The ID of the transit gateway attachment to associate with
...
# Required Parameters
- `SecurityGroupId`: The ID of one or more of the VPC's security groups. You cannot specify
...
- `InstanceId`: The ID of the instance.
- `VolumeId`: The ID of the EBS volume. The volume and instance must be within the same
  Availability Zone.
...
# Required Parameters
- `ClientVpnEndpointId`: The ID of the Client VPN endpoint.
- `TargetNetworkCidr`: The IPv4 address range, in CIDR notation, of the network for which
...
- `GroupId`: The ID of the security group. You must specify either the security group ID or
  the security group name in the request. For security groups in a nondefault VPC, you must
  specify the security group ID.
- `GroupName`: [EC2-Classic, default VPC] The name of the security group. You must specify
  either the security group ID or the security group name in the request.
- `IpPermissions`: The sets of IP permissions.
- `IpProtocol`: The IP protocol name (tcp, udp, icmp) or number (see Protocol Numbers). To
...
- `SourceSecurityGroupOwnerId`: [nondefault VPC] The AWS account ID for the source security
  ...
  protocol, the start of the port range, and the end of the port range. Creates rules that
...
- `terminateInstances`: Indicates whether to terminate instances for a Spot Fleet request
...
# Optional Parameters
- `ClientToken`: Unique, case-sensitive identifier you provide to ensure idempotency of the
...
- `kmsKeyId`: The identifier of the symmetric AWS Key Management Service (AWS KMS) customer
  master key (CMK) to use when creating encrypted volumes. If this parameter is not
...
- `InstanceType`: The instance type for which to reserve capacity. For more information,
...
- `Authentication`: Information about the authentication method to be used to authenticate
  clients.
- `ClientCidrBlock`: The IPv4 address range, in CIDR notation, from which to assign client
  ...
  be changed after the Client VPN endpoint has been created. The CIDR block should be /22 or
  greater.
...
- `ServerCertificateArn`: The ARN of the server certificate. For more information, see the
...
- `SelfServicePortal`: Specify whether to enable the self-service portal for the Client VPN
...
- `SplitTunnel`: Indicates whether split-tunnel is enabled on the AWS Client VPN endpoint.
  ...
  VPN endpoints, see Split-Tunnel AWS Client VPN Endpoint in the AWS Client VPN Administrator
  Guide.
- `TagSpecification`: The tags to apply to the Client VPN endpoint during creation.
- `TransportProtocol`: The transport protocol to be used by the VPN session. Default value:
  udp
...
- `DestinationCidrBlock`: The IPv4 address range, in CIDR notation, of the route
...
- `TargetVpcSubnetId`: The ID of the subnet through which you want to route traffic. The
...
- `TagSpecification`: The key-value pair for tagging the EC2 Fleet request on creation. The
  value for ResourceType must be fleet, otherwise the fleet request fails. To tag instances
  at launch, specify the tags in the launch template. For information about tagging after
  launch, see Tagging your resources.
...
# Required Parameters
- `ResourceId`: The ID of the subnet, network interface, or VPC for which you want to
  create a flow log. Constraints: Maximum of 1000 resources
- `ResourceType`: The type of resource for which to create the flow log. For example, if
  you specified a VPC ID for the ResourceId property, specify VPC for this property.
- `TrafficType`: The type of traffic to log. You can log traffic that the resource accepts
...
- `LogDestinationType`: Specifies the type of destination to which the flow log data is to
  be published. Flow log data can be published to CloudWatch Logs or Amazon S3. To publish
...
- `LogGroupName`: The name of a new or existing CloudWatch Logs log group where Amazon EC2
  publishes your flow logs. If you specify LogDestinationType as s3, do not specify
...
- `MaxAggregationInterval`: The maximum interval of time during which a flow of packets is
  captured and aggregated into a flow log record. You can specify 60 seconds (1 minute) or
...
- `TagSpecification`: The tags to apply to the AMI and snapshots on creation. You can tag
  ...
  CreateTags.
- `blockDeviceMapping`: The block device mappings. This parameter cannot be used to modify
  the encryption status of existing volumes or snapshots. To create an AMI with encrypted
...
# Optional Parameters
- `ClientToken`: Unique, case-sensitive identifier you provide to ensure the idempotency of
  the request. For more information, see Ensuring Idempotency. Constraint: Maximum 128 ASCII
  characters.
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
...
- `LaunchTemplateName`: The name of the launch template. You must specify either the launch
  template ID or launch template name in the request.
...
- `PrefixListName`: A name for the prefix list. Constraints: Up to 255 characters in
  length. The name cannot start with com.amazonaws.

# Optional Parameters
- `ClientToken`: Unique, case-sensitive identifier you provide to ensure the idempotency of
  the request. For more information, see Ensuring Idempotency. Constraints: Up to 255 UTF-8
...
# Required Parameters
- `AllocationId`: The allocation ID of an Elastic IP address to associate with the NAT
...
- `ruleNumber`: The rule number for the entry (for example, 100). ACL entries are processed
  in ascending order by rule number. Constraints: Positive integer from 1 to 32766. The range
  32767 to 65535 is reserved for internal use.
...
- `dryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `ipv6AddressCount`: The number of IPv6 addresses to assign to a network interface. Amazon
  EC2 automatically selects the IPv6 addresses from the subnet range. You can't use this
  option if specifying specific IPv6 addresses. If your subnet has the
...
- `clientToken`: Unique, case-sensitive identifier you provide to ensure idempotency of
  ...
  Idempotency.
- `instanceCount`: The number of instances that are a part of a Reserved Instance account
...
- `priceSchedules`: A list specifying the price of the Standard Reserved Instance for each
...
# Optional Parameters
- `CarrierGatewayId`: The ID of the carrier gateway. You can only use this option when the
...
- `gatewayId`: The ID of an internet gateway or virtual private gateway attached to your
  VPC.
- `instanceId`: The ID of a NAT instance in your VPC. The operation fails if you specify an
...
- `GroupName`: The name of the security group. Constraints: Up to 255 characters in length.
  Cannot start with sg-. Constraints for EC2-Classic: ASCII characters Constraints for
...
# Required Parameters
- `InstanceSpecification`: The instance to specify which volumes should be included in the
  snapshots.
...
- `Ipv6CidrBlock`: The IPv6 network range for the subnet, in CIDR notation. The subnet size
...
# Required Parameters
- `ResourceId`: The IDs of the resources, separated by spaces. Constraints: Up to 1000
...
- `TransportTransitGatewayAttachmentId`: The ID of the transit gateway attachment. You can
  specify a VPC attachment or a AWS Direct Connect attachment.
...
# Required Parameters
- `PeerAddress`: The peer IP address (GRE outer IP address) on the appliance side of the
  Connect peer.
...
- `PeerTransitGatewayId`: The ID of the peer transit gateway with which to create the
  peering attachment.
- `TransitGatewayId`: The ID of the transit gateway.
...
- `MultiAttachEnabled`: Indicates whether to enable Amazon EBS Multi-Attach. If you enable
  Multi-Attach, you can attach the volume to up to 16 Instances built on the Nitro System in
  the same Availability Zone. This parameter is supported with io1 and io2 volumes only. For
  more information, see  Amazon EBS Multi-Attach in the Amazon Elastic Compute Cloud User
  Guide.
...
- `dryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `instanceTenancy`: The tenancy options for instances launched into the VPC. For default,
...
# Required Parameters
- `ServiceName`: The service name. To get a list of available services, use the
...
- `PolicyDocument`: (Interface and gateway endpoints) A policy to attach to the endpoint
  ...
  service.
- `PrivateDnsEnabled`: (Interface endpoint) Indicates whether to associate a private hosted
  zone with the specified VPC. The private hosted zone contains a record set for the default
...
- `SubnetId`: (Interface and Gateway Load Balancer endpoints) The ID of one or more subnets
  in which to create an endpoint network interface. For a Gateway Load Balancer endpoint, you
...
- `ClientToken`: Unique, case-sensitive identifier that you provide to ensure the
  idempotency of the request. For more information, see How to Ensure Idempotency.
- `ConnectionEvents`: One or more endpoint events for which to receive notifications. Valid
...
# Required Parameters
- `ClientVpnEndpointId`: The ID of the Client VPN endpoint from which the route is to be
  deleted.
- `DestinationCidrBlock`: The IPv4 address range, in CIDR notation, of the route to be
  deleted.
...
- `TerminateInstances`: Indicates whether to terminate the instances when the EC2 Fleet is
...
# Required Parameters
- `LaunchTemplateVersion`: The version numbers of one or more launch template versions to
  delete.
...
- `destinationIpv6CidrBlock`: The IPv6 CIDR range for the route. The value you specify must
  match the CIDR for the route exactly.
- `dryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
...
- `dryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `tag`: The tags to delete. Specify a tag key and an optional tag value to delete specific
  tags. If you specify a tag key without a tag value, we delete any tag with this key
  regardless of its value. If you specify a tag key with an empty string as the tag value, we
  delete the tag only if its value is an empty string. If you omit this parameter, we delete
  all user-defined tags for the specified resources. We do not delete AWS-generated tags
...
# Optional Parameters
- `AllAvailabilityZones`: Include all Availability Zones, Local Zones, and Wavelength Zones
  regardless of your opt-in status. If you do not use this parameter, the results include
...
- `dryRun`: Checks whether you have the required permissions for the action, without
  ...
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `maxResults`: The maximum number of results to return with a single call. To retrieve the
  remaining results, make another call with the returned nextToken value. Constraint: If the
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters. Filter names and values are case-sensitive.    description
  - The description of the authorization rule.    destination-cidr - The CIDR of the network
  to which the authorization rule applies.    group-id - The ID of the Active Directory group
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters. Filter names and values are case-sensitive.    endpoint-id
  - The ID of the Client VPN endpoint.    transport-protocol - The transport protocol (tcp |
  udp).
- `MaxResults`: The maximum number of results to return for the request in a single page.
  The remaining results can be seen by sending another request with the nextToken value.
- `NextToken`: The token to retrieve the next page of results.
...
- `DhcpOptionsId`: The IDs of one or more DHCP options sets. Default: Describes all your
  DHCP options sets.
- `Filter`: One or more filters.    dhcp-options-id - The ID of a DHCP options set.    key
  - The key for one of the options (for example, domain-name).    value - The value for one
  of the options.    owner-id - The ID of the AWS account that owns the DHCP options set.
  tag:&lt;key&gt; - The key/value combination of a tag assigned to the resource. Use the tag
  key in the filter name and the tag value as the filter value. For example, to find all
  resources that have a tag with the key Owner and the value TeamA, specify tag:Owner for the
  filter name and TeamA for the filter value.    tag-key - The key of a tag assigned to the
...
- `Filter`: The filters.    availability-zone - The Availability Zone in which the Elastic
...
# Required Parameters
- `principalArn`: The ARN of the principal, which can be an IAM role, IAM user, or the root
  user.
...
- `Filter`: The filters.    architecture - The image architecture (i386 | x86_64 | arm64).
  ...
  block-device-mapping.volume-type - The volume type of the EBS volume (gp2 | io1 | io2 | st1
  | sc1 | standard).    block-device-mapping.encrypted - A Boolean that indicates whether the
  EBS volume is encrypted.    description - The description of the image (provided during
  image creation).    ena-support - A Boolean that indicates whether enhanced networking with
  ENA is enabled.    hypervisor - The hypervisor type (ovm | xen).    image-id - The ID of
  the image.    image-type - The image type (machine | kernel | ramdisk).    is-public - A
  Boolean that indicates whether the image is public.    kernel-id - The kernel ID.
  manifest-location - The location of the image manifest.    name - The name of the AMI
  (provided during image creation).    owner-alias - The owner alias (amazon |
  aws-marketplace). The valid aliases are defined in an Amazon-maintained list. This is not
  the AWS account alias that can be set using the IAM console. We recommend that you use the
  Owner request parameter instead of this filter.    owner-id - The AWS account ID of the
  ...
  The product code.    product-code.type - The type of the product code (devpay |
  marketplace).    ramdisk-id - The RAM disk ID.    root-device-name - The device name of the
  root device volume (for example, /dev/sda1).    root-device-type - The type of the root
  device volume (ebs | instance-store).    state - The state of the image (available |
...
# Optional Parameters
- `Filter`: One or more filters.    attachment.state - The current state of the attachment
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters.    create-time - The time the launch template version was
...
- `LaunchTemplateId`: The ID of the launch template. To describe one or more versions of a
  specified launch template, you must specify either the launch template ID or the launch
...
- `LaunchTemplateName`: The name of the launch template. To describe one or more versions
  ...
  versions in your account, you must omit this parameter.
- `LaunchTemplateVersion`: One or more versions of the launch template. Valid values depend
  on whether you are describing a specified launch template (by ID or name) or all launch
  templates in your account. To describe one or more versions of a specified launch template,
  valid values are Latest, Default, and numbers. To describe all launch templates in your
  account that are defined as the latest version, the valid value is Latest. To describe all
  launch templates in your account that are defined as the default version, the valid value
  is Default. You can specify Latest and Default in the same call. You cannot specify numbers.
- `MaxResults`: The maximum number of results to return in a single call. To retrieve the
  remaining results, make another call with the returned NextToken value. This value can be
  between 1 and 200.
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters.    create-time - The time the launch template was created.
     launch-template-name - The name of the launch template.    tag:&lt;key&gt; - The
...
# Optional Parameters
- `MaxResults`: The maximum number of items to return for this request. The request returns
  a token that you can specify in a subsequent call to get the next set of results. You
  cannot specify this parameter and the network interface IDs parameter in the same request.
...
- `MaxResults`: The maximum number of results to return in a single call. This value can be
  between 5 and 300. The default value is 300. To retrieve the remaining results, make
  another call with the returned NextToken value.
...
- `Filter`: The filters.    availability-zone - The Availability Zone (for example,
  ...
  platform (Linux/UNIX or Windows).
- `MaxResults`: The maximum number of results to return in a single call. This value can be
  between 5 and 300. The default value is 100. To retrieve the remaining results, make
  another call with the returned NextToken value.
- `NextToken`: The token for the next set of results.
...
- `GroupName`: [EC2-Classic and default VPC only] The names of the security groups. You can
  specify either the security group name or the security group ID. For security groups in a
  nondefault VPC, use the group-name filter to describe security groups by name. Default:
...
# Optional Parameters
- `Filter`: One or more filters.    availability-zone-group - The Availability Zone group.
...
- `MaxResults`: The maximum number of results to return in a single call. Specify a value
  ...
  NextToken value.
- `NextToken`: The token to request the next set of results. This value is null when there
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters. The possible values are:    association.state - The state
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters. The possible values are:    options.protocol - The tunnel
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters. The possible values are:    transit-gateway-attachment-id
  - The ID of the transit gateway attachment.    local-owner-id - The ID of your AWS account.
     remote-owner-id - The ID of the AWS account in the remote Region that owns the transit
  gateway.    state - The state of the peering attachment. Valid values are available |
  deleted | deleting | failed | failing | initiatingRequest | modifying | pendingAcceptance |
  pending | rollingBack | rejected | rejecting).    tag:&lt;key&gt; - The key/value
  combination of a tag assigned to the resource. Use the tag key in the filter name and the
  tag value as the filter value. For example, to find all resources that have a tag with the
  key Owner and the value TeamA, specify tag:Owner for the filter name and TeamA for the
  filter value.    tag-key - The key of a tag assigned to the resource. Use this filter to
  find all resources that have a tag with a specific key, regardless of the tag value.
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: The filters.    modification-state - The current modification state (modifying
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters.    connection-notification-arn - The ARN of the SNS topic
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters.    principal - The ARN of the principal.    principal-type
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters.    service-name - The name of the service.    service-type
  - The type of service (Interface | Gateway).    tag:&lt;key&gt; - The key/value combination
  of a tag assigned to the resource. Use the tag key in the filter name and the tag value as
  the filter value. For example, to find all resources that have a tag with the key Owner and
  the value TeamA, specify tag:Owner for the filter name and TeamA for the filter value.
  tag-key - The key of a tag assigned to the resource. Use this filter to find all resources
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters.    service-name - The name of the service.    vpc-id - The
  ID of the VPC in which the endpoint resides.    vpc-endpoint-id - The ID of the endpoint.
   vpc-endpoint-state - The state of the endpoint (pendingAcceptance | pending | available |
  deleting | deleted | rejected | failed).    vpc-endpoint-type - The type of VPC endpoint
  (Interface | Gateway | GatewayLoadBalancer).    tag:&lt;key&gt; - The key/value combination
  of a tag assigned to the resource. Use the tag key in the filter name and the tag value as
  the filter value. For example, to find all resources that have a tag with the key Owner and
  the value TeamA, specify tag:Owner for the filter name and TeamA for the filter value.
  tag-key - The key of a tag assigned to the resource. Use this filter to find all resources
...
- `MaxResults`: The maximum number of results to return with a single call. To retrieve the
  remaining results, make another call with the returned nextToken value.
- `NextToken`: The token for the next page of results.
- `VpcPeeringConnectionId`: One or more VPC peering connection IDs. Default: Describes all
...
# Optional Parameters
- `Filter`: One or more filters.    cidr - The primary IPv4 CIDR block of the VPC. The CIDR
  block you specify must exactly match the VPC's CIDR block for information to be returned
  for the VPC. Must contain the slash followed by one or two digits (for example, /28).
...
- `InstanceId`: The ID of the instance. If you are detaching a Multi-Attach enabled volume,
...
- `ClientVpnEndpointId`: The ID of the Client VPN endpoint from which to disassociate the
  target network.
...
# Required Parameters
- `AvailabilityZone`: One or more Availability Zones. For example, us-east-2a.
- `SourceSnapshotId`: The IDs of one or more snapshots. For example,
...
# Required Parameters
- `GatewayId`: The ID of the virtual private gateway that is attached to a VPC. The virtual
  private gateway must be attached to the same VPC that the routing tables are associated
  with.
- `RouteTableId`: The ID of the route table. The routing table must be associated with the
...
- `S3ExportLocation`: Information about the destination Amazon S3 bucket. The bucket must
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `RoleName`: The name of the role that grants VM Import/Export permission to export images
  to your Amazon S3 bucket. If this parameter is not specified, the default role is named
  'vmimport'.
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `Filter`: One or more filters. The possible values are:    transit-gateway-route-table-id
...
# Required Parameters
- `CertificateRevocationList`: The client certificate revocation list file. For more
  information, see Generate a Client Certificate Revocation List in the AWS Client VPN
  Administrator Guide.
- `ClientVpnEndpointId`: The ID of the Client VPN endpoint to which the client certificate
...
- `KmsKeyId`: An identifier for the symmetric AWS Key Management Service (AWS KMS) customer
  master key (CMK) to use when creating the encrypted AMI. This parameter is only required if
  you want to use a non-default CMK; if this parameter is not specified, the default CMK for
  EBS is used. If a KmsKeyId is specified, the Encrypted flag must also be set.  The CMK
  identifier may be provided in any of the following formats:    Key ID   Key alias. The
  alias ARN contains the arn:aws:kms namespace, followed by the Region of the CMK, the AWS
  account ID of the CMK owner, the alias namespace, and then the CMK alias. For example,
  arn:aws:kms:us-east-1:012345678910:alias/ExampleAlias.   ARN using key ID. The ID ARN
  contains the arn:aws:kms namespace, followed by the Region of the CMK, the AWS account ID
  of the CMK owner, the key namespace, and then the CMK ID. For example,
...
- `publicKeyMaterial`: The public key. For API calls, the text must be base64-encoded. For
...
- `Encrypted`: Specifies whether the destination snapshot of the imported image should be
  ...
  Encryption in the Amazon Elastic Compute Cloud User Guide.
- `KmsKeyId`: An identifier for the symmetric AWS Key Management Service (AWS KMS) customer
  master key (CMK) to use when creating the encrypted snapshot. This parameter is only
...
# Required Parameters
- `GroupName`: The name of the Availability Zone group, Local Zone group, or Wavelength
  Zone group.
- `OptInStatus`: Indicates whether you are opted in to the Local Zone group or Wavelength
...
# Required Parameters
- `CpuCredits`: The credit option for CPU usage of the instance family. Valid Values:
  standard | unlimited
- `InstanceFamily`: The instance family.
...
# Required Parameters
- `principalArn`: The ARN of the principal, which can be an IAM user, IAM role, or the root
  user. Specify all to modify the ID format for all IAM users, IAM roles, and the root user
  of the account.
- `resource`: The type of resource: bundle | conversion-task | customer-gateway |
  dhcp-options | elastic-ip-allocation | elastic-ip-association | export-task | flow-log |
  image | import-task | internet-gateway | network-acl | network-acl-association |
...
- `ProductCode`: The DevPay product codes. After you add a product code to an AMI, it can't
  be removed.
...
- `disableApiTermination`: If the value is true, you can't terminate the instance using the
  Amazon EC2 console, CLI, or API; otherwise, you can. You cannot use this parameter for Spot
  Instances.
- `dryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
...
- `userData`: Changes the instance's user data to the specified value. If you are using an
  AWS SDK or command line tool, base64-encoding is performed for you, and you can load the
...
- `HttpPutResponseHopLimit`: The desired HTTP PUT response hop limit for instance metadata
  ...
  parameter is specified, the existing state is maintained. Possible values: Integers from 1
  to 64
...
# Required Parameters
- `ReservedInstancesConfigurationSetItemType`: The configuration settings for the Reserved
  Instances to modify.
...
- `DestinationCidrBlock`: The destination CIDR block to assign to the Traffic Mirror rule.
- `DestinationPortRange`: The destination ports that are associated with the Traffic Mirror
  rule.
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
...
- `SessionNumber`: The session number determines the order in which sessions are evaluated
  ...
  the one that mirrors the packets. Valid values are 1-32766.
- `TrafficMirrorFilterId`: The ID of the Traffic Mirror filter.
- `TrafficMirrorTargetId`: The Traffic Mirror target. The target must be in the same VPC as
...
# Optional Parameters
- `AddSubnetIds`: The IDs of one or more subnets to add. You can specify at most one subnet
...
- `MultiAttachEnabled`: Specifies whether to enable Amazon EBS Multi-Attach. If you enable
  Multi-Attach, you can attach the volume to up to 16  Nitro-based instances in the same
  Availability Zone. This parameter is supported with io1 and io2 volumes only. For more
...
- `Throughput`: The target throughput of the volume, in MiB/s. This parameter is valid only
  for gp3 volumes. The maximum value is 1,000. Default: If no throughput value is specified,
  the existing value is retained. Valid Range: Minimum value of 125. Maximum value of 1000.
- `VolumeType`: The target EBS volume type of the volume. For more information, see Amazon
  EBS volume types in the Amazon Elastic Compute Cloud User Guide. Default: If no type is
  specified, the existing type is retained.
"""
...
# Optional Parameters
- `AddRouteTableId`: (Gateway endpoint) One or more route tables IDs to associate with the
  endpoint.
- `AddSecurityGroupId`: (Interface endpoint) One or more security group IDs to associate
  with the network interface.
- `AddSubnetId`: (Interface and Gateway Load Balancer endpoints) One or more subnet IDs in
  which to serve the endpoint. For a Gateway Load Balancer endpoint, you can specify only one
  subnet.
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
...
- `RemoveRouteTableId`: (Gateway endpoint) One or more route table IDs to disassociate from
  the endpoint.
...
- `DryRun`: Checks whether you have the required permissions for the action, without
  actually making the request, and provides an error response. If you have the required
  permissions, the error response is DryRunOperation. Otherwise, it is UnauthorizedOperation.
- `RequesterPeeringConnectionOptions`: The VPC peering connection options for the requester
  VPC.
"""
...
# Required Parameters
- `DestinationCidrBlock`: The CIDR range used for the destination match. Routing decisions
  are based on the most specific match.
- `TransitGatewayRouteTableId`: The ID of the route table.
...
- `reasonCode`: The reason codes that describe the health state of your instance.
...
- `LaunchSpecification`: The launch specification.
- `TagSpecification`: The key-value pair for tagging the Spot Instance request on creation.
  The value for ResourceType must be spot-instances-request, otherwise the Spot Instance
  request fails. To tag the Spot Instance request after it has been created, see CreateTags.
- `availabilityZoneGroup`: The user-specified name for a logical grouping of requests. When
  you specify an Availability Zone group in a Spot Instance request, all Spot Instances in
  the request are launched in the same Availability Zone. Instance proximity is maintained
...
- `blockDurationMinutes`: The required duration for the Spot Instances (also known as Spot
  blocks), in minutes. This value must be a multiple of 60 (60, 120, 180, 240, 300, or 360).
  The duration period starts as soon as your Spot Instance receives its instance ID. At the
  end of the duration period, Amazon EC2 marks the Spot Instance for termination and provides
  a Spot Instance termination notice, which gives the instance a two-minute warning before it
  terminates. You can't specify an Availability Zone group or a launch group if you specify a
  duration. New accounts or accounts with no previous billing history with AWS are not
...
- `validUntil`: The end date of the request, in UTC format (YYYY-MM-DDTHH:MM:SSZ).   For a
...
# Required Parameters
- `ClientVpnEndpointId`: The ID of the Client VPN endpoint with which the authorization
  rule is associated.
- `TargetNetworkCidr`: The IPv4 address range, in CIDR notation, of the network for which
...
- `IpProtocol`: The IP protocol name (tcp, udp, icmp) or number (see Protocol Numbers). Use
  -1 to specify all.
- `SourceSecurityGroupName`: [EC2-Classic, default VPC] The name of the source security
  group. You can't specify this parameter in combination with the following parameters: the
  CIDR IP address range, the start of the port range, the IP protocol, and the end of the
...
# Required Parameters
- `MaxCount`: The maximum number of instances to launch. If you specify more instances than
  ...
  the Amazon EC2 FAQ.
- `MinCount`: The minimum number of instances to launch. If you specify a minimum that is
...
- `CpuOptions`: The CPU options for the instance. For more information, see Optimizing CPU
...
- `CreditSpecification`: The credit option for CPU usage of the burstable performance
  ...
  (T3/T3a instances)
- `ElasticGpuSpecification`: An elastic GPU to associate with the instance. An Elastic GPU
  is a GPU resource that you can attach to your Windows instance to accelerate the graphics
  performance of your applications. For more information, see Amazon EC2 Elastic GPUs in the
  Amazon EC2 User Guide.
...
- `HibernationOptions`: Indicates whether an instance is enabled for hibernation. For more
  information, see Hibernate your instance in the Amazon EC2 User Guide. You can't enable
  hibernation and AWS Nitro Enclaves on the same instance.
...
- `InstanceType`: The instance type. For more information, see Instance types in the Amazon
  EC2 User Guide. Default: m1.small
...
- `Ipv6AddressCount`: [EC2-VPC] The number of IPv6 addresses to associate with the primary
  network interface. Amazon EC2 chooses the IPv6 addresses from the range of your subnet. You
  cannot specify this option and the option to assign specific IPv6 addresses in the same
  request. You can specify this option if you've specified a minimum number of instances to
  launch. You cannot specify this option and the network interfaces option in the same
  request.
...
- `LaunchTemplate`: The launch template to use to launch the instances. Any parameters that
  you specify in RunInstances override the same parameters in the launch template. You can
...
- `SecurityGroupId`: The IDs of the security groups. You can create a security group using
  CreateSecurityGroup. If you specify a network interface, you must specify any security
  groups as part of the network interface.
- `SubnetId`: [EC2-VPC] The ID of the subnet to launch the instance into. If you specify a
...
- `clientToken`: Unique, case-sensitive identifier you provide to ensure the idempotency of
  the request. If you do not specify a client token, a randomly generated token is used for
  the request to ensure idempotency. For more information, see Ensuring Idempotency.
...
- `networkInterface`: The network interfaces to associate with the instance. If you specify
  a network interface, you must specify any security groups and subnets as part of the
  network interface.
- `privateIpAddress`: [EC2-VPC] The primary IPv4 address. You must specify a value from the
  IPv4 address range of the subnet. Only one private IP address can be designated as primary.
  You can't specify this option if you've specified the option to designate a private IP
  address as the primary IP address in a network interface specification. You cannot specify
  this option if you're launching more than one instance in the request. You cannot specify
...
# Required Parameters
- `LaunchSpecification`: The launch specification. You must match the instance type,
...
# Required Parameters
- `InstanceId`: The IDs of the instances. Constraints: Up to 1000 instance IDs. We
