...
const SERVICE_PARAMETER_MAP = AWS.LittleDict("duration_seconds" => "DurationSeconds", "policy" => "Policy", "policy_arns" => "PolicyArns", "tags" => "Tags", "external_id" => "ExternalId", "serial_number" => "SerialNumber", "source_identity" => "SourceIdentity", "token_code" => "TokenCode", "transitive_tag_keys" => "TransitiveTagKeys", "provider_id" => "ProviderId")
...
User Guide.  Permissions  The temporary security credentials created by AssumeRole can be
used to make API calls to any Amazon Web Services service with the following exception: You
cannot call the Amazon Web Services STS GetFederationToken or GetSessionToken API
...
policies. You can use the role's temporary credentials in subsequent Amazon Web Services
API calls to access resources in the account that owns the role. You cannot use session
policies to grant more permissions than those allowed by the identity-based policy of the
role that is being assumed. For more information, see Session Policies in the IAM User
Guide. When you create a role, you create two policies: A role trust policy that specifies
who can assume the role and a permissions policy that specifies what can be done with the
role. You specify the trusted principal who is allowed to assume the role in the role trust
policy. To assume a role from a different account, your Amazon Web Services account must be
trusted by the role. The trust relationship is defined in the role's trust policy when the
role is created. That trust policy states which accounts are allowed to delegate that
access to users in the account.  A user who wants to access a role in a different account
must also have permissions that are delegated from the user account administrator. The
administrator must attach a policy that allows the user to call AssumeRole for the ARN of
the role in the other account. To allow a user to assume a role in the same account, you
can do either of the following:   Attach a policy to the user that allows the user to call
AssumeRole (as long as the role's trust policy trusts the account).   Add the user as a
principal directly in the role's trust policy.   You can do either because the role’s
trust policy acts as an IAM resource-based policy. When a resource-based policy grants
access to a principal in the same account, no additional identity-based policy is required.
For more information about trust policies and resource-based policies, see IAM Policies in
the IAM User Guide.  Tags  (Optional) You can pass tag key-value pairs to your session.
These tags are called session tags. For more information about session tags, see Passing
Session Tags in STS in the IAM User Guide. An administrator must grant you the permissions
...
Tags for Attribute-Based Access Control in the IAM User Guide. You can set the session tags
...
- `policy_arns`: The Amazon Resource Names (ARNs) of the IAM managed policies that you want
  ...
  IAM User Guide.
- `serial_number`: The identification number of the MFA device that is associated with the
...
- `tags`: A list of session tags that you want to pass. Each session tag consists of a key
  name and an associated value. For more information about session tags, see Tagging Amazon
  Web Services STS Sessions in the IAM User Guide. This parameter is optional. You can pass
  up to 50 session tags. The plaintext session tag keys can’t exceed 128 characters, and
  the values can’t exceed 256 characters. For these and additional limits, see IAM and STS
  Character Limits in the IAM User Guide.  An Amazon Web Services conversion compresses the
  ...
  tags override a role tag with the same key.  Tag key–value pairs are not case sensitive,
  but case is preserved. This means that you cannot have separate Department and department
  tag keys. Assume that the role has the Department=Marketing tag and you pass the
...
- `token_code`: The value provided by the MFA device, if the trust policy of the role being
...
"""
function assume_role(RoleArn, RoleSessionName; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...)
    params = amazonify(SERVICE_PARAMETER_MAP, kwargs)
    return sts("AssumeRole", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("RoleArn"=>RoleArn, "RoleSessionName"=>RoleSessionName), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
has a separate limit. Your request can fail for this limit even if your plaintext meets the
other requirements. The PackedPolicySize response element indicates by percentage how close
the policies and tags for your request are to the upper size limit.   You can pass a
session tag with the same key as a tag that is attached to the role. When you do, session
tags override the role's tags with the same key. An administrator must grant you the
...
information, see Chaining Roles with Session Tags in the IAM User Guide.  SAML
Configuration  Before your application can call AssumeRoleWithSAML, you must configure your
...
entity in your Amazon Web Services account that represents your identity provider. You must
also create an IAM role that specifies this SAML provider in its trust policy.  For more
information, see the following resources:    About SAML 2.0-based Federation in the IAM
User Guide.     Creating SAML Identity Providers in the IAM User Guide.     Configuring a
Relying Party and Claims in the IAM User Guide.     Creating a Role for SAML 2.0 Federation
...
- `policy_arns`: The Amazon Resource Names (ARNs) of the IAM managed policies that you want
  ...
  IAM User Guide.
"""
function assume_role_with_saml(PrincipalArn, RoleArn, SAMLAssertion; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...)
    params = amazonify(SERVICE_PARAMETER_MAP, kwargs)
    return sts("AssumeRoleWithSAML", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("PrincipalArn"=>PrincipalArn, "RoleArn"=>RoleArn, "SAMLAssertion"=>SAMLAssertion), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- `provider_id`: The fully qualified host component of the domain name of the identity
  ...
  ID tokens.
"""
function assume_role_with_web_identity(RoleArn, RoleSessionName, WebIdentityToken; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...)
    params = amazonify(SERVICE_PARAMETER_MAP, kwargs)
    return sts("AssumeRoleWithWebIdentity", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("RoleArn"=>RoleArn, "RoleSessionName"=>RoleSessionName, "WebIdentityToken"=>WebIdentityToken), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
"""
function decode_authorization_message(EncodedMessage; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...)
    params = amazonify(SERVICE_PARAMETER_MAP, kwargs)
    return sts("DecodeAuthorizationMessage", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("EncodedMessage"=>EncodedMessage), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
"""
function get_access_key_info(AccessKeyId; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...)
    params = amazonify(SERVICE_PARAMETER_MAP, kwargs)
    return sts("GetAccessKeyInfo", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AccessKeyId"=>AccessKeyId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function get_caller_identity(; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...)
    params = amazonify(SERVICE_PARAMETER_MAP, kwargs)
    return sts("GetCallerIdentity", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
- `policy_arns`: The Amazon Resource Names (ARNs) of the IAM managed policies that you want
  ...
  Web Services General Reference. This parameter is optional. However, if you do not pass any
  session policies, then the resulting federated user session has no permissions. When you
  pass session policies, the session permissions are the intersection of the IAM user
  policies and the session policies that you pass. This gives you a way to further restrict
  the permissions for a federated user. You cannot use session policies to grant more
  permissions than those that are defined in the permissions policy of the IAM user. For more
  information, see Session Policies in the IAM User Guide. The resulting credentials can be
  used to access a resource that has a resource-based policy. If that policy specifically
  references the federated user session in the Principal element of the policy, the session
  has the permissions allowed by the policy. These permissions are granted in addition to the
  permissions that are granted by the session policies.  An Amazon Web Services conversion
  compresses the passed session policies and session tags into a packed binary format that
  has a separate limit. Your request can fail for this limit even if your plaintext meets the
  other requirements. The PackedPolicySize response element indicates by percentage how close
  the policies and tags for your request are to the upper size limit.
- `tags`: A list of session tags. Each session tag consists of a key name and an associated
  ...
  User Guide. This parameter is optional. You can pass up to 50 session tags. The plaintext
  ...
  tags, and the session tag passed in the request takes precedence over the role tag.
"""
function get_federation_token(Name; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...)
    params = amazonify(SERVICE_PARAMETER_MAP, kwargs)
    return sts("GetFederationToken", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
end
...
function get_session_token(; aws_config::AbstractAWSConfig=global_aws_config(), kwargs...)
    params = amazonify(SERVICE_PARAMETER_MAP, kwargs)
    return sts("GetSessionToken", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
end
