...
Adds or overwrites one or more tags for the specified resource. *Tags* are metadata that
you can assign to your automations, documents, managed nodes, maintenance windows,
...
consists of a key and an optional value, both of which you define. For example, you could
define a set of tags for your account's managed nodes that helps you track each node's
owner and stack level. For example:</p> - `Key=Owner,Value=DbAdmin`
...
You can search and filter the resources based on the tags you add. Tags don't have any
semantic meaning to and are interpreted strictly as a string of characters.

 <p>For more information about using tags with Amazon Elastic Compute Cloud (Amazon EC2)
instances, see [Tag your Amazon EC2 resources](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/Using_Tags.html)
...
- `resource_id`: The resource ID you want to tag.
  ...
  (ARN) for the object. Specifically, `ResourceID` is created from the strings that come
  after the word `opsmetadata` in the ARN. For example, an OpsMetadata object with an ARN
  of `arn:aws:ssm:us-east-2:1234567890:opsmetadata/aws/ssm/MyGroup/appmanager` has a
  ...
  nodes. You must specify the name of the managed node in the following format: `mi-
  *ID_number* `. For example, `mi-1a2b3c4d5e6f`.
- `resource_type`: Specifies the type of resource you are tagging.

  !!! note
      The `ManagedInstance` type for this API operation is for on-premises managed nodes.
  You must specify the name of the managed node in the following format: `mi-*ID_number* `.
...
"""
add_tags_to_resource(ResourceId, ResourceType, Tags; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("AddTagsToResource", Dict{String, Any}("ResourceId"=>ResourceId, "ResourceType"=>ResourceType, "Tags"=>Tags); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
add_tags_to_resource(ResourceId, ResourceType, Tags, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("AddTagsToResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourceId"=>ResourceId, "ResourceType"=>ResourceType, "Tags"=>Tags), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
cancel_command(CommandId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CancelCommand", Dict{String, Any}("CommandId"=>CommandId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
cancel_command(CommandId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CancelCommand", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CommandId"=>CommandId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
cancel_maintenance_window_execution(WindowExecutionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CancelMaintenanceWindowExecution", Dict{String, Any}("WindowExecutionId"=>WindowExecutionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
cancel_maintenance_window_execution(WindowExecutionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CancelMaintenanceWindowExecution", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowExecutionId"=>WindowExecutionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
managing on-premises machines using Systems Manager, see [Setting up Amazon Web Services Systems Manager for hybrid and multicloud environments](https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-managedinstances.html)
...
- `"Tags"`: Optional metadata that you assign to a resource. Tags enable you to categorize
  ...
  might want to tag an activation to identify which servers or virtual machines (VMs) in
  your on-premises environment you intend to activate. In this case, you could specify the
  ...
  activation are automatically applied to the on-premises servers or VMs.You can't add tags
  to or delete tags from an existing activation. You can tag your on-premises servers, edge
  devices, and VMs after they connect to Systems Manager for the first time and are
  assigned a managed node ID. This means they are listed in the Amazon Web Services Systems
  Manager console with an ID that is prefixed with "mi-". For information about how to add
  tags to your managed nodes, see <a>AddTagsToResource</a>. For information about how to
  remove tags from your managed nodes, see <a>RemoveTagsFromResource</a>.
"""
create_activation(IamRole; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateActivation", Dict{String, Any}("IamRole"=>IamRole); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_activation(IamRole, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateActivation", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("IamRole"=>IamRole), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `name`: The name of the SSM Command document or Automation runbook that contains the
  ...
  For Systems Manager documents (SSM documents) that are shared with you from other Amazon
  ...
  account, you only need to specify the document name. For example, `AWS-
  ApplyPatchBaseline` or `My-Document`.
...
- `"DocumentVersion"`: The document version you want to associate with the targets. Can be
  ...
  document if that document is shared from another account. State Manager always runs the
  `default` version of a document if shared from another account, even though the Systems
  Manager console shows that a new version was processed. If you want to run an association
...
- `"Duration"`: The number of hours the association can run before it is canceled. Duration
  ...
  made available again immediately, without a reboot.

  The `Duration` parameter applies only when both these conditions are true: - The
  association for which you specify a duration is cancelable according to the parameters of
...
- `"InstanceId"`: The managed node ID.
  ...
  use the `Targets` parameter. Requests that include the parameter `InstanceID` with
  Systems Manager documents (SSM documents) that use schema version 2.0 or later will fail.
  In addition, if you use the parameter `InstanceId`, you can't use the parameters
...
- `"MaxConcurrency"`: The maximum number of targets allowed to run the association at the
  same time. You can specify a number, for example 10, or a percentage of the target set,
  for example 10%. The default value is 100%, which means all targets run the association
  at the same time.
...
- `"MaxErrors"`: The number of errors that are allowed before the system stops sending
  ...
  is returned. If you run an association on 50 managed nodes and set `MaxError` to 10%,
...
- `"ScheduleOffset"`: Number of days to wait after the scheduled day to run an association.
  For example, if you specified a cron schedule of `cron(0 0 ? * THU#2 *)`, you could
  specify an offset of 3 to run the association each Sunday after the second Thursday of
  the month. For more information about cron schedules for associations, see [Reference: Cron and rate expressions for Systems Manager](https://docs.aws.amazon.com/systems-manager/latest/userguide/reference-cron-and-rate-expressions.html)
  in the *Amazon Web Services Systems Manager User Guide*.
...
- `"SyncCompliance"`: The mode for generating association compliance. You can specify
  `AUTO` or `MANUAL`. In `AUTO` mode, the system uses the status of the association
  execution to determine the compliance status. If the association execution runs
  successfully, then the association is `COMPLIANT`. If the association execution doesn't
  ...
  <a>PutComplianceItems</a> API operation. In this case, compliance data isn't managed by
  State Manager. It is managed by your direct call to the <a>PutComplianceItems</a> API
  operation.
...
- `"Targets"`: The targets for the association. You can target managed nodes by using tags,
  ...
  information about choosing targets for an association, see [About targets and rate controls in State Manager associations](https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-state-manager-targets-and-rate-controls.html)
  in the *Amazon Web Services Systems Manager User Guide*.
"""
create_association(Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateAssociation", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_association(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateAssociation", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
the specified managed nodes or targets.

When you associate a document with one or more managed nodes using IDs or tags, Amazon Web
Services Systems Manager Agent (SSM Agent) running on the managed node processes the
...
"""
create_association_batch(Entries; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateAssociationBatch", Dict{String, Any}("Entries"=>Entries); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_association_batch(Entries, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateAssociationBatch", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Entries"=>Entries), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `content`: The content for the new SSM document in JSON or YAML format. The content of
  the document must not exceed 64KB. This quota also includes the content specified for
  input parameters at runtime. We recommend storing the contents for your new document in
  an external JSON or YAML file and referencing the file in a command.

  For examples, see the following topics in the *Amazon Web Services Systems Manager User
  Guide*. - [Create an SSM document (console)](https://docs.aws.amazon.com/systems-manager/latest/userguide/documents-using.html#create-ssm-console)
...
- `name`: A name for the SSM document.
  ...
  Amazon Web Services for use as document name prefixes: - `aws`
   - `amazon`
 - `amzn`
 - `AWSEC2`
...
- `"DisplayName"`: An optional field where you can specify a friendly name for the SSM
  ...
  value at a later time using the <a>UpdateDocument</a> operation.
...
- `"Requires"`: A list of SSM documents required by a document. This parameter is used
  ...
  document for validation purposes. For more information, see [What is AppConfig?](https://docs.aws.amazon.com/appconfig/latest/userguide/what-is-appconfig.html)
  in the *AppConfig User Guide*.
- `"Tags"`: Optional metadata that you assign to a resource. Tags enable you to categorize
  a resource in different ways, such as by purpose, owner, or environment. For example, you
  might want to tag an SSM document to identify the types of targets or the environment
  where it will run. In this case, you could specify the following key-value pairs: -
...
- `"TargetType"`: Specify a target type to define the kinds of resources the document can
  ...
  list of valid resource types, see [Amazon Web Services resource and property types reference](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-template-resource-type-ref.html)
...
"""
create_document(Content, Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateDocument", Dict{String, Any}("Content"=>Content, "Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_document(Content, Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateDocument", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Content"=>Content, "Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
value you specify for `Cutoff` is one hour, no maintenance window tasks can start after 5
PM.
...
- `"ScheduleTimezone"`: The time zone that the scheduled maintenance window executions are
  ...
  "America/Los_Angeles", "UTC", or "Asia/Seoul". For more information, see the [Time Zone Database](https://www.iana.org/time-zones)
  on the IANA website.
...
- `"Tags"`: Optional metadata that you assign to a resource. Tags enable you to categorize
  a resource in different ways, such as by purpose, owner, or environment. For example, you
  might want to tag a maintenance window to identify the type of tasks it will run, the
  types of targets, and the environment it will run in. In this case, you could specify the
  following key-value pairs: - `Key=TaskType,Value=AgentUpdate`
   - `Key=OS,Value=Windows`
   - `Key=Environment,Value=Production`


  !!! note
      To add tags to an existing maintenance window, use the <a>AddTagsToResource</a>
  operation.
"""
...
create a new OpsItem. For more information, see [Set up OpsCenter](https://docs.aws.amazon.com/systems-manager/latest/userguide/OpsCenter-setup.html)
in the *Amazon Web Services Systems Manager User Guide*.
...
of their Amazon Web Services resources. For more information, see [Amazon Web Services Systems Manager OpsCenter](https://docs.aws.amazon.com/systems-manager/latest/userguide/OpsCenter.html)
in the *Amazon Web Services Systems Manager User Guide*.
...
- `"OperationalData"`: Operational data is custom data that provides useful reference
  ...
  keys, troubleshooting tips, or other relevant data. You enter operational data as key-
  value pairs. The key has a maximum length of 128 characters. The value has a maximum size
  of 20 KB.</p>

  !!! important
      Operational data keys *can't* begin with the following: `amazon`, `aws`, `amzn`,
  `ssm`, `/amazon`, `/aws`, `/amzn`, `/ssm`.You can choose to make the data searchable by
  other users in the account or you can restrict search access. Searchable data means that
  all users with access to the OpsItem Overview page (as provided by the
  <a>DescribeOpsItems</a> API operation) can view and search on the specified data.
  Operational data that isn't searchable is only viewable by users who have access to the
  OpsItem (as provided by the <a>GetOpsItem</a> API operation).

   <p>Use the `/aws/resources` key in OperationalData to specify a related resource in the
  ...
  keys, see [Create OpsItems manually](https://docs.aws.amazon.com/systems-manager/latest/userguide/OpsCenter-manually-create-OpsItems.html)
  in the *Amazon Web Services Systems Manager User Guide*.
- `"OpsItemType"`: The type of OpsItem to create. Systems Manager supports the following
  types of OpsItems:</p> - `/aws/issue`
...
- `"Tags"`: Optional metadata that you assign to a resource.
  ...
  an existing OpsItem, use the <a>AddTagsToResource</a> operation.
"""
create_ops_item(Description, Source, Title; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateOpsItem", Dict{String, Any}("Description"=>Description, "Source"=>Source, "Title"=>Title); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_ops_item(Description, Source, Title, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateOpsItem", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Description"=>Description, "Source"=>Source, "Title"=>Title), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"Tags"`: Optional metadata that you assign to a resource. You can specify a maximum of
  ...
  different ways, such as by purpose, owner, or environment. For example, you might want to
  tag an OpsMetadata object to identify an environment or target Amazon Web Services
  Region. In this case, you could specify the following key-value pairs: -
  `Key=Environment,Value=Production`
 - `Key=Region,Value=us-east-2`
"""
create_ops_metadata(ResourceId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateOpsMetadata", Dict{String, Any}("ResourceId"=>ResourceId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_ops_metadata(ResourceId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateOpsMetadata", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourceId"=>ResourceId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"ApprovedPatches"`: A list of explicitly approved patches for the baseline.
  ...
  patches, see [About package name formats for approved and rejected patch lists](https://docs.aws.amazon.com/systems-manager/latest/userguide/patch-manager-approved-rejected-package-name-formats.html)
  in the *Amazon Web Services Systems Manager User Guide*.
...
- `"RejectedPatchesAction"`: The action for Patch Manager to take on patches included in
  the `RejectedPackages` list.</p> <dl> <dt>ALLOW_AS_DEPENDENCY</dt> <dd> **Linux and
  macOS**: A package in the rejected patches list is installed only if it is a dependency
  of another package. It is considered compliant with the patch baseline, and its status is
  reported as `INSTALLED_OTHER`. This is the default action if no option is specified.

   <p> **Windows Server**: Windows Server doesn't support the concept of package
  dependencies. If a package in the rejected patches list and already installed on the
  node, its status is reported as `INSTALLED_OTHER`. Any package not already installed on
  the node is skipped. This is the default action if no option is specified. </dd>
  <dt>BLOCK</dt> <dd> **All OSs**: Packages in the rejected patches list, and packages that
  include them as dependencies, aren't installed by Patch Manager under any circumstances.
  ...
  baseline and its status is reported as `INSTALLED_REJECTED`. </dd> </dl>
...
- `"Tags"`: Optional metadata that you assign to a resource. Tags enable you to categorize
  ...
  following key-value pairs: - `Key=PatchSeverity,Value=Critical`
   - `Key=OS,Value=Windows`
...
"""
create_patch_baseline(Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreatePatchBaseline", Dict{String, Any}("Name"=>Name, "ClientToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_patch_baseline(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreatePatchBaseline", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name, "ClientToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
Web Services Systems Manager offers two types of resource data sync: `SyncToDestination`
and `SyncFromSource`.
...
`EntireOrganization` by using Organizations. For more information, see [Setting up Systems Manager Explorer to display data from multiple accounts and Regions](https://docs.aws.amazon.com/systems-manager/latest/userguide/Explorer-resource-data-sync.html)
in the *Amazon Web Services Systems Manager User Guide*.
...
status of a sync, use the <a>ListResourceDataSync</a>.
...
- `"SyncType"`: Specify `SyncToDestination` to create a resource data sync that
  synchronizes data to an S3 bucket for Inventory. If you specify `SyncToDestination`, you
  must provide a value for `S3Destination`. Specify `SyncFromSource` to synchronize data
  from a single account and multiple Regions, or multiple Amazon Web Services accounts and
  Amazon Web Services Regions, as listed in Organizations for Explorer. If you specify
  `SyncFromSource`, you must provide a value for `SyncSource`. The default value is
  `SyncToDestination`.
"""
create_resource_data_sync(SyncName; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateResourceDataSync", Dict{String, Any}("SyncName"=>SyncName); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
create_resource_data_sync(SyncName, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("CreateResourceDataSync", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SyncName"=>SyncName), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
delete_activation(ActivationId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteActivation", Dict{String, Any}("ActivationId"=>ActivationId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_activation(ActivationId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteActivation", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ActivationId"=>ActivationId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
of the node. To change the configuration state of a managed node after you disassociate a
...
"""
delete_association(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteAssociation"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_association(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteAssociation", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
Before you delete the document, we recommend that you use <a>DeleteAssociation</a> to
...
- `"VersionName"`: The version name of the document that you want to delete. If not
  provided, all versions of the document are deleted.
"""
delete_document(Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteDocument", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_document(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteDocument", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"DryRun"`: Use this option to view a summary of the deletion request without deleting
  any data or the data type. This option is useful when you only want to understand what
  will be deleted. Once you validate that the data to be deleted is what you intend to
  delete, you can run the same command without specifying the `DryRun` option.
- `"SchemaDeleteOption"`: Use the `SchemaDeleteOption` to delete a custom inventory type
  (schema). If you don't choose this option, the system only deletes existing inventory
  data associated with the custom inventory type. Choose one of the following options:
...
"""
delete_inventory(TypeName; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteInventory", Dict{String, Any}("TypeName"=>TypeName, "ClientToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_inventory(TypeName, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteInventory", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("TypeName"=>TypeName, "ClientToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
delete_maintenance_window(WindowId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteMaintenanceWindow", Dict{String, Any}("WindowId"=>WindowId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_maintenance_window(WindowId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteMaintenanceWindow", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! important
    Note the following important information about this operation. - Deleting an OpsItem is
irreversible. You can't restore a deleted OpsItem.
 - This operation uses an *eventual consistency model*, which means the system can take a
few minutes to complete this operation. If you delete an OpsItem and immediately call, for
example, <a>GetOpsItem</a>, the deleted OpsItem might still appear in the response.
...
management account can't delete OpsItems in other accounts, even if OpsCenter has been set
up for cross-account administration. For more information about cross-account
administration, see [Setting up OpsCenter to centrally manage OpsItems across accounts](https://docs.aws.amazon.com/systems-manager/latest/userguide/OpsCenter-setting-up-cross-account.html)
...
"""
delete_ops_item(OpsItemId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteOpsItem", Dict{String, Any}("OpsItemId"=>OpsItemId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_ops_item(OpsItemId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteOpsItem", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("OpsItemId"=>OpsItemId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
delete_ops_metadata(OpsMetadataArn; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteOpsMetadata", Dict{String, Any}("OpsMetadataArn"=>OpsMetadataArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_ops_metadata(OpsMetadataArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteOpsMetadata", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("OpsMetadataArn"=>OpsMetadataArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `name`: The name of the parameter to delete.
  ...
  name itself.
"""
delete_parameter(Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteParameter", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_parameter(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteParameter", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `names`: The names of the parameters to delete. After deleting a parameter, wait for at
  ...
  name itself.
"""
delete_parameters(Names; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteParameters", Dict{String, Any}("Names"=>Names); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_parameters(Names, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteParameters", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Names"=>Names), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
delete_patch_baseline(BaselineId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeletePatchBaseline", Dict{String, Any}("BaselineId"=>BaselineId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_patch_baseline(BaselineId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeletePatchBaseline", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("BaselineId"=>BaselineId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
delete_resource_data_sync(SyncName; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteResourceDataSync", Dict{String, Any}("SyncName"=>SyncName); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_resource_data_sync(SyncName, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteResourceDataSync", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SyncName"=>SyncName), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
Resource Access Manager (RAM). For more information about cross-account sharing of
parameters, see [Working with shared parameters](https://docs.aws.amazon.com/systems-manager/latest/userguide/parameter-store-shared-parameters.html)
in the *Amazon Web Services Systems Manager User Guide*.
...
- `resource_arn`: Amazon Resource Name (ARN) of the resource to which the policies are
  attached.
"""
delete_resource_policy(PolicyHash, PolicyId, ResourceArn; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteResourcePolicy", Dict{String, Any}("PolicyHash"=>PolicyHash, "PolicyId"=>PolicyId, "ResourceArn"=>ResourceArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
delete_resource_policy(PolicyHash, PolicyId, ResourceArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeleteResourcePolicy", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("PolicyHash"=>PolicyHash, "PolicyId"=>PolicyId, "ResourceArn"=>ResourceArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `instance_id`: The ID assigned to the managed node when you registered it using the
  activation process.
"""
deregister_managed_instance(InstanceId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeregisterManagedInstance", Dict{String, Any}("InstanceId"=>InstanceId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
deregister_managed_instance(InstanceId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeregisterManagedInstance", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("InstanceId"=>InstanceId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `patch_group`: The name of the patch group that should be deregistered from the patch
  baseline.
"""
deregister_patch_baseline_for_patch_group(BaselineId, PatchGroup; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeregisterPatchBaselineForPatchGroup", Dict{String, Any}("BaselineId"=>BaselineId, "PatchGroup"=>PatchGroup); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
deregister_patch_baseline_for_patch_group(BaselineId, PatchGroup, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeregisterPatchBaselineForPatchGroup", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("BaselineId"=>BaselineId, "PatchGroup"=>PatchGroup), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"Safe"`: The system checks if the target is being referenced by a task. If the target is
  ...
  maintenance window.
"""
deregister_target_from_maintenance_window(WindowId, WindowTargetId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeregisterTargetFromMaintenanceWindow", Dict{String, Any}("WindowId"=>WindowId, "WindowTargetId"=>WindowTargetId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
deregister_target_from_maintenance_window(WindowId, WindowTargetId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeregisterTargetFromMaintenanceWindow", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId, "WindowTargetId"=>WindowTargetId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
deregister_task_from_maintenance_window(WindowId, WindowTaskId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeregisterTaskFromMaintenanceWindow", Dict{String, Any}("WindowId"=>WindowId, "WindowTaskId"=>WindowTaskId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
deregister_task_from_maintenance_window(WindowId, WindowTaskId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DeregisterTaskFromMaintenanceWindow", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId, "WindowTaskId"=>WindowTaskId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_activations(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeActivations"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_activations(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeActivations", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"AssociationVersion"`: Specify the association version to retrieve. To view the latest
  ...
  list of all associations for a managed node, use <a>ListAssociations</a>. To get a list
  of versions for a specific association, use <a>ListAssociationVersions</a>.
- `"InstanceId"`: The managed node ID.
- `"Name"`: The name of the SSM document.
"""
describe_association(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAssociation"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_association(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAssociation", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
"""
describe_association_execution_targets(AssociationId, ExecutionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAssociationExecutionTargets", Dict{String, Any}("AssociationId"=>AssociationId, "ExecutionId"=>ExecutionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
"""
describe_association_executions(AssociationId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAssociationExecutions", Dict{String, Any}("AssociationId"=>AssociationId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_association_executions(AssociationId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAssociationExecutions", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AssociationId"=>AssociationId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_automation_executions(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAutomationExecutions"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_automation_executions(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAutomationExecutions", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_automation_step_executions(AutomationExecutionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAutomationStepExecutions", Dict{String, Any}("AutomationExecutionId"=>AutomationExecutionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_automation_step_executions(AutomationExecutionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAutomationStepExecutions", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AutomationExecutionId"=>AutomationExecutionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    Currently, `DescribeAvailablePatches` supports only the Amazon Linux 1, Amazon Linux 2,
...
- `"Filters"`: Each element in the array is a structure containing a key-value pair.
  ...
  Supported keys for Windows Server managed node patches include the following:</p> - **
  `PATCH_SET` **
 ...
 <p>Sample values: `1463241`
- `"MaxResults"`: The maximum number of patches to return (per page).
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_available_patches(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAvailablePatches"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_available_patches(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeAvailablePatches", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_document(Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeDocument", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_document(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeDocument", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `permission_type`: The permission type for the document. The permission type can be
  *Share*.
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_document_permission(Name, PermissionType; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeDocumentPermission", Dict{String, Any}("Name"=>Name, "PermissionType"=>PermissionType); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_document_permission(Name, PermissionType, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeDocumentPermission", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name, "PermissionType"=>PermissionType), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_effective_instance_associations(InstanceId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeEffectiveInstanceAssociations", Dict{String, Any}("InstanceId"=>InstanceId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_effective_instance_associations(InstanceId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeEffectiveInstanceAssociations", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("InstanceId"=>InstanceId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_effective_patches_for_patch_baseline(BaselineId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeEffectivePatchesForPatchBaseline", Dict{String, Any}("BaselineId"=>BaselineId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_effective_patches_for_patch_baseline(BaselineId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeEffectivePatchesForPatchBaseline", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("BaselineId"=>BaselineId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_instance_associations_status(InstanceId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstanceAssociationsStatus", Dict{String, Any}("InstanceId"=>InstanceId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_instance_associations_status(InstanceId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstanceAssociationsStatus", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("InstanceId"=>InstanceId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
not return information for nodes that are either Stopped or Terminated.

If you specify one or more node IDs, the operation returns information for those managed
nodes. If you don't specify node IDs, it returns information for all your managed nodes. If
...
EC2 instance configured with a Systems Manager Quick Setup host management configuration or
...
- `"InstanceInformationFilterList"`: This is a legacy method. We recommend that you don't
  use this method. Instead, use the `Filters` data type. `Filters` enables you to return
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_instance_information(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstanceInformation"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_instance_information(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstanceInformation", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_instance_patch_states(InstanceIds; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstancePatchStates", Dict{String, Any}("InstanceIds"=>InstanceIds); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_instance_patch_states(InstanceIds, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstancePatchStates", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("InstanceIds"=>InstanceIds), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_instance_patch_states_for_patch_group(PatchGroup; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstancePatchStatesForPatchGroup", Dict{String, Any}("PatchGroup"=>PatchGroup); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_instance_patch_states_for_patch_group(PatchGroup, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstancePatchStatesForPatchGroup", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("PatchGroup"=>PatchGroup), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"Filters"`: Each element in the array is a structure containing a key-value pair.
  ...
  Supported keys for `DescribeInstancePatches`include the following:</p> - **
  `Classification` **
  ...
  Sample values: `Installed` | `InstalledOther` | `InstalledPendingReboot`
   ...
   <p>For lists of all `State` values, see [Understanding patch compliance state values](https://docs.aws.amazon.com/systems-manager/latest/userguide/patch-manager-compliance-states.html)
  in the *Amazon Web Services Systems Manager User Guide*.
- `"MaxResults"`: The maximum number of patches to return (per page).
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_instance_patches(InstanceId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstancePatches", Dict{String, Any}("InstanceId"=>InstanceId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_instance_patches(InstanceId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstancePatches", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("InstanceId"=>InstanceId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token provided by a previous request to use to return the next set of
  properties.
"""
describe_instance_properties(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstanceProperties"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_instance_properties(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInstanceProperties", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
"""
describe_inventory_deletions(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInventoryDeletions"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_inventory_deletions(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeInventoryDeletions", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"Filters"`: Optional filters used to scope down the returned task invocations. The
  supported filter key is `STATUS` with the corresponding values `PENDING`, `IN_PROGRESS`,
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_maintenance_window_execution_tasks(WindowExecutionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowExecutionTasks", Dict{String, Any}("WindowExecutionId"=>WindowExecutionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_maintenance_window_execution_tasks(WindowExecutionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowExecutionTasks", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowExecutionId"=>WindowExecutionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_maintenance_window_executions(WindowId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowExecutions", Dict{String, Any}("WindowId"=>WindowId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_maintenance_window_executions(WindowId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowExecutions", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_maintenance_window_schedule(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowSchedule"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_maintenance_window_schedule(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowSchedule", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_maintenance_window_targets(WindowId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowTargets", Dict{String, Any}("WindowId"=>WindowId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_maintenance_window_targets(WindowId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowTargets", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
max-errors` and `--max-concurrency`. Instead, the system inserts a placeholder value of
`1`, which may be reported in the response to this command. These values don't affect the
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_maintenance_window_tasks(WindowId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowTasks", Dict{String, Any}("WindowId"=>WindowId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_maintenance_window_tasks(WindowId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowTasks", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_maintenance_windows(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindows"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_maintenance_windows(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindows", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_maintenance_windows_for_target(ResourceType, Targets; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowsForTarget", Dict{String, Any}("ResourceType"=>ResourceType, "Targets"=>Targets); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_maintenance_windows_for_target(ResourceType, Targets, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeMaintenanceWindowsForTarget", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourceType"=>ResourceType, "Targets"=>Targets), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
to query a list of OpsItems. For more information, see [Set up OpsCenter](https://docs.aws.amazon.com/systems-manager/latest/userguide/OpsCenter-setup.html)
in the *Amazon Web Services Systems Manager User Guide*.
...
- `"MaxResults"`: The maximum number of items to return for this call. The call also
  returns a token that you can specify in a subsequent call to get the next set of results.
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
- `"OpsItemFilters"`: One or more filters to limit the response.</p> - Key: CreatedTime
  ...
  *The Equals operator for Title matches the first 100 characters. If you specify more than
  ...
  limit.
   ...
   <p>**If you filter the response by using the OperationalData operator, specify a key-
  value pair by using the following JSON format: {"key":"key_name","value":"a_value"}
"""
describe_ops_items(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeOpsItems"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_ops_items(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeOpsItems", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
when you enable the [Shared](https://docs.aws.amazon.com/systems-manager/latest/APIReference/API_DescribeParameters.html#systemsmanager-DescribeParameters-request-Shared)
option.
...
the matching values up to that point and a `NextToken`. You can specify the `NextToken` in
...
- `"Shared"`: Lists parameters that are shared with you.</p>

  !!! note
      By default when using this option, the command returns parameters that have been
  shared using a standard Resource Access Manager Resource Share. In order for a parameter
  that was shared using the <a>PutResourcePolicy</a> command to be returned, the associated
  `RAM Resource Share Created From Policy` must have been promoted to a standard Resource
  ...
  API operation.
   ...
   <p>For more information about sharing parameters, see [Working with shared parameters](https://docs.aws.amazon.com/systems-manager/latest/userguide/parameter-store-shared-parameters.html)
  in the *Amazon Web Services Systems Manager User Guide*.
"""
describe_parameters(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeParameters"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_parameters(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeParameters", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"Filters"`: Each element in the array is a structure containing a key-value pair.
  ...
  Supported keys for `DescribePatchBaselines` include the following:</p> - ** `NAME_PREFIX`
  **
 ...
 <p>Sample values: `AMAZON_LINUX` | `SUSE` | `WINDOWS`
- `"MaxResults"`: The maximum number of patch baselines to return (per page).
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_patch_baselines(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribePatchBaselines"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_patch_baselines(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribePatchBaselines", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
describe_patch_group_state(PatchGroup; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribePatchGroupState", Dict{String, Any}("PatchGroup"=>PatchGroup); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_patch_group_state(PatchGroup, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribePatchGroupState", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("PatchGroup"=>PatchGroup), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"Filters"`: Each element in the array is a structure containing a key-value pair.
  ...
  Supported keys for `DescribePatchGroups` include the following:</p> - ** `NAME_PREFIX` **
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_patch_groups(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribePatchGroups"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_patch_groups(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribePatchGroups", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
<a>CreatePatchBaseline</a>, <a>UpdatePatchBaseline</a>, <a>DescribeAvailablePatches</a>,
and <a>DescribePatchBaselines</a>.

The following section lists the properties that can be used in filters for each major
operating system type: <dl> <dt>AMAZON_LINUX</dt> <dd>Valid properties: `PRODUCT` |
`CLASSIFICATION` | `SEVERITY`  </dd> <dt>AMAZON_LINUX_2</dt> <dd>Valid properties:
`PRODUCT` | `CLASSIFICATION` | `SEVERITY`  </dd> <dt>AMAZON_LINUX_2023</dt> <dd>Valid
properties: `PRODUCT` | `CLASSIFICATION` | `SEVERITY`  </dd> <dt>CENTOS</dt> <dd>Valid
properties: `PRODUCT` | `CLASSIFICATION` | `SEVERITY`  </dd> <dt>DEBIAN</dt> <dd>Valid
properties: `PRODUCT` | `PRIORITY`  </dd> <dt>MACOS</dt> <dd>Valid properties: `PRODUCT` |
`CLASSIFICATION`  </dd> <dt>ORACLE_LINUX</dt> <dd>Valid properties: `PRODUCT` |
`CLASSIFICATION` | `SEVERITY`  </dd> <dt>REDHAT_ENTERPRISE_LINUX</dt> <dd>Valid properties:
`PRODUCT` | `CLASSIFICATION` | `SEVERITY`  </dd> <dt>SUSE</dt> <dd>Valid properties:
`PRODUCT` | `CLASSIFICATION` | `SEVERITY`  </dd> <dt>UBUNTU</dt> <dd>Valid properties:
`PRODUCT` | `PRIORITY`  </dd> <dt>WINDOWS</dt> <dd>Valid properties: `PRODUCT` |
`PRODUCT_FAMILY` | `CLASSIFICATION` | `MSRC_SEVERITY`  </dd> </dl>
...
- `"PatchSet"`: Indicates whether to list patches for the Windows operating system or for
  applications released by Microsoft. Not applicable for the Linux or macOS operating
  systems.
"""
describe_patch_properties(OperatingSystem, Property; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribePatchProperties", Dict{String, Any}("OperatingSystem"=>OperatingSystem, "Property"=>Property); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_patch_properties(OperatingSystem, Property, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribePatchProperties", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("OperatingSystem"=>OperatingSystem, "Property"=>Property), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
describe_sessions(State; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeSessions", Dict{String, Any}("State"=>State); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
describe_sessions(State, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DescribeSessions", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("State"=>State), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
disassociate_ops_item_related_item(AssociationId, OpsItemId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DisassociateOpsItemRelatedItem", Dict{String, Any}("AssociationId"=>AssociationId, "OpsItemId"=>OpsItemId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
disassociate_ops_item_related_item(AssociationId, OpsItemId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("DisassociateOpsItemRelatedItem", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AssociationId"=>AssociationId, "OpsItemId"=>OpsItemId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `automation_execution_id`: The unique identifier for an existing automation execution to
  examine. The execution ID is returned by StartAutomationExecution when the execution of
  an Automation runbook is initiated.
"""
get_automation_execution(AutomationExecutionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetAutomationExecution", Dict{String, Any}("AutomationExecutionId"=>AutomationExecutionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_automation_execution(AutomationExecutionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetAutomationExecution", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AutomationExecutionId"=>AutomationExecutionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"AtTime"`: (Optional) The specific time for which you want to get calendar state
  information, in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format. If you don't
  specify a value or `AtTime`, the current time is used.
"""
get_calendar_state(CalendarNames; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetCalendarState", Dict{String, Any}("CalendarNames"=>CalendarNames); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_calendar_state(CalendarNames, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetCalendarState", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CalendarNames"=>CalendarNames), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
previous API command.
 ...
 `GetCommandInvocation` only gives the execution status of a plugin in a document. To get
the command execution status on a specific managed node, use <a>ListCommandInvocations</a>.
...
- `instance_id`: (Required) The ID of the managed node targeted by the command. A *managed
  node* can be an Amazon Elastic Compute Cloud (Amazon EC2) instance, edge device, and on-
  premises server or VM in your hybrid environment that is configured for Amazon Web
...
- `"PluginName"`: The name of the step for which you want detailed results. If the document
  ...
  want details for. Alternatively, use <a>ListCommandInvocations</a> with the `CommandId`
  and `Details` parameters. The `PluginName` is the `Name` attribute of the `CommandPlugin`
  object in the `CommandPlugins` list.
"""
get_command_invocation(CommandId, InstanceId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetCommandInvocation", Dict{String, Any}("CommandId"=>CommandId, "InstanceId"=>InstanceId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_command_invocation(CommandId, InstanceId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetCommandInvocation", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("CommandId"=>CommandId, "InstanceId"=>InstanceId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
get_connection_status(Target; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetConnectionStatus", Dict{String, Any}("Target"=>Target); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_connection_status(Target, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetConnectionStatus", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Target"=>Target), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"OperatingSystem"`: Returns the default patch baseline for the specified operating
  system.
"""
get_default_patch_baseline(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetDefaultPatchBaseline"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_default_patch_baseline(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetDefaultPatchBaseline", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
system attempts to use your local Amazon Web Services credentials and the operation fails.
To avoid this, you can run the command in the Amazon Web Services Systems Manager console.
Use Run Command, a capability of Amazon Web Services Systems Manager, with an SSM document
that enables you to target a managed node with a script or command. For example, run the
command using the `AWS-RunShellScript` document or the `AWS-RunPowerShellScript` document.
...
"""
get_deployable_patch_snapshot_for_instance(InstanceId, SnapshotId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetDeployablePatchSnapshotForInstance", Dict{String, Any}("InstanceId"=>InstanceId, "SnapshotId"=>SnapshotId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_deployable_patch_snapshot_for_instance(InstanceId, SnapshotId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetDeployablePatchSnapshotForInstance", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("InstanceId"=>InstanceId, "SnapshotId"=>SnapshotId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"VersionName"`: An optional field specifying the version of the artifact associated with
  ...
  and can't be changed.
"""
get_document(Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetDocument", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_document(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetDocument", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
get_inventory(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetInventory"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_inventory(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetInventory", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"Aggregator"`: Returns inventory schemas that support aggregation. For example, this
  call returns the `AWS:InstanceInformation` type, because it supports aggregation based on
  the `PlatformName`, `PlatformType`, and `PlatformVersion` attributes.
- `"MaxResults"`: The maximum number of items to return for this call. The call also
  returns a token that you can specify in a subsequent call to get the next set of results.
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
...
"""
get_inventory_schema(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetInventorySchema"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_inventory_schema(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetInventorySchema", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
get_maintenance_window(WindowId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetMaintenanceWindow", Dict{String, Any}("WindowId"=>WindowId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_maintenance_window(WindowId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetMaintenanceWindow", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
get_maintenance_window_execution(WindowExecutionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetMaintenanceWindowExecution", Dict{String, Any}("WindowExecutionId"=>WindowExecutionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_maintenance_window_execution(WindowExecutionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetMaintenanceWindowExecution", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowExecutionId"=>WindowExecutionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `window_execution_id`: The ID of the maintenance window execution that includes the task.
"""
get_maintenance_window_execution_task(TaskId, WindowExecutionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetMaintenanceWindowExecutionTask", Dict{String, Any}("TaskId"=>TaskId, "WindowExecutionId"=>WindowExecutionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_maintenance_window_execution_task(TaskId, WindowExecutionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetMaintenanceWindowExecutionTask", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("TaskId"=>TaskId, "WindowExecutionId"=>WindowExecutionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `window_execution_id`: The ID of the maintenance window execution for which the task is a
  part.
"""
get_maintenance_window_execution_task_invocation(InvocationId, TaskId, WindowExecutionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetMaintenanceWindowExecutionTaskInvocation", Dict{String, Any}("InvocationId"=>InvocationId, "TaskId"=>TaskId, "WindowExecutionId"=>WindowExecutionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_maintenance_window_execution_task_invocation(InvocationId, TaskId, WindowExecutionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetMaintenanceWindowExecutionTaskInvocation", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("InvocationId"=>InvocationId, "TaskId"=>TaskId, "WindowExecutionId"=>WindowExecutionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
max-errors` and `--max-concurrency`. Instead, the system inserts a placeholder value of
`1`, which may be reported in the response to this command. These values don't affect the
running of your task and can be ignored.To retrieve a list of tasks in a maintenance
window, instead use the <a>DescribeMaintenanceWindowTasks</a> command.
...
"""
get_maintenance_window_task(WindowId, WindowTaskId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetMaintenanceWindowTask", Dict{String, Any}("WindowId"=>WindowId, "WindowTaskId"=>WindowTaskId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_maintenance_window_task(WindowId, WindowTaskId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetMaintenanceWindowTask", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId, "WindowTaskId"=>WindowTaskId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
Access Management (IAM) to view information about an OpsItem. For more information, see [Set up OpsCenter](https://docs.aws.amazon.com/systems-manager/latest/userguide/OpsCenter-setup.html)
in the *Amazon Web Services Systems Manager User Guide*.
...
"""
get_ops_item(OpsItemId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetOpsItem", Dict{String, Any}("OpsItemId"=>OpsItemId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_ops_item(OpsItemId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetOpsItem", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("OpsItemId"=>OpsItemId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
"""
get_ops_metadata(OpsMetadataArn; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetOpsMetadata", Dict{String, Any}("OpsMetadataArn"=>OpsMetadataArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_ops_metadata(OpsMetadataArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetOpsMetadata", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("OpsMetadataArn"=>OpsMetadataArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
get_ops_summary(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetOpsSummary"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_ops_summary(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetOpsSummary", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
get_parameter(Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetParameter", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_parameter(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetParameter", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `name`: The name or Amazon Resource Name (ARN) of the parameter for which you want to
  review history. For parameters shared with you from another account, you must use the
  full ARN.
...
- `"WithDecryption"`: Return decrypted values for secure string parameters. This flag is
  ignored for `String` and `StringList` parameter types.
"""
get_parameter_history(Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetParameterHistory", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_parameter_history(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetParameterHistory", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
!!! note
    To get information about a single parameter, you can use the <a>GetParameter</a>
operation instead.

# Arguments

- `names`: The names or Amazon Resource Names (ARNs) of the parameters that you want to
  query. For parameters shared with you from another account, you must use the full ARNs.
  ...
  responses.For information about shared parameters, see [Working with shared parameters](https://docs.aws.amazon.com/systems-manager/latest/userguide/parameter-store-shared-parameters.html)
  in the *Amazon Web Services Systems Manager User Guide*.
...
- `"WithDecryption"`: Return decrypted secure string value. Return decrypted values for
  secure string parameters. This flag is ignored for `String` and `StringList` parameter
  types.
"""
get_parameters(Names; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetParameters", Dict{String, Any}("Names"=>Names); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_parameters(Names, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetParameters", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Names"=>Names), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"MaxResults"`: The maximum number of items to return for this call. The call also
  returns a token that you can specify in a subsequent call to get the next set of results.
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
- `"ParameterFilters"`: Filters to limit the request results.</p>

  !!! note
      The following `Key` values are supported for `GetParametersByPath`: `Type`, `KeyId`,
  and `Label`.

   <p>The following `Key` values aren't supported for `GetParametersByPath`: `tag`,
...
"""
get_parameters_by_path(Path; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetParametersByPath", Dict{String, Any}("Path"=>Path); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_parameters_by_path(Path, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetParametersByPath", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Path"=>Path), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `baseline_id`: The ID of the patch baseline to retrieve.
  ...
  the full Amazon Resource Name (ARN) of the baseline. For example, for the baseline `AWS-
  AmazonLinuxDefaultPatchBaseline`, specify `arn:aws:ssm:us-east-
  2:733109147000:patchbaseline/pb-0e392de35e7c563b7` instead of `pb-0e392de35e7c563b7`.
"""
get_patch_baseline(BaselineId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetPatchBaseline", Dict{String, Any}("BaselineId"=>BaselineId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_patch_baseline(BaselineId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetPatchBaseline", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("BaselineId"=>BaselineId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"OperatingSystem"`: Returns the operating system rule specified for patch groups using
  the patch baseline.
"""
get_patch_baseline_for_patch_group(PatchGroup; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetPatchBaselineForPatchGroup", Dict{String, Any}("PatchGroup"=>PatchGroup); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_patch_baseline_for_patch_group(PatchGroup, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetPatchBaselineForPatchGroup", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("PatchGroup"=>PatchGroup), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
"""
get_resource_policies(ResourceArn; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetResourcePolicies", Dict{String, Any}("ResourceArn"=>ResourceArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_resource_policies(ResourceArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetResourcePolicies", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourceArn"=>ResourceArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
setting. Use the <a>UpdateServiceSetting</a> API operation to change the default setting.
Or use the <a>ResetServiceSetting</a> to change the value back to the original value
...
"""
get_service_setting(SettingId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetServiceSetting", Dict{String, Any}("SettingId"=>SettingId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
get_service_setting(SettingId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("GetServiceSetting", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SettingId"=>SettingId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
Parameter labels have the following requirements and restrictions. - A version of a
parameter can have a maximum of 10 labels.
 - You can't attach the same label to different versions of the same parameter. For
example, if version 1 has the label Production, then you can't attach Production to version
2.
...
- `"ParameterVersion"`: The specific version of the parameter on which you want to attach
  one or more labels. If no version is specified, the system attaches the label to the
  latest version.
"""
label_parameter_version(Labels, Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("LabelParameterVersion", Dict{String, Any}("Labels"=>Labels, "Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
label_parameter_version(Labels, Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("LabelParameterVersion", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Labels"=>Labels, "Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
"""
list_association_versions(AssociationId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListAssociationVersions", Dict{String, Any}("AssociationId"=>AssociationId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_association_versions(AssociationId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListAssociationVersions", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AssociationId"=>AssociationId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
list_associations(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListAssociations"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_associations(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListAssociations", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
list_command_invocations(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListCommandInvocations"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_command_invocations(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListCommandInvocations", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: (Optional) The token for the next set of items to return. (You received
  this token from a previous call.)
"""
list_commands(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListCommands"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_commands(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListCommands", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
list_compliance_items(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListComplianceItems"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_compliance_items(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListComplianceItems", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"MaxResults"`: The maximum number of items to return for this call. Currently, you can
  specify null or 50. The call also returns a token that you can specify in a subsequent
  call to get the next set of results.
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
"""
list_compliance_summaries(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListComplianceSummaries"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_compliance_summaries(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListComplianceSummaries", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
list_document_metadata_history(Metadata, Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListDocumentMetadataHistory", Dict{String, Any}("Metadata"=>Metadata, "Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_document_metadata_history(Metadata, Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListDocumentMetadataHistory", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Metadata"=>Metadata, "Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
list_document_versions(Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListDocumentVersions", Dict{String, Any}("Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_document_versions(Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListDocumentVersions", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"Filters"`: One or more `DocumentKeyValuesFilter` objects. Use a filter to return a more
  ...
  have been applied to a document. Other valid keys include `Owner`, `Name`,
  `PlatformTypes`, `DocumentType`, and `TargetType`. For example, to return documents you
  own use `Key=Owner,Values=Self`. To specify a custom key-value pair, use the format
  `Key=tag:tagName,Values=valueName`.

  !!! note
      This API operation only supports filtering documents by using a single tag key and
  one or more tag values. For example: `Key=tag:tagName,Values=valueName1,valueName2`
- `"MaxResults"`: The maximum number of items to return for this call. The call also
  returns a token that you can specify in a subsequent call to get the next set of results.
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
list_documents(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListDocuments"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_documents(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListDocuments", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: The token for the next set of items to return. (You received this token
  from a previous call.)
"""
list_inventory_entries(InstanceId, TypeName; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListInventoryEntries", Dict{String, Any}("InstanceId"=>InstanceId, "TypeName"=>TypeName); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_inventory_entries(InstanceId, TypeName, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListInventoryEntries", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("InstanceId"=>InstanceId, "TypeName"=>TypeName), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
"""
list_ops_item_events(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListOpsItemEvents"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_ops_item_events(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListOpsItemEvents", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"OpsItemId"`: The ID of the OpsItem for which you want to list all related-item
  resources.
"""
list_ops_item_related_items(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListOpsItemRelatedItems"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_ops_item_related_items(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListOpsItemRelatedItems", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
"""
list_ops_metadata(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListOpsMetadata"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_ops_metadata(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListOpsMetadata", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"NextToken"`: A token to start the list. Use this token to get the next set of results.
"""
list_resource_compliance_summaries(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListResourceComplianceSummaries"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_resource_compliance_summaries(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListResourceComplianceSummaries", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
list_resource_data_sync(; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListResourceDataSync"; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_resource_data_sync(params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListResourceDataSync", params; aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
list_tags_for_resource(ResourceId, ResourceType; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListTagsForResource", Dict{String, Any}("ResourceId"=>ResourceId, "ResourceType"=>ResourceType); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
list_tags_for_resource(ResourceId, ResourceType, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ListTagsForResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourceId"=>ResourceId, "ResourceType"=>ResourceType), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"AccountIdsToRemove"`: The Amazon Web Services users that should no longer have access
  to the document. The Amazon Web Services user can either be a group of account IDs or
  *All*. This action has a higher priority than `AccountIdsToAdd`. If you specify an ID to
...
"""
modify_document_permission(Name, PermissionType; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ModifyDocumentPermission", Dict{String, Any}("Name"=>Name, "PermissionType"=>PermissionType); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
modify_document_permission(Name, PermissionType, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ModifyDocumentPermission", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name, "PermissionType"=>PermissionType), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
ComplianceType can be one of the following: - ExecutionId: The execution ID when the patch,
association, or custom compliance item was applied.
...
- `"ItemContentHash"`: MD5 or SHA-256 content hash. The content hash is used to determine
  if existing information should be overwritten or ignored. If the content hashes match,
...
- `"UploadType"`: The mode for uploading compliance items. You can specify `COMPLETE` or
  `PARTIAL`. In `COMPLETE` mode, the system overwrites all existing compliance information
  for the resource. You must provide a full list of compliance items each time you send the
  request.
...
"""
put_inventory(InstanceId, Items; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("PutInventory", Dict{String, Any}("InstanceId"=>InstanceId, "Items"=>Items); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
put_inventory(InstanceId, Items, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("PutInventory", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("InstanceId"=>InstanceId, "Items"=>Items), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `name`: The fully qualified name of the parameter that you want to add to the system.</p>

  !!! note
      You can't enter the Amazon Resource Name (ARN) for a parameter, only the parameter
  name itself.The fully qualified name includes the complete hierarchy of the parameter
  path and name. For parameters in a hierarchy, you must include a leading forward slash
  character (/) when you create or reference a parameter. For example:
  `/Dev/DBServer/MySQL/db-string13`

  Naming Constraints: - Parameter names are case sensitive.
  ...
  characters reserved for internal use by Systems Manager. The maximum length for a
  parameter name that you create is 1011 characters. This includes the characters in the
  ARN that precede the name you specify, such as `arn:aws:ssm:us-east-
  2:111122223333:parameter/`.
...
- `"AllowedPattern"`: A regular expression used to validate the parameter value. For
  example, for String types with values restricted to numbers, you can specify the
...
- `"DataType"`: The data type for a `String` parameter. Supported data types include plain
  ...
  Systems Manager validates the parameter value is in the required format, such as `ami-
  12345abcdeEXAMPLE`, and that the specified AMI is available in your Amazon Web Services
  account.
  ...
  parameters are created successfully, see [Setting up notifications or trigger actions based on Parameter Store events](https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-paramstore-cwe.html).
  For more information about AMI format validation , see [Native parameter support for Amazon Machine Image IDs](https://docs.aws.amazon.com/systems-manager/latest/userguide/parameter-store-ec2-aliases.html).
...
- `"KeyId"`: The Key Management Service (KMS) ID that you want to use to encrypt a
  ...
  Amazon Web Services account which is not as secure as using a custom key. - To use a
  custom KMS key, choose the `SecureString` data type with the `Key ID` parameter.
...
- `"Policies"`: One or more policies to apply to a parameter. This operation takes a JSON
  ...
  updating the policy. Updating the *parameter* doesn't affect the expiration date and
  ...
  example, a secret needs to be changed within a period of time, but it hasn't been
  changed.

  All existing policies are preserved until you send new policies or an empty policy. For
  more information about parameter policies, see [Assigning parameter policies](https://docs.aws.amazon.com/systems-manager/latest/userguide/parameter-store-policies.html).
- `"Tags"`: Optional metadata that you assign to a resource. Tags enable you to categorize
  a resource in different ways, such as by purpose, owner, or environment. For example, you
  might want to tag a Systems Manager parameter to identify the type of resource to which
  it applies, the environment, or the type of configuration data referenced by the
...
- `"Tier"`: The parameter tier to assign to a parameter.
  ...
  information, see [Managing parameter tiers](https://docs.aws.amazon.com/systems-manager/latest/userguide/parameter-store-advanced-parameters.html)
  in the *Amazon Web Services Systems Manager User Guide*.
  ...
  for an advanced parameter, you must delete it and recreate it as a new standard
  parameter.

   **Using the Default Tier Configuration**

  In `PutParameter` requests, you can specify the tier to create the parameter in. Whenever
  you specify a tier in the request, Parameter Store creates or updates the parameter
  according to that request. However, if you don't specify a tier in a request, Parameter
  Store assigns the tier based on the current Parameter Store default tier configuration.

  The default tier when you begin using Parameter Store is the standard-parameter tier. If
  you use the advanced-parameter tier, you can specify one of the following as the
  default:</p> - **Advanced**: With this option, Parameter Store evaluates all requests as
  advanced parameters.
   - **Intelligent-Tiering**: With this option, Parameter Store evaluates each request to
  determine if the parameter is standard or advanced.
  ...
  Options that require an advanced parameter include the following: - The content size of
  the parameter is more than 4 KB.
...
- `"Type"`: The type of parameter that you want to add to the system.
  ...
  !!! note
      `SecureString` isn't currently supported for CloudFormation templates.Items in a
  `StringList` must be separated by a comma (,). You can't use other punctuation or special
  character to escape items in the list. If you have a parameter value that requires a
...
"""
put_parameter(Name, Value; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("PutParameter", Dict{String, Any}("Name"=>Name, "Value"=>Value); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
put_parameter(Name, Value, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("PutParameter", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Name"=>Name, "Value"=>Value), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
Manager resources. The following resources support Systems Manager resource policies.</p> -
`OpsItemGroup` - The resource policy for `OpsItemGroup` enables Amazon Web Services
accounts to view and interact with OpsCenter operational work items (OpsItems).
 - `Parameter` - The resource policy is used to share a parameter with other accounts using
...
parameter tiers, see [Managing parameter tiers](https://docs.aws.amazon.com/systems-manager/latest/userguide/parameter-store-advanced-parameters.html).
...
To share a `SecureString` parameter, it must be encrypted with a customer managed key, and
you must share the key separately through Key Management Service. Amazon Web Services
managed keys cannot be shared. Parameters encrypted with the default Amazon Web Services
managed key can be updated to use a customer managed key instead. For KMS key definitions,
see [KMS concepts](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html) in
...
Resource Share using the RAM [PromoteResourceShareCreatedFromPolicy](https://docs.aws.amazon.com/ram/latest/APIReference/API_PromoteResourceShareCreatedFromPolicy.html)
API operation. Otherwise, the parameter won't be returned by the Systems Manager [DescribeParameters](https://docs.aws.amazon.com/systems-manager/latest/APIReference/API_DescribeParameters.html)
API operation using the `--shared` option.

 <p>For more information, see [Sharing a parameter](https://docs.aws.amazon.com/systems-manager/latest/userguide/parameter-store-shared-parameters.html#share)
in the *Amazon Web Services Systems Manager User Guide*
...
"""
put_resource_policy(Policy, ResourceArn; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("PutResourcePolicy", Dict{String, Any}("Policy"=>Policy, "ResourceArn"=>ResourceArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
put_resource_policy(Policy, ResourceArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("PutResourcePolicy", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Policy"=>Policy, "ResourceArn"=>ResourceArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
patch baseline Amazon Resource Name (ARN) as the baseline ID value. For example, for
CentOS, specify `arn:aws:ssm:us-east-2:733109147000:patchbaseline/pb-0574b43a65ea646ed`
...
"""
register_default_patch_baseline(BaselineId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("RegisterDefaultPatchBaseline", Dict{String, Any}("BaselineId"=>BaselineId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
register_default_patch_baseline(BaselineId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("RegisterDefaultPatchBaseline", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("BaselineId"=>BaselineId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
register_patch_baseline_for_patch_group(BaselineId, PatchGroup; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("RegisterPatchBaselineForPatchGroup", Dict{String, Any}("BaselineId"=>BaselineId, "PatchGroup"=>PatchGroup); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
register_patch_baseline_for_patch_group(BaselineId, PatchGroup, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("RegisterPatchBaselineForPatchGroup", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("BaselineId"=>BaselineId, "PatchGroup"=>PatchGroup), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `targets`: The targets to register with the maintenance window. In other words, the
  ...
  task the same priority level.You can specify targets using managed node IDs, resource
  group names, or tags that have been applied to managed nodes.

   **Example 1**: Specify managed node IDs

   `Key=InstanceIds,Values=&lt;instance-id-1&gt;,&lt;instance-id-2&gt;,&lt;instance-id-
  3&gt;`
   ...
   **Example 5**: Use filters for resource group types
   ...
   `Key=resource-groups:ResourceTypeFilters,Values=&lt;resource-type-1&gt;,&lt;resource-
  type-2&gt;`
  ...
  following format
   ...
   <p> `Key=resource-groups:ResourceTypeFilters,Values=AWS::EC2::INSTANCE,AWS::EC2::VPC`For
  more information about these examples formats, including the best use case for each one,
  see [Examples: Register targets with a maintenance window](https://docs.aws.amazon.com/systems-manager/latest/userguide/mw-cli-tutorial-targets-examples.html)
  in the *Amazon Web Services Systems Manager User Guide*.
...
- `"CutoffBehavior"`: Indicates whether tasks should continue to run after the cutoff time
  specified in the maintenance windows is reached.  - `CONTINUE_TASK`: When the cutoff time
  is reached, any tasks that are running continue. The default value.
   - `CANCEL_TASK`: <ul> <li>For Automation, Lambda, Step Functions tasks: When the cutoff
  ...
  <a>CancelCommand</a> operation that attempts to cancel the command associated with the
  task. However, there is no guarantee that the command will be terminated and the
  underlying process stopped.
  The status for tasks that are not completed is `TIMED_OUT`. </li> </ul>
...
- `"MaxConcurrency"`: The maximum number of targets this task can be run for, in
  parallel.</p>

  !!! note
      Although this element is listed as "Required: No", a value can be omitted only when
  you are registering or updating a [targetless task](https://docs.aws.amazon.com/systems-manager/latest/userguide/maintenance-windows-targetless-tasks.html)
  You must provide a value in all other cases.

   <p>For maintenance window tasks without a target specified, you can't supply a value for
...
- `"Priority"`: The priority of the task in the maintenance window, the lower the number
  the higher the priority. Tasks in a maintenance window are scheduled in priority order
...
- `"ServiceRoleArn"`: The Amazon Resource Name (ARN) of the IAM service role for Amazon Web
  ...
  specify a service role ARN, Systems Manager uses a service-linked role in your account.
  If no appropriate service-linked role for Systems Manager exists in your account, it is
...
- `"Targets"`: The targets (either managed nodes or maintenance window targets).</p>
  ...
  don't specify targets, see [Registering maintenance window tasks without targets](https://docs.aws.amazon.com/systems-manager/latest/userguide/maintenance-windows-targetless-tasks.html)
  in the *Amazon Web Services Systems Manager User Guide*.Specify managed nodes using the
  following format:
 ...
 <p> `Key=WindowTargetIds,Values=&lt;window-target-id-1&gt;,&lt;window-target-id-2&gt;`
...
- `"TaskParameters"`: The parameters that should be passed to the task when it is run.
  ...
  maintenance window task types, see <a>MaintenanceWindowTaskInvocationParameters</a>.
"""
register_task_with_maintenance_window(TaskArn, TaskType, WindowId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("RegisterTaskWithMaintenanceWindow", Dict{String, Any}("TaskArn"=>TaskArn, "TaskType"=>TaskType, "WindowId"=>WindowId, "ClientToken"=>string(uuid4())); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
register_task_with_maintenance_window(TaskArn, TaskType, WindowId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("RegisterTaskWithMaintenanceWindow", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("TaskArn"=>TaskArn, "TaskType"=>TaskType, "WindowId"=>WindowId, "ClientToken"=>string(uuid4())), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `resource_type`: The type of resource from which you want to remove a tag.
...
"""
remove_tags_from_resource(ResourceId, ResourceType, TagKeys; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("RemoveTagsFromResource", Dict{String, Any}("ResourceId"=>ResourceId, "ResourceType"=>ResourceType, "TagKeys"=>TagKeys); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
remove_tags_from_resource(ResourceId, ResourceType, TagKeys, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("RemoveTagsFromResource", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("ResourceId"=>ResourceId, "ResourceType"=>ResourceType, "TagKeys"=>TagKeys), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
define the default value for a `SettingId`. You can't create a new `SettingId`, but you can
overwrite the default value if you have the `ssm:UpdateServiceSetting` permission for the
setting. Use the <a>GetServiceSetting</a> API operation to view the current value. Use the
...
- `setting_id`: The Amazon Resource Name (ARN) of the service setting to reset. The setting
  ID can be one of the following. - `/ssm/managed-instance/default-ec2-instance-management-
  role`
   - `/ssm/automation/customer-script-log-destination`
   - `/ssm/automation/customer-script-log-group-name`
   - `/ssm/documents/console/public-sharing-permission`
   ...
   - `/ssm/parameter-store/high-throughput-enabled`
"""
reset_service_setting(SettingId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ResetServiceSetting", Dict{String, Any}("SettingId"=>SettingId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
reset_service_setting(SettingId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ResetServiceSetting", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SettingId"=>SettingId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
resume_session(SessionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ResumeSession", Dict{String, Any}("SessionId"=>SessionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
resume_session(SessionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("ResumeSession", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SessionId"=>SessionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"Payload"`: The data sent with the signal. The data schema depends on the type of signal
  ...
  For `StartStep` and `Resume` signal types, you must send the name of the Automation step
  to start or resume as the payload. For example:
...
"""
send_automation_signal(AutomationExecutionId, SignalType; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("SendAutomationSignal", Dict{String, Any}("AutomationExecutionId"=>AutomationExecutionId, "SignalType"=>SignalType); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
send_automation_signal(AutomationExecutionId, SignalType, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("SendAutomationSignal", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AutomationExecutionId"=>AutomationExecutionId, "SignalType"=>SignalType), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `document_name`: The name of the Amazon Web Services Systems Manager document (SSM
  ...
  document belonging to another account, specify the document Amazon Resource Name (ARN).
  For more information about how to use shared documents, see [Sharing SSM documents](https://docs.aws.amazon.com/systems-manager/latest/userguide/ssm-using-shared.html)
  in the *Amazon Web Services Systems Manager User Guide*.
...
- `"InstanceIds"`: The IDs of the managed nodes where the command should run. Specifying
  ...
  For more information about how to use targets, see [Run commands at scale](https://docs.aws.amazon.com/systems-manager/latest/userguide/send-commands-multiple.html)
  in the *Amazon Web Services Systems Manager User Guide*.
- `"MaxConcurrency"`: (Optional) The maximum number of managed nodes that are allowed to
  run the command at the same time. You can specify a number such as 10 or a percentage
...
- `"MaxErrors"`: The maximum number of errors allowed without the command failing. When the
  command fails one more time beyond the value of `MaxErrors`, the systems stops sending
  the command to additional targets. You can specify a number like 10 or a percentage like
  10%. The default value is `0`. For more information about how to use `MaxErrors`, see [Using error controls](https://docs.aws.amazon.com/systems-manager/latest/userguide/send-commands-multiple.html#send-commands-maxerrors)
  in the *Amazon Web Services Systems Manager User Guide*.
...
- `"ServiceRoleArn"`: The ARN of the Identity and Access Management (IAM) service role to
  ...
  information about creating and using this service role, see [Monitoring Systems Manager status changes using Amazon SNS notifications](https://docs.aws.amazon.com/systems-manager/latest/userguide/monitoring-sns-notifications.html)
  in the *Amazon Web Services Systems Manager User Guide*.
...
"""
send_command(DocumentName; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("SendCommand", Dict{String, Any}("DocumentName"=>DocumentName); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
send_command(DocumentName, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("SendCommand", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DocumentName"=>DocumentName), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
start_associations_once(AssociationIds; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("StartAssociationsOnce", Dict{String, Any}("AssociationIds"=>AssociationIds); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_associations_once(AssociationIds, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("StartAssociationsOnce", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AssociationIds"=>AssociationIds), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `document_name`: The name of the SSM document to run. This can be a public document or a
  ...
  document ARN. For more information about how to use shared documents, see [Sharing SSM documents](https://docs.aws.amazon.com/systems-manager/latest/userguide/documents-ssm-sharing.html)
  in the *Amazon Web Services Systems Manager User Guide*.
...
- `"MaxErrors"`: The number of errors that are allowed before the system stops running the
  automation on additional targets. You can specify either an absolute number of errors,
  for example 10, or a percentage of the target set, for example 10%. If you specify 3, for
  example, the system stops running the automation when the fourth error is received. If
  you specify 0, then the system stops running the automation on additional targets after
  the first error result is returned. If you run an automation on 50 resources and set max-
  errors to 10%, then the system stops running the automation on additional targets when
...
- `"Tags"`: Optional metadata that you assign to a resource. You can specify a maximum of
  ...
  automation to identify an environment or operating system. In this case, you could
  specify the following key-value pairs: - `Key=environment,Value=test`
   - `Key=OS,Value=Windows`
...
- `"Targets"`: A key-value mapping to target resources. Required if you specify
  TargetParameterName.
"""
start_automation_execution(DocumentName; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("StartAutomationExecution", Dict{String, Any}("DocumentName"=>DocumentName); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_automation_execution(DocumentName, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("StartAutomationExecution", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DocumentName"=>DocumentName), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"AutoApprove"`: Indicates whether the change request can be approved automatically
  ...
  If `AutoApprovable` is enabled in a change template, then setting `AutoApprove` to `true`
  in `StartChangeRequestExecution` creates a change request that bypasses approver review.
...
- `"ChangeDetails"`: User-provided details about the change. If no details are provided,
  content specified in the **Template information** section of the associated change
  template is added.
...
- `"ScheduledTime"`: The date and time specified in the change request to run the
  ...
  required approvals for the change request have been received.
- `"Tags"`: Optional metadata that you assign to a resource. You can specify a maximum of
  five tags for a change request. Tags enable you to categorize a resource in different
  ways, such as by purpose, owner, or environment. For example, you might want to tag a
  change request to identify an environment or target Amazon Web Services Region. In this
  case, you could specify the following key-value pairs: -
  `Key=Environment,Value=Production`
 - `Key=Region,Value=us-east-2`
"""
start_change_request_execution(DocumentName, Runbooks; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("StartChangeRequestExecution", Dict{String, Any}("DocumentName"=>DocumentName, "Runbooks"=>Runbooks); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_change_request_execution(DocumentName, Runbooks, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("StartChangeRequestExecution", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DocumentName"=>DocumentName, "Runbooks"=>Runbooks), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
sending input and receiving outputs.</p>

!!! note
    Amazon Web Services CLI usage: `start-session` is an interactive command that requires
the Session Manager plugin to be installed on the client machine making the call. For
information, see [Install the Session Manager plugin for the Amazon Web Services CLI](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-working-with-install-plugin.html)
in the *Amazon Web Services Systems Manager User Guide*.

 <p>Amazon Web Services Tools for PowerShell usage: Start-SSMSession isn't currently
...
- `"DocumentName"`: The name of the SSM document you want to use to define the type of
  session, input parameters, or preferences for the session. For example, `SSM-
  SessionManagerRunShell`. You can call the <a>GetDocument</a> API to verify the document
...
"""
start_session(Target; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("StartSession", Dict{String, Any}("Target"=>Target); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
start_session(Target, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("StartSession", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Target"=>Target), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
stop_automation_execution(AutomationExecutionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("StopAutomationExecution", Dict{String, Any}("AutomationExecutionId"=>AutomationExecutionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
stop_automation_execution(AutomationExecutionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("StopAutomationExecution", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AutomationExecutionId"=>AutomationExecutionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
terminate_session(SessionId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("TerminateSession", Dict{String, Any}("SessionId"=>SessionId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
terminate_session(SessionId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("TerminateSession", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SessionId"=>SessionId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
unlabel_parameter_version(Labels, Name, ParameterVersion; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UnlabelParameterVersion", Dict{String, Any}("Labels"=>Labels, "Name"=>Name, "ParameterVersion"=>ParameterVersion); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
unlabel_parameter_version(Labels, Name, ParameterVersion, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UnlabelParameterVersion", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Labels"=>Labels, "Name"=>Name, "ParameterVersion"=>ParameterVersion), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
you call `UpdateAssociation`, the system removes all optional parameters from the request
and overwrites the association with null values for those parameters. This is by design.
You must specify all optional parameters in the call, even if you are not changing the
parameters. This includes the `Name` parameter. Before calling this API action, we
recommend that you call the <a>DescribeAssociation</a> API operation and make a note of all
...
call the <a>DescribeAssociation</a> API operation. If you don't have permission to call
...
targets. You can add the `ApplyOnlyAtCronInterval` parameter to run the association during
...
- `"ApplyOnlyAtCronInterval"`: By default, when you update an association, the system runs
  ...
  association or you make changes to the SSM document on which that association is based
  (by using the Documents page in the console), State Manager applies the association at
  the next specified cron interval. For example, if you chose the `Latest` version of an
  SSM document when you created an association and you edit the association by choosing a
  different document version on the Documents page, State Manager applies the association
...
- `"AssociationVersion"`: This parameter is provided for concurrency control purposes. You
  must specify the latest association version in the service. If you want to ensure that
...
- `"SyncCompliance"`: The mode for generating association compliance. You can specify
  ...
  In `MANUAL` mode, you must specify the `AssociationId` as a parameter for the
  <a>PutComplianceItems</a> API operation. In this case, compliance data isn't managed by
  State Manager, a capability of Amazon Web Services Systems Manager. It is managed by your
  direct call to the <a>PutComplianceItems</a> API operation.
...
"""
update_association(AssociationId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateAssociation", Dict{String, Any}("AssociationId"=>AssociationId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_association(AssociationId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateAssociation", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("AssociationId"=>AssociationId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
associated with the specified managed node.
 ...
 `UpdateAssociationStatus` is primarily used by the Amazon Web Services Systems Manager
Agent (SSM Agent) to report status updates about your associations and is only used for
...
- `name`: The name of the SSM document.
"""
update_association_status(AssociationStatus, InstanceId, Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateAssociationStatus", Dict{String, Any}("AssociationStatus"=>AssociationStatus, "InstanceId"=>InstanceId, "Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"DocumentVersion"`: The version of the document that you want to update. Currently,
  Systems Manager supports updating only the latest version of the document. You can
  ...
  immediately runs the association unless you previously specifed the `apply-only-at-cron-
  interval` parameter.
- `"TargetType"`: Specify a new target type for the document.
- `"VersionName"`: An optional field specifying the version of the artifact you are
  updating with the document. For example, 12.6. This value is unique across all versions
  of a document, and can't be changed.
"""
update_document(Content, Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateDocument", Dict{String, Any}("Content"=>Content, "Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_document(Content, Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateDocument", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("Content"=>Content, "Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
update_document_default_version(DocumentVersion, Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateDocumentDefaultVersion", Dict{String, Any}("DocumentVersion"=>DocumentVersion, "Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_document_default_version(DocumentVersion, Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateDocumentDefaultVersion", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DocumentVersion"=>DocumentVersion, "Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"DocumentVersion"`: The version of a change template in which to update approval
  metadata.
"""
update_document_metadata(DocumentReviews, Name; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateDocumentMetadata", Dict{String, Any}("DocumentReviews"=>DocumentReviews, "Name"=>Name); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_document_metadata(DocumentReviews, Name, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateDocumentMetadata", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("DocumentReviews"=>DocumentReviews, "Name"=>Name), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"Replace"`: If `True`, then all fields that are required by the
  <a>CreateMaintenanceWindow</a> operation are also required for this API request. Optional
...
- `"StartDate"`: The date and time, in ISO-8601 Extended format, for when you want the
  ...
  current date and time are used as the start date.
"""
update_maintenance_window(WindowId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateMaintenanceWindow", Dict{String, Any}("WindowId"=>WindowId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_maintenance_window(WindowId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateMaintenanceWindow", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
target, and resource group. For more information, see <a>Target</a>.
...
- `"OwnerInformation"`: User-provided value that will be included in any Amazon CloudWatch
  Events events raised while running tasks for these targets in this maintenance window.
- `"Replace"`: If `True`, then all fields that are required by the
  <a>RegisterTargetWithMaintenanceWindow</a> operation are also required for this API
  request. Optional fields that aren't specified are set to null.
- `"Targets"`: The targets to add or replace.
"""
update_maintenance_window_target(WindowId, WindowTargetId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateMaintenanceWindowTarget", Dict{String, Any}("WindowId"=>WindowId, "WindowTargetId"=>WindowTargetId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_maintenance_window_target(WindowId, WindowTargetId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateMaintenanceWindowTarget", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId, "WindowTargetId"=>WindowTargetId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
can change the following values: - `TaskARN`. For example, you can change a `RUN_COMMAND`
...
(Automation, Lambda, and Step Functions). For more information about running tasks that
don't specify targets, see [Registering maintenance window tasks without targets](https://docs.aws.amazon.com/systems-manager/latest/userguide/maintenance-windows-targetless-tasks.html)
in the *Amazon Web Services Systems Manager User Guide*.If the value for a parameter in
`UpdateMaintenanceWindowTask` is null, then the corresponding field isn't modified. If you
set `Replace` to true, then all fields required by the
<a>RegisterTaskWithMaintenanceWindow</a> operation are required for this request. Optional
fields that aren't specified are set to null.
...
different `OutputS3BucketName` value, the values for `Comment` and `NotificationConfig` are
removed.
...
- `"MaxErrors"`: The new `MaxErrors` value to specify. `MaxErrors` is the maximum number of
  errors that are allowed before the task stops being scheduled.</p>
...
- `"Replace"`: If True, then all fields that are required by the
  <a>RegisterTaskWithMaintenanceWindow</a> operation are also required for this API
  request. Optional fields that aren't specified are set to null.
- `"ServiceRoleArn"`: The Amazon Resource Name (ARN) of the IAM service role for Amazon Web
  Services Systems Manager to assume when running a maintenance window task. If you do not
  specify a service role ARN, Systems Manager uses a service-linked role in your account.
...
- `"TaskInvocationParameters"`: The parameters that the task should use during execution.
  ...
  are removed.
- `"TaskParameters"`: The parameters to modify.</p>

  !!! note
      `TaskParameters` has been deprecated. To specify parameters to pass to a task when it
  runs, instead use the `Parameters` option in the `TaskInvocationParameters` structure.
  For information about how Systems Manager handles these options for the supported
  maintenance window task types, see <a>MaintenanceWindowTaskInvocationParameters</a>.The
  map has the following format:

  Key: string, between 1 and 255 characters

 <p>Value: an array of strings, each string is between 1 and 255 characters
"""
update_maintenance_window_task(WindowId, WindowTaskId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateMaintenanceWindowTask", Dict{String, Any}("WindowId"=>WindowId, "WindowTaskId"=>WindowTaskId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_maintenance_window_task(WindowId, WindowTaskId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateMaintenanceWindowTask", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("WindowId"=>WindowId, "WindowTaskId"=>WindowTaskId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
nodes during the activation process. For more information, see <a>CreateActivation</a>.
...
"""
update_managed_instance_role(IamRole, InstanceId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateManagedInstanceRole", Dict{String, Any}("IamRole"=>IamRole, "InstanceId"=>InstanceId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_managed_instance_role(IamRole, InstanceId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateManagedInstanceRole", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("IamRole"=>IamRole, "InstanceId"=>InstanceId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
to update an OpsItem. For more information, see [Set up OpsCenter](https://docs.aws.amazon.com/systems-manager/latest/userguide/OpsCenter-setup.html)
in the *Amazon Web Services Systems Manager User Guide*.
...
- `"OperationalData"`: Add new keys or edit existing key-value pairs of the OperationalData
  ...
  a maximum length of 128 characters. The value has a maximum size of 20 KB.</p>
...
- `"Title"`: A short heading that describes the nature of the OpsItem and the impacted
  resource.
"""
update_ops_item(OpsItemId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateOpsItem", Dict{String, Any}("OpsItemId"=>OpsItemId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_ops_item(OpsItemId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateOpsItem", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("OpsItemId"=>OpsItemId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
"""
update_ops_metadata(OpsMetadataArn; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateOpsMetadata", Dict{String, Any}("OpsMetadataArn"=>OpsMetadataArn); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_ops_metadata(OpsMetadataArn, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateOpsMetadata", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("OpsMetadataArn"=>OpsMetadataArn), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
- `"RejectedPatchesAction"`: The action for Patch Manager to take on patches included in
  ...
  installed outside of Patch Manager afterward, it's considered noncompliant with the patch
  baseline and its status is reported as `INSTALLED_REJECTED`. </dd> </dl>
- `"Replace"`: If True, then all fields that are required by the <a>CreatePatchBaseline</a>
  operation are also required for this API request. Optional fields that aren't specified
  are set to null.
- `"Sources"`: Information about the patches to use to update the managed nodes, including
  target operating systems and source repositories. Applies to Linux managed nodes only.
"""
update_patch_baseline(BaselineId; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdatePatchBaseline", Dict{String, Any}("BaselineId"=>BaselineId); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_patch_baseline(BaselineId, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdatePatchBaseline", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("BaselineId"=>BaselineId), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
change the account options for that sync. For example, if you create a sync in the us-east-
2 (Ohio) Region and you choose the `Include only the current account` option, you can't
...
"""
update_resource_data_sync(SyncName, SyncSource, SyncType; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateResourceDataSync", Dict{String, Any}("SyncName"=>SyncName, "SyncSource"=>SyncSource, "SyncType"=>SyncType); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_resource_data_sync(SyncName, SyncSource, SyncType, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateResourceDataSync", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SyncName"=>SyncName, "SyncSource"=>SyncSource, "SyncType"=>SyncType), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
...
define the default value for a `SettingId`. You can't create a new `SettingId`, but you can
overwrite the default value if you have the `ssm:UpdateServiceSetting` permission for the
setting. Use the <a>GetServiceSetting</a> API operation to view the current value. Or, use
the <a>ResetServiceSetting</a> to change the value back to the original value defined by
...
- `setting_id`: The Amazon Resource Name (ARN) of the service setting to update. For
  example, `arn:aws:ssm:us-east-1:111122223333:servicesetting/ssm/parameter-store/high-
  throughput-enabled`. The setting ID can be one of the following. - `/ssm/managed-
  ...
  !!! note
      Permissions to update the `/ssm/managed-instance/default-ec2-instance-management-
  role` setting should only be provided to administrators. Implement least privilege access
  when allowing individuals to configure or modify the Default Host Management
  Configuration.
- `setting_value`: The new value to specify for the service setting. The following list
  specifies the available values for each setting. - For `/ssm/managed-instance/default-ec2-
  instance-management-role`, enter the name of an IAM role.
...
"""
update_service_setting(SettingId, SettingValue; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateServiceSetting", Dict{String, Any}("SettingId"=>SettingId, "SettingValue"=>SettingValue); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
update_service_setting(SettingId, SettingValue, params::AbstractDict{String}; aws_config::AbstractAWSConfig=current_aws_config()) = ssm("UpdateServiceSetting", Dict{String, Any}(mergewith(_merge, Dict{String, Any}("SettingId"=>SettingId, "SettingValue"=>SettingValue), params)); aws_config=aws_config, feature_set=SERVICE_FEATURE_SET)
